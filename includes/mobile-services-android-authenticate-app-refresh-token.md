Nuestra caché del token debería funcionar en un caso sencillo pero, ¿qué ocurre cuando el token expira o se revoca? Tenemos que poder detectar un token expirado. El token puede expirar si la aplicación no se está ejecutando. Lo que podría significar que la caché del token no es válida. El token también puede expirar mientras la aplicación se está ejecutando durante una llamada que la aplicación realiza directamente o una llamada realizada por la biblioteca de Servicios móviles. El resultado puede ser un código de estado HTTP 401 "No autorizado". Por tanto necesitamos una forma de detectarlo y de actualizar el token. Para ello, usamos un [ServiceFilter][ServiceFilter] de la [Biblioteca de cliente Android][Biblioteca de cliente Android].
...
2.  En el archivo ToDoActivity.java, agregue los miembros siguientes a la clase ToDoActivity.
...
6.  En el archivo ToDoActivity.java, agregue la siguiente definición para la clase `RefreshTokenCacheFilter` en la clase ToDoActivity:
...
7.  En el archivo ToDoActivity.java, actualice el método `onCreate` de la forma siguiente:
    ...
    En este código, se utiliza RefreshTokenCacheFilter además de ProgressFilter. También, durante `onCreate` queremos cargar la caché del token. Por tanto, se pasa un valor false al método `authenticate`.

  [ServiceFilter]: http://dl.windowsazure.com/androiddocs/com/microsoft/windowsazure/mobileservices/ServiceFilter.html
  [Biblioteca de cliente Android]: http://dl.windowsazure.com/androiddocs/
