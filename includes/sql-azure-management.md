...
<p>Puede usar la versi&oacute;n SQL Server 2014, SQL Server 2012 o SQL Server 2008 R2 de SSMS. Las versiones anteriores no son compatibles.</p>
</div>
...
Management Studio es un entorno integrado para administrar
bases de datos SQL. Para administrar
bases de datos en Azure, puede usar la aplicación Management Studio instalada con SQL Server o descargar
...
5.  En la pantalla **Selección de características**, aparece preseleccionada la opción **Herramientas de administración -
    Básica**. Esto se debe a que se encuentra en ejecución el instalador de Management Studio. Si ejecuta la instalación de todas las ediciones de SQL Server Express, seleccione la opción **Herramientas de administración - Básica** y, a continuación, haga clic en **Siguiente**.

6.  En la pantalla **Informes de errores**, si lo desea, puede seleccionar la opción para enviar
    informes de errores a Microsoft. No es obligatorio usar SSMSE. Haga clic en
    **Siguiente** para iniciar la instalación.

7.  Cuando se haya completado la instalación, se abrirá la página **Operación completada**
    . Haga clic en **Cerrar**.

## <span id="Step2"></span> </a>Paso 2: Conectarse a Base de datos SQL
...
3.  En la página principal Bases de datos SQL, haga clic en **SERVERS** en la parte superior de la página para ver todos los servidores asociados con la suscripción. Encuentre el nombre del servidor con el que desea establecer la conexión y cópielo en el Portapapeles.
    ...
    que permita todas las conexiones desde la máquina local. Para ello, agregue la dirección IP de las máquinas locales a la lista de excepciones del firewall.
...
8.  Haga clic en **SAVE** en la parte inferior de la página.
    ...
    **Nota:** Los cambios de la configuración del
    cortafuegos pueden tardar hasta cinco minutos en aplicarse.

    Ya puede conectarse a Base de datos SQL con Management Studio.

9.  En la barra de tareas, haga clic en **Inicio**, vaya a **Todos los programas** y a
    **Microsoft SQL Server 2012** y, a continuación, haga clic en **SQL Server
    Management Studio**.

10. En **Conectar a servidor**, especifique *serverName*.database.windows.net como
    nombre completo del servidor. En Azure, el nombre del servidor es una cadena generada automáticamente compuesta de caracteres alfanuméricos.
...
Muchos de los asistentes de SSMS para tareas como
crear y modificar inicios de sesión y bases de datos en una base de datos SQL Server
no están disponibles para las bases de datos SQL en Azure, por lo que tendrá que
...
Mientras está conectado a la base de datos **maestra**, puede crear bases de datos
nuevas en el servidor y modificar o anular las existentes. En los pasos
siguientes se describe cómo realizar varias tareas comunes de administración
de bases de datos con Management Studio. Para realizar estas tareas, asegúrese de que está conectado a la base de datos
**maestra** con el inicio de sesión principal a nivel de servidor que
creó cuando creó el servidor.
...
-   La base de datos maestra tiene la vista **sys.databases**, que puede usar
    para ver los detalles acerca de todas las bases de datos. Para ver todas las bases de datos existentes,
    ejecute la siguiente instrucción:

        SELECT * FROM sys.databases;

-   En Base de datos SQL, la instrucción **USE** no es compatible para cambiar
    entre bases de datos. En su lugar, necesita establecer una conexión
    directa con la base de datos de destino.
...
otras instrucciones de Transact-SQL. Para obtener m&aacute;s informaci&oacute;n, consulte la informaci&oacute;n espec&iacute;fica de las instrucciones disponible en los v&iacute;nculos mencionados anteriormente.</p>
</div>

</div>

## <span id="Step4"></span> </a>Paso 4: Crear y administrar los inicios de sesión

La base de datos **maestra** realiza un seguimiento de los inicios de sesión y de cuáles tienen
permiso para crear bases de datos u otros inicios de sesión. Para administrar inicios de sesión,
conéctese a la base de datos **maestra** con el inicio de sesión
...
-   Use la instrucción **CREATE USER** para conceder permisos a nivel
...
2.  Haga clic con el botón secundario en la base de datos **myTestDB** y seleccione **Nueva consulta**.
    ...
    1.  Ejecute la siguiente instrucción en la base de datos myTestDB para
        crear un usuario de la base de datos con nombre **login1User** que se corresponda con el inicio de sesión a
        nivel de servidor **login1**.
...
-   Use la instrucción **ALTER LOGIN** para modificar un inicio de sesión existente, por
    ...
    Reemplace **newPassword** por la contraseña que desee y
    **oldPassword** por la contraseña actual de inicio de sesión.
...
-   Use la instrucción **DROP LOGIN** para eliminar un inicio de sesión existente.
    Al eliminar un inicio de sesión a nivel de servidor también se eliminan todas las cuentas de usuario
    de la base de datos asociadas. Para obtener más información,
    consulte [DROP LOGIN (Base de datos SQL)][DROP DATABASE (Base de datos SQL)]. La instrucción **DROP LOGIN**
     debe ejecutarse en la base de datos **maestra**. La instrucción
    siguiente elimina el inicio de sesión **login1**.

        DROP LOGIN login1;

-   La base de datos maestra tiene la vista **sys.sql\_logins**, que puede usar
    para ver los inicios de sesión. Para ver todos los inicios de sesión existentes, ejecute
    la siguiente instrucción:

        SELECT * FROM sys.sql_logins;

## <span id="Step5"></span> </a>Paso 5: Supervisar Base de datos SQL mediante las vistas de administración dinámica
...
-   Para consultar una vista de administración dinámica, es necesario disponer de los permisos **VER ESTADO DE BASE DE DATOS**
    ...
    usuario específico de la base de datos, conéctese a la base de datos que desea administrar
    con el inicio de sesión principal a nivel de servidor y ejecute la siguiente
    instrucción en la base de datos:

        GRANT VIEW DATABASE STATE TO login1User;

-   Calcule el tamaño de la base de datos; para ello, use la vista **sys.dm\_db\_partition\_stats**
    . La vista **sys.dm\_db\_partition\_stats** devuelve información sobre la página y
    el recuento de filas para cada partición de la base de datos, que
    puede usar para calcular el tamaño de la base de datos. La siguiente consulta devuelve
...
-   Use las vistas **sys.dm\_exec\_connections** y **sys.dm\_exec\_sessions**
     para recuperar información sobre las conexiones de usuario actuales y las tareas
    internas asociadas con la base de datos. La consulta siguiente
...
-   Use la vista **sys.dm\_exec\_query\_stats** para recuperar estadísticas de agregado acerca
    del rendimiento para los planes de consulta en caché. La siguiente consulta
    devuelve información acerca de las cinco consultas principales clasificadas en función del
...
-   [Referencia de Transact-SQL (Base de datos SQL)][]
  ...
  [\*iniciodesesión\*@\*NombreServidor\*]: mailto:*login*@*yourServerName*
  ...
  [sp\_addrolemember (Transact-SQL)]: http://msdn.microsoft.com/es-es/library/ms187750.aspx
