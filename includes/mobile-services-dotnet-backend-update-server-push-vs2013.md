...
4. Agregue el código siguiente que implementa el método POST en el controlador:
   ...
   > [!NOTE]
   > Se puede llamar a este método POST con cualquier cliente que tenga la clave de la aplicación, que no es segura. Si quiere proteger el extremo, aplique el atributo `[AuthorizeLevel(AuthorizationLevel.User)]` al método o la clase para requerir autenticación.
   > 
   > 
