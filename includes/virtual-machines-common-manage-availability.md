...
Si usa Azure Managed Disks, puede omitir las instrucciones siguientes. Azure Managed Disks proporciona alta disponibilidad y redundancia porque los discos se almacenan en dominios de error que se alinean con los conjuntos de disponibilidad de máquinas virtuales. Para más información, consulte [Azure Managed Disks overview](../articles/storage/storage-managed-disks-overview.md) (Información general sobre Azure Managed Disks).

Si usa discos no administrados, deben seguirse unos procedimientos recomendados con relación a las cuentas de almacenamiento utilizadas por los discos duros virtuales en la máquina virtual. Cada disco (VHD) es un blob en páginas en una cuenta de Azure Storage. Es importante asegurarse de que hay aislamiento y redundancia en las cuentas de almacenamiento con el fin de proporcionar una alta disponibilidad para las máquinas virtuales en el conjunto de disponibilidad.
...
3. **Utilice una cuenta de almacenamiento independiente para cada máquina virtual de un conjunto de disponibilidad.** Varias máquinas virtuales en el mismo conjunto de disponibilidad NO deben compartir las cuentas de almacenamiento. Las máquinas virtuales de distintos conjuntos de disponibilidad pueden compartir las cuentas de almacenamiento siempre que sigan los procedimientos recomendados anteriores.
