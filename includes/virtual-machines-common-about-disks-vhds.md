...
El disco temporal se crea automáticamente. En las máquinas virtuales Windows, este disco está etiquetado de forma predeterminada como unidad D: y se usa para almacenar el archivo pagefile.sys. En las máquinas virtuales de Linux, el disco es normalmente /dev/sdb y es formateado y montado en /mnt/resource por el agente de Linux de Azure.

El tamaño del disco temporal varía, según el tamaño de la máquina virtual. Para más información, consulte los [tamaños de máquinas virtuales Linux](../articles/virtual-machines/virtual-machines-linux-sizes.md) o los [tamaños de máquinas virtuales Windows](../articles/virtual-machines/virtual-machines-windows-sizes.md).

>[AZURE.WARNING] No almacene datos en el disco temporal. Proporciona almacenamiento temporal para aplicaciones y procesos y está destinado únicamente a almacenar datos como archivos de páginas o de intercambio. Para reasignar este disco a una letra de unidad diferente para una máquina virtual Windows, consulte [Cambiar las letras de las unidades](../articles/virtual-machines/virtual-machines-windows-classic-change-drive-letter.md).

Para más información sobre cómo usa Azure el disco temporal, consulte [Understanding the temporary drive on Microsoft Azure Virtual Machines](https://blogs.msdn.microsoft.com/mast/2013/12/06/understanding-the-temporary-drive-on-windows-azure-virtual-machines/) (Descripción de la unidad temporal en máquinas virtuales de Microsoft Azure).
...
>[AZURE.NOTE] Para más información sobre las capacidades de las máquinas virtuales, consulte los [tamaños de máquinas virtuales Linux](../articles/virtual-machines/virtual-machines-linux-sizes.md) o los [tamaños de máquinas virtuales Windows](../articles/virtual-machines/virtual-machines-windows-sizes.md).
...
Puede agregar discos de datos a una máquina virtual en cualquier momento **acoplando** el disco a la máquina virtual. Puede usar un disco duro virtual cargado o copiado por usted en su cuenta de almacenamiento o uno creado por Azure para usted. Al acoplar un disco de datos se asocia el archivo VHD de la cuenta de almacenamiento a la máquina virtual, y se coloca una "concesión" en el disco duro virtual para que no pueda eliminarse del almacenamiento mientras esté todavía acoplado.
