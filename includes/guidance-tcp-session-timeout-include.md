...
* **saliente** mediante [SNAT][snat] (Source NAT). Este tiempo de espera está establecido en 4 minutos y no se puede ajustar.

Para garantizar que no se pierden conexiones más allá del límite de tiempo de espera, debe asegurarse de que su aplicación mantiene la sesión activa, o bien configurar el sistema operativo subyacente para ello. Las opciones de configuración que se usarán son diferentes en los sistemas Linux y Windows, como se muestra aquí.

En [Linux][linux], es preciso cambiar estás variables de kernel.
net.ipv4.tcp_keepalive_time = 120 net.ipv4.tcp_keepalive_intvl = 30 net.ipv4.tcp_keepalive_probes = 8

En [Windows][windows], es preciso cambiar los siguientes valores del Registro.
