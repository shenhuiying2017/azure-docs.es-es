...
> Se puede descargar un documento de Visio que incluye este diagrama de arquitectura, y que está disponible en el [Centro de descarga Microsoft][visio-download]. Este diagrama está en la página "Compute - single VM".
> 
> 
...
* **Discos de datos.** Un [disco de datos][data-disk] es un disco duro virtual persistente para los datos de la aplicación. Los discos de datos se almacenan en Azure Storage, como el disco del sistema operativo.
...
Azure ofrece muchos recursos y tipos de recursos diferentes, y esta arquitectura de referencia se puede aprovisionar de muchas maneras distintas. Para implementar esta arquitectura, tal como se muestra en la ilustración anterior, consulte la sección [Implementación de la solución](#solution-deployment) al final de este documento para más información. Si elige crear su propia arquitectura de referencia, debe seguir estas recomendaciones a menos que una de las recomendaciones no cumpla un requisito concreto.

### <a name="vm-recommendations"></a>Recomendaciones de VM

Azure ofrece varios tamaños de máquinas virtuales diferentes, pero se recomiendan las series DS y GS porque estos tamaños de máquina admiten [Premium Storage][premium-storage]. Seleccione uno de estos tamaños de máquina a menos que tenga una carga de trabajo especializada, como puede ser el caso de la informática de alto rendimiento. Para más información, consulte [Tamaños de máquinas virtuales][virtual-machine-sizes]. 

Si desplaza una carga de trabajo existente a Azure, comience con el tamaño de máquina virtual que más se acerque a los servidores locales. Luego, mida el rendimiento de la carga de trabajo real con respecto a la CPU, la memoria y las operaciones de entrada/salida por segundo (IOPS) de disco, y ajuste el tamaño, si es necesario. Si necesita varias NIC para la máquina virtual, tenga en cuenta que el número máximo de NIC disponibles es una función del [tamaño de máquina virtual][vm-size-tables].   

Cuando aprovisiona la VM y otros recursos, debe especificar una ubicación. Por lo general, se recomienda elegir una ubicación más cercana a los usuarios internos o clientes. Sin embargo, no todos los tamaños de máquina virtual están disponibles en todas las ubicaciones. Para más información, consulte [Servicios por región][services-by-region]. Para ver una lista de los tamaños de máquina virtual disponibles en una ubicación dada, ejecute el siguiente comando de la interfaz de la línea de comandos (CLI) de Azure:

```
    azure vm sizes --location <location>
```

Para más información sobre cómo elegir una imagen de máquina virtual publicada, consulte [Navegación y selección de imágenes de máquina virtual Windows en Azure con PowerShell o la CLI][select-vm-image].
...
Aprovisione suficientes cuentas de almacenamiento para el número de máquinas virtuales que piensa implementar. Cada una de las máquinas virtuales necesita un disco de sistema operativo y un disco temporal y, en función de los [límites del disco de la cuenta de almacenamiento][storage-account-limits], una cuenta Premium Storage puede admitir entre 1 y 20 máquinas virtuales. Si opta por agregar discos de datos a la máquina virtual, se quita el formato tras su creación. Tendrá que iniciar sesión en la máquina virtual para dar formato a los discos de datos.
...
* Reserve una [dirección IP estática][static-ip] si necesita una dirección IP fija que no cambie &mdash;, por ejemplo, si tiene que crear un registro D en DNS o necesita que la dirección IP se agregue a una lista segura.
...
Puede escalar y reducir verticalmente una máquina virtual [cambiando su tamaño][vm-resize]. Para escalar horizontalmente, coloque dos o más máquinas virtuales en un conjunto de disponibilidad detrás de un equilibrador de carga. Para más información, consulte [Ejecución de varias máquinas virtuales Windows en Azure][multi-vm].
...
**Detención de una máquina virtual.** Azure hace una distinción entre los estados "Detenido" y "Desasignado". Se le cobrará cuando el estado de la máquina virtual sea "Detenido". No se le cobrará cuando la máquina virtual esté desasignada.
...
Hay disponible una carpeta [deployment][github-folder] para una arquitectura de referencia que implementa estos procedimientos recomendados. Esta arquitectura de referencia incluye una red virtual (VNet), un grupo de seguridad de red (NSG) y una máquina virtual única (VM).

Hay varias maneras de implementar esta arquitectura de referencia. La manera más fácil es seguir estos pasos: 
...
Para obtener información sobre otras maneras de implementar esta arquitectura de referencia, consulte el archivo Léame en la carpeta de Github [guidance-single-vm][github-folder]]. 
