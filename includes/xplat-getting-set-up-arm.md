...
### Paso 1: Comprobación de la versión de xplat-cli

Para usar xplat-cli para comandos imperativos y plantillas de ARM, debe tener al menos la versión 0.8.17. Para comprobar su versión, escriba `azure --version`. Debería ver algo parecido a lo siguiente:

    $ azure --version
    0.8.17 (node: 0.10.25)
    
Si necesita actualizar la versión de xplat-cli, consulte [xplat-cli](https://github.com/Azure/azure-xplat-cli).
...
Si tiene solo una suscripción en su cuenta de Azure, xplat-cli se asocia a esa suscripción de manera predeterminada. Si tiene más de una suscripción, debe seleccionar la suscripción que desea usar y escriba `azure account set <subscription id or name> true` donde _identificador de suscripción o nombre_ es el identificador de suscripción o el nombre de la suscripción con la que desea trabajar en la sesión actual.
...
### Paso 4: Ubicación de xplat-cli en modo ARM

Para utilizar el modo de administración de recursos de Azure \(ARM\) con xplat-cli, escriba `azure config mode arm`. Debe ver algo parecido a lo siguiente:
