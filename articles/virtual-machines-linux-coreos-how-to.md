...
3. Demonios [etcd](https://github.com/coreos/etcd) y [fleet](https://github.com/coreos/fleet) integrados (servicios) para una comunicación y una administración dinámicas entre máquina virtual y clúster
...
Use las instrucciones indicadas en [Utilización de SSH con Linux en Azure](http://azure.microsoft.com/en-us/documentation/articles/virtual-machines-linux-use-ssh-key/) para crear una clave pública y privada para SSH. (Los pasos básicos se encuentran en las instrucciones a continuación). Va a utilizar estas claves para conectar con las máquinas virtuales en el clúster para verificar que funcionan y que pueden comunicarse entre sí.

> [WACOM.NOTE] Este tema supone que no tiene estas claves, y requiere que cree un archivo **"myPrivateKey.pem"** y **"myCert.pem"** para una mayor claridad. Si ya dispone de un par de claves pública y privada guardado en **`~/.ssh/id_rsa`**, puede escribir "openssl req -x509 -key ~/.ssh/id_rsa -nodes -days 365 -newkey rsa:2048 -out myCert.pem" para obtener el archivo .pem que necesita cargar en Azure.
...
> [WACOM.NOTE] Recuerde escribir "cat etcdid" para recuperar el identificador de detección etcd del archivo `etcdid` que ha creado anteriormente y reemplace **"<token>"** en el siguiente archivo **cloud-config.yaml** con el número generado del archivo "etcdid". Si no puede validar el clúster al final, es posible que este sea uno de los pasos que ha pasado por alto.
...
(Para obtener información más completa sobre el archivo cloud-config, consulte [Uso de Cloud-Config](https://coreos.com/docs/cluster-management/setup/cloudinit-cloud-config/) en la documentación de CoreOS).
...
Una nota: Si observa el trabajo realizado hasta ahora en el [nuevo portal](https://portal.azure.com), verá que el nombre del servicio en la nube es un dominio y un grupo de recursos, como se muestra en la imagen siguiente:
...
> [WACOM.NOTE] Si ya está utilizando la clave **~/.ss h/id_rsa**, entonces agréguela con "ssh-add ~/.ssh/id_rsa".
...
Ahora debería tener un clúster de CoreOS de tres nodos ejecutándose en Azure. A partir de aquí, puede explorar cómo crear clústeres más complejos y utilizar Docker y crear aplicaciones más interesantes leyendo el [tutorial sobre CoreOS de Tim Park], el [tutorial sobre CoreOS de Patrick Chanezo], la documentación de [Docker] y la [información general sobre CoreOS].
...
[Tutorial sobre CoreOS de Patrick Chanezo]: https://github.com/chanezon/azure-linux/tree/master/coreos/cloud-init
