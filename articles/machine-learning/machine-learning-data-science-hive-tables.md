...
2. [Enviar las consultas de subárbol en la línea de comandos de Hadoop en el nodo principal](machine-learning-data-science-hive-queries.md).

Los usuarios también pueden usar la [consola de consultas (editor de subárbol)] escribiendo la URL en un explorador web `https://<nombre de clúster de Hadoop>.azurehdinsight.net/Home/HiveEditor (se le pedirá que introduzca las credenciales de clúster de Hadoop para iniciar sesión) o puede enviar [trabajos de subárbol mediante PowerShell](hdinsight-submit-hadoop-jobs-programmatically.md). 

- [Paso 1: crear tablas y base de datos de subárbol](#create-tables)
- [Paso 2: cargar datos en tablas de subárbol](#load-data)
- [Temas avanzados: tabla con particiones y almacenar datos de subárbol en formato ORC](#partition-orc)
...
- `<ruta de acceso a datos blob>`: Si el archivo blob que se va a cargar en la tabla de subárbol se encuentra en el contenedor predeterminado del clúster de Hadoop de HDInsight, la `<ruta de acceso a datos blob>` debe tener el formato `'wasb:///<directorio de este contenedor>/<nombre de archivo blob>'`. El archivo de blob también debe estar en el contenedor adicional del clúster de Hadoop de HDInsight. En este caso, `<ruta de acceso a datos blob>` debe tener el formato `'wasb://<nombre del contenedor>@<nombre de cuenta de almacenamiento>.blob.windows.core.net/<nombre de archivo blob>'`.
...
Además de crear particiones de la tabla, también es beneficioso almacenar los datos de subárbol en formato ORC. ORC significa "Columnas de filas optimizadas".  Consulte "[El uso de archivos ORC mejora el rendimiento cuando el subárbol lee, escribe y procesa datos](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+ORC#LanguageManualORC-ORCFiles)."_ para obtener más detalles.
...
Al consultar tablas con particiones, se recomienda agregar la condición de partición al **principio** de la `where` cláusula para que se pueda mejorar significativamente la eficacia de la búsqueda. 
...
4. Es seguro quitar el `<nombre de tabla de archivo de texto externo>` mediante la siguiente consulta cuando todos los datos se han insertado en `<nombre de base de datos>.<nombre de tabla ORC>`:
