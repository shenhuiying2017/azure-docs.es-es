...
## <a name="launch-azure-machine-learning-workbench"></a>Inicio de Azure Machine Learning Workbench
Siga la [guía de inicio de instalación y creación](quickstart-installation.md) para instalar la aplicación Azure Machine Learning Workbench, que también incluye la interfaz de la línea de comandos (CLI). Inicie la aplicación Azure Machine Learning Workbench e inicie sesión si es necesario.

## <a name="create-a-new-project"></a>Crear un nuevo proyecto
1. En el panel **Proyectos**, haga clic en el icono **+** para crear un **Nuevo proyecto**.
...
3. Seleccione la opción **File(s)/Directory** (Archivos o directorio) y elija el archivo local `iris.csv`. Acepte la configuración predeterminada para todas las pantallas y, finalmente, haga clic en **Finalizar**. 
...
6. Haga clic en el botón **Preparar** situado junto al botón **Métricas** (o en el botón **Datos** si se encuentra en la vista de métricas). Aparecerá el cuadro de diálogo **Preparar**. El proyecto de ejemplo ya contiene el archivo `iris.dprep`, por lo que de forma predeterminada se le pide que cree un nuevo flujo de datos en ese paquete de preparación de datos existente **iris.dprep**. Cambie el valor de la lista desplegable a **+New Data Preparation Package** (+Nuevo paquete de preparación de datos), escriba el valor nuevo "iris-1" y, a continuación, haga clic en **Aceptar**.
...
Ahora vamos a preparar algunos datos simples. Para cambiar los nombres de la columna, haga clic en el encabezado de columna y haga que el texto del encabezado se pueda modificar. Escriba `Sepal Length`, `Sepal Width`, `Petal Length`, `Petal Width` y `Species` para las cinco columnas respectivamente.

![cambiar el nombre de las columnas](media/tutorial-classifying-iris/rename_column.png)

Seleccione la columna `Species` y haga clic con el botón derecho en ella. Elija **Value Counts** (Recuentos de valores). 

![recuento de valores](media/tutorial-classifying-iris/value_count.png)

Esta acción crea un histograma con cuatro barras. Observe que la columna de destino tiene tres valores distintos: `Iris_virginica`, `Iris_versicolor` y `Iris-setosa`. También hay una fila con el valor `(null)`. Vamos a deshacernos de esta fila seleccionando la barra que representa el valor NULL y, para quitarla, haremos clic en el botón de filtro **-**. 

![recuento de valores](media/tutorial-classifying-iris/filter_out.png)

Mientras realiza el cambio de nombre de la columna y filtra la fila con el valor NULL, todas las acciones que realiza se registran como un paso de preparación de datos en el panel **PASOS**. Puede modificarlos (para ajustar su configuración), reordenarlos, o incluso quitarlos.
...
Ahora, cierre el editor de DataPrep. (No se preocupe, se guarda automáticamente). Haga doble clic en el archivo **iris 1.dprep** para mostrar el menú contextual y elija **Generate Data Access Code File** (Generar archivo de código de acceso a datos). 

![generar código](media/tutorial-classifying-iris/generate_code.png)

Se crea un archivo **iris-1.py** con las dos líneas de código siguientes rellenadas previamente (junto con algunos comentarios):
...
```
Este fragmento de código invoca la lógica que creó como un paquete de preparación de datos. En función del contexto en el que se ejecuta este código, `df` puede ser una clase [pandas DataFrame](https://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.html) si se ejecuta en tiempo de ejecución de Python, o una clase [Spark DataFrame](https://spark.apache.org/docs/latest/sql-programming-guide.html) si se ejecuta en un contexto de Spark. Para obtener más información sobre cómo preparar los datos en Azure Machine Learning Workbench, haga referencia a la guía [Getting Started with Data Preparation](data-prep-getting-started.md) (Introducción a la preparación de datos).
