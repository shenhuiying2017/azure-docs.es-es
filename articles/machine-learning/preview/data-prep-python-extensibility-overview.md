...
description: "En este documento se proporciona información general y algunos ejemplos detallados sobre cómo usar el código de Python para ampliar la funcionalidad de la preparación de datos"
...
---
# <a name="data-prep-python-extensions"></a>Extensiones de Python para la preparación de datos
Como método para completar vacíos de funcionalidad entre características integradas, Preparación de datos incluye una extensibilidad en varios niveles. En este documento se indica la extensibilidad mediante un script de Python. 
...
3. Agregar columna
4. Filtro avanzado
5. Transformar flujo de datos
6. Transformar partición
...
Por ejemplo, podría agregar una nueva columna que calculara el registro de otra columna de las dos maneras siguientes: Expresión: 
...
Si usa el tipo de bloque Expresión, puede seguir agregando instrucciones de importación al código, pero todas ellas se deben agrupar en las líneas superiores del código. Correcto: 
...
## <a name="installing-new-packages"></a>Instalar paquetes nuevos
...
Este punto de extensión le permite controlar completamente el proceso de lectura de un archivo en un flujo de datos. El sistema llama al código y pasa la lista de archivos que se deben procesar. Luego, el código deberá crear y devolver una trama de datos de Pandas. 
...
A este punto de extensión se obtiene acceso desde el asistente Abrir origen de datos. Seleccione File (Archivo) en la primera página y, luego, elija la ubicación del archivo. En la página "Choose File Parameters" (Seleccionar parámetros de archivo), abra la lista desplegable "File Type" (Tipo de archivo) y seleccione "Custom File (Script)" (Archivo personalizado (script)). 

El código obtiene una trama de datos de Pandas denominada "df" que contiene información sobre los archivos que debe leer. Si decide abrir un directorio que contiene varios archivos, la trama de datos contendrá más de una fila.  
...
### <a name="purpose"></a>Propósito 
El punto de extensión del escritor le permite controlar completamente el proceso de escritura de datos desde un flujo de datos. El sistema llama al código y pasa una trama de datos. Luego, el código podrá usar la trama de datos para escribir datos como quiera. 

>[!NOTE]
>El punto de extensión del escritor no funciona en Spark. 
...
Este bloque de escritura personalizado puede existir en medio de una lista de pasos. Así, si usa un módulo, la función de escritura deberá devolver la trama de datos que representa la entrada al paso siguiente. 

## <a name="add-column"></a>Agregar columna 
### <a name="purpose"></a>Propósito
Este punto de extensión le permite escribir Python para calcular una columna nueva. El código que escriba tiene acceso a la fila completa. Debe devolver un nuevo valor de columna para cada fila. 
...
### <a name="purpose"></a>Propósito 
Este punto de extensión le permite escribir un filtro personalizado. Tiene acceso a toda la fila y el código debe devolver True (incluir la fila) o False (excluir la fila). 
...
### <a name="purpose"></a>Propósito 
El punto de extensión le permite transformar completamente el flujo de datos. Tiene acceso a una trama de datos de Pandas que contiene todas las columnas y filas que está procesando. Además, el código debe devolver una trama de datos de Pandas con los datos nuevos. 

>[!NOTE]
>En Python, se cargarán todos los datos en memoria en una trama de datos de Pandas si se usa esta extensión. 

En Spark, todos los datos se recopilan en un solo nodo de trabajo. Esto podría dar lugar a que un nodo de trabajo se quedara sin memoria en el caso de que haya muchos datos. Úselo con precaución.
...
### <a name="purpose"></a>Propósito 
Este punto de extensión le permite transformar una partición del flujo de datos. Tiene acceso a una trama de datos de Pandas que contiene todas las columnas y filas de esa partición. Además, el código debe devolver una trama de datos de Pandas con los datos nuevos. 
...
En Preparación de datos existe el concepto de valores de error. Su creación y su motivo de existencia se tratan aquí <link to error values doc>. 

Se pueden detectar valores de error en el código personalizado de Python. Son instancias de una clase de Python denominada `DataPrepError`. Esta clase encapsula una excepción de Python y tiene un par de propiedades, que contienen información sobre el error que se produjo al procesar el valor original, así como el valor original. 
...
``` 
#### <a name="how-to-use"></a>Modo de uso 
Python se puede ejecutar en un punto de extensión para generar clases DataPrepErrors como valores devueltos mediante el método de creación mencionado antes. Es mucho más probable que los errores DataPrepErrors se detecten al procesar datos en un punto de extensión. En este momento, el código personalizado de Python necesita tratar DataPrepError como un tipo de datos válido. 
