...
* Se han cargado los datos en tablas de subárbol en clústeres de Hadoop de HDInsight de Azure. De no ser así, siga [Crear y cargar datos en tablas de subárbol](machine-learning-data-science-move-hive-tables.md) para cargar los datos en tablas de subárbol primero.
...
Los usuarios pueden especificar también la consola de comandos de subárbol ejecutando el comando `hive` desde la línea de comandos de Hadoop y, a continuación, enviar consultas de subárbol desde la consola de comandos de subárbol. Aquí tiene un ejemplo.  
...
En el ejemplo siguiente, el resultado de la consulta de subárbol se escribe en un directorio de blob `queryoutputdir` dentro del contenedor predeterminado del clúster de Hadoop. Aquí solo debe proporcionar el nombre del directorio, sin el nombre del blob. Se generará un error si proporciona tanto el directorio como el nombre del blob, como  `wasb:///queryoutputdir/queryoutput.txt`. 
...
https://<nombre de clúster de Hadoop>.azurehdinsight.net/Home/HiveEditor  

en un explorador web. Tenga en cuenta que se le pedirá que escriba las credenciales de clúster de Hadoop para iniciar sesión. También puede [Enviar trabajos de subárbol mediante PowerShell](../hdinsight/hdinsight-submit-hadoop-jobs-programmatically.md#hive-powershell). 
...
En esta consulta, si `<campo de fecha y hora>` tiene un patrón como `03/26/2015 12:04:39`, el `"<patrón del campo de fecha y hora>"` debería ser `"MM/dd/aaaa HH:mm:ss"`.

		select from_unixtime(unix_timestamp('05/15/2015 09:32:10','MM/dd/yyyy HH:mm:ss'))
		from hivesampletable limit 1;

La  `hivesampletable` de esta consulta viene preinstalado en todos los clústeres de Hadoop de HDInsight de Azure de forma predeterminada cuando se aprovisionan los clústeres. 
...
Los campos que se utilizan en esta consulta son las coordenadas GPS de la ubicaciones de recogida y entrega, denominadas pickup_longitude, pickup_latitude, dropoff_longitude y dropoff_latitude. Las consultas que calculan la distancia directa entre las coordenadas de recogida y entrega son:
...
Las ecuaciones matemáticas que calculan la distancia entre dos coordenadas GPS se pueden encontrar en el sitio [Movable Type Scripts](http://www.movable-type.co.uk/scripts/latlong.html), creado por Peter Lapisu. En su Javascript, la función toRad() es `lat_or_lon*pi/180`, que convierte grados a radianes. Aquí, `lat_or_lon` es la latitud o la longitud. Debido a que Subárbol no proporciona la función `atan2`, pero sí proporciona la función `atan`, la función `atan2` la implementa la función `atan` en la consulta Subárbol anterior mediante la definición proporcionada en [Wikipedia](http://en.wikipedia.org/wiki/Atan2). 

![Create workspace][1]

Se puede encontrar una lista completa de los UDF incrustados de subárbol en la sección **Funciones integradas** de la [wiki de subárbol de Apache](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+UDF#LanguageManualUDF-MathematicalFunctions). 
...
4. **Especificación del número de asignadores al subárbol**: Aunque Hadoop permite al usuario establecer el número de reductores, no suele establecer el número de asignadores. Un truco que permite cierto grado de control en este número es elegir las variables de Hadoop, *mapred.min.split.size* y *mapred.max.split.size* ya que el tamaño de cada asignación de tarea viene determinado por:
