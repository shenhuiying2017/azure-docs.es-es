...
> [Obtención de Jupyter Notebook](https://aka.ms/aml-packages/forecasting/notebooks/financial_forecasting)
...
Ahora puede calcular los errores de previsión en el conjunto de pruebas. Aquí puede usar el error porcentual absoluto medio (MAPE). MAPE es el error absoluto medio relativo a los valores de ventas reales. La función ```calc_error``` proporciona algunas funciones integradas para las métricas de error de uso frecuente. También puede definir la función de error personalizada y pasarla al argumento ```err_fun```.
...
Es importante ver la distribución de estos errores en las 249 series temporales en los datos. Para ver la distribución de los errores, use el argumento `by` en `calc_error` para calcular los errores de cada serie. Luego, cree un diagrama de caja para visualizarlos.
...
En general, el modelo Naive parece crear mejores previsiones a pesar de algunos valores atípicos, los que son menos precisos. 

## <a name="build-machine-learning-models"></a>Generación de modelos de aprendizaje automático
Además de los modelos univariados tradicionales, Azure Machine Learning Package for Forecasting también permite crear modelos de aprendizaje automático. Y
...
El modelo de aprendizaje automático pudo usar las características agregadas y las similitudes entre las series para obtener una previsión más precisa.
...
En esta sección, se implementa una canalización como un servicio web Azure Machine Learning y se consume para entrenamiento y puntuación.
Las canalizaciones no se ajustan para la implementación. Puntuar el servicio web implementado vuelve a entrenar el modelo y genera previsiones según los datos nuevos. 
