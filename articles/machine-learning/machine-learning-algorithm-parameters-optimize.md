<properties 
	pageTitle="Cómo elegir parámetros para optimizar los algoritmos de Aprendizaje automático de Azure | Azure" 
...
1.	**Definir el espacio de parámetros**: Para el algoritmo, primero decidimos los valores de parámetro exactos que nos gustaría tener en cuenta. 
...
El módulo de **partición y ejemplo** se puede usar para asignar pliegues a los datos de forma aleatoria. En la siguiente ilustración, vemos un ejemplo de configuración para el módulo donde definimos 5 subconjuntos y asignamos aleatoriamente un número de subconjunto a las instancias de ejemplo. 

![image4](./media/machine-learning-algorithm-parameters-optimize/fig4.png)


## Definir métrica
El módulo **Parámetros de barrido** proporciona compatibilidad para elegir empíricamente el mejor conjunto de parámetros para un algoritmo determinado y el conjunto de datos. El panel de propiedades de este módulo incluye, además de otra información referente a entrenar el modelo, la métrica que se utilizará para determinar el mejor conjunto de parámetros. Tiene dos listas desplegables diferentes para los algoritmos de clasificación y regresión, respectivamente. Si el algoritmo en cuestión es un algoritmo de clasificación, se ignorará la métrica de regresión y viceversa. En este ejemplo concreto, elegimos **Precisión** como la métrica.   
 
![image5](./media/machine-learning-algorithm-parameters-optimize/fig5.png)

## Formar, evaluar y comparar:  
El mismo módulo **Parámetros de barrido** entrena todos los modelos correspondientes al conjunto de parámetros, evalúa varias métricas y, a continuación, genera el mejor modelo formado en función de la métrica elegida por el usuario. Este módulo contiene dos entradas obligatorias
