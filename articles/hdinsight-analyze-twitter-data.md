<properties 
	pageTitle="Análisis de datos de Twitter con Hadoop en HDInsight | Azure" 
...
- **Un clúster de HDInsight de Azure**. Para obtener instrucciones acerca del aprovisionamiento del clúster, consulte [Introducción al uso de HDInsight][hdinsight-get-started] de Azure o [Aprovisionamiento de clústeres de HDInsight][hdinsight-provision]. Necesitará el nombre del clúster más adelante en el tutorial.
...
En este tutorial, verá las [API de streaming de Twitter][twitter-streaming-ap]. La API de streaming de Twitter específica que utilizará es [statuses/filter][twitter-statuses-filter].

>[WACOM.NOTE] Un archivo que contiene 10 000 tweets y el archivo de script de Hive (que se describe en la sección siguiente) se han cargado en un contenedor de Blob público. Puede omitir esta sección si va a utilizar los archivos cargados. 
...
En este tutorial, pasarán a PowerShell para realizar una llamada de servicio web. Para obtener un ejemplo de .NET C#, consulte [Análisis de opiniones de Twitter con HBase en HDInsight][hdinsight-hbase-twitter-sentiment]. La otra herramienta popular para realizar llamadas de servicios web [*Curl*][curl]. Es posible descargar la Curl [aquí][curl-download].
...
3. Establecen los primeros cinco a ocho variables en el script:	
	...
	<tr><td>$clusterName</td><td>El nombre del clúster de HDInsight donde desea ejecutar la aplicación.</td></tr><tr><td>$oauth_consumer_key</td><td>Esta es la aplicación Twitter. <strong>de la aplicación Twitter</strong> que ha escrito antes cuando ha creado la aplicación Twitter.</td></tr>
	<tr><td>$oauth_consumer_secret</td><td>Esta es la aplicación Twitter. <strong>de la aplicación Twitter</strong> que ha escrito antes.</td></tr>
	<tr><td>$oauth_token</td><td>Esta es la aplicación Twitter. <strong>de la aplicación Twitter</strong> que ha escrito antes.</td></tr>
	<tr><td>$oauth_token_secret</td><td>Esta es la aplicación Twitter. <strong>de la aplicación Twitter</strong> que ha escrito antes.</td></tr>	
...
>[WACOM.NOTE] El archivo de script de Hive y un archivo que contiene 10 000 tweets se han cargado en un contenedor de Blob público. Puede omitir esta sección si va a utilizar los archivos cargados.
...
>[WACOM.NOTE] Para usar los tweets y el script de HiveQL que ha cargado en las dos últimas secciones, establezca $hqlScriptFile en "/ tutorials/twitter/twitter.hql"; para utilizar los que se han cargado en un blob público, establezca $hqlScriptFile en "wasb://twittertrend@hditutorialdata.blob.core.windows.net/twitter.hql"
...
- [Realizar análisis de opinión en Twitter en tiempo real con HBase en HDInsight][hdinsight-hbase-twitter-sentiment]
...
[powershell-script]: http://technet.microsoft.com/es-es/library/ee176961.aspx
