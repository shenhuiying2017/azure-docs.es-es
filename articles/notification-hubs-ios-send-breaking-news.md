...
4.  Cree una nueva clase y nómbrela `Notifications`. Copie el siguiente código en la sección de interfaz del archivo Notifications.h:
...
7.  En el código anterior, reemplace los marcadores de posición `<hub name>` y `<connection string with listen access>` por el nombre de su centro de notificaciones y la cadena de conexión para *DefaultListenSharedAccessSignature* que obtuvo anteriormente.
...
<div class="dev-callout"><strong>Nota:</strong> 
    <p>Puesto que el token del dispositivo asignado por el Servicio de notificaciones de inserci&oacute;n de Apple (APNS) puede cambiar en cualquier momento, debe registrar las notificaciones con frecuencia para evitar errores de notificaci&oacute;n. En este ejemplo se registra la notificaci&oacute;n cada vez que se inicia la aplicaci&oacute;n. En las aplicaciones que se ejecutan con frecuencia, m&aacute;s de una vez al d&iacute;a, es posible que pueda omitir el registro para conservar el ancho de banda si pasa menos de un d&iacute;a del registro previo.</p>
</div>
