<properties 
	pageTitle="Conceptos clave de Análisis de transmisiones | Azure" 
	description="Obtenga información sobre los componentes clave de un trabajo de Análisis de transmisiones,lo que incluye entradas y salidas admitidas, detalles de configuración del trabajo y métricas expuestas." 
...
# Conceptos clave de Análisis de transmisiones de Azure 
...
Cada definición de trabajo de Análisis de transmisiones debe contener al menos un origen de entrada de streaming de datos para su uso y transformación por parte del trabajo. [Almacenamiento de blobs de Azure](azure.blob.storage) y [Centros de eventos de Azure](azure.event.hubs) se admiten como orígenes de entrada de streaming de datos. Los orígenes de entrada de Centros de eventos se usan para recopilar streaming de eventos procedentes de varios dispositivos y servicios diferentes, mientras que el almacenamiento de blobs se puede usar como origen de entrada para la introducción de grandes cantidades de datos. Como los blobs no transmiten datos, los trabajos de Análisis de transmisiones por blobs no serán de carácter temporal a menos que los registros del blob contengan marcas de tiempo.
...
### Serialización
Para garantizar el comportamiento correcto de las consultas, Stream Analytics debe conocer el formato de serialización que se utiliza en las secuencias de datos de entrada. Los formatos admitidos actualmente son JSON, CSV y Avro para los datos de streaming y CSV para los datos de referencia.
...
## Consultar
La lógica para filtrar, manipular y procesar los datos de entrada se define en la consulta de trabajos de Análisis de transmisiones. Las consultas se escriben utilizando el lenguaje de consulta de Análisis de transmisiones, un lenguaje parecido a SQL que es principalmente un subconjunto de sintaxis estándar de Transact-SQL con algunas extensiones específicas para consultas temporales.
