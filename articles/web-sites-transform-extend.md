...
Puede transformar el archivo [ApplicationHost.config](http://www.iis.net/learn/get-started/planning-your-iis-architecture/introduction-to-applicationhostconfig) en los sitios web de Azure mediante las declaraciones de [XML Document Transformation](http://msdn.microsoft.com/es-es/library/dd465326.aspx) (XDT). También puede usar las declaraciones XDT para agregar extensiones de sitios privados para habilitar acciones de administración del sitio personalizado. Este artículo incluye un sitio del administrador PHP de ejemplo que habilita la administración de la configuración de PHP a través de una interfaz web.
...
Para obtener ejemplos adicionales, consulte <https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions>.
...
**Nota**: la información adicional para estos elementos clave se encuentra disponible en <https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions>. Se incluye un ejemplo detallado para mostrar los pasos para la creación y habilitación de una extensión de sitio privado. El código fuente para el administrador PHP siguiente puede descargarse en <https://github.com/projectkudu/PHPManager>.
...
La única lógica especial necesaria para la E/S del archivo es indicar dónde se encuentra el directorio wwwroot del sitio. Puesto que se muestra el siguiente ejemplo de código, la variable de entorno "HOME" indica la ruta raíz del sitio y la ruta wwwroot puede construirse anexando "site\\wwwroot":
    ...
    /// <summary>
    /// Proporciona la ubicación del archivo .user.inim, incluso si no existe aún.
    /// </summary>
    private static string GetUserSettingsFilePath()
    {
            var rootPath = Environment.GetEnvironmentVariable("HOME"); // Para uso en Sitios web Azure
            if (rootPath == null)
            {
                rootPath = System.IO.Path.GetTempPath(); // Para propósitos de prueba
            };
	...
	<system.applicationHost>
             <sites>
                    <site name="%XDT_SCMSITENAME%" xdt:Locator="Match(name)">
                        <!-- NOTA: agregue el nombre de extensión en las rutas de aplicación a continuación. -->
                        <application path="/[nombre-de-extensión]" xdt:Locator="Match(path)" xdt:Transform="Remove" />
                        <application path="/[nombre-de-extensión]" applicationPool="%XDT_APPPOOLNAME%" xdt:Transform="Insert">
...
Esto provoca la suma de una nueva ruta de aplicación a la lista de sitios `system.applicationHost` en el sitio SCM. El sitio SCM es un punto final de administración que especifica credenciales de acceso. Dispone de la dirección URL `https://[nombre-de-sitio].scm.azurewebsites.net`.

    <system.applicationHost>
	...
                <bindings>
                    <binding protocol="http" bindingInformation="*:80: [sitio-web].scm.azurewebsites.net" />
                    <binding protocol="https" bindingInformation="*:443: [sitio-web].scm.azurewebsites.net" />
                </bindings>
                <traceFailedRequestsLogging enabled="false" directory="C:\DWASFiles\Sites\[sitio-web]\VirtualDirectory0\LogFiles" />
                <detailedErrorLogging enabled="false" directory="C:\DWASFiles\Sites\[sitio-web]\VirtualDirectory0\LogFiles\DetailedErrors" />
                <logFile logSiteId="false" />
                <application path="/" applicationPool="[sitio-web]">
                    <virtualDirectory path="/" physicalPath="D:\Program Files (x86)\SiteExtensions\Kudu.24.20926.5" />
                </application>
                <!-- Tenga en cuenta los cambios personalizados que van aquí. -->
                <application path="/[nombre-de-extensión]" applicationPool="[sitio-web]">
                    <virtualDirectory path="/" physicalPath="C:\DWASFiles\Sites\[sitio-web]\VirtualDirectory0\SiteExtensions\[nombre-de-extensión]" />
                </application>
            </site>
	</sites>
      ...
...
Para instalar la extensión del sitio, puede usar el FTP para copiar todos los archivos de la aplicación web en la carpeta `\SiteExtensions\[nombre-de-extensión]` del sitio en el que desee instalar la extensión. Asegúrese de copiar el archivo ApplicationHost.xdt en la ubicación también.
...
`https://[nombre-del-sitio].scm.azurewebsites.net/[nombre-de-la-extensión]`
