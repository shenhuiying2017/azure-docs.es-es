...
# <a name="accelerate-real-time-big-data-analytics-with-the-spark-to-documentdb-connector"></a>Aceleración de análisis de macrodatos en tiempo real con el conector de Spark a DocumentDB

El conector de Spark a DocumentDB permite que Azure DocumentDB actúe como un origen de entrada o un receptor de salida para trabajos de Apache Spark. La conexión de [Spark](http://spark.apache.org/) a [DocumentDB](https://azure.microsoft.com/services/documentdb/) acelera la capacidad de resolver problemas de ciencia de datos de avance rápido, donde los datos pueden persistir y consultarse con rapidez mediante DocumentDB. El conector de Spark a DocumentDB utiliza de forma eficaz los índices administrados nativos de DocumentDB y habilita las columnas actualizables al realizar analíticas, presionar el filtrado de predicados con respecto a los datos distribuidos globalmente sujetos a cambios rápidos, desde IoT y ciencia de datos hasta escenarios de análisis. 

## <a name="download"></a>Descargar

Comience con la descarga del conector de Spark a DocumentDB (versión preliminar) en el repositorio [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark/) de GitHub.

## <a name="connector-components"></a>Componentes del conector

El conector utiliza los siguientes componentes:

* [DocumentDB](http://documentdb.com), el [sistema de base de datos globalmente distribuido](documentdb-distribute-data-globally.md) y multiinquilino de Microsoft, diseñado para la nube. DocumentDB permite a los clientes aprovisionar y escalar el rendimiento y el almacenamiento de forma elástica a través de cualquier número de regiones geográficas. El servicio ofrece a los desarrolladores una latencia baja garantizada en un 99 %, una disponibilidad alta del 99,99 % garantizada y [varios modelos bien definidos de coherencia](documentdb-consistency-levels.md).
...
Existen dos enfoques para conectar Apache Spark y Azure DocumentDB:
...
El [SDK de pyDocumentDB](https://github.com/Azure/azure-documentdb-python) actual permite conectar Spark a DocumentDB como se muestra en el diagrama siguiente:

![Flujo de datos de Spark a DocumentDB a través de pyDocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-pydocumentdb.png)
...
La información importante es que la comunicación entre Spark y DocumentDB se limita al nodo principal de Spark y a los nodos de puerta de enlace de DocumentDB.  Las consultas van tan rápido como la capa de transporte entre estos dos nodos.
...
### <a name="connecting-spark-to-documentdb-via-pydocumentdb"></a>Conexión de Spark a DocumentDB a través de pyDocumentDB 
A cambio de la sencillez del transporte de comunicación, la ejecución de una consulta de Spark en DocumentDB con pyDocumentDB es relativamente sencilla.
...
La conexión de Spark a DocumentDB con pyDocumentDB suele utilizarse en escenarios en que:
...
## <a name="spark-to-documentdb-connector"></a>Conector de Spark a DocumentDB

El conector de Spark a DocumentDB usa el [SDK de Java para Azure DocumentDB](https://github.com/Azure/azure-documentdb-java) y transfiere los datos entre los nodos de trabajo de Spark y DocumentDB, tal y como se muestra en el diagrama siguiente:

![Flujo de datos del conector de Spark a DocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-connector.png)

El flujo de datos es el siguiente:

1. Una conexión se realiza desde el nodo principal de Spark hasta el nodo de puerta de enlace de DocumentDB para obtener el mapa de la partición.  Tenga en cuenta que el usuario especifica solo las conexiones de Spark y DocumentDB; el hecho de que se conecte con los nodos principal y de puerta de enlace correspondientes es transparente para el usuario.
2. Esta información se proporciona en el nodo principal de Spark.  En este momento, debe tener la capacidad de analizar la consulta para determinar a qué particiones (y sus ubicaciones) dentro de DocumentDB necesita acceder.
3. Esta información se transmite a los nodos de trabajo de Spark...
4. Esto permite a los nodos de trabajo de Spark conectarse directamente a las particiones de DocumentDB para extraer los datos necesarios y devolver los datos a las particiones de Spark dentro de los nodos de trabajo de Spark.

La información importante es que la comunicación entre Spark y DocumentDB es mucho más rápida porque la transferencia de datos se realiza entre los nodos de trabajo de Spark y los nodos de datos de DocumentDB (particiones).

### <a name="building-the-spark-to-documentdb-connector"></a>Creación del conector de Spark a DocumentDB
...
### <a name="connecting-spark-to-documentdb-using-the-connector"></a>Conexión de Spark a DocumentDB mediante el conector
Mientras que el transporte de comunicación es un poco más complicado, ejecutar una consulta de Spark en DocumentDB mediante el conector es una acción mucho más rápida.
...
### <a name="why-use-the-spark-to-documentdb-connector-implementation"></a>¿Por qué usar la implementación del conector de Spark a DocumentDB?

La conexión de Spark a DocumentDB con el conector suele utilizarse en escenarios en que:
...
* Tiene una gran cantidad de datos que se van a transferir entre Apache Spark y DocumentDB.
...
En esta sección se proporcionan algunos ejemplos de cómo se pueden realizar agregaciones distribuidas y análisis de forma conjunta con la utilización de Apache Spark y Azure DocumentDB.  Tenga en cuenta que Azure DocumentDB ya es compatible con agregaciones, como se describe en el [blog Planet scale aggregates with Azure DocumentDB](https://azure.microsoft.com/blog/planet-scale-aggregates-with-azure-documentdb/) (Agregaciones de escala mundial con Azure DocumentDB), por lo que se muestra cómo puede llevarlas al siguiente nivel con Apache Spark.

Tenga en cuenta que estas agregaciones guardan relación con el [cuaderno del conector de Spark a DocumentDB](https://github.com/Azure/azure-documentdb-spark/blob/master/samples/notebooks/Spark-to-DocumentDB_Connector.ipynb).

### <a name="connecting-to-flights-sample-data"></a>Conexión a los datos de ejemplo sobre vuelos
Para estos ejemplos de agregaciones, se accede a algunos datos de rendimiento sobre vuelos almacenados en la base de datos **DoctorWho** de DocumentDB.  Para conectarse a ella, debe utilizar el siguiente fragmento de código:
...
Con este fragmento de código, también se va a ejecutar una consulta base que transfiere el conjunto filtrado de datos deseado de DocumentDB a Spark (donde el último puede realizar agregaciones distribuidas).  En este caso, se pregunta por los vuelos que salen de Seattle (SEA).
...
En el conjunto siguiente, se van a ejecutar consultas **GROUP BY** con facilidad en la base de datos de DocumentDB:
...
Si aún no lo ha hecho, descargue el conector de Spark a DocumentDB del repositorio de GitHub [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark) y explore los recursos adicionales en el repositorio:
