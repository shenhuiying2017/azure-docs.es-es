...
---
# <a name="storage-and-predictable-performance-provisioning-in-azure-cosmos-db"></a>Almacenamiento y aprovisionamiento de rendimiento predecible en Azure Cosmos DB
Azure Cosmos DB es un servicio de bases de datos NoSQL orientado a documentos totalmente administrado y escalable para documentos JSON. Con Azure Cosmos DB, no tendrá que alquilar máquinas virtuales, implementar software ni supervisar bases de datos. Los ingenieros de Microsoft operan y supervisan de forma continua Azure Cosmos DB para ofrecer disponibilidad, rendimiento y protección de datos universales.  

Para comenzar con Azure Cosmos DB, [cree una cuenta de base de datos](documentdb-create-account.md) y, después, una [colección y una base de datos de Azure Cosmos DB](documentdb-create-collection.md) mediante [Azure Portal](https://portal.azure.com/). Las bases de datos de Azure Cosmos DB se ofrecen en unidades de rendimiento y almacenamiento respaldado de unidad de estado sólido (SSD). Estas unidades de almacenamiento se aprovisionan al crear colecciones de bases de datos, y cada colección tiene capacidad de proceso reservada que se puede escalar o reducir verticalmente en cualquier momento para satisfacer las demandas de la aplicación.
...
Como suscriptor de Azure, puede aprovisionar una o más cuentas de base de datos de Azure Cosmos DB para administrar los recursos de base de datos. Cada suscripción está asociada con una única suscripción de Azure.

Se pueden crear cuentas de Azure Cosmos DB mediante [Azure Portal](documentdb-create-account.md) o mediante [una plantilla de ARM o la CLI de Azure](documentdb-automation-resource-manager-cli.md).

## <a name="databases"></a>Bases de datos
Una única base de datos de Azure Cosmos DB puede contener una cantidad prácticamente ilimitada de almacenamiento de documentos agrupado por colecciones. Las colecciones proporcionan aislamiento del rendimiento; cada colección puede aprovisionarse con rendimiento que no se comparte con otras colecciones de la misma base de datos o cuenta. Una base de datos de Azure Cosmos DB es elástica en tamaño, comprendiendo desde unos pocos GB hasta TB de almacenamiento de documento respaldado y rendimiento aprovisionado de SSD. A diferencia de la base de datos RDBMS tradicional, una base de datos en Azure Cosmos DB no está dirigida a un único equipo y puede abarcar varios equipos o clústeres.  

Con Azure Cosmos DB, a medida que escala sus aplicaciones, podrá crear más colecciones o bases de datos o ambas. Las bases de datos se pueden crear a través de [Azure Portal](documentdb-create-database.md) o mediante cualquiera de los [SDK de Azure Cosmos DB](documentdb-dotnet-samples.md).   

## <a name="database-collections"></a>Colecciones de base de datos
Cada base de datos de Azure Cosmos DB puede contener una o más colecciones. Las colecciones funcionan como particiones de datos de alta disponibilidad para el procesamiento y almacenamiento de documentos. Cada colección puede almacenar documentos con esquema heterogéneo. Las capacidades de consulta e indexación automática de Azure Cosmos DB le permiten filtrar y recuperar documentos con facilidad. Una colección proporciona el ámbito del almacenamiento de documentos y la ejecución de consultas. Una colección también es un dominio de transacciones para todos los documentos contenidos en la misma. A las colecciones se les asigna capacidad de proceso en función del valor establecido en el Portal de Azure o mediante los SDK.
...
Las colecciones se pueden crear a través del [Azure Portal](documentdb-create-collection.md) o mediante cualquiera de los [SDK de DocumentDB](documentdb-sdk-dotnet.md).   

## <a name="request-units-and-database-operations"></a>Solicitud de unidades y las operaciones de base de datos
...
Hay varios factores que afectan a las unidades de solicitud usadas para una operación en una cuenta de base de datos de Azure Cosmos DB. Entre estos factores se incluyen:
...
Para más información, consulte [Unidades de solicitud de Azure Cosmos DB](documentdb-request-units.md).
...
Para obtener instrucciones sobre cómo cambiar el nivel de coherencia en Azure Portal, consulte [Administración de una cuenta de Azure Cosmos DB](documentdb-manage-account.md#consistency). O bien, para obtener más información sobre los niveles de coherencia, consulte [Uso de los niveles de coherencia](documentdb-consistency-levels.md).

## <a name="provisioned-document-storage-and-index-overhead"></a>Sobrecarga de índice y almacenamiento de documentos aprovisionados
Azure Cosmos DB admite la creación de colecciones de una sola partición y con varias particiones. Cada partición en Azure Cosmos DB admite hasta 10 GB de almacenamiento respaldado de SSD. Los 10 GB de almacenamiento de documentos incluyen más almacenamiento para el índice. De forma predeterminada, una colección de Azure Cosmos DB se configura para que indexe automáticamente todos los documentos sin solicitar de forma explícita ningún índice o esquema secundario. En función de las aplicaciones que usan Azure Cosmos DB, la sobrecarga de índice típica es de entre el 2 % y el 20 %. La tecnología de indexación que usa Azure Cosmos DB garantiza que, a pesar de los valores de las propiedades, la sobrecarga de índice no supera el 80 % del tamaño de los documentos con la configuración predeterminada.

De forma predeterminada, Azure Cosmos DB indexa todos los documentos automáticamente. Sin embargo, en caso de que desee ajustar la sobrecarga de índice, puede elegir eliminar determinados documentos de la indexación en el momento de insertar o reemplazar un documento, según lo descrito en [Directivas de indexación de Azure Cosmos DB](documentdb-indexing-policies.md). Puede configurar una colección de Azure Cosmos DB para que excluya todos los documentos de la colección de la indexación. También puede configurar una colección de Azure Cosmos DB para indexar de manera selectiva solo ciertas propiedades o rutas de acceso con caracteres comodín de los documentos JSON, tal como se describe en la [configuración de la directiva de indexación de una colección](documentdb-indexing-policies.md#CustomizingIndexingPolicy). Excluir propiedades o documentos también mejora la capacidad de proceso de escritura; lo que significa que consumirá menos unidades de solicitud.   

## <a name="next-steps"></a>Pasos siguientes
Para continuar aprendiendo acerca del funcionamiento de Azure Cosmos DB, consulte [Partición y escalado en Azure Cosmos DB](documentdb-partition-data.md).

Para obtener instrucciones sobre cómo supervisar los niveles de rendimiento en Azure Portal, consulte [Supervisión de una cuenta de Azure Cosmos DB](documentdb-monitor-accounts.md). Para obtener más información sobre la elección de los niveles de rendimiento para las colecciones, consulte [Niveles de rendimiento en Azure Cosmos DB](documentdb-performance-levels.md).
