...
***Actualización del 2 de abril de 2014.***
...
4.  En la lista desplegable **Base de datos**, elija **Crear una base de datos SQL gratuita**.

5.  En la lista desplegable **Región**, elija la misma región que seleccionó para el sitio web.
    Este valor especifica el centro de datos en el que se ejecutará la máquina virtual.
6.  En el cuadro **Nombre de la cadena de conexión de BD**, no modifique el valor predeterminado *DefaultConnection*.
7.  Haga clic en la flecha que apunta hacia la derecha en la parte inferior del cuadro.
    El asistente avanza hasta el paso **Especificar configuración de base de datos**.

8.  En el cuadro **Nombre**, escriba *ContactDB*. (consulte la imagen que aparece a continuación).
9.  En el cuadro **Servidor**, seleccione **Nuevo servidor de bases de datos SQL** (consulte la imagen que aparece a continuación). Si anteriormente creó una base de datos de SQL Server, también tiene la opción de seleccionar ese SQL Server en el control desplegable.
...
8.  Presione CTRL + F5 para ejecutar la aplicación y haga clic en el enlace **CM Demo**; o bien navegue a http://localhost:(port\#)/Cm.
...
1.  Siga las instrucciones del tutorial [Creación de una aplicación de ASP.NET MVC 5 con Facebook y Google OAuth2 inicio de sesión OpenID][Create an ASP.NET MVC 5 App with Facebook and Google OAuth2 and OpenID Sign-on] en **Creación de una aplicación de Google para OAuth 2 para configurar una aplicación de Google para OAuth2**.
2.  Abra el archivo *App\_Start\\Startup.Auth.cs*. Quite los caracteres de comentario del método *app.UseGoogleAuthentication()* y escriba los valores de **clientId** y **clientSecret**.
...
7.  Inicie sesión como *user1@contoso.com* con la contraseña "P\_assw0rd1" (el "0" en "word" es un cero). Se le redirigirá a la página de edición que seleccionó anteriormente.
...
1.  En Visual Studio, haga clic con el botón secundario en el proyecto, en el **Explorador de soluciones**, y seleccione **Publicar** en el menú contextual.
    ...
    ![Opción Publicar del menú contextual del proyecto][7]
