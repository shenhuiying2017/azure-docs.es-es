...
3. Cree una nueva salida seleccionando **New Stream** (Nueva transmisión) y, a continuación, haga clic en **Add Output** (Agregar salida).
...
4. Ahora que se creó la salida, haga clic en **Add Stream** (Agregar transmisión). Ahora pueden configurarse las opciones de salida.
5. Desplácese hasta la transmisión "Stream 1" que acaba de crear, haga clic en la pestaña **Video** a la izquierda y expanda la sección de configuración **Advanced** (Avanzada). 
...
6. Obtenga la dirección URL de entrada del canal.
	...
	Vaya de nuevo a la herramienta AMSE y compruebe el estado de finalización del canal. Una vez que se cambió el estado de **Iniciando** a **En ejecución**, puede obtener la dirección URL de entrada.
...
7. Haga clic en **Crear** (si se creó un nuevo evento) o **Actualizar** (si está modificando un evento ya existente) y, a continuación, inicie el codificador.

>[AZURE.IMPORTANT]Antes de hacer clic en **Start** (Iniciar) en la interfaz web de Elemental Live, **debe** asegurarse de que el canal está listo. Además, asegúrese de no dejar el canal en un estado Listo sin un evento durante más de 15 minutos.
...
6. Una vez confirmado, haga clic con el botón derecho de nuevo en el programa y seleccione **Copiar la URL de salida en el Portapapeles** (o recupere esta información de la opción **Información y configuración del programa** en el menú). 
