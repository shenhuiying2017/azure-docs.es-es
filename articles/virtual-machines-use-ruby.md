<properties urlDisplayName="What is Azure Multi-Factor Authentication?" pageTitle="&iquest;Qu&eacute; es Azure Multi-Factor Authentication?" metaKeywords="" description="Obtenga m&aacute;s informaci&oacute;n acerca de Azure Multi-Factor Authentication, un m&eacute;todo de autenticaci&oacute;n que requiere el uso de m&aacute;s de un m&eacute;todo de verificaci&oacute;n y agrega una segunda capa importante de seguridad a las transacciones e inicios de sesi&oacute;n del usuario." metaCanonical="" services="active-directory,multi-factor-authentication" documentationCenter="" title="Administraci&oacute;n de M&aacute;quinas virtuales de Azure usando Ruby" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
-   [Conceptos][Conceptos]
-   [Creación de un certificado de administración][Creación de un certificado de administración]
-   [Creación de una aplicación de Ruby][Creación de una aplicación de Ruby]
-   [Configuración de la aplicación para que use el SDK][Configuración de la aplicación para que use el SDK]
-   [Configuración de una conexión de administración de Azure][Configuración de una conexión de administración de Azure]
-   [Eliminación de Máquinas virtuales][Eliminación de Máquinas virtuales]
-   [Eliminación de imágenes y discos][Eliminación de imágenes y discos]
-   [Eliminación de Servicios en la nube][Eliminación de Servicios en la nube]
-   [Eliminación de servicios de almacenamiento][Eliminación de servicios de almacenamiento]
-   [Pasos siguientes][Pasos siguientes]

## <a name="what-is"> </a>Qué es la administración de servicios

Azure proporciona las [API REST para las operaciones de administración de servicios][API REST para las operaciones de administración de servicios], entre las que se incluyen la administración de Máquinas virtuales de Azure. El SDK de Azure para Ruby expone operaciones de administración para Máquinas virtuales mediante la clase **Azure::VirtualMachineSerivce**. Se puede obtener acceso a gran parte de la funcionalidad de administración de máquinas virtuales en el [Portal de administración de Azure][Portal de administración de Azure] usando esta clase.
...
2.  Para exportar el certificado de administración, use el comando siguiente:
    ...
    Cuando el comando finalice, el certificado se exportará a un archivo llamado \<azure-subscription-name\>.pem. Por ejemplo, si la suscripción se llama **mygreatsubscription**, el archivo creado se llamará **mygreatsubscription.pem**.
...
## <a name="virtual-machine"> </a>Direccionamiento del Máquinas virtuales
...
### Reproducción de Crear una máquina virtual

Para crear una máquina virtual, use el método **create\_virtual\_machine**. Este método acepta un hash que contiene los parámetros siguientes y devuelve una instancia de **Azure::VirtualMachineManagement::VirtualMachine** que describe la máquina virtual creada:
...
A continuación encontrará las opciones disponibles al usar el método **create\_virtual\_machine**:

-   **:storage\_account\_name**: Nombre de la cuenta de almacenamiento que se va a utilizar para almacenar imágenes de disco. Si la cuenta de almacenamiento no existe todavía, se crea una nueva. Si se omite, se crea una nueva cuenta de almacenamiento con un nombre basado en el parámetro :vm\_name.

-   **:cloud\_service\_name**: Nombre del servicio en la nube que hay que usar para hospedar la máquina virtual. Si el servicio en la nube no existe todavía, se crea uno nuevo. Si se omite, se crea una nueva cuenta de servicio en la nube con un nombre basado en el parámetro :vm\_name.
...
-   **:vm\_size**: Tamaño de la máquina virtual. Determina el tamaño de la memoria, el número de núcleos, el ancho de banda y otras características físicas de la máquina virtual. Consulte [Tamaños de máquinas virtuales y servicios en la nube de Azure][Tamaños de máquinas virtuales y servicios en la nube de Azure] para ver las características físicas y los tamaños disponibles.

-   **:winrm\_transport**: Matriz de transportes disponible para su uso con WinRM. Los transportes válidos son http y https. Si se especifica "https" como transporte, también debe usar **:ssh\_private\_key\_file** y **:ssh\_certificate\_file** para especificar el certificado usado para proteger las comunicaciones HTTPS.
...
### Reproducción de máquinas virtuales

Para enumerar las máquinas virtuales existentes de la suscripción de Azure, use el método **list\_virtual\_machines**. Este método devuelve una matriz de objetos **Azure::VirtualMachineManagement::VirtualMachine**:

    vm_mgr = Azure::VirtualMachineService.new
    virtual_machines = vm_mgr.list_virtual_machines

### Reproducción de información acerca de una máquina virtual
...
### Reproducción de una máquina virtual

Para eliminar una máquina virtual, use el método **delete\_virtual\_machine** y proporcione los nombres de la máquina virtual y del servicio en la nube:
...
**Advertencia**
El método **delete\_virtual\_machine** elimina el servicio en la nube y todos los discos asociados a la máquina virtual.

</div>

### Reproducción de una máquina virtual

Para apagar una máquina virtual, use el método **shutdown\_virtual\_machine** y proporcione los nombres de la máquina virtual y del servicio en la nube:
...
Para iniciar una máquina virtual, use el método **start\_virtual\_machine** y proporcione los nombres de la máquina virtual y del servicio en la nube:

    vm_mgr = Azure::VirtualMachineService.new
    vm = vm_mgr.start_virtual_machine('myvm', 'mycloudservice')

## <a name="vm-images"> </a>Direccionamiento del discos e imágenes de la máquina virtual
...
### Reproducción de imágenes de máquinas virtuales

Para enumerar las imágenes de las máquinas virtuales disponibles, use el método **list\_virtual\_machine\_images**. Este método devuelve una matriz de objetos **Azure::VirtualMachineImageService**.
...
Para enumerar los discos de la suscripción de Azure, use el método **list\_virtual\_machine\_disks**. Este método devuelve una matriz de objetos **Azure::VirtualMachineImageManagement::VirtualMachineDisk**.
...
Para eliminar un disco, use el método **delete\_virtual\_machine\_disk** y especifique el nombre del disco que desee eliminar:

    disk_mgr = Azure::VirtualMachineImageManagement::VirtualMachineDiskManagementService.new
    disk_mgr.delete_virtual_machine_disk

## <a name="cloud-services"> </a>Direccionamiento del Servicios en la nube

Las operaciones de administración de Servicios en la nube de Azure se realizan usando la clase **Azure::CloudService**.

### Reproducción de un servicio en la nube

Para crear un servicio en la nube, use el método **create\_cloud\_service** y proporcione un nombre y un hash de opciones. Las opciones válidas son:
...
### Reproducción de servicios en la nube

Para enumerar los servicios en la nube de la suscripción de Azure, use el método **list\_cloud\_services**. Este método devuelve una matriz de objetos **Azure::CloudServiceManagement::CloudService**:

    cs_mgr = Azure::CloudService.new
    cloud_services = cs_mgr.list_cloud_services

### Reproducción de la existencia de un servicio en la nube
...
Para eliminar un servicio en la nube, use el método **delete\_cloud\_service** y proporcione el nombre del servicio en la nube:

    cs_mgr = Azure::CloudService.new
    cs_mgr.delete_cloud_service('mycloudservice')

### Reproducción de una implementación

Para eliminar la implementación de un servicio en la nube, use el método **delete\_cloud\_service\_deployment** y proporcione el nombre del servicio en la nube:

    cs_mgr = Azure::CloudService.new
    cs_mgr.delete_cloud_service_deployment('mycloudservice')

## <a name="storage-services"> </a>Direccionamiento del servicios de almacenamiento

Las operaciones de administración de Servicios en la nube de Azure se realizan usando la clase **Azure::StorageService**.

### Reproducción de Crear una cuenta de almacenamiento

Para crear una cuenta de almacenamiento, use el método **create\_storage\_account** y proporcione un nombre y un hash de opciones. Las opciones válidas son:
...
### Reproducción de cuentas de almacenamiento

Para obtener una lista con las cuentas de almacenamiento de la suscripción de Azure, use el método **list\_storage\_accounts**. Este método devuelve una matriz de objetos **Azure::StorageManagement::StorageAccount**:

    storage_mgr = Azure::StorageService.new
    accounts = storage_mgr.list_storage_accounts

### Reproducción de la existencia de una cuenta de almacenamiento

Para ver si existe una cuenta de almacenamiento concreta, use el método **get\_storage\_account** y especifique el nombre de la cuenta de almacenamiento. Este método devuelve **true** si ya existe la cuenta de almacenamiento o **false** si no existe.

    storage_mgr = Azure::StorageService.new
    store_exists = storage_mgr.get_storage_account('mystorage')

### Reproducción de una cuenta de almacenamiento
...
-   Visite la página de características de las [máquinas virtuales][máquinas virtuales].
-   Consulte la referencia de MSDN: [Máquinas virtuales][Máquinas virtuales]
-   Más información acerca de cómo hospedar una [Aplicación de Ruby on Rails en una máquina virtual][Aplicación de Ruby on Rails en una máquina virtual]
  ...
  [Eliminación de Máquinas virtuales]: #virtual-machine
  [Eliminación de imágenes y discos]: #vm-images
  [Eliminación de Servicios en la nube]: #cloud-services
  [Eliminación de servicios de almacenamiento]: #storage-services
  [Pasos siguientes]: #next-steps
  [API REST para las operaciones de administración de servicios]: http://msdn.microsoft.com/es-es/library/windowsazure/ee460799.aspx
  [Portal de administración de Azure]: https://manage.windowsazure.com
  [Instalación y configuración de la interfaz de línea de comandos entre plataformas de Azure]: http://www.windowsazure.com/es-es/manage/install-and-configure-cli/
  [Tamaños de máquinas virtuales y servicios en la nube de Azure]: http://msdn.microsoft.com/es-es/library/windowsazure/dn197896.aspx
  [máquinas virtuales]: http://www.windowsazure.com/es-es/documentation/services/virtual-machines/
  [Máquinas virtuales]: http://msdn.microsoft.com/es-es/library/windowsazure/jj156003.aspx
  [Aplicación de Ruby on Rails en una máquina virtual]: http://www.windowsazure.com/es-es/develop/ruby/tutorials/web-app-with-linux-vm/
