...
## <a id="subheading-3"></a>Prácticas para el uso en producción
...
En el entorno de producción, es probable que necesite auditar todo el tráfico a la base de datos desde todas las aplicaciones y herramientas. Para eso, modifique **Acceso habilitado por seguridad** de *Optional* a *Required* y guarde la directiva. Una vez que se configure *Required*, no habrá opción de tener acceso a la base de datos a través de la cadena de conexión original, sino solo a través de la cadena de conexión habilitada por seguridad.
...
1. En el cuadro de configuración de auditoría (descrito anteriormente en la sección de auditoría de configuración), cambie la **Clave de acceso de almacenamiento** de *Primary* a *Secondary* y presione **Guardar**.

![][10]

2. Vaya a la tarjeta de configuración de almacenamiento y **genere de nuevo** la *Primary Access Key*.

3. Vuelva al módulo de configuración de auditoría, cambie la **Clave de acceso de almacenamiento** de *Secondary* a *Primary* y presione **Guardar**.

4. Vuelva a la interfaz de usuario de almacenamiento y **genere de nuevo** la *Secondary Access Key* (como preparación para el siguiente ciclo de actualización de las claves).
...
> [AZURE.NOTE] El módulo AzureResourceManager está actualmente en vista previa. Puede que no ofrezca las mismas capacidades de administración que el módulo de Azure.

 Para acceder al modo [Administrador de recursos de Azure](https://msdn.microsoft.com/library/dn654592.aspx) ejecute el cmdlet Switch-AzureMode (`Switch-AzureMode AzureResourceManager`). Cuando esté en el modo de Administrador de recursos de Azure, ejecute `Get-Command *AzureSql*` para enumerar los cmdlets disponibles.
...
[Configuración de la auditoría para su base de datos con el Portal de Azure clásico]: #subheading-4
