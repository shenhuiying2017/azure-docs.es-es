---
title: Personalización de la plantilla de Resource Manager exportada | Microsoft Docs
...
---
# Personalización de una plantilla de Azure Resource Manager exportada
En este artículo se muestra cómo modificar una plantilla exportada para permitir que pase valores adicionales como parámetros. Se basa en los pasos realizados en el artículo [Exportación de plantillas de Azure Resource Manager desde recursos existentes](resource-manager-export-template.md), aunque no es esencial que complete primero ese artículo. Puede encontrar la plantilla y los scripts que necesita en este artículo.
...
Si no ha completado el tutorial anterior, cree un archivo denominado **template.json** y agregue el siguiente contenido de la plantilla exportada al archivo.
...
La plantilla template.json funciona bien si desea crear el mismo tipo de cuenta de almacenamiento en la misma región con una red virtual que usa el mismo prefijo de dirección y el mismo prefijo de subred para cada implementación. No obstante, Resource Manager proporciona opciones para que pueda implementar plantillas con mucha más flexibilidad. Por ejemplo, durante la implementación, puede especificar el tipo de cuenta de almacenamiento que desea crear o los valores que prefiere para el prefijo de la dirección de red virtual y de la subred.
...
El archivo de parámetros descargado ya no coincide con los parámetros de su plantilla. No es necesario utilizar un archivo de parámetros, aunque puede simplificar el proceso al volver a implementar un entorno. Usará los valores predeterminados definidos en la plantilla para muchos de los parámetros, por lo que el archivo de parámetros solo necesita dos valores.
...
Puede usar Azure PowerShell o la interfaz de la línea de comandos (CLI) de Azure para implementar la plantilla personalizada y los archivos de parámetros. Si es necesario, instale [Azure PowerShell](powershell-install-configure.md) o la [CLI de Azure](xplat-cli-install.md). Puede usar los scripts que descargó con la plantilla cuando exportó la plantilla original o escribir el suyo propio para implementar la plantilla. En este artículo se muestran ambas opciones.

1. Para implementarla con su propio script, use uno de los siguientes.
...
2. Si ha descargado la plantilla y los scripts exportados, busque el archivo **deploy.ps1** (para PowerShell) o **deploy.sh** (para la CLI de Azure).
...
## Pasos siguientes
* El [Tutorial de la plantilla de Resource Manager](resource-manager-template-walkthrough.md) se basa en lo que ha aprendido en este artículo y en él se crea una plantilla para una solución más complicada. Le ayuda a comprender mejor los recursos que están disponibles y cómo determinar los valores que se deben proporcionar.
