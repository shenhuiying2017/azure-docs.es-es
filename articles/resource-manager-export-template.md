...
Entender cómo se crean las plantillas de Azure Resource Manager puede resultar un tanto desalentador. Afortunadamente, Resource Manager le ayuda con esa tarea porque puede exportar una plantilla de recursos existentes en su suscripción. Puede usar esa plantilla generada para aprender sobre la sintaxis de plantillas o para automatizar la nueva implementación de su solución según sea necesario.
...
4. El Administrador de recursos recupera los cinco archivos siguientes para usted:
   ...
   - La plantilla que define la infraestructura de la solución. Cuando creó la cuenta de almacenamiento por medio del portal, Resource Manager usó una plantilla para implementarla y la guardó para futura referencia.

   - Un archivo de parámetros que puede usar para pasar valores durante la implementación. Contiene los valores que proporcionó durante la primera implementación, pero puede cambiar cualquiera de ellos al volver a implementar la plantilla.

   - Un archivo de script de Azure PowerShell que puede usar para implementar la plantilla.

   - Un archivo de script de la interfaz de la línea de comandos (CLI) de Azure que puede usar para implementar la plantilla.

   - Una clase .NET que puede utilizar para implementar la plantilla.
     ...
     Observe que la plantilla define los parámetros para el nombre de la cuenta de almacenamiento, el tipo y la ubicación. Un parámetro también indica si está habilitado el cifrado y el valor predeterminado es **false**. En la sección **resources**, verá la definición de la cuenta de almacenamiento que se implementará.

Los corchetes contienen una expresión que se evalúa durante la implementación. Las expresiones entre corchetes mostradas de la plantilla sirven para obtener valores de parámetros durante la implementación. Puede usar muchas más expresiones y verá ejemplos de ellas más adelante en este artículo. Para ver la lista completa, consulte [Funciones de plantilla de Azure Resource Manager](resource-group-template-functions.md).
...
1. En la hoja del grupo de recursos, seleccione **Agregar** y después **red virtual** entre los recursos disponibles.
