...
1. Haga clic en **File (Archivo) > New (Nuevo) > Dynamic Web Project (Proyecto web dinámico)**. Asígnele el nombre `JSPHello`. No es necesario cambiar ninguna otra configuración en este cuadro de diálogo. Haga clic en **Finalizar**
    ...
    ![][3]
2. En el Explorador de proyectos, expanda el proyecto **JSPHello**, haga clic con el botón derecho en **WebContent**. A continuación, haga clic en **New (Nuevo) > JSP File (Archivo JSP)**. En el cuadro de diálogo Nuevo archivo JSP, asigne al archivo el nombre `index.jsp`. Haga clic en **Next**.
