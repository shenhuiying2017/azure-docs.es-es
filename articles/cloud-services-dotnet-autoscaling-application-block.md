...
1.  Abra la ventana **NuGet Package Manager Console**. En el menú **Tools**, seleccione **Library Package Manager** y, a continuación, **Package Manager Console**.

2.  Escriba el siguiente comando en la ventana NuGet Package Manager Console:
...
Su proyecto debe tener como destino .NET Framework 4. Para cambiar o verificar el marco de destino:

1.  En el Explorador de soluciones, haga clic con el botón secundario en el nombre del proyecto y seleccione **Properties**.
...
Use el método **IServiceLocator.GetInstance** para crear una instancia de autoescala y, a continuación, llame al método **Autoscaler.Start** para ejecutar **Autoscaler**.
...
-   **[subscriptionid]:** El ID único de la suscripción de Azure que contiene la aplicación en la cual desea usar la autoescala.
    ...
    2.  En la lista de Servicios en la nube, haga clic en el servicio que hospeda la aplicación en la cual desea usar la autoescala. El panel Quick Glance de la derecha muestra **Subscription ID**.
    ...
    2.  En la lista de cuentas de almacenamiento, seleccione la cuenta de almacenamiento que está usando. La columna **Name** muestra el **nombre**.

    3.  Haga clic en el botón **Manage Keys** en la parte inferior de la pantalla para obtener la clave de acceso primaria.
    ...
    1.  En el Portal de administración de Azure, haga clic en **Settings**.

    2.  La columna **Thumbprint** muestra la **huella digital**.

        ![imagen](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling09.png)
...
El siguiente código de ejemplo muestra un conjunto de reglas de ejemplo en un archivo **rules.xml**:
    ...
    <rules xmlns="http://schemas.microsoft.com/practices/2011/entlib/autoscaling/rules">
      <constraintRules>
        <rule name="default" enabled="true" rank="1" description="La regla de restricción predeterminada">
          <actions>
      ...
      <reactiveRules>
        <rule name="ScaleUpOnHighUtilization" rank="10" description="Escalar verticalmente el rol web" habilitado="true" >
          <when>
            <any>
        ...
        <rule name="ScaleDownOnLowUtilization" rank="10" description="Escalar verticalmente el rol web" habilitado="true" >
          <when>
            <all>
...
1.  Haga clic con el botón secundario en el archivo **App.config** en el Explorador de soluciones y, a continuación, haga clic en **Edit Configuration File**.

2.  En el menú **Blocks**, haga clic en **Add Autoscaling Settings**:
     ![imagen](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling10.png)

3.  Expanda **Autoscaling Settings** y, a continuación, haga clic en los puntos suspensivos (...) junto a **Data Points Store Storage Account**; agregue el **nombre de cuenta** y la **clave de cuenta** de la cuenta de almacenamiento de Azure donde el bloque va a almacenar los puntos de datos que recopila (consulte [Definición de su modelo de servicio][] si no está seguro de dónde puede encontrar estos valores) y, a continuación, haga clic en **OK**:

    ![imagen](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling11.png)

4.  Expanda la sección **Autoscaling Settings** para mostrar las secciones **Rules Store** y **Service Information Store**. De manera predeterminada, están configuradas para usar el almacenamiento de blobs de Azure:
     ![imagen](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling12.png)

5.  Haga clic en el signo más (+) junto a **Rules Store**, seleccione **Set Rules Store**, haga clic en **Use Local File Rules Store** y, a continuación, haga clic en **Yes**.

6.  En el cuadro **File Name**, escriba **rules.xml**. Este es el nombre del archivo que contiene sus reglas de autoescala:
     ![imagen](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling13.png)

7.  Haga clic en el signo más (+) junto a **Service Information Store**, seleccione **Set Service Information Store**, haga clic en **Use Local File Service Information Store** y, a continuación, haga clic en **Yes**.

8.  En el cuadro **File Name**, escriba **services.xml**. Este es el nombre del archivo que contiene sus reglas de autoescala:
     ![imagen](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling14.png)

9.  En la ventana de configuración de las bibliotecas empresariales, en el menú **File**, haga clic en **Save** para guardar los cambios realizados en la configuración. Posteriormente, en la ventana de la configuración de las bibliotecas empresariales, en el menú **File**, haga clic en **Exit**.
...
1.  En Visual Studio, haga doble clic en el archivo **App.config** en el Explorador de soluciones para abrirlo en el editor. Posteriormente, agregue la sección **system.diagnostics** como se muestra en el siguiente ejemplo:
        ...
        <configuration>
          ...
          <system.diagnostics>
            <sources>
              <sourcename="Autoescala general" switchName="SourceSwitch" switchType="System.Diagnostics.SourceSwitch" />
              <sourcename="Actualizaciones de autoescala" switchName="SourceSwitch" switchType="System.Diagnostics.SourceSwitch" />
            </sources>
            <switches>
...
Ahora puede ejecutar su aplicación de consola de host del bloque de autoescala de la aplicación y observar cómo las reglas de autoescala funcionan con su aplicación de Azure de destino. Al ejecutar la aplicación de consola de host, debe ver mensajes similares a los siguientes en la ventana de salida de Visual Studio. Estos mensajes de registro le ayudan a comprender el comportamiento del bloque. Por ejemplo, indican las reglas que el bloque está combinando y las acciones que el bloque está realizando.
    ...
    Detalle de la autoescala: 1002 : Coincidencia de regla.
    [BEGIN DATA]{"EvaluationId":"6b27dfa0-b671-44a3-adf1-bb1e0b7c3726",
    "MatchingRules":[{"RuleName":"default","RuleDescription":"La regla de restricción predeterminada",
    "Targets":["AutoscalingApplicationRole"]},{"RuleName":"ScaleUp","RuleDescription":"Escalar horizontalmente el rol web",
    "Targets":[]},{"RuleName":"ScaleDown","RuleDescription":"Escalar horizontalmente el rol web","Targets":[]}]}

    Detalle de actualizaciones de autoescala: 3001 : La configuración de implementación actual para un servicio hospedado está a punto de comprobarse
    para determinar si se requiere un cambio (para el escalado del rol o cambios en la configuración).
    [BEGIN DATA]{"EvaluationId":"6b27dfa0-b671-44a3-adf1-bb1e0b7c3726",
    ...
    Detalle de actualizaciones de autoescala: 3003 : Están a punto de enviarse las solicitudes de escalado del rol para el servicio hospedado.
    [BEGIN DATA]{"EvaluationId":"6b27dfa0-b671-44a3-adf1-bb1e0b7c3726",
    "HostedServiceDetails":{"Subscription":"AutoscalingHowTo","HostedService":"autoscalingtarget",
    ...
    Información sobre actualizaciones de autoescala: 3002 : Se enviaron los cambios de configuración del rol para la implementación.
    [BEGIN DATA]{"EvaluationId":"6b27dfa0-b671-44a3-adf1-bb1e0b7c3726",
    "HostedServiceDetails":{"Subscription":"AutoscalingHowTo","HostedService":"autoscalingtarget",
    "DeploymentSlot":"Staging"},"ScaleRequests":{"AutoscalingApplicationRole":{"Min":2,"Max":6,"AbsoluteDelta":0,
    "RelativeDelta":0,"MatchingRules":"default"}},"SettingChangeRequests":{},
