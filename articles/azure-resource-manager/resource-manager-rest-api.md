...
El token de acceso que ha recibido en la llamada HTTP anterior debe pasarse para todas las solicitudes a la API de ARM como encabezado llamado "Autorización" con el valor "Bearer YOUR_ACCESS_TOKEN". Observe el espacio entre "Bearer" y el token de acceso.
...
### <a name="deploy-resources-to-a-resource-group-using-an-arm-template"></a>Implementación de recursos en un grupo de recursos mediante una plantilla de ARM
