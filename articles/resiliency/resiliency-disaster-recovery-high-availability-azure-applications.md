<properties
   pageTitle="Recuperación ante desastres y alta disponibilidad para aplicaciones creadas en Microsoft Azure | Microsoft Azure"
...
##Introducción
Este artículo se centra en la alta disponibilidad (HA) para aplicaciones que se ejecutan en Azure. Una estrategia global para la alta disponibilidad abarca también el área de la recuperación ante desastres (DR). Para elaborar planes ante errores y desastres en la nube, es necesario reconocer los errores con rapidez. A continuación, se implementa una estrategia que se adapte al tiempo de inactividad de la aplicación que se pueda tolerar. Además, se debe considerar el grado de pérdida de datos que la aplicación puede admitir sin causar repercusiones negativas para la empresa cuando se restauren.

Cuando preguntamos a los clientes si están preparados para errores temporales y a gran escala, la mayoría dice estarlo. Sin embargo, antes de contestar esa pregunta, piense en las siguientes: ¿ensayan en su empresa estos errores? ¿Han probado la recuperación de bases de datos para asegurarse de que estén instaurados los procesos correctos? Lo más probable es que no lo hayan hecho. Eso es porque una recuperación ante desastres satisfactoria comienza con gran cantidad de planeamiento y diseño para implementar estos procesos. Al igual que muchos otros requisitos no funcionales, como la seguridad, la recuperación ante desastres no consigue casi nunca el análisis inicial y la asignación de tiempo que realmente requiere. Además, la mayoría de los clientes carecen de presupuesto para regiones distribuidas geográficamente con capacidad redundante. Por lo tanto, es frecuente que incluso las aplicaciones críticas queden excluidas de un planeamiento satisfactorio de la recuperación ante desastres.
...
##Características de las aplicaciones en la nube resistentes
Una aplicación bien diseñada puede resistir errores de capacidad en un nivel táctico y también tolerar errores estratégicos en todo el sistema en el nivel de región. En las secciones siguientes, se define la terminología que se utiliza en todo el documento para describir los diversos aspectos de los servicios en la nube resistentes.
...
1. Utilice esta información para definir el RTO y RPO para cada aplicación.
1. Diseñe teniendo en cuenta un error potencial, empezando por la arquitectura de la aplicación.
1. Implemente procedimientos recomendados para la alta disponibilidad, al mismo tiempo que equilibra el costo, la complejidad y el riesgo.
1. Implemente planes y procesos de recuperación ante desastres.
...
1. Defina un propietario específico para los procesos, la automatización y las pruebas de la recuperación ante desastres. El propietario debe administrar todo el proceso y responsabilizarse de él.
1. Documente los procesos de modo que se puedan repetir con facilidad. Aunque existe un propietario, debería haber varias personas que entiendan y puedan seguir los procesos en caso de emergencia.
1. Entrene al personal para implementar el proceso.
1. Realice con regularidad simulaciones de desastres, tanto de entrenamiento como para validación del proceso.

##Resumen
Cuando se produce un error en el hardware o las aplicaciones dentro de Azure, las técnicas y estrategias para su administración son diferentes a las empleadas cuando el error se produce en sistemas locales. La razón principal es que las soluciones en la nube suelen poseer más dependencias de infraestructura que está distribuida por una región de Azure y se administran como servicios independientes. Debe tratar los errores parciales por medio de técnicas de alta disponibilidad. Para administrar los errores más graves, posiblemente causados por un desastre, utilice estrategias de recuperación ante desastres.
...
[Best Practices for the design of large-scale services on Azure Cloud Services](https://azure.microsoft.com/blog/best-practices-for-designing-large-scale-services-on-windows-azure/) (Procedimientos recomendados para el diseño de servicios a gran escala en Servicios en la nube de Azure)
