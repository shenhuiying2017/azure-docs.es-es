...
Cuando se compila una configuración de DSC, se generan una o más configuraciones de nodo, dependiendo de los bloques de nodo en la configuración. Una configuración de nodo es lo mismo que un "MOF" o "documento de configuración" (si está familiarizado con esos términos de DSC de PS) y representa un "rol", como servidor web o un trabajador, cuyo estado deseado deben asumir uno o más nodos o se debe comprobar que lo cumplen. Los nombres de las configuraciones de nodo en DSC de Azure Automation adoptan la forma de "Configuration Name.NodeConfigurationBlockName".
...
DSC de Azure Automation facilita la incorporación de nodos para su administración y permite cambiar la configuración de los nodos asignada al lado servidor de cada nodo, de modo que la próxima vez que un nodo busque en el servidor instrucciones, asumirá un rol distinto y cambiará el modo en que está configurado y el estado de cumplimiento que debe notificar para que coincida.
