...
Los runbooks secundarios no se devuelven al runbook primario de manera segura debido al tiempo. Además, ciertas variables como $VerbosePreference, $WarningPreference, entre otras, no se pueden propagar a los runbooks secundarios. Para evitar estos problemas, puede invocar los runbooks secundarios como trabajos de Automation independientes mediante el uso del cmdlet `Start-AzureRmAutomationRunbook` con el modificador `-Wait`. Esto bloquea el runbook primario hasta que se completa el runbook secundario.

Si no quiere bloquear el runbook primario durante la espera, puede invocar al runbook secundario mediante el cmdlet `Start-AzureRmAutomationRunbook` sin el modificador `-Wait`. Luego tendría que usar `Get-AzureRmAutomationJob` para esperar a que se complete el trabajo y `Get-AzureRmAutomationJobOutput` y `Get-AzureRmAutomationJobOutputRecord` para recuperar los resultados.
