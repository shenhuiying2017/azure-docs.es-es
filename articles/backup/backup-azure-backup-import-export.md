...
4. Prepare una *ubicación de ensayo* que podría ser un recurso compartido de red o la unidad de disco adicional en la máquina. Asegúrese de que la ubicación de ensayo tiene suficiente espacio en disco para almacenar la copia inicial. Por ejemplo, si intenta realizar una copia de seguridad en un servidor de archivos de 500GB, asegúrese de que el área de ensayo es de al menos 500GB (aunque se usará una cantidad menor). El área de ensayo es de almacenamiento transitorio, y se utiliza temporalmente durante este flujo de trabajo.

5. Escritor de unidad SATA externo y una unidad SATA externa de 3,5 pulgadas. El servicio de importación y exportación solo admite unidades de disco duro SATA II/III de 3,5 pulgadas. No se admiten unidades de disco duro con un tamaño superior a 4 TB. Es posible conectar un disco SATA II/III externo a la mayoría de los equipos con un adaptador USB SATA II/III. Consulte la documentación de importación y exportación para el conjunto más reciente de las unidades compatibles con el servicio.

5. Habilite BitLocker en la máquina a la que está conectado el sistema de escritura de la unidad SATA.

6. Descargue la herramienta de importación y exportación [aquí](http://go.microsoft.com/fwlink/?LinkID=301900&clcid=0x409) en la máquina a la que está conectado el sistema de escritura de la unidad SATA.
...
1. Como parte de la programación de una copia de seguridad, aparecerá la pantalla siguiente (en Windows Server, el cliente de Windows o SCDPM). <br/> ![ImportScreen][1]

  La pantalla correspondiente en SCDPM tiene el siguiente aspecto. <br/> ![Pantalla de importación de DPM][8]
...
Complete el flujo de trabajo y seleccione **Copia de seguridad ahora** en el mmc de Copia de seguridad de Azure para iniciar la copia de seguridad sin conexión. La copia de seguridad inicial se escribe en el área de ensayo como parte de este paso.<br/>

  ![Realizar copia de seguridad ahora][2]

El flujo de trabajo correspondiente en SCDPM se habilita haciendo clic en el **Grupo de protección** y eligiendo la opción **Crear punto de recuperación**. Tras esto, se debe elegir la opción **Protección en línea**.<br/> ![Realizar copia de seguridad de DPM ahora][9]

Una vez completada la operación, se crea un archivo *. AIBBlob* y un archivo *. BaseBlob* en la ubicación de ensayo. <br/> ![Salida][3]
...
2. Descomprima el archivo *WAImportExport.zip*. Ejecute la herramienta *WAImportExport* que formateará la unidad SATA, escriba los datos de copia de seguridad en la unidad SATA y cífrela. Antes de ejecutar el siguiente comando, asegúrese de que BitLocker está habilitado en la máquina. <br/>
...
|/dstdir:<*DestinationBlobVirtualDirectory*> | La ruta de acceso al directorio virtual de destino en la cuenta de almacenamiento de Microsoft Azure. Asegúrese de utilizar nombres de contenedor válidos al especificar los directorios virtuales de destino o blobs. Tenga en cuenta que los nombres de contenedor deben estar en minúsculas.|
  ...
  ![Salida de PowerShell][4]

### Crear trabajo de importación en el portal de Azure
1. Diríjase a su cuenta de almacenamiento en el [Portal de administración](https://manage.windowsazure.com/) y haga clic en la pestaña **Importación/Exportación** y luego en **Crear trabajo de importación** en el panel de tareas. <br/> ![Portal][5]
...
5. A continuación, seleccione en la lista la región de su centro de datos. La región del centro de datos indicará el centro de datos y la dirección donde debe enviar el paquete. <br/> ![CD][6]
...
7. Envíe el disco y escriba el número de seguimiento para realizar un seguimiento del estado del envío. Una vez que llega el disco al centro de datos, se copia en la cuenta de almacenamiento y el estado se actualiza. <br/>
