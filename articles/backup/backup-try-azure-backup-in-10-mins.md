...
3. Seleccione las opciones de **redundancia de almacenamiento**.
    ...
    Si está usando Azure como un extremo de almacenamiento de copia de seguridad principal (por ejemplo, copia de seguridad en Azure desde un servidor de Windows), debe considerar seleccionar la opción de almacenamiento con redundancia geográfica (valor predeterminado).

    Si está utilizando Azure como un punto de conexión de almacenamiento de copia de seguridad terciario (por ejemplo, emplea SCDPM para tener una copia de seguridad local y utiliza Azure para sus necesidades de retención a largo plazo), debería considerar elegir el almacenamiento con redundancia local. Esto reduce el costo de almacenamiento de datos en Azure, a la vez que ofrece un menor nivel de durabilidad de los datos, el cual podría ser aceptable para copias terciarias.
    ...
    >[AZURE.NOTE] Para más información acerca de cómo elegir las opciones de redundancia de almacenamiento consulte este artículo de [introducción general](../storage/storage-redundancy.md).
...
6. Haga clic en **Guardar** para descargar las credenciales del almacén en la carpeta de **descargas** de la cuenta local, o elija **Guardar como** en el menú **Guardar** para especificar una ubicación para las credenciales de almacén.
...
7. Haga clic en **Servicios de recuperación** en el panel de navegación izquierdo y, a continuación, haga clic en el almacén de copia de seguridad que desea registrar en un servidor.

    ![Seleccionar almacén de copia de seguridad](./media/backup-try-azure-backup-in-10-mins/recovery-services-select-vault.png)

8. En la página Inicio rápido, haga clic en **Agente para Windows Server, System Center Data Protection Manager o cliente de Windows > Guardar** (o seleccione la opción **Guardar como** en el menú **Guardar** para especificar una ubicación para el agente).
...
2. Elija la **carpeta de instalación** y la **carpeta de caché** necesarias para el agente.
...
3. Aún puede conectarse a Internet a través de la configuración predeterminada del proxy o, si va a utilizar un servidor proxy para conectarse a Internet, en la pantalla **Configuración de proxy**, compruebe la casilla **Configuración de servidor proxy personalizada** y especifique los detalles del servidor proxy.
...
5. Una vez que el agente esté instalado, haga clic en **Continuar con el registro** para seguir con el flujo de trabajo.

    ![Registro](./media/backup-try-azure-backup-in-10-mins/register.png)

6. En la pantalla **Identificación del almacén**, busque y seleccione el **archivo de credenciales del almacén** que se descargó anteriormente. Asegúrese de que el archivo de almacén de credenciales está disponible en una ubicación a la que puede tener acceso la aplicación de instalación.
...
7. En la pantalla **Configuración de cifrado**, puede generar una frase de contraseña o proporcionarla (mínimo de 16 caracteres). Recuerde guardar la frase de contraseña en una ubicación segura.
    ...
    El **Asistente para registrar servidor** registra el servidor mediante Copia de seguridad de Microsoft Azure.

    ![Cifrado](./media/backup-try-azure-backup-in-10-mins/registering-server.png)

8. Una vez que la **clave de cifrado** está establecida, deje la casilla **Inicio del agente de Servicios de recuperación de Microsoft Azure** activada y haga clic en **Cerrar**.
...
1. En el complemento mmc (que se abrió automáticamente si ha dejado la casilla **Inicio del agente de Servicios de recuperación de Microsoft Azure** activada) haga clic en **Programar copia de seguridad**.
...
3. En la pantalla **Seleccionar elementos de los que realizar copia de seguridad**, haga clic en **Agregar elementos**.
    ...
    Seleccione los elementos de los cuales desea tener una copia de seguridad y haga clic en **Aceptar**. Copia de seguridad de Azure en Windows Server o en un cliente Windows le permite proteger archivos y carpetas.
...
5. Seleccione la **Directiva de retención** para la copia de seguridad. Puede modificar la directiva de retención diaria, semanal, mensual y anual según sus necesidades.
...
6. Elija el tipo de copia de seguridad inicial.
    ...
    Puede hacer una copia de seguridad automáticamente en la red, o puede realizar una copia sin conexión. El resto de este artículo sigue el proceso de copia de seguridad automática. Si prefiere hacer una copia de seguridad sin conexión, revise este artículo para obtener información adicional sobre el [flujo de trabajo de copia de seguridad sin conexión en Copia de seguridad de Azure](backup-azure-backup-import-export.md).
...
8. Una vez que finalice el asistente para crear la **programación de copia de seguridad**, haga clic en **Cerrar**.

    ![Copia de seguridad inicial de Windows Server](./media/backup-try-azure-backup-in-10-mins/backup-schedule-created.png)

9. En el complemento mmc haga clic en **Hacer copia de seguridad ahora** para completar la propagación inicial a través de la red.

    ![Realizar copia de seguridad de Windows Server ahora](./media/backup-try-azure-backup-in-10-mins/snap-in-backup-now.png)

10. En la pantalla **Confirmación**, revise la configuración que utilizará el asistente para realizar la copia de seguridad del equipo y haga clic en **Realizar una copia de seguridad**.

    ![Realizar copia de seguridad de Windows Server ahora](./media/backup-try-azure-backup-in-10-mins/backup-now-confirmation.png)

11. Haga clic en **Cerrar** para cerrar el asistente. Puede hacer esto antes de que el **proceso de copia de seguridad** se termine y se seguirá ejecutando en segundo plano.

    ![Realizar copia de seguridad de Windows Server ahora](./media/backup-try-azure-backup-in-10-mins/backup-progress.png)

12. Una vez completada la copia de seguridad inicial, la vista **Trabajos** de la consola de Copia de seguridad de Azure indica el estado "Trabajo completado".
...
- Obtenga más detalles acerca de la [preparación del entorno para realizar la copia de seguridad de equipos Windows](backup-configure-vault.md)
- Obtenga más detalles acerca de la [copia de seguridad de un equipo de Windows Server](backup-azure-backup-windows-server.md)
