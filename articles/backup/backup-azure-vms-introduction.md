...
El servicio Copia de seguridad de Azure inicia el trabajo de copia de seguridad a la hora programada y desencadena la extensión de copia de seguridad para tomar una instantánea de un momento en el tiempo. Si se trata de máquinas virtuales con Windows, esta instantánea se toma en coordinación con el Servicio de instantáneas de volumen (VSS) para obtener una instantánea coherente de los discos de la máquina virtual sin tener que apagarla. Si se trata de máquinas virtuales con Linux, los clientes pueden escribir scripts personalizados que se invocarán al realizar una instantánea de máquina virtual, a fin de garantizar la coherencia, como se describe más adelante en este artículo. 
...
Azure Backup proporciona un marco que ofrece a los clientes la flexibilidad que necesitan para controlar el entorno y el flujo de trabajo de copia de seguridad mediante la ejecución del script anterior y posterior como parte de la copia de seguridad de la máquina virtual, de tal forma que se garantice la coherencia con las aplicaciones. Se invocará el script anterior antes de realizar la instantánea de máquina virtual y el script posterior tras la finalización de dicha instantánea. Para obtener más información, vea la información sobre [copia de seguridad de máquinas virtuales con Linux coherentes con la aplicación mediante los script anterior y posterior](https://docs.microsoft.com/azure/backup/backup-azure-linux-app-consistent). 
> [!NOTE]
> Azure Backup solo invoca los script anterior y posterior escritos por el cliente; no obstante, es necesario garantizar la coherencia con la aplicación con estos script que el cliente controla. Azure Backup marcará el punto de recuperación como coherente con la aplicación si la ejecución de los script anterior y posterior se realiza correctamente. 
>
...
Cuando los datos de copia de seguridad se copian de una cuenta de almacenamiento, se tienen en cuenta las métricas de salida (o rendimiento) y de las operaciones de entrada y salida por segundo (IOPS) de la cuenta de almacenamiento. Al mismo tiempo, las máquinas virtuales también consumen IOPS y el rendimiento. El objetivo es asegurarse de que el tráfico de copia de seguridad y máquina virtual no supera los límites de la cuenta de almacenamiento.

### <a name="number-of-disks"></a>Número de discos
El proceso de copia de seguridad intenta realizar el trabajo lo más rápido posible. Con ello, se consume tantos recursos como es posible. Pero todas las operaciones de E/S están limitadas por la *capacidad de proceso de destino de un blob*, que tiene un límite de 60 MB por segundo. En un intento por maximizar la velocidad, el proceso de copia de seguridad trata de hacer copia de seguridad de cada uno de los discos de la máquina virtual *en paralelo*. Si una máquina virtual tiene cuatro discos, el servicio tratará de hacer copia de seguridad de los cuatro discos en paralelo. El factor más importante que determina el tráfico de copia de seguridad que sale de una cuenta de almacenamiento de cliente es el **número de discos** de los que se va a realizar una copia de seguridad.
...
* Tiempo de copia de datos: si es necesario, los datos se copian primero desde el almacén en la cuenta de almacenamiento del cliente. Si los datos deben copiarse a la cuenta de almacenamiento del cliente antes de que el servicio de copia de seguridad escriba los datos desde el almacén, el tiempo de copia será mayor. Para reducir el tiempo de copia durante el proceso de restauración, seleccione una cuenta de almacenamiento que no esté cargada con otras lecturas y escrituras de aplicaciones.
...
El tamaño *real* en este caso es el tamaño de la máquina virtual de 17 GB + 30 GB + 0 GB = 47 GB. Este tamaño de instancias protegidas (47 GB) se convierte en la base de la factura mensual. A medida que crece la cantidad de datos en la máquina virtual, el tamaño de instancia protegida usado para la facturación también cambiará en consecuencia.
