...
Esta guía le indica cómo enfrentarse a determinadas situaciones habituales usando un cliente Android para Servicios móviles de Azure. Entre las situaciones tratadas se encuentran la consulta, la inserción, la actualización y la eliminación de los datos, la autenticación de los usuarios, la administración de los errores y la personalización del cliente. Si no tiene experiencia en el uso de Servicios móviles, considere primero la opción de completar la [guía de inicio rápido de Servicios móviles][guía de inicio rápido de Servicios móviles] (en inglés). El tutorial de la guía de inicio rápido le ayuda a configurar la cuenta y crear el primer servicio móvil.
...
-   [Direccionamiento del referencia de tabla][Direccionamiento del referencia de tabla]
    ...
    -   [La estructura de API][La estructura de API]
-   [Direccionamiento del desde un servicio móvil][Direccionamiento del desde un servicio móvil]
    ...
    -   [Direccionamiento del métodos de consulta][Direccionamiento del métodos de consulta]
...
-   [Direccionamiento del un elemento específico][Direccionamiento del un elemento específico]
-   [Direccionamiento del datos sin tipo][Direccionamiento del datos sin tipo]
-   [Direccionamiento del de datos a la interfaz de usuario][Direccionamiento del de datos a la interfaz de usuario]

    -   [Direccionamiento del diseño][Direccionamiento del diseño]
    -   [Direccionamiento del adaptador][Direccionamiento del adaptador]
    -   [Direccionamiento del adaptador][3]
-   [Direccionamiento del de usuarios][Direccionamiento del de usuarios]

    -   [de tokens de autenticación en la caché][de tokens de autenticación en la caché]
...
Se asume que ha creado un servicio móvil y una tabla. Para obtener más información, consulte [Crear una tabla][Crear una tabla]. En el código usado en este tema, se supone que el nombre de la tabla es *ToDoItem* y que dispondrá de las siguientes columnas:
...
Cuando está habilitado el esquema dinámico, Servicios móviles de Azure genera automáticamente columnas nuevas basadas en el objeto de la solicitud de inserción o actualización. Para obtener más información, consulte [Esquema dinámico][Esquema dinámico].
...
El código siguiente crea el objeto [MobileServiceClient][MobileServiceClient] que se usa para obtener acceso al servicio móvil.
...
La forma más sencilla de consultar o modificar los datos de un servicio móvil es utilizar el *modelo de programación con tipo*, ya que Java es un lenguaje con una fuerte presencia de tipos (más adelante trataremos el modelo *sin tipo*). Este modelo proporciona serialización y deserialización directas para JSON usando la biblioteca de [gson][gson] al enviar datos entre el cliente y el servicio móvil: El desarrollador no tiene que hacer nada más, el marco se encarga de todo.

Lo primero que debe hacer para consultar o modificar los datos es crear un objeto [MobileServiceTable][MobileServiceTable] llamando al método **getTable** en [**MobileServiceClient**][MobileServiceClient]. Abordaremos dos sobrecargas de este método:
...
La [primera sobrecarga][primera sobrecarga] se usa cuando el nombre de la clase y el de la tabla son iguales:

        MobileServiceTable<ToDoItem> mToDoTable = mClient.getTable(ToDoItem.class);

La [segunda sobrecarga][segunda sobrecarga] se usa cuando el nombre de la tabla es diferente del nombre del tipo.

        MobileServiceTable<ToDoItem> mToDoTable = mClient.getTable("ToDoItemBackup", ToDoItem.class);

### <a name="api"></a>La estructura de API

Las operaciones de la tabla de Servicios móviles usan el modelo de devolución de llamada asincrónico. Los métodos que implican consultas y operaciones, como las inserciones, las actualizaciones y las eliminaciones, tienen un parámetro que es un objeto de devolución de llamada. Este objeto siempre contiene un método **OnCompleted**. El método **onCompleted** contiene un parámetro que es un objeto **Exception**, que a su vez se puede probar para determinar si la llamada al método es correcta. Un objeto **Exception** nulo indica que es correcta, de lo contrario, el objeto **Exception** describe el motivo del error.

Hay varios objetos de devolución de llamada diferentes y cada uno depende de si está consultando, modificando o eliminando datos. Los parámetros para el método *onCompleted* varían en función de qué objeto de devolución de llamada forme parte.
...
Las consultas como esta usan el objeto de devolución de llamada [**TableQueryCallback\<E\>**][**TableQueryCallback\<E\>**].

El parámetro *result* devuelve el resultado configurado en la consulta, y el código que hay dentro de la rama correcta de la prueba *exception* muestra cómo analizar las filas individuales.
...
Inicie un filtro con una llamada de método [**where**][**where**] en la referencia de tabla. A esto le sigue una llamada de método [**field**][**field**] seguida de una llamada de método que especifica el predicado lógico. Entre los posibles métodos de predicado se incluyen [**eq**][**eq**], [**ne**][**ne**], [**gt**][**gt**], [**ge**][**ge**], [**lt**][**lt**], [**le**][**le**], entre otros.
...
Por ejemplo, puede filtrar por fechas. Puede comparar todo el campo de fecha, pero también puede comparar partes de las fechas con métodos como [**year**][**year**], [**month**][**month**], [**day**][**day**], [**hour**][**hour**], [**minute**][**minute**] y [**second**][**second**]. El código parcial siguiente agrega un filtro para los elementos cuyo valor *due date* sea igual a 2013.

        mToDoTable.where().year("due").eq(2013)

Puede hacer una amplia variedad de filtros complejos en campos de cadena con métodos como [**startsWith**][**startsWith**], [**endsWith**][**endsWith**], [**concat**][**concat**], [**subString**][**subString**], [**indexOf**][**indexOf**], [**replace**][**replace**], [**toLower**][**toLower**], [**toUpper**][**toUpper**], [**trim**][**trim**] y [**length**][**length**]. El código parcial siguiente filtra las filas de tabla donde la columna *text* empieza por "PRI0".

        mToDoTable.where().startsWith("text", "PRI0")

Los campos de número también permiten una amplia variedad de filtros más complejos con métodos como [**add**][**add**], [**sub**][**sub**], [**mul**][**mul**], [**div**][**div**], [**mod**][**mod**], [**floor**][**floor**], [**ceiling**][**ceiling**] y [**round**][**round**]. El código parcial siguiente filtra las filas de tabla cuya columna *duration* empieza por un número par.

        mToDoTable.where().field("duration").mod(2).eq(0)

Puede combinar predicados con métodos como [**and**][**and**], [**or**][**or**] y [**not**][**not**]. Este código parcial combina dos de los ejemplos anteriores.
...
Para obtener más información y ver ejemplos de filtrado, consulte [Exploración de la riqueza del modelo de consulta del cliente Android de Servicios móviles][Exploración de la riqueza del modelo de consulta del cliente Android de Servicios móviles] (en inglés).
...
El primer parámetro del método [**orderBy**][**orderBy**] es una cadena igual al nombre del campo por el que realizar la ordenación.

El segundo parámetro usa la enumeración [**QueryOrder**][**QueryOrder**] para especificar si la ordenación será ascendente o descendente.
...
El método [**select**][**select**] tiene que seguir métodos como [**where**][4] y [**orderBy**][5], si están presentes. Pueden ir seguidos de métodos como [**top**][**top**].

### <a name="chaining"></a>Direccionamiento del métodos de consulta
...
Lo que hace este trabajo es que los métodos de consulta que usa devuelven objetos [**MobileServiceQuery\<T\>**][**MobileServiceQuery\<T\>**], que a su vez pueden tener métodos adicionales invocados en ellos. Para finalizar las series de métodos y ejecutar la consulta, llame al método [**execute**][**execute**].
...
A continuación, llame al método [**insert**][**insert**].
...
Para las operaciones **insert**, el objeto de devolución de llamada es [**TableOperationCallback\<ToDoItem\>**][**TableOperationCallback\<ToDoItem\>**].

El parámetro vacío del método **onCompleted** contiene el objeto recién insertado. El código correcto indica cómo acceder al *id* de la fila insertada.
...
## <a name="updating"></a><span class="short-header">Actualización de datos</span>Actualización de datos en un servicio móvil

El código siguiente muestra cómo actualizar los datos de una tabla. En este ejemplo, *mToDoItem* es una referencia a un elemento de la tabla *ToDoItem* y actualizamos su propiedad *duration*.
...
Fíjese en que el objeto de devolución de llamada y los parámetros del método *onCompleted* son los mismos que cuando realizamos una inserción.

## <a name="deleting"></a><span class="short-header">Eliminación de datos</span>Eliminación de datos en un servicio móvil

El código siguiente muestra cómo eliminar datos de una tabla. Elimina un elemento existente de la tabla ToDoItem usando una referencia al elemento, en este caso *mToDoItem*.
...
Fíjese en que en el caso de *delete*, el objeto de devolución de llamada es [**TableDeleteCallback**][**TableDeleteCallback**] y el método **onCompleted** es algo diferente donde no se devuelven filas de tabla.
...
## <a name="lookup"></a><span class="short-header">Búsqueda de datos</span>Búsqueda de un elemento específico

A veces puede que quiera consultar un elemento específico por su *id*, a diferencia de las consultas donde normalmente se obtiene una serie de elementos que satisfacen ciertos criterios. El código siguiente indica cómo hacer esto, para *id* = "37BBF396-11F0-4B39-85C8-B319C729AF6D".
...
Como en el modelo con tipo, se empieza obteniendo una referencia de tabla, aunque en este caso se trate de un objeto [MobileServicesJsonTable][MobileServicesJsonTable]. La referencia se obtiene llamando al método [getTable()][getTable()] en una instancia del cliente de Servicio móviles.
...
La mayoría de las llamadas de API para obtener acceso a los datos son similares a las llamadas de programación con tipo. La principal diferencia es que en el modelo sin tipo se invocan métodos en el objeto **MobileServiceJsonTable**, en lugar del objeto **MobileServiceTable**. El uso del objeto de devolución de llamada y el método **onCompleted** es muy parecido al modelo con tipo.

### <a name="json_instance"></a>Direccionamiento del una instancia de tabla sin tipo
...
### <a name="json_insert"></a>Direccionamiento del una tabla sin tipo

El código siguiente muestra cómo realizar una inserción. El primer paso es crear un [**JsonObject**][**JsonObject**], que forma parte de la biblioteca de [gson][gson].
...
El paso siguiente es insertar el objeto. La función de devolución de llamada pasada al método [**insert**][6] es una instancia de la clase [**TableJsonOperationCallback**][**TableJsonOperationCallback**]. Fíjese en cómo el primer parámetro del método *onCompleted* es un JsonObject.
...
### <a name="json_get"></a>Direccionamiento del todas las filas de una tabla sin tipo

El código siguiente muestra cómo recuperar una tabla completa. Fíjese en que el modelo de programación sin tipo usa un objeto de devolución de llamada diferente: [**TableJsonQueryCallback**][**TableJsonQueryCallback**].
...
## <a name="binding"></a><span class="short-header">Enlace de datos</span>Enlace de datos a la interfaz de usuario
...
### <a name="layout"></a>Direccionamiento del diseño
...
### <a name="adapter"></a>Direccionamiento del adaptador
...
Debe reemplazar el método *getView* del adaptador. Este código de ejemplo es una muestra de cómo hacerlo: (los detalles podrían variar según su aplicación).
...
Fíjese en que el segundo parámetro para el constructor ToDoItemAdapter es una referencia al diseño. A la llamada al constructor le sigue este código, que obtiene en primer lugar una referencia a la **ListView** y, a continuación, llama a *setAdapter* para configurarse él mismo a fin de usar el adaptador que se acaba de crear:

        ListView listViewToDo = (ListView) findViewById(R.id.listViewToDo);
        listViewToDo.setAdapter(mAdapter);

### <a name="use-adapter"></a>Direccionamiento del adaptador
...
1.  Registrar la aplicación para la autenticación con un proveedor y configurar Servicios móviles
2.  Restringir los permisos de tabla a los usuarios autenticados únicamente
3.  código de autenticación a su aplicación
...
Estas dos primeras tareas se realizan usando el [Portal de administración de Azure][Portal de administración de Azure]. Para obtener más información, consulte [Introducción a la autenticación][Introducción a la autenticación].

### <a name="caching"></a>Direccionamiento del código de autenticación a su aplicación
...
2.  En el método **onCreate** de la clase de actividad, agregue la línea de código siguiente después del código que crea el objeto `MobileServiceClient`. Se supone que la referencia al objeto `MobileServiceClient` es *mClient*.
...
<p>Si usa un proveedor de identidades que no sea Google, cambie el valor que ha pasado al m&eacute;todo <strong>login</strong> anterior por uno de los siguientes: <i>MicrosoftAccount</i>, <i>Facebook</i>, <i>Twitter</i> o <i>WindowsAzureActiveDirectory</i>.</p>
</div>

3.  Cuando ejecute la aplicación, inicie sesión con el proveedor de identidades que haya elegido.

### <a name="caching"></a>Direccionamiento del de tokens de autenticación en la caché
...
Puede ver un ejemplo de la validación y la gestión de los errores [aquí][aquí], donde la validación se implementa por medio de scripts de servidor que devuelven excepciones en el error y código de cliente que gestiona las excepciones.
...
Cada uno de ellos tiene un método **OnCompleted**, cuyo segundo parámetro es un objeto **java.lang.Exception**. Puede incluir estos objetos de devolución de llamada en subclases e implementar su propio método **onCompleted** que comprueba si el parámetro de excepción es nulo. Si lo es, no hay error y simplemente debe llamar a **super.OnCompleted()**.
...
Ahora el desarrollador puede usar sus devoluciones de llamada repartidas en subclases y dejar de preocuparse por comprobar la excepción, ya que se controla en un lugar centralizado (\#2) para todas las instancias de la devolución de llamada.
...
### <a name="columns"></a>Direccionamiento del nombres de servidor y cliente diferentes
...
Debe serializar los nombres de cliente en nombres JSON que coincidan con los nombres de columna de la tabla *ToDoItem* del servidor. El código siguiente, que usa la biblioteca [gson][gson], lo hace.
...
### <a name="table"></a>Direccionamiento del nombres de tabla diferentes entre el cliente y los servicios móviles

Asignar el nombre de la tabla de cliente a una tabla de servicios móviles diferente es fácil, simplemente usamos una de los reemplazos de la función
[getTable()][segunda sobrecarga], como se observa en el código siguiente.

        mToDoTable = mClient.getTable("ToDoItemBackup", ToDoItem.class);

### <a name="conversions"></a>Direccionamiento del automática de nombres de columna

Asignar nombres de columna para una tabla estrecha con solo unas pocas columnas no es muy difícil, como hemos visto en la sección anterior. Pero supongamos que nuestra tabla tiene muchas columnas, unas 20 o 30. Resulta que podemos llamar a la API de [gson][gson] y especificar una estrategia de conversión que aplicaremos a todas las columnas y evitaremos tener que anotar cada uno de los nombres de las columnas.

Para ello, usamos la biblioteca de [gson][gson] que utiliza la biblioteca de cliente Android en segundo plano para serializar los objetos de Java en datos JSON, que a su vez se envían a Servicios móviles de Azure.
...
### <a name="complex"></a>Direccionamiento del una propiedad de objeto o matriz en una tabla
...
Para ver un ejemplo de cómo hacerlo, consulte nuestra entrada de blog [Personalización de la serialización usando la biblioteca de][Personalización de la serialización usando la biblioteca de][gson][gson] en el cliente Android de Servicios móviles</a> (en inglés).
...
Encontrará la referencia de Javadocs para la API del cliente Android [][]<http://dl.windowsazure.com/androiddocs/com/microsoft/windowsazure/mobileservices/package-summary.html></a>

<!-- Anchors. -->
<!-- Images. -->
<!-- URLs. -->

  [guía de inicio rápido de Servicios móviles]: /es-es/develop/mobile/tutorials/get-started-android/
  [SDK de Servicios móviles]: http://go.microsoft.com/fwlink/p/?linkid=280126
  [SDK de Android]: https://go.microsoft.com/fwLink/p/?LinkID=280125&clcid=0x409
  ...
  [Direccionamiento del métodos de consulta]: #chaining
  [Direccionamiento del datos en un servicio móvil]: #inserting
  [1]: #updating
  [2]: #deleting
  [Direccionamiento del un elemento específico]: #lookup
  [Direccionamiento del datos sin tipo]: #untyped
  [Direccionamiento del de datos a la interfaz de usuario]: #binding
  [Direccionamiento del diseño]: #layout
  [Direccionamiento del adaptador]: #adapter
  [3]: #use-adapter
  [Direccionamiento del de usuarios]: #authentication
  [de tokens de autenticación en la caché]: #caching
  ...
  [Crear una tabla]: http://go.microsoft.com/fwlink/p/?LinkId=298592
  ...
  [primera sobrecarga]: http://go.microsoft.com/fwlink/p/?LinkId=296839
  [segunda sobrecarga]: http://go.microsoft.com/fwlink/p/?LinkId=296840
  ...
  [Exploración de la riqueza del modelo de consulta del cliente Android de Servicios móviles]: http://hashtagfail.com/post/46493261719/mobile-services-android-querying
  [4]: http://go.microsoft.com/fwlink/p/?LinkId=296296
  [5]: http://go.microsoft.com/fwlink/p/?LinkId=296313
  ...
  [6]: http://go.microsoft.com/fwlink/p/?LinkId=298535
  [Introducción a la autenticación]: http://go.microsoft.com/fwlink/p/?LinkId=296316
  [Portal de administración de Azure]: https://manage.windowsazure.com/
  [aquí]: https://www.windowsazure.com/es-es/develop/mobile/tutorials/validate-modify-and-augment-data-dotnet/
  [Personalización de la serialización usando la biblioteca de]: http://hashtagfail.com/post/44606137082/mobile-services-android-serialization-gson
  []: http://go.microsoft.com/fwlink/p/?LinkId=298735 "aquí"
