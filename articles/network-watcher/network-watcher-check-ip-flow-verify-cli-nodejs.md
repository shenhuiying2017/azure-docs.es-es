...
Se necesita la dirección IP de una NIC en la máquina virtual. Recuperación de las NIC para una máquina virtual con el comando siguiente. Si ya conoce la dirección IP que desea probar en la máquina virtual, puede omitir este paso.
...
Ejecute el cmdlet `network watcher ip-flow-verify` para probar el tráfico. En este ejemplo se usa la primera dirección IP de la primera NIC:
...
Si se está bloqueando el tráfico y no debería ser así, consulte [Administración de grupos de seguridad de red](../virtual-network/manage-network-security-group.md) para realizar un seguimiento del grupo de seguridad de red y de las reglas de seguridad definidas.
