<properties urlDisplayName="Blob Service" pageTitle="Uso de almacenamiento de blobs (Node.js) | Microsoft Azure" metaKeywords="Get started Azure blob, Azure unstructured data, Azure unstructured storage, Azure blob, Azure blob storage, Azure blob Node.js" description="Aprenda a utilizar el servicio BLOB de Azure para cargar, descargar, incluir en un listado y eliminar contenido de blobs. Ejemplos escritos en Node.js." metaCanonical="" services="storage" documentationCenter="nodejs" title="How to Use the Blob Service from Node.js" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
Esta guía muestra cómo realizar algunas tareas comunes a través del servicio BLOB de Azure. Los ejemplos están escritos usando la API Node.js. Los escenarios cubiertos incluyen **cargar**, **escuchar**, **descargar** y **eliminar** blobs. Para obtener más información sobre blobs,
consulte la sección [Pasos siguientes][].

## Tabla de contenido

* [¿Qué es el servicio BLOB?][]    
* [Conceptos][]    
...
##<a name="create-account"></a>Crear una cuenta de almacenamiento de Azure
...
3.  Puede ejecutar manualmente el comando **ls** para verificar si se ha creado la carpeta **node\_modules**. Dentro de dicha carpeta, busque el paquete **azure-storage**, que contiene las bibliotecas necesarias para el acceso al almacenamiento.
...
código siguiente crea un objeto **BlobService**. Agregue lo siguiente en
la parte superior de **server.js**:

    var blobSvc = azure.createBlobService();

> [WACOM.NOTE] Puede obtener acceso a un blob de forma anónima mediante el uso de **createBlobServiceAnonymous** y proporcionando la dirección del host. Por ejemplo, "var blobSvc = azure.createBlobService('https://myblob.blob.core.windows.net/');".
...
Si el contenedor se crea, "result" será true. Si el contenedor ya existe, "result" será false. "response" contendrá información sobre el funcionamiento, incluida la información sobre [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) para el contenedor.
...
El "resultado" que devuelven estos métodos contendrá información acerca de la operación, como el valor **ETag** del blob.
...
El "resultado" contendrá una colección de "entradas", que es una matriz de objetos que describen cada blob. Si todos los blobs no se pueden devolver, el "resultado" proporcionará también un "continuationToken", que se puede usar como el segundo parámetro para recuperar entradas adicionales.

## <a name="download-blob"> </a>Reproducción de blobs
...
El "resultado" contendrá información acerca del blob, lo que incluye información de **ETag**.

## <a name="delete-blob"> </a>Reproducción de un blob
...
##<a name="concurrent-access"></a>Reproducción de simultáneo

Para permitir el acceso simultáneo a un blob desde varios clientes o varias instancias de proceso, puede usar etiquetas **ETags** o **concesiones**.
...
## <a name="sas"></a>Reproducción de firmas de acceso compartido
...
La aplicación cliente usa entonces la SAS con **BlobServiceWithSAS** para realizar operaciones con el blob. Lo siguiente obtiene información sobre **myblob**.
...
-   Visite el repositorio del [SDK de almacenamiento de Azure para Node.js][] en GitHub.
  ...
  [Trabajo de un contenedor]: #create-container
  [Trabajo de un blob en un contenedor]: #upload-blob
  [Trabajo de los blobs de un contenedor]: #list-blob
  [Trabajo de blobs]: #download-blobs
  [Trabajo de un blob]: #delete-blobs
  [Trabajo simultáneo]: #concurrent-access
...
[Creación e implementación de una aplicación Node.js en un sitio web de Azure]: /en-us/develop/nodejs/tutorials/create-a-website-(mac)/
  [Servicio en la nube Node.js con almacenamiento]: /en-us/documentation/articles/storage-nodejs-use-table-storage-cloud-service-app/
  [Aplicación web Node.js con almacenamiento]: /en-us/documentation/articles/storage-nodejs-use-table-storage-web-site/
 [Sitio web con WebMatrix]: /en-us/documentation/articles/web-sites-nodejs-use-webmatrix/
  [Uso de la API de REST]: http://msdn.microsoft.com/en-us/library/windowsazure/hh264518.aspx
  [Portal de administración de Azure]: http://manage.windowsazure.com
  [Servicio en la nube de Node.js]: /en-us/documentation/articles/cloud-services-nodejs-develop-deploy-app/
