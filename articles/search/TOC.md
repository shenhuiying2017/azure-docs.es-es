# Información general
## [Acerca de Azure Search](search-what-is-azure-search.md)

# Inicios rápidos

## [Creación de un servicio](search-create-service-portal.md)
## Portal
### [Creación de un índice](search-create-index-portal.md)
### [Importación de datos](search-import-data-portal.md)
### [Explorador de búsqueda](search-explorer.md)
## .NET
### [Creación de un índice](search-create-index-dotnet.md)
### [Importación de datos](search-import-data-dotnet.md)
### [Search](search-query-dotnet.md)
## REST
### [Creación de un índice](search-create-index-rest-api.md)
### [Importación de datos](search-import-data-rest-api.md)
### [Search](search-query-rest-api.md)
### [Prueba con clientes REST](search-fiddler.md)

# Tutoriales

## [1 - Indexación de Azure SQL Database](search-indexer-tutorial.md)
## [2 - Indexación de blobs de Azure](search-semi-structured-data.md)
## [3 - Incorporación de sinónimos en .NET](search-synonyms-tutorial-sdk.md)
## [4 - Tutorial del portal](search-get-started-portal.md)

# Muestras
## [Ejemplos de código de Azure](https://azure.microsoft.com/resources/samples/?service=search)
## [Ejemplos de GitHub](https://github.com/Azure-Samples/?utf8=%E2%9C%93&query=search)

# Conceptos
## [Búsqueda de texto completo](search-lucene-query-architecture.md)
## [Indexación](search-what-is-data-import.md)
## [Definición de índice](search-what-is-an-index.md)
## [Análisis lingüístico](search-analyzers.md)

# Guías de procedimientos

## Planeamiento y diseño
### [Elección de una SKU](search-sku-tier.md)
### [Límites de servicio](search-limits-quotas-capacity.md)
### [Escalabilidad del servicio](search-capacity-planning.md)
### [Patrones de diseño para varios inquilinos](search-modeling-multitenant-saas-applications.md)
### [Rendimiento y optimización](search-performance-optimization.md)

## Seguridad
### [Seguridad operativa y de datos](search-security-overview.md)
### [Protección mediante filtros de identidad](search-security-trimming-for-azure-search.md)
### [Protección mediante Active Directory](search-security-trimming-for-azure-search-with-aad.md)

## Desarrollo
### [Versiones de API](search-api-versions.md)
### [Desarrollo en .NET](search-howto-dotnet-sdk.md)
### [Desarrollo en Node.js](search-get-started-nodejs.md)
### [Desarrollo en Java](search-get-started-java.md)
### Actualización del SDK
#### [.NET SDK 3.0](search-dotnet-sdk-migration.md)
#### [.NET SDK 1.1](search-dotnet-sdk-migration-version-1.md)
#### [SDK de administración de .NET](search-dotnet-mgmt-sdk-migration.md)
### [Actualización de la API de REST](search-api-migration.md)
### [Tipos de datos complejos de modelo](search-howto-complex-data-types.md)
### [Manejo de actualizaciones simultáneas](search-howto-concurrency.md)
### [Paginación de resultados](search-pagination-page-layout.md)

## Análisis de texto
### [Analizadores de idiomas](https://docs.microsoft.com/rest/api/searchservice/language-support)
### [Analizadores personalizados](https://docs.microsoft.com/rest/api/searchservice/custom-analyzers-in-azure-search)

## Datos de indexación
### [Introducción a los indexadores](search-indexer-overview.md)
### [Indexador de Azure Blob Storage](search-howto-indexing-azure-blob-storage.md)
### [Indexador de Azure Table Storage](search-howto-indexing-azure-tables.md)
### [Indexador de Azure SQL](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers.md)
### [Indexador de Azure Cosmos DB](search-howto-index-cosmosdb.md)
### [Indexación de blobs CSV](search-howto-index-csv-blobs.md)
### [Indexación de blobs JSON](search-howto-index-json-blobs.md)
### [Configuración de conexiones de indexador a SQL Server en la máquina virtual de Azure](search-howto-connecting-azure-sql-iaas-to-azure-search-using-indexers.md)
### [Asignaciones de campos en indexadores](search-indexer-field-mappings.md)
##  Search
### [Construcción de consultas básicas](search-query-overview.md)
### [Sintaxis de consulta simplificada](https://docs.microsoft.com/rest/api/searchservice/simple-query-syntax-in-azure-search)
### [Sintaxis de consulta de Lucene](https://docs.microsoft.com/rest/api/searchservice/lucene-query-syntax-in-azure-search)
### [Ejemplos de consulta de sintaxis de Lucene](search-query-lucene-examples.md)
### [Navegación por facetas](search-faceted-navigation.md)
### [Perfiles de puntuación](https://docs.microsoft.com/rest/api/searchservice/add-scoring-profiles-to-a-search-index)
### [Sugerencias](https://docs.microsoft.com/rest/api/searchservice/suggesters)
### [Sinónimos (en versión preliminar)](search-synonyms.md)
## Filtros
### [Introducción sobre los filtros](search-filters.md)
### [Filtros de faceta](search-filters-facets.md)
### [Sintaxis de expresión de OData](https://docs.microsoft.com/rest/api/searchservice/odata-expression-syntax-for-azure-search)

## Administración y análisis
### [Administración (portal)](search-manage.md)
### [PowerShell (Azure Search)](search-manage-powershell.md)
### [Supervisión del uso y las estadísticas](search-monitor-usage.md)
### [Búsqueda de análisis de tráfico](search-traffic-analytics.md)

# Referencia

## [.NET](/dotnet/api/?term=microsoft.azure.search)
## [.NET (administración)](/dotnet/api/?term=microsoft.azure.management.search)
## [Python (administración)](http://azure-sdk-for-python.readthedocs.io/en/latest/ref/azure.mgmt.search.html)
## [REST](/rest/api/searchservice)
## [REST (administración)](/rest/api/searchmanagement)
## [REST de servicio (Versión preliminar)](search-api-2016-09-01-preview.md)

# Recursos

## [P+F: Preguntas más frecuentes](search-faq-frequently-asked-questions.md)
## [Precios](https://azure.microsoft.com/pricing/details/search/)
## [Actualizaciones del servicio](https://azure.microsoft.com/updates/?product=search)
## Cursos y tutoriales
### [Vídeos y tutoriales](search-video-demo-tutorial-list.md)
### [Academia virtual](https://mva.microsoft.com/training-courses/using-windows-azure-search-10540?l=ADkxnd97_9304984382)
## Sitios de demostración
### [Demostración del analizador de búsquedas](http://alice.unearth.ai/)
### [Aplicaciones de demostración en vivo](https://searchsamples.azurewebsites.net/)
### [Aplicación de listados de trabajos](http://aka.ms/azjobsdemo)
## Partner y comunidad
### [Foro de MSDN sobre Azure Search](https://social.msdn.microsoft.com/forums/azure/home?forum=AzureSearch)
### [Desbordamiento de la pila](http://stackoverflow.com/questions/tagged/azure-search)
### [Blog: Modelado de datos relacionales](http://blogs.technet.com/b/onsearch/archive/2015/09/08/modeling-the-adventureworks-inventory-database-for-azure-search.aspx)
### [Blog: Aplicación de facetas de varios niveles](http://blogs.technet.com/b/onsearch/archive/2015/09/09/multi-level-taxonomy-facets-in-azure-search.aspx)
