---
title: Conexión de Base de datos SQL de Azure a Búsqueda de Azure con indexadores | Microsoft Docs
...
El servicio Búsqueda de Azure es un servicio de búsqueda hospedado en la nube que facilita ofrecer una excelente experiencia de búsqueda. Antes de buscar, deberá rellenar un índice de Búsqueda de Azure con los datos. Si los datos residen en una base de datos SQL de Azure, el nuevo **indexador de Azure Search para Azure SQL Database** (o **indexador Azure SQL**, para abreviar) de Azure Search puede automatizar el proceso de indexación. Esto significa que tendrá que escribir menos código y tendrá menos infraestructura para mantener.
...
Para instalar y configurar un indexador de Azure SQL, puede llamar a la [API de REST de Azure Search](http://go.microsoft.com/fwlink/p/?LinkID=528173) para crear y administrar **indexadores** y **orígenes de datos**. 

También puede utilizar la [clase Indexer](https://msdn.microsoft.com/library/azure/microsoft.azure.search.models.indexer.aspx) en el [SDK de .NET](https://msdn.microsoft.com/library/azure/dn951165.aspx), o el Asistente para la importación de datos del [Azure Portal](https://portal.azure.com), para crear y programar un indexador.
...
* Todos los datos proceden de una sola tabla o vista.
  * Si los datos están distribuidos en varias tablas, puede crear una vista y usarla con el indizador. Sin embargo, tenga en cuenta que si usa una vista, no podrá usar la detección de cambios integrada de SQL Server. Consulte [esta sección](#CaptureChangedRows) para obtener más detalles. 
...
4. Suponga que esta ejecución tarda más de una hora (haría falta un gran número de documentos para que esto sucediera en la realidad, pero es una ilustración útil), por ejemplo, 70 minutos, de modo que finaliza en torno a las 2:10 a. m.
...
## <a name="capturing-new,-changed-and-deleted-rows"></a>Capturar filas nuevas, cambiadas y eliminadas
...
| char, nchar, varchar, nvarchar |Edm.String<br/>Collection(Edm.String) |Transformar una columna de cadenas en Collection(Edm.String) requiere el uso de una API de vista previa versión 2015-02-28. Consulte [este artículo](search-api-indexers-2015-02-28-Preview.md#CreateIndexer) para obtener más información. |
...
R: Sí. Sin embargo, debe permitir que el servicio de búsqueda se conecte a su base de datos haciendo las dos cosas siguientes. Consulte el artículo [Configuración de una conexión desde un indexador de Azure Search a SQL Server en una máquina virtual de Azure](search-howto-connecting-azure-sql-iaas-to-azure-search-using-indexers.md) para más información.

1. Quizá deba también configurar la base de datos con un certificado de confianza para que el servicio de búsqueda pueda abrir conexiones SSL con ella.
2. Configure el firewall para permitir el acceso a la dirección IP del servicio de búsqueda.
