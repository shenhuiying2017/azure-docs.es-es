...
> [AZURE.IMPORTANT]Este artículo no ofrece información general conceptual sobre el servicio Factoría de datos de Azure. Para una información general detallada del servicio, consulte [Introducción a la Factoría de datos de Azure](data-factory-introduction.md).
> 
...
- **Azure PowerShell**. Siga las instrucciones del artículo [Cómo instalar y configurar Azure PowerShell](../powershell-install-configure.md) para instalar la última versión de Azure PowerShell en el equipo local. 
...
1. Inicie Azure PowerShell y ejecute los comandos siguientes. Mantenga Azure PowerShell abierto hasta el final de este tutorial. Si lo cierra y vuelve a abrirlo, deberá ejecutar los comandos de nuevo.
	1. Ejecute **Add-AzureAccount** y escriba el mismo nombre de usuario y contraseña que usó para iniciar sesión en el Portal de Azure.
...
4.	Ahora, ejecute el cmdlet **New-AzureRmDataFactoryLinkedService** para crear el servicio vinculado **StorageLinkedService**.
	...
	Si no hubiera ejecutado el cmdlet **Get-AzureRmDataFactory** y asignado el resultado a la variable **$df**, tendría que especificar valores para los parámetros *ResourceGroupName* y *DataFactoryName* de la siguiente forma.
...
1.	Cree un archivo JSON con el nombre OutputTable.json en la carpeta C:\\ADFGetStartedPSH con el siguiente contenido:
	...
	En el ejemplo anterior, crea un conjunto de datos llamado **AzureBlobOutput** y especifique la estructura de los datos que generará el script de Hive. Además, especifica que los resultados se almacenan en el contenedor de blobs llamado **data** y en la carpeta llamada **partitioneddata**. La sección **availability** especifica que el conjunto de datos de salida se genera mensualmente.
...
1.	Cree un archivo JSON con el nombre MyFirstPipelinePSH.json en la carpeta C:\\ADFGetStartedPSH con el siguiente contenido:
	...
	El archivo de script de Hive, partitionweblogs.hql, se almacena en la cuenta de almacenamiento de Azure (especificada mediante la propiedad scriptLinkedService, llamada StorageLinkedService) y en un contenedor denominado **script**.
...
3.	Ejecute **Get-AzureRmDataFactoryRun** para obtener la información de la actividad que se ejecuta para un segmento específico.
