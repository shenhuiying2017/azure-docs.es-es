...
| accessToken | Token de acceso de API que puede utilizarse para tener acceso a los datos del Shopify. El token no expira si está en modo sin conexión. Puede elegir marcar este campo como SecureString para almacenarlo de forma segura en ADF, o almacenar la contraseña en Azure Key Vault y permitir que se extraiga la copia de la actividad desde allí al realizar la copia de datos. Obtenga más información sobre el [Almacenamiento de credenciales en Key Vault](store-credentials-in-key-vault.md). | Sí |
...
Para copiar datos de Shopify, establezca la propiedad type del conjunto de datos en **ShopifyObject**. No hay ninguna propiedad específica de type adicional en este tipo de conjunto de datos.
