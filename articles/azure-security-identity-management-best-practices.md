...
Puede aprender más sobre SSO de Azure AD en el artículo [Administración y personalización de AD FS con Azure AD Connect](./active-directory/active-directory-aadconnect-federation-management.md).

## Implementación de la administración de contraseñas

En escenarios donde tenga varios inquilinos o quiera permitir que los usuarios [restablezcan su propia contraseña](./active-directory/active-directory-passwords-update-your-own-password.md), es importante que utilice directivas de seguridad adecuadas para evitar un uso inadecuado. En Azure, puede aprovechar la funcionalidad de autoservicio de restablecimiento de contraseña y personalizar las opciones de seguridad para adaptarlas a sus requisitos empresariales.

Es especialmente importante conocer las experiencias de estos usuarios cuando intentan realizar estos pasos y aprender de ellas. En función de estas experiencias, elabore un plan para mitigar los posibles problemas que pueden producirse durante la implementación en el caso de un grupo más grande. También se recomienda que utilice la [Visión operativa con los informes de la administración de contraseñas](./active-directory/active-directory-passwords-get-insights.md) para supervisar los usuarios que se registran.
...
Una alternativa para las organizaciones que quieren mantener todo el control de autenticación en el entorno local es usar [Servidor Azure Multi-Factor Authentication](./multi-factor-authentication/multi-factor-authentication-get-started-server.md), también denominado MFA en el entorno local. Mediante este método, podrá seguir aplicando la autenticación multifactor y mantener el servidor MFA local.

Para obtener más información sobre Azure MFA, lea el artículo [Introducción a Azure Multi-Factor Authentication en la nube](./multi-factor-authentication/multi-factor-authentication-get-started-cloud.md).
...
Las organizaciones que no supervisen activamente sus sistemas de identidad corren el riesgo de comprometer las credenciales de los usuarios. Si las organizaciones no saben que están teniendo lugar actividades sospechosas con estas credenciales, no podrán mitigar este tipo de amenaza. Puede aprender más sobre Azure Active Directory en [Azure Active Directory Identity Protection](./active-directory/active-directory-identityprotection.md).
