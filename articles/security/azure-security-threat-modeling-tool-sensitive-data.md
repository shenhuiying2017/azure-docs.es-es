...
| Límites de confianza de máquinas virtuales de IaaS de Azure | <ul><li>[Use Azure Disk Encryption para cifrar discos usados por máquinas virtuales](#disk-vm)</li></ul> | 
...
| Cliente móvil | <ul><li>[Cifre la información confidencial o de identificación personal guardada en el almacenamiento local de teléfonos](#pii-phones)</li><li>[Ofusque los archivos binarios generados antes de distribuirlos a los usuarios finales](#binaries-end)</li></ul> | 
...
## <a name="a-idbinaries-infoaensure-that-binaries-are-obfuscated-if-they-contain-sensitive-information"></a><a id="binaries-info"></a>Asegúrese de que los archivos binarios estén ofuscados si contienen información confidencial
...
| Attributes              | N/D  |
| Referencias              | N/D  |
| Pasos | Asegúrese de que los archivos binarios estén ofuscados si contienen información confidencial como secretos comerciales o lógica de negocios confidencial que no se deba revelar. Esto sirve para detener la ingeniería inversa de ensamblados. Se pueden usar herramientas como CryptoObfuscator para este propósito. |

## <a name="a-idefs-useraconsider-using-encrypted-file-system-efs-is-used-to-protect-confidential-user-specific-data"></a><a id="efs-user"></a>Considere la posibilidad de usar el sistema de cifrado de archivos (EFS) para proteger información confidencial específica del usuario
...
## <a name="a-idfilesystemaensure-that-sensitive-data-stored-by-the-application-on-the-file-system-is-encrypted"></a><a id="filesystem"></a>Asegúrese de que se cifre la información confidencial almacenada por la aplicación en el sistema de archivos
...
## <a name="a-idcache-browseraensure-that-sensitive-content-is-not-cached-on-the-browser"></a><a id="cache-browser"></a>Asegúrese de que no se almacene contenido confidencial en la memoria caché del explorador
...
## <a name="a-idencrypt-dataaencrypt-sections-of-web-apps-configuration-files-that-contain-sensitive-data"></a><a id="encrypt-data"></a>Cifre las secciones de los archivos de configuración de la aplicación web que contengan información confidencial
...
## <a name="a-idautocomplete-inputaexplicitly-disable-the-autocomplete-html-attribute-in-sensitive-forms-and-inputs"></a><a id="autocomplete-input"></a>Deshabilite explícitamente el atributo HTML autocomplete en formularios y entradas confidenciales
...
## <a name="a-iddata-maskaensure-that-sensitive-data-displayed-on-the-user-screen-is-masked"></a><a id="data-mask"></a>Asegúrese de que la información confidencial que se muestra en la pantalla del usuario esté enmascarada
...
## <a name="a-iddynamic-usersaimplement-dynamic-data-masking-to-limit-sensitive-data-exposure-non-privileged-users"></a><a id="dynamic-users"></a>Implemente el enmascaramiento dinámico de datos para limitar la exposición de información confidencial a usuarios sin privilegios
...
| Attributes              | Versión de SQL: V12, versión de SQL: MsSQL2016 |
...
## <a name="a-idsalted-hashaensure-that-passwords-are-stored-in-salted-hash-format"></a><a id="salted-hash"></a>Asegúrese de que las contraseñas se almacenen en formato de hash con sal
...
| Pasos | Las contraseñas no se deberían almacenar en bases de datos de almacén de usuarios personalizadas. En su lugar, los hash de contraseña se deben almacenar con valores sal. Asegúrese de que la sal para el usuario siempre sea única y aplique bcrypt, scrypt o PBKDF2 antes de almacenar la contraseña, con un número mínimo de iteraciones de factor de trabajo de 150 000 bucles para eliminar la posibilidad de que se use fuerza bruta.| 

## <a name="a-iddb-encryptedaensure-that-sensitive-data-in-database-columns-is-encrypted"></a><a id="db-encrypted"></a>Asegúrese de que la información confidencial en las columnas de la base de datos esté cifrada
...
## <a name="a-idtde-enabledaensure-that-database-level-encryption-tde-is-enabled"></a><a id="tde-enabled"></a>Asegúrese de que el cifrado de base de datos (TDE) esté habilitado
...
## <a name="a-idbackupaensure-that-database-backups-are-encrypted"></a><a id="backup"></a>Asegúrese de que las copias de seguridad de la base de datos estén cifradas
...
| Fase de SDL               | Compilación |  
| Tecnologías aplicables | SQL Azure, OnPrem |
| Attributes              | Versión de SQL: V12, versión de SQL: MsSQL2014 |
...
## <a name="a-idapi-browseraensure-that-sensitive-data-relevant-to-web-api-is-not-stored-in-browsers-storage"></a><a id="api-browser"></a>Asegúrese de que la información confidencial relevante para la API web no se guarde en el almacenamiento del explorador
...
## <a name="a-idencrypt-docdbaencrypt-sensitive-data-stored-in-documentdb"></a><a id="encrypt-docdb"></a>Cifre la información confidencial almacenada en DocumentDB
...
## <a name="a-iddisk-vmause-azure-disk-encryption-to-encrypt-disks-used-by-virtual-machines"></a><a id="disk-vm"></a>Use Azure Disk Encryption para cifrar discos usados por máquinas virtuales
...
## <a name="a-idfabric-appsaencrypt-secrets-in-service-fabric-applications"></a><a id="fabric-apps"></a>Cifre los secretos en aplicaciones de Service Fabric
...
## <a name="a-idmodeling-teamsaperform-security-modeling-and-use-business-unitsteams-where-required"></a><a id="modeling-teams"></a>Realice el modelado de seguridad y use unidades de negocio y equipos cuando sea necesario
...
## <a name="a-identitiesaminimize-access-to-share-feature-on-critical-entities"></a><a id="entities"></a>Minimice el acceso a la característica para compartir en entidades críticas
...
## <a name="a-idgood-practicesatrain-users-on-the-risks-associated-with-the-dynamics-crm-share-feature-and-good-security-practices"></a><a id="good-practices"></a>Entrene a los usuarios sobre los riesgos asociados con la característica Compartir de Dynamics CRM y los procedimientos recomendados de seguridad
...
## <a name="a-idexception-mgmtainclude-a-development-standards-rule-proscribing-showing-config-details-in-exception-management"></a><a id="exception-mgmt"></a>Incluya una regla de estándares de desarrollo que prohíba mostrar detalles de configuración en la administración de excepciones
...
## <a name="a-idsse-previewause-azure-storage-service-encryption-sse-for-data-at-rest-preview"></a><a id="sse-preview"></a>Use el cifrado del servicio Azure Storage (SSE) para datos en reposo (versión preliminar)
...
## <a name="a-idclient-storageause-client-side-encryption-to-store-sensitive-data-in-azure-storage"></a><a id="client-storage"></a>Use el cifrado en el cliente para almacenar información confidencial en Azure Storage
...
## <a name="a-idpii-phonesaencrypt-sensitive-or-pii-data-written-to-phones-local-storage"></a><a id="pii-phones"></a>Cifre la información confidencial o de identificación personal guardada en el almacenamiento local de teléfonos
...
### <a name="example"></a>Ejemplo
Intune puede configurarse con las siguientes directivas de seguridad para proteger la información confidencial: 
```C#
...
## <a name="a-idbinaries-endaobfuscate-generated-binaries-before-distributing-to-end-users"></a><a id="binaries-end"></a>Ofusque los archivos binarios generados antes de distribuirlos a los usuarios finales
...
| Pasos | Se deberían ofuscar los archivos binarios generados (ensamblados dentro de apk) para detener la ingeniería inversa de ensamblados. Se pueden usar herramientas como CryptoObfuscator para este propósito. |

## <a name="a-idcertaset-clientcredentialtype-to-certificate-or-windows"></a><a id="cert"></a>Establezca clientCredentialType en Certificate o Windows
...
## <a name="a-idsecurityawcf-security-mode-is-not-enabled"></a><a id="security"></a>El modo de seguridad de WCF no está habilitado
...
| Attributes              | Modo de seguridad: Transport, modo de seguridad: Message |
