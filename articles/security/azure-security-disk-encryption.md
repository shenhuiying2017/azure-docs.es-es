...
4. Azure actualiza el modelo de servicio de la máquina virtual y la máquina virtual IaaS se marca como descifrada. El contenido de la máquina virtual no se cifra en reposo.
5. La deshabilitación de la operación del cifrado no elimina el almacén de claves de cliente y el material de clave de cifrado, claves de cifrado de BitLocker para Windows y frase de contraseña para Linux.
...
> [AZURE.NOTE] Actualmente no se admite la autenticación basada en certificados de AAD en máquinas virtuales Linux.
...
> [AZURE.NOTE] Todos los procesos de espacio de usuario que no se ejecutan como servicios `systemd` deben terminarse con un `SIGKILL`. Deberá reiniciar la máquina virtual. Planee un tiempo de inactividad de la máquina virtual al habilitar el cifrado del disco del sistema operativo en una máquina virtual en ejecución.
