...
[Log Analytics](https://azure.microsoft.com/services/log-analytics) puede proporcionar a Contoso Webstore un registro completo de toda la actividad del usuario y del sistema, además de un registro de los datos de los titulares de las tarjetas. Se puede revisar y comprobar la exactitud de los cambios. 
...
- **Archivado de registros:** todos los registros de diagnóstico se configuran para escribir en una cuenta de almacenamiento de Azure centralizada y cifrada y que así queden archivados durante un período de retención definido (2 días). A continuación, esos registros se conectan a Azure Log Analytics para procesarlos, almacenarlos y agregarlos a los paneles. [Log Analytics](https://azure.microsoft.com/services/log-analytics) es un servicio que le ayudará a recopilar y analizar los datos que generaron los recursos en los entornos locales o en la nube.
...
- El [control de acceso basado en roles de Azure (RBAC)](/azure/role-based-access-control/role-assignments-portal) permite administrar al detalle el control de acceso de Azure. El acceso a la suscripción está limitado al administrador de dicha suscripción y el acceso a Azure Key Vault está restringido para todos los usuarios.
