...
| Referencias              | [Azure File Storage](../storage/storage-files-introduction.md), [Procedimiento para montar un recurso compartido de archivos de Azure en Windows](../storage/storage-file-how-to-use-files-windows.md) |
| Pasos | Azure File Storage admite HTTPS cuando se usa la API de REST, pero se usa con más frecuencia como un recurso compartido de archivos de SMB conectado a una máquina virtual. SMB 2.1 no admite el cifrado, por lo que solo se permiten las conexiones dentro de la misma región de Azure. Sin embargo, SMB 3.0 admite el cifrado y se puede utilizar con Windows Server 2016, Windows Server 2012 R2, Windows 8.1 y Windows 10, lo que permite el acceso entre regiones e incluso el acceso en el escritorio. |
