...
Hive ofrece un modo de ejecutar trabajos de MapReduce de Hadoop mediante un lenguaje de scripting de tipo SQL, denominado *[HiveQL][hadoop-hiveql]*, que se puede utilizar para resumir, consultar y analizar grandes volúmenes de datos. 

Una de las principales ventajas de HDInsight es la separación del almacenamiento de datos y el proceso. HDInsight usa el almacenamiento de blobs de Azure para el almacenamiento de datos. Un proceso normal de MapReduce se puede dividir en 3 partes:
...
La parte principal del tutorial le muestra cómo utilizar un script de PowerShell para realizar lo siguiente:
...
* [Aprovisionamiento de clústeres de HDInsight y ejecución de trabajos de Hive/Sqoop (M1)](#runjob)
* [Apéndice A: Carga de datos de retraso de vuelos en el almacenamiento de blobs de Azure (A1)](#appendix-a)
* [Apéndice B: Creación y carga de un script de HiveQL (A2)](#appendix-b)
* [Apéndice C: Preparación de la base de datos SQL de Azure para el resultado del trabajo Sqoop (A3)](#appendix-c)
* [Pasos siguientes](#nextsteps)

##<a id="prerequisite"></a>Requisitos previos
...
En este tutorial se utiliza el rendimiento puntual de los datos de vuelos de aerolíneas de [Research and Innovative Technology Administration, Bureau of Transportation Statistics][rita-website] (RITA). Los datos se han cargado en un contenedor de almacenamiento de blobs de Azure con el permiso de acceso Blob público. Puesto que es un contenedor de blobs público, no es necesario que enlace esta cuenta de almacenamiento al clúster de HDInsight que ejecuta el script Hive. El script de HiveQL se carga también en el mismo contenedor de blobs. Si desea más información sobre cómo obtener o cargar los datos en su propia cuenta de almacenamiento y cómo crear o cargar el archivo de scripts de HiveQL, consulte el [apéndice A](#appendix-a) y el [apéndice B](#appendix-b).

La siguiente tabla enumera los archivos utilizados en este tutorial:

<table border="1">
<tr><th>Archivos</th><th>Descripción</th></tr>
<tr><td>wasb://flightdelay@hditutorialdata.blob.core.windows.net/flightdelays.hql</td><td>Archivo de scripts de HiveQL que necesita el trabajo de Hive que va a ejecutar. Este script se ha cargado en una cuenta de almacenamiento de blobs de Azure con el público.  El <a href="#appendix-b">apéndice B</a> cuenta con las instrucciones sobre preparar y cargar este archivo a su propia cuenta de almacenamiento de blobs de Azure.</td></tr>
<tr><td>wasb://flightdelay@hditutorialdata.blob.core.windows.net/2013Data</td><td>Datos de entrada para los trabajos de Hive. Los datos se han cargado en una cuenta de almacenamiento de blobs de Azure con el acceso público.  El <a href="#appendix-a">apéndice A</a> cuenta con la instrucción sobre obtener y cargar los datos a su propia cuenta de almacenamiento de blobs de Azure.</td></tr>
<tr><td>\tutorials\flightdelays\output</td><td>Ruta de acceso de salida para el trabajo de Hive. El contenedor predeterminado se usa para almacenar los datos de salida.</td></tr>
...
- El comando CREATE TABLE mueve el archivo de datos a la carpeta /hive/warehouse/<NombreTabla>.
...
2. Prepare los parámetros:
	...
	<tr><td>$hdinsightClusterName</td><td>Nombre del clúster de HDInsight. Si el clúster no existe, el script creará uno de los nombres introducidos.</td></tr>
	...
	<tr><td>$blobContainerName</td><td>Se usará el contenedor de blobs como sistema de archivos predeterminado. Si lo deja en blanco, se usará el valor de $hdinsightClusterName. </td></tr>
	<tr><td>$sqlDatabaseServerName</td><td>Nombre del servidor de base de datos SQL de Azure Tiene que ser un servidor existente. Consulte <a href="#appendix-c">Apéndice C</a> para crear uno.</td></tr>
	...
	<tr><td>$sqlDatabaseName</td><td>La Base de datos SQL hacia donde Sqoop exportará los datos. El nombre predeterminado es "HDISqoop". El nombre de tabla para el resultado del trabajo de Sqooop es "AvgDelays". </td></tr>
	</table>
2. Abra PowerShell ISE.
...
1. Prepare los parámetros:
	...
	<table border="1">
	<tr><th>Nombre de la variable</th><th>Notas</th></tr>
	<tr><td>$storageAccountName</td><td>La cuenta de almacenamiento de Azure en donde desea cargar los datos.</td></tr>
...
1. Prepare los parámetros:
	...
	<table border="1">
	<tr><th>Nombre de la variable</th><th>Notas</th></tr>
	<tr><td>$storageAccountName</td><td>La cuenta de almacenamiento de Azure en donde desea cargar el script de HiveQL.</td></tr>
...
---
##<a id="appendix-c"></a>Apéndice C: Preparación de la base de datos SQL de Azure para el resultado del trabajo Sqoop
...
1. Prepare los parámetros:
	...
	<tr><td>$sqlDatabaseName</td><td>La Base de datos SQL utilizada para crear la tabla AvgDelays para el trabajo de Sqoop. Si se deja en blanco, se creará una base de datos denominada "HDISqoop". El nombre de tabla para el resultado del trabajo de Sqooop es "AvgDelays". </td></tr>
	</table>
2. Abra PowerShell ISE. 
2. Copie y pegue el siguiente script en el panel de scripts:
