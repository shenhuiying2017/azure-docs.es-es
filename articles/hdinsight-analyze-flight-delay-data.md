...
# Análisis de la información de retraso de vuelos usando Hadoop en HDInsight

Hive ofrece el modo de ejecutar trabajos de MapReduce de Hadoop mediante un lenguaje de scripting de tipo SQL, llamado *[HiveQL][HiveQL]*, que se puede aplicar al resumen, la consulta y el análisis de grandes volúmenes de datos. En este tutorial se muestra cómo utilizar Hive para calcular retrasos promedio en aeropuertos y cómo utilizar Sqoop para exportar los resultados a la Base de datos SQL.

**Requisitos previos:**

Antes de empezar este tutorial, debe contar con lo siguiente:

-   Un clúster de HDInsight de Azure. Para obtener información acerca de cómo aprovisionar un clúster de HDInsight, consulte [Introducción a HDInsight][Introducción a HDInsight] o [Aprovisionamiento de clústeres de HDInsight][Aprovisionamiento de clústeres de HDInsight].
...
-   [Creación y carga del script de HiveQL][Creación y carga del script de HiveQL]
-   [Ejecución del script de HiveQL][Ejecución del script de HiveQL]
-   [Exportación del resultado a la Base de datos SQL de Azure][Exportación del resultado a la Base de datos SQL de Azure]
-   [Pasos siguientes][Pasos siguientes]

## <span id="prepare"></span></a>Preparación del tutorial

En este tutorial se utiliza el rendimiento puntual de los datos de vuelos de aerolíneas de [Research and Innovative Technology Administration, Bureau of Transportation Statistics][Research and Innovative Technology Administration, Bureau of Transportation Statistics] (RITA) en su estación de trabajo. Realizará lo siguiente:
...
<table border="1">
<tr><td><strong>Archivos</strong></td><td><strong>Descripción</strong></td></tr>
<tr><td>\tutotirals\flightdelays\data</td><td>datos de vuelos de entrada para Hive</td></tr>
<tr><td>\tutorials\flightdelays\output</td><td>salida para Hive</td></tr>
<tr><td>\tutorials\flightdelays\flightdelays.hql</td><td>archivo de script de HiveQL</td></tr>
<tr><td>\tutorials\flightdelays\jobstatus</td><td>estado del trabajo de Hadoop</td></tr>
</table>
...
3.  Configure las tres primeras variables y, a continuación, ejecute los siguientes comandos.
	...
	<table border="1">
	<tr><td><strong>Nombre de la variable</strong></td><td><strong>Descripción</strong></td></tr>
	<tr><td>$subscriptionName</td><td>Su nombre de suscripción a Azure.</td></tr>
	<tr><td>$storageAccountName</td><td>La cuenta de almacenamiento de Azure que se usa para almacenar los archivos de datos de vuelo. Se recomienda utilizar la cuenta de almacenamiento predeterminada.</td></tr>
	<tr><td>$containerName</td><td>El contenedor de almacenamiento de blobs de Azure que se utiliza para almacenar los archivos de datos de vuelo. Se recomienda utilizar el contenedor de blobs del sistema de archivos de clúster de HDInsight predeterminado. De manera predeterminada, tiene el mismo nombre que el del clúster de HDInsight.</td></tr>
	<tr><td>$localFolder</td><td>Esta es la carpeta de la estación de trabajo donde se almacenan los archivos de retrasos de vuelos.</td></tr>
	<tr><td>$destFolder</td><td>Esta es la ruta de acceso de WASB donde se cargarán los datos de retraso de los vuelos. La ruta de acceso de Hadoop (HDInsight) distingue mayúsculas de minúsculas.</td></tr>
	<tr><td>$month</td><td>Si no descargó los 12 archivos, necesita actualizar esta variable.</td></tr>
	</table>
...
3.  Establezca las seis primeras variables en el siguiente script y, a continuación, ejecute los comandos:
	...
	<tr><td><strong>Nombre de la variable</strong></td><td><strong>Descripción</strong></td></tr>
	<tr><td>$subscriptionName</td><td>Su nombre de suscripción a Azure.</td></tr>
	<tr><td>$sqlDatabaseServer</td><td>El nombre del servidor de Base de datos SQL utilizado por Sqoop para exportar los datos. Si lo deja como está, el script creará uno automáticamente. De lo contrario, especifique una Base de datos SQL o SQL Server existente.</td></tr>
	<tr><td>$sqlDatabaseUsername</td><td>Nombre de usuario de Base de datos SQL/SQL Server.</td></tr>
	<tr><td>$sqlDatabasePassword</td><td>Contraseña de usuario de Base de datos SQL/SQL Server.</td></tr>
	<tr><td>$sqlDatabaseLocation</td><td>Solo se utiliza si desea que el script cree un servidor de Base de datos SQL automáticamente.</td></tr>
	<tr><td>$sqlDatabaseName</td><td>El nombre de la Base de datos SQL utilizado por Sqoop para exportar los datos. Si lo deja como está, el script creará uno automáticamente. De lo contrario, especifique una Base de datos SQL o SQL Server existente.</td></tr>
	<tr><td>$sqlDatabaseMaxSizeGB</td><td>Solo se utiliza si desea que el script cree una Base de datos SQL automáticamente.</td></tr>
	</table>
...
## <span id="createscript"></span></a>Creación y carga del script de HiveQL
...
3.  Configure las dos primeras variables y, a continuación, ejecute los siguientes comandos.
	...
	<table border="1">
	<tr><td><strong>Nombre de la variable</strong></td><td><strong>Descripción</strong></td></tr>
	<tr><td>$storageAccountName</td><td>La cuenta de almacenamiento de Azure que se usa para almacenar el archivo de script de HiveQL. Los scripts de PowerShell que se proporcionan en este tutorial requieren los archivos de datos de vuelo y el archivo de script ubicado en la misma cuenta de almacenamiento de Azure y el contenedor de almacenamiento de blobs.</td></tr>
	<tr><td>$containerName</td><td>El contenedor de almacenamiento de blobs de Azure que se usa para almacenar el archivo de script de HiveQL. Los scripts de PowerShell que se proporcionan en este tutorial requieren los archivos de datos de vuelo y el archivo de script ubicado en la misma cuenta de almacenamiento de Azure y el contenedor de almacenamiento de blobs.</td></tr>
	<tr><td>$hqlLocalFileName</td><td>El nombre de los archivos locales para el script de HiveQL antes de cargarlo a WASB. Para simplificar el script de PowerShell, debe escribir el archivo localmente y luego usar el cmdlet Set-AzureStorageBlobContent para cargar el archivo de script en HDInsight.</td></tr>
	<tr><td>$hqlBlobName</td><td>Este es el nombre del archivo de script con la ruta de acceso en WASB</td></tr>
	<tr><td>$srcDataFolder</td><td>Esta es la carpeta de WASB de donde el script de HiveQL extrae los datos.</td></tr>
	<tr><td>$dstDataFolder </td><td>Esta es la carpeta de WASB hacia donde el script de HiveQL envía los resultados. Más adelante en el tutorial, utilizará Sqoop para exportar los datos de esta carpeta a la Base de datos SQL de Azure.</td></tr>
	</table>
...
## <span id="executehqlscript"></span></a>Ejecución del script de HiveQL

Hay varios cmdlets de Azure PowerShell que puede utilizar para ejecutar Hive. En este tutorial se utiliza Invoke-Hive. Para otros métodos, consulte [Uso de Hive con HDInsight][Uso de Hive con HDInsight]. Con Invoke-Hive, puede ejecutar una instrucción de HiveQL o un script de HiveQL. Usará el script de HiveQL que ha creado y cargado al almacenamiento de blobs de Azure.

Hay un problema conocido de la ruta de acceso de Hive. Las instrucciones para solucionarlo se pueden encontrar en [TechNet Wiki][TechNet Wiki].
...
4.  Configure las tres primeras variables y, a continuación, ejecútelas:
	...
	<table border="1">
	<tr><td><strong>Nombre de la variable</strong></td><td><strong>Descripción</strong></td></tr>
	<tr><td>$clusterName</td><td>El clúster de HDInsight que ejecutará el script de Hive y la exportación de Sqoop.</td></tr>
	<tr><td>$storageAccountName</td><td>La cuenta de almacenamiento de Azure se utiliza para almacenar el script de HiveQL. Consulte [Creación y carga del script de HiveQL][1].</td></tr>
	<tr><td>$containerName</td><td>El contenedor de almacenamiento de blobs de Azure que se usa para almacenar el script de HiveQL. Consulte [Creación y carga del script de HiveQL][1].</td></tr>
	<tr><td>$hqlScriptFile</td><td>Esta es el URI para el archivo de script de HiveQL.</td></tr>
	<tr><td>$outputBlobName</td><td>Este es el archivo de salida del script de HiveQL. El nombre predeterminado es *000000\_0*.</td></tr>
	</table>
...
3.  Configure las cinco primeras variables y, a continuación, ejecute los siguientes comandos:
    ...
    Estas son las variables y su descripción:
	<table border="1">
	<tr><td><strong>Nombre de la variable</strong></td><td><strong>Nota:</strong></td></tr>
	<tr><td>$clusterName</td><td>Nombre del clúster de HDInsight.</td></tr>
	<tr><td>$sqlDatabaseServer</td><td>El servidor de Base de datos SQL hacia donde Sqoop exportará los datos.</td></tr>
	<tr><td>$sqlDatabaseUsername</td><td>Nombre de usuario de Base de datos SQL.</td></tr>
	<tr><td>$sqlDatabasePassword</td><td>Contraseña de usuario de Base de datos SQL.</td></tr>
	<tr><td>$sqlDatabaseName</td><td>La Base de datos SQL hacia donde Sqoop exportará los datos. El nombre predeterminado es \*HDISqoop".</td></tr>
	<tr><td>$sqlDatabaseTableName</td><td>La Base de datos SQL hacia donde Sqoop exportará los datos. El nombre predeterminado es AvgDelays. Esta es la tabla que creó anteriormente en el tutorial.</td></tr>
	<tr><td>$exportDir</td><td>Esta es la ubicación del archivo de salida de Hive. Sqoop exportará los archivos de esta ubicación a la Base de datos SQL.</td></tr>
	</table>
