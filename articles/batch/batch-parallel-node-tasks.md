...
La ejecución en paralelo de tareas a gran escala es una característica esencial de Lote de Azure. Esta capacidad se extiende no solo a varios nodos de proceso que ejecutan las tareas, sino también a la ejecución de tareas simultáneas en esos nodos. Con Lote, la ejecución en paralelo de tareas escala tanto vertical como horizontalmente.

Al habilitar la ejecución de tareas simultáneas en los nodos de proceso de un grupo, puede maximizar el uso de recursos en un número menor de nodos del grupo. Si bien algunos escenarios se beneficiarán de que todos los recursos de un nodo estén disponibles para asignarse a una sola tarea, en otras situaciones será conveniente permitir que varias tareas compartan esos recursos:
...
[Batch Explorer][batch_explorer], una de las [aplicaciones de ejemplo][github_samples] de Lote de Azure, contiene una característica denominada *Mapa térmico* que proporciona visualización de la ejecución de tareas. Cuando ejecuta la aplicación de ejemplo [ParallelTasks][parallel_tasks_sample], puede usar la característica Mapa térmico para visualizar fácilmente la ejecución en paralelo de tareas en cada nodo.
