<properties
 pageTitle="Arquitectura de seguridad de IoT | Microsoft Azure"
...
## La seguridad comienza con un modelo de riesgos
...
### Cómo crear un modelo de riesgos
...
Todos los elementos del diagrama arquitectónico están sujetos a diversas amenazas; usaremos el mnemotécnico STRIDE. Para más información acerca de los elementos de STRIDE, consulte el artículo [Threat Modeling Again, STRIDE](https://blogs.msdn.microsoft.com/larryosterman/2007/09/04/threat-modeling-again-stride/) (Modelado de riesgos otra vez, STRIDE).
...
### La zona de la puerta de enlace en la nube
...
### Dispositivos de información frente a dispositivos especiales
...
## Modelado de riesgos de la arquitectura de referencia de IoT de Azure
...
### Dispositivo y orígenes de datos o transporte de datos
...
**Manipulación**: un atacante puede sacar provecho del material clave extraído para interceptar y suprimir datos del dispositivo en la ruta de acceso de comunicación y reemplazarlos por datos falsos que se han autenticado con el material clave robado.
...
**Denegación de servicio**: el dispositivo puede desactivarse o activarse en un modo en el que la comunicación no sea posible (lo que es intencionado en muchas máquinas industriales).
...
| Puerta de enlace de campo Puerta de enlace en la nube | TID | TLS (PSK/RSA) para cifrar el tráfico. | La interceptación o interferencia de la comunicación entre el dispositivo y la puerta de enlace | Seguridad en el nivel de protocolo (HTTP(S)/AMQP/MQTT/CoAP). En el caso de los protocolos personalizados, es preciso averiguar cómo protegerlos. |
| Puerta de enlace en la nube de dispositivo | TID | TLS (PSK/RSA) para cifrar el tráfico. | La interceptación o interferencia de la comunicación entre el dispositivo y la puerta de enlace | Seguridad en el nivel de protocolo (HTTP(S)/AMQP/MQTT/CoAP). En el caso de los protocolos personalizados, es preciso averiguar cómo protegerlos. |

Estos son algunos ejemplos de las amenazas de esta categoría:

**Denegación de servicio**: los dispositivos restringidos suelen estar ser bajo la amenaza de denegación de servicio cuando escuchan activamente conexiones entrantes o datagramas no solicitados en una red, porque un atacante puede abrir muchas conexiones en paralelo y no prestarles servicio o prestárselo muy lentamente, o el dispositivo se puede inundar con tráfico no solicitado. En ambos casos, el dispositivo puede representarse eficazmente como no operativo en la red.

**Suplantación, revelación de información**: tanto los dispositivos restringidos como los dispositivos especiales a menudo tienen funciones de seguridad del tipo «una para todos», como la protección mediante PIN o contraseña, o se basan totalmente en confiar en la red, lo que significa que concederán acceso a la información cuando un dispositivo se encuentre en la misma red y a menudo dicha red solo está protegida por una clave compartida. Esto significa que cuando el secreto compartido se revela al dispositivo o a la red, es posible controlar el dispositivo u observar los datos emitidos desde el dispositivo.

**Suplantación**: un atacante puede interceptar o invalidad parcialmente la difusión y suplantar el originador ("Man in the middle")

**Manipulación**: un atacante puede interceptar o invalidad parcialmente la difusión y enviar información falsa. Divulgación de información: un atacante puede interceptar una difusión y obtener información sin autorización. Denegación de servicio: un atacante puede bloquear la señal de difusión y denegar la distribución de la información
