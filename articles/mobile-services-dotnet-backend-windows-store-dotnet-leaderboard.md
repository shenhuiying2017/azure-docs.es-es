...
-   [Acerca de la aplicación de ejemplo][Acerca de la aplicación de ejemplo]
-   [Adición de modelos de datos][Adición de modelos de datos]
-   [Adición de controladores Web API][Adición de controladores Web API]
-   [Uso de un objeto DTO para devolver entidades relacionadas][Uso de un objeto DTO para devolver entidades relacionadas]
-   [Definición de una API personalizada para enviar puntuaciones][Definición de una API personalizada para enviar puntuaciones]
-   [Creación de una aplicación de la Tienda Windows][Creación de una aplicación de la Tienda Windows]
-   [Adición de clases de modelo][Adición de clases de modelo]
-   [Creación de un modelo de vista][Creación de un modelo de vista]
-   [Adición de una instancia de MobileServiceClient][Adición de una instancia de MobileServiceClient]
-   [Creación de la página principal][Creación de la página principal]
-   [Publicación del servicio móvil][Publicación del servicio móvil]
...
Este tutorial utiliza [Visual Studio 2013 Update 2][Visual Studio 2013 Update 2].
...
Descargue el proyecto completo [aquí][aquí].
...
En Visual Studio 2013 Update 2, el proyecto de aplicación web ASP.NET incluye ahora una plantilla para Servicios móviles de Windows Azure. Seleccione esta plantilla y haga clic en **Aceptar**.
...
Va a usar [EF Code First][EF Code First] para definir las tablas de la base de datos. En la carpeta DataObjects, agregue una clase denominada `Player`.
...
Observe que ambas clases se heredan de la clase **EntityData**. Al derivarse de **EntityData**, facilitan a la aplicación el uso de los datos con la biblioteca de cliente multiplataforma para Servicios móviles de Azure. **EntityData** facilita también que la aplicación pueda [controlar los conflictos de escritura en la base de datos][controlar los conflictos de escritura en la base de datos].

La clase `PlayerRank` tiene una [propiedad navigation][propiedad navigation] que apunta a la entidad `Player` relacionada. El atributo **[ForeignKey]** le indica a EF que la propiedad `Player` representa una clave externa.

# Adición de controladores Web API

Ahora va a agregar controladores Web API para `Player` y `PlayerRank`. En lugar de controladores Web API estándar, va a agregar un tipo especial de controlador denominado *controlador de tabla*, diseñado específicamente para Servicios móviles de Azure.

Haga clic con el botón secundario en la carpeta Controllers, seleccione Agregar y elija Nuevo elemento de scaffolding.

![][5]

En el cuadro de diálogo **Agregar scaffold**, expanda **Común** a la izquierda y seleccione **Servicios móviles de Windows Azure**. Después seleccione **Controlador de tabla de Servicios móviles de Windows Azure**. Haga clic en **Add**.

En el cuadro de diálogo **Agregar controlador**:

1.  En **Clase de modelo**, seleccione Player.
2.  En **Clase de contexto de datos**, seleccione LeaderboardContext.
3.  Dele el nombre “PlayerContoller” al controlador.
4.  Haga clic en **Add**.
...
-   Enrutamiento: La ruta predeterminada para una clase **TableController** es `/tables/{table_name}/{id}`, donde *table\_name* coincide con el nombre de entidad. Por tanto, la ruta para el controlador Player es */tables/player/{id}*. Esta convención de enrutamiento hace que **TableController** sea coherente con la [API REST][API REST] de Servicios móviles.
...
En la clase `PlayerRankController`, usaremos el método **Select** de LINQ para convertir las instancias de `PlayerRank` en instancias de `PlayerRankDto`. Actualice los métodos de controlador `GetAllPlayerRank` y `GetPlayerRank` del siguiente modo:
...
En la clase `PlayerRankController`, mueva la variable `LeaderboardContext` del constructor a una variable de clase:
...
También puede poner el método en un controlador aparte. Ningún método es mejor que el otro, depende de cómo desee organizar el código.
Para obtener más información sobre el atributo **[Route]**, consulte [Enrutamiento de atributos en Web API][Enrutamiento de atributos en Web API].

## Creación de una aplicación de la Tienda Windows

En esta sección, se describe la aplicación de la Tienda Windows que usa el servicio móvil. Sin embargo, no nos centraremos demasiado en el código XAML ni la interfaz de usuario. Más bien nos centraremos en la lógica de la aplicación. Puede descargar el proyecto completo [aquí][aquí].
...
En el cuadro de diálogo **Publicar**, haga clic en **Servicios móviles de Windows Azure**.
...
-   [Más información acerca de Servicios móviles de Azure][Más información acerca de Servicios móviles de Azure]
-   [Más información acerca de Web API][ASP.NET Web API]
-   [Control de conflictos de escritura de bases de datos][Control de conflictos de escritura de bases de datos]
...
<!-- URLs. -->
  ...
  [propiedad navigation]: http://msdn.microsoft.com/en-us/data/jj713564.aspx
  ...
  [Asignación entre tipos de base de datos y tipos de cliente en el backend .NET usando AutoMapper]: http://blogs.msdn.com/b/azuremobile/archive/2014/05/19/mapping-between-database-types-and-client-type-in-the-net-backend-using-automapper.aspx
  [Enrutamiento de atributos en Web API]: http://www.asp.net/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2
  ...
  [Más información acerca de Servicios móviles de Azure]: /en-us/develop/mobile/resources/
