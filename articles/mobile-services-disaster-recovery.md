<properties urlDisplayName="Recover your mobile service in the event of a disaster" pageTitle="Recuperación del servicio móvil en caso de un desastre - Servicios móviles de Azure" metaKeywords="" description="Learn how to recover your mobile service in the event of a disaster." metaCanonical="" services="" documentationCenter="Mobile" title="Recover your mobile service in the event of a disaster" authors="brettsam" solutions="" manager="dwrede" editor="" />
...
+ **Copia de seguridad de los datos de la base de datos SQL del servicio móvil de Azure**
	<br/>Los datos de aplicación de servicio móvil se almacenan en una Base de datos SQL de Azure. Le recomendamos que haga una copia de seguridad tal como se indica en la [Guía sobre continuidad empresarial en Base de datos SQL].
...
+ Se muestra un banner con la palabra **Incorrecto** en la parte superior de cada pestaña del servicio móvil en el portal Azure y las operaciones de administración originan mensajes de error.
...
4. Ahora puede utilizar el servicio secundario para recuperar el principal.
	...
	<p>Cuando ejecuta el comando en este paso, el servicio secundario se elimina, de tal forma que su capacidad se puede utilizar para recuperar el servicio principal. Le recomendamos que haga una copia de seguridad de los scripts y configuraciones antes de ejecutar el comando, si desea conservarlos.</p>
    </div>
   ...
   Cuando esté preparado, ejecute el comando siguiente:
	...
	<p>Pueden pasar unos minutos después de la finalización del comando hasta que pueda ver los cambios en el portal.</p>
    </div>
...
6. Asegúrese de que el servicio recuperado se comunica con la Base de datos SQL de Azure. El comando de recuperación recupera el servicio móvil, pero conserva la conexión con la base de datos original. Si el problema de la región principal de Azure también afecta a la base de datos, es posible que el servicio recuperado no se esté ejecutando correctamente. Puede utilizar el Panel de servicios de Azure para examinar el estado de la base de datos de una región específica. Si la base de datos original no funciona, puede recuperarla:
	+ Recupere la Base de datos SQL de Azure en la región de Azure donde acaba de recuperar el servicio móvil, tal como se describe en [Guía sobre continuidad empresarial en Base de datos SQL].
	+ En el portal de Azure, en la pestaña **"Configurar"** del servicio móvil, elija "Cambiar base de datos" y seleccione la base de datos recién recuperada.
...
<!-- URLs. -->
[Guía sobre continuidad empresarial en Base de datos SQL]: http://msdn.microsoft.com/es-es/library/windowsazure/hh852669.aspx
