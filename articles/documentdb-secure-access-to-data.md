<properties 
	pageTitle="Aprenda a proteger el acceso a datos en Base de datos de documentos | Azure" 
	description="Obtenga información sobre los conceptos de control de acceso en Base de datos de documentos, incluidas las claves maestras, las claves de solo lectura, los usuarios y los permisos." 
	services="documentdb" 
	authors="stephbaron" 
	manager="johnmac" 
	editor="monicar" 
...
# Protección del acceso a los datos de Base de datos de documentos #

En este artículo se proporciona información general sobre la protección del acceso a los datos almacenados en [Base de datos de documentos de Microsoft Azure](../../services/documentdb/). 

Después de leer dicha información, podrá responder a las preguntas siguientes:  

-	¿Qué son las claves maestras de Base de datos de documentos?
-	¿Qué son las claves de solo lectura de Base de datos de documentos?
...
En el contexto de estas dos categorías, Base de datos de documentos admite tres tipos de roles de control de acceso: administrador de cuenta, administrador de solo lectura y usuario de base de datos.  Los derechos de cada rol de control de acceso son:
 
- Administrador de cuenta: acceso completo a todos los recursos (administrativos y de aplicación) en una cuenta de Base de datos de documentos determinada.
- Administrador de solo lectura: acceso de solo lectura a todos los recursos (administrativos y de aplicación) en una cuenta de Base de datos de documentos determinada. 
- Usuario de base de datos: recurso de usuario de Base de datos de documentos asociado a un conjunto específico de recursos propios de Base de datos de documentos (por ejemplo, colecciones, documentos, scripts).  Puede haber uno o más recursos de usuario asociados a una base de datos determinada y cada uno de estos puede tener uno o varios permisos asociados.

Teniendo en cuenta las categorías y los recursos mencionados, el modelo de control de acceso de Base de datos de documentos define tres tipos de construcciones de acceso:

- Claves maestras: tras crear una cuenta de Base de datos de documentos, se crean dos claves maestras (principal y secundaria).  Estas claves habilitan el acceso administrativo completo a todos los recursos de dicha cuenta.

![DocumentDB master keys illustration](./media/documentdb-secure-access-to-data/masterkeys.png)

- Claves de solo lectura: tras crear una cuenta de Base de datos de documentos, se crean dos claves de solo lectura (principal y secundaria).  Estas claves habilitan el acceso de solo lectura a todos los recursos de dicha cuenta.

![DocumentDB read-only keys illustration](./media/documentdb-secure-access-to-data/readonlykeys.png)

- Tokens de recursos: un token de recurso se asocia a un recurso de permiso de Base de datos de documentos y captura la relación entre el usuario de una base de datos y el permiso que este tiene para un recurso de aplicación de Base de datos de documentos específico (por ejemplo, colección o documento).
...
El token de recurso de Base de datos de documentos ofrece una alternativa segura que permite a los clientes leer, escribir y eliminar recursos en la cuenta de Base de datos de documentos en función de los permisos concedidos y sin necesidad de una clave maestra o de solo lectura.
...
2. Dicho servicio tiene la clave maestra de la cuenta de Base de datos de documentos.
...
7. La aplicación de teléfono puede seguir usando el token de recurso para obtener acceso directo a los recursos de Base de datos de documentos con los permisos definidos por el token de recurso y para el intervalo permitido por dicho token. 
...
> [AZURE.NOTE] Cada usuario de Base de datos de documentos tiene una propiedad PermissionsLink que se puede usar para recuperar la lista de permisos asociados al usuario.

Un recurso de permiso de Base de datos de documentos se asocia a un usuario de dicha base de datos.  Cada usuario puede contener cero o más permisos de Base de datos de documentos.  Un recurso de permiso proporciona acceso a un token de seguridad que el usuario necesita al intentar obtener acceso a un recurso de aplicación específico.
...
> [AZURE.NOTE] Para ejecutar procedimientos almacenados de Base de datos de documentos, el usuario debe tener el permiso Todo en la colección donde se va a ejecutar el procedimiento almacenado.
