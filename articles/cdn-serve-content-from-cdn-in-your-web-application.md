<properties 
	...
	description="Un tutorial que le enseña a usar el contenido de una red CDN para mejorar el rendimiento de su aplicación web." 
	services="cdn" 
...
-	Una [cuenta de Microsoft Azure] activa (/account/) Puede registrarse para una cuenta de prueba.
...
6. En Visual Studio 2013, en el Explorador de soluciones, haga clic en el botón **Conectar a Windows Azure**.
...
8.  Una vez registrado, expanda **Windows Azure > Almacenamiento > su cuenta de almacenamiento**. Haga clic con el botón secundario en **Blob** y seleccione **Crear contenedor de blobs**.
...
11.	Ahora que ha cargado los blobs, debe hacerlos públicos para que pueda obtener acceso a ellos. En el Explorador de servidores, haga clic con el botón secundario en el contenedor y seleccione **Propiedades**. Establezca la propiedad **Acceso de lectura público** en **Blob**.
...
6. Cuando haya descargado el archivo de configuración de publicación, ejecute lo siguiente: 
		...
		Import-AzurePublishSettingsFile "<suRutaAccesArchivoDescargado>"
...
2. Haga clic con el botón secundario en el archivo descargado y haga clic en **Propiedades**.
3. Haga clic en **Desbloquear**.
4. Abra una ventana de PowerShell y ejecute lo siguiente:

		cd <carpetaProyecto>
		.\UploadContentToAzureBlobs.ps1 -StorageAccount "<suNombreCuentaAlmacenamiento>" -StorageContainer "<suNombreContenedor>"
...
Para un ejemplo de la integración de scripts de PowerShell en su configuración de entrega continua, consulte [Entrega continua para Servicio en la nube de Azure](cloud-services-dotnet-continuous-delivery.md). 
...
También puede hacer en el script de PowerShell para establecer todos los encabezados de Cache-Control de los blobs. Para el script de [Carga de contenido automatizada desde la aplicación ASP .NET al extremo de la red CDN](#upload), encuentre el fragmento de código siguiente:
...
Con [Sitios web Azure](/services/websites/) y [Servicios en la nube de Azure](/services/cloud-services/), consigue la mejor integración de red CDN de Azure con [Unión y minificación de ASP.NET](http://www.asp.net/mvc/tutorials/mvc-4/bundling-and-minification). 
...
- Integración de la implementación de contenido (imágenes, scripts y hojas de estilo) como parte del proceso de [implementación continua](app-service-web/web-sites-publish-source-control.md) de su sitio web Azure
...
- En los archivos .cshtml, transforme las etiquetas de CSS/scripts representadas para utilizar la red CDN de Azure. Por ejemplo:
		...
		@Html.Raw(Styles.Render("~/Content/css").ToString().Insert(0, "http://<suNombreCDN>.vo.msecnd.net"))
