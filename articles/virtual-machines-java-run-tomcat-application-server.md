<properties urlDisplayName="Tomcat on Virtual Machine" pageTitle="Tomcat en una máquina virtual - Tutorial de Azure" metaKeywords="Azure vm, creating vm Tomcat, configuring vm Tomcat" description="Aprenda a crear una máquina virtual de Windows y configurarla para ejecutar un servidor de aplicaciones Apache Tomcat." metaCanonical="" services="virtual-machines" documentationCenter="Java" title="How to run a Java application server on a virtual machine" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" scriptId="" videoId="" />
...
6. En el cuadro de diálogo <strong>Configuración de la máquina virtual</strong> :
    ...
    4. Haga clic en **Siguiente**.
7. En el cuadro de diálogo final <strong>Configuración de la máquina virtual</strong> :
...
1. Mientras tiene una sesión iniciada en la máquina virtual, abra una sesión de explorador en <http://tomcat.apache.org/download-70.cgi>.
...
4. En el asistente para la **instalación de Apache Tomcat**, siga las indicaciones para instalar Tomcat. En este tutorial, es adecuado aceptar los valores predeterminados. Cuando llegue al cuadro de diálogo **Completing the Apache Tomcat Setup Wizard** (Finalización del asistente para la instalación de Apache Tomcat), si lo desea, puede marcar **Run Apache Tomcat** (Ejecutar Apache Tomcat) para iniciar Tomcat ahora. Haga clic en **Finish** (Finalizar) para finalizar el proceso de instalación de Tomcat.
...
6. En el cuadro de diálogo **Agregar extremo**, asegúrese de que la casilla **Agregar extremo independiente** está marcada y, a continuación, haga clic en **Siguiente**.
7. En el cuadro de diálogo <strong>Detalles del nuevo extremo</strong>:
...
6. Para la nueva regla, seleccione **Puerto** en**Tipo de regla** y, a continuación, haga clic en **Siguiente**.

 ![New inbound rule port][NewRulePort]

7. Seleccione**TCP** para el protocolo y especifique **8080**para el puerto; a continuación, haga clic en **Siguiente**.

 ![New inbound rule ][NewRuleProtocol]

8. Elija **Permitir la conexión** y, a continuación, haga clic en **Siguiente**.

 ![New inbound rule action][NewRuleAction]

9. Asegúrese de que las casillas **Dominio**, **Privado** y **Público** estén activadas para el perfil y, a continuación, haga clic en **Siguiente**.

 ![New inbound rule profile][NewRuleProfile]

10. Especifique un nombre para la regla, por ejemplo **HttpIn** (sin embargo, no es necesario que el nombre de la regla coincida con el nombre del extremo) y haga clic en **Finalizar**.  

 ![New inbound rule name][NewRuleName]

En este momento, el sitio web de Tomcat se debería ver desde un explorador externo, usando una URL con el formato **http://*your\_DNS\_name*.cloudapp.net**, donde ***your\_DNS\_name*** es el nombre DNS que especificó cuando creó la máquina virtual.
