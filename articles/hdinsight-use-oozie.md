<properties 
	pageTitle="Uso de Oozie con Hadoop en HDInsight | Azure" 
	description="Use Oozie con Hadoop en HDInsight, una solución para grandes volúmenes de datos. Aprenda a definir un flujo de trabajo de Oozie y envíe un trabajo de Oozie." 
	services="hdinsight" 
	documentationCenter="" 
	authors="mumian" 
	manager="paulettm" 
...
1. Una acción de Hive ejecuta un script de HiveQL para contar las apariciones de cada tipo de nivel de registro en un archivo log4j. Cada archivo log4j consta de una línea de campos que contiene uno llamado [LOG LEVEL] que muestra el tipo y la gravedad, por ejemplo:
...
> [AZURE.NOTE]Este tutorial funciona en las versiones 3.0 y 2.1 de HDInsight. Este tutorial no se ha probado en un emulador de HDInsight.
...
- **Una estación de trabajo** con Azure PowerShell instalado y configurado. Para obtener instrucciones, consulte [Instalación y configuración de Azure PowerShell][powershell-install-configure]. Para ejecutar scripts de Windows PowerShell, debe ejecutar como administrador y establecer la directiva de ejecución en *RemoteSigned*. Para obtener más información, consulte [Ejecutar scripts de Windows PowerShell][powershell-script].
- **Un clúster de HDInsight**. Para obtener información sobre la creación de un clúster de HDInsight, consulte [Aprovisionamiento de clústeres de Hadoop en HDInsight con opciones personalizadas][hdinsight-provision-clusters] o [Introducción al uso de Hadoop con Hive en HDInsight para analizar el uso de datos móviles][hdinsight-get-started]. Para completar el tutorial, necesitará los datos siguientes:
...
<tr><td>Nombre del contenedor de blobs de Azure</td><td>$containerName</td><td></td><td>Para este ejemplo, use el nombre del contenedor de blobs utilizado para el sistema de archivos predeterminado del clúster de HDInsight. De manera predeterminada, tiene el mismo nombre que el del clúster de HDInsight.</td></tr>
</table>

- **Una base de datos SQL de Azure**. Debe configurar una regla de firewall para que la base de datos SQL permita el acceso desde la estación de trabajo. Para obtener instrucciones sobre la creación de una base de datos SQL de Azure y la configuración del firewall, consulte [Introducción a la base de datos SQL de Microsoft Azure][sqldatabase-get-started]. En este artículo se proporciona un script de Windows PowerShell para crear la tabla de base de datos SQL de Azure requerida para este tutorial.
...
<tr><td>Nombre del servidor de base de datos SQL</td><td>$sqlDatabaseServer</td><td></td><td>Base de datos SQL de Azure en la que Sqoop exportará los datos. </td></tr>
<tr><td>Nombre de inicio de sesión de la base de datos SQL</td><td>$sqlDatabaseLogin</td><td></td><td>Nombre de inicio de sesión de la base de datos SQL de Azure.</td></tr>
<tr><td>Contraseña de inicio de sesión de la base de datos SQL</td><td>$sqlDatabaseLoginPassword</td><td></td><td>Contraseña de inicio de sesión de la base de datos SQL de Azure.</td></tr>
<tr><td>Nombre de la base de datos SQL</td><td>$sqlDatabaseName</td><td></td><td>Base de datos SQL de Azure en la que Sqoop exportará los datos. </td></tr>
</table>> [AZURE.NOTE]De forma predeterminada, una base de datos SQL de Azure permite realizar conexiones desde servicios de Azure, como HDInsight de Azure. Si la configuración del firewall está deshabilitada, debe habilitarla en el portal de Azure. Para obtener instrucciones sobre la creación de una base de datos SQL y la configuración de las reglas de firewall, consulte [Creación y configuración de una base de datos SQL][sqldatabase-create-configue].


> [AZURE.NOTE]Completar los valores de las tablas le resultará útil para completar este tutorial.


##Definición del flujo de trabajo de Oozie y el script de HiveQL relacionado

Las definiciones de los flujos de trabajo de Oozie se escriben en hPDL (un lenguaje de definición de proceso XML). El nombre de archivo de flujo de trabajo predeterminado es *workflow.xml*. Guardará el archivo de flujo de trabajo localmente y lo implementará en el clúster de HDInsight con Windows PowerShell posteriormente en este tutorial.
...
Hay un problema conocido de la ruta de acceso de Hive. Lo observará cuando envíe un trabajo de Oozie. Las instrucciones para corregirlo pueden encontrarse en Wiki de TechNet: [Error de Hive de HDInsight: No se puede cambiar de nombre][technetwiki-hive-error].
...
1. Cree un archivo de texto con el siguiente contenido:
	...
	RunHiveScript tiene distintas variables. Pasará los valores cuando envíe el trabajo de Oozie desde la estación de trabajo con Windows PowerShell.
...
Ejecutará un script de Windows PowerShell para realizar las siguientes acciones:
...
HDInsight usa los blobs del Almacenamiento de Azure para el almacenamiento de datos. Para obtener más información, consulte [Uso de Almacenamiento de blobs de Azure con HDInsight][hdinsight-storage].

Cuando se aprovisiona un clúster de HDInsight, se designan una cuenta de almacenamiento de Azure y un contenedor de blobs específico de dicha cuenta como el sistema de archivos predeterminado, de la misma forma que en HDFS. Además de esta cuenta de almacenamiento, puede agregar más cuentas de almacenamiento desde la misma suscripción de Azure o desde otra diferente durante el proceso de aprovisionamiento. Para obtener instrucciones sobre cómo agregar otras cuentas de almacenamiento, consulte [Aprovisionamiento de clústeres de Hadoop en HDInsight][hdinsight-provision]. Para simplificar el script de Windows PowerShell que se utiliza en este tutorial, todos los archivos se almacenan en el contenedor del sistema de archivos predeterminado, ubicado en */tutorials/useoozie*. De forma predeterminada, este contenedor tiene el mismo nombre que el del clúster de HDInsight. La sintaxis es:

	wasb[s]://<ContainerName>@<StorageAccountName>.blob.core.windows.net/<path>/<filename>

> [AZURE.NOTE]La sintaxis *wasb://* solo es compatible en los clústeres de HDInsight versión 3.0. La antigua sintaxis *asv://* es compatible con los clústeres de HDInsight 2.1 y 1.6, pero no es compatible con los clústeres de HDInsight 3.0.
...
Para acceder a un archivo almacenado en el contenedor del sistema de archivos predeterminado desde HDInsight se puede usar cualquiera de los URI siguientes (el siguiente código usa workflow.xml como ejemplo):
...
Existen determinados aspectos que debe conocer en relación con la tablas internas y externas de Hive:
...
4. Haga clic en **Ejecutar script** o presione **F5** para ejecutar el script. La salida debe ser similar a la siguiente:

	![Resultado de preparación del tutorial][img-preparation-output]

##Ejecución del proyecto de Oozie
...
3. Copie el siguiente script en el panel de scripts y, a continuación, configure las diez primeras variables (omita la variable $storageUri).
...
3. Agregue lo siguiente al script. Este script define la carga de Oozie.
...
4. Agregue lo siguiente al script. Este script comprueba el estado del servicio web de Oozie.
...
6. Agregue lo siguiente al script. Este script comprueba el estado del trabajo de Oozie.
...
8. Haga clic en **Ejecutar script** o presione **F5** para ejecutar el script. La salida debe ser similar a la siguiente:

	![Resultado del flujo de trabajo de ejecución del tutorial][img-runworkflow-output]

8. Conéctese a la base de datos SQL de Azure para ver los datos exportados.

**Para comprobar el registro de errores del trabajo**

Para solucionar los problemas de un flujo de trabajo, puede encontrar el archivo de registro de Oozie en *C:\\apps\\dist\\oozie-3.3.2.1.3.2.0-05\\oozie-win-distro\\logs\\Oozie.log* o en *C:\\apps\\dist\\oozie-4.0.0.2.0.7.0-1528\\oozie-win-distro\\logs\\Oozie.log* desde el nodo principal del clúster. Para obtener información acerca de, vea [Administración de clústeres de Hadoop en HDInsight mediante el Portal de administración de Azure][hdinsight-admin-portal].
...
##Pasos siguientes
En este tutorial ha aprendido a definir un flujo de trabajo de Oozie y a ejecutar un trabajo de Oozie mediante Windows PowerShell. Para obtener más información, consulte los artículos siguientes:
