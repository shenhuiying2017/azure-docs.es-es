...
Vea [Invocar programas MapReduce de la factoría de datos][data-factory-map-reduce] para obtener detalles acerca la ejecución de programas MapReduce en un clúster de HDInsight desde una canalización de la factoría de datos de Azure mediante transformaciones de MapReduce de la actividad de HDInsight. En este artículo se describe el uso de la transformación de Pig y Hive de la actividad de HDInsight.

## En este artículo

Sección | Descripción
------- | -----------
[Ejemplo de JSON de Pig](#PigJSON) | Esta sección proporciona el esquema JSON para definir una actividad de HDInsight que usa una transformación de Pig. 
[Ejemplo de JSON de Hive](#HiveJSON) | Esta sección proporciona esquema JSON para definir una actividad de HDInsight que usa una transformación de Hive. 
[Uso de scripts de Pig y Hive que se almacenan en el almacenamiento de blobs de Azure](#ScriptInBlob) | Describe cómo hacer referencia a scripts de Pig y Hive almacenados en un almacenamiento de blobs de Azure desde una actividad de HDInsight mediante la transformación de Pig y Hive.
[Consultas de Pig y Hive parametrizadas](#ParameterizeQueries) | Describe cómo especificar especificar valores de parámetros que se usan en los scripts de Pig y Hive mediante la propiedad **extendedProperties** en JSON.
[Tutorial: Uso de Hive con la Factoría de datos de Azure](#Waltkthrough) | Proporciona instrucciones detalladas para crear una canalización que use Hive para procesar datos.  
...
1. Cree un archivo JSON llamado **HiveInputBlobTable.json** en la carpeta **C:\ADFGetStarted\Hive** con el siguiente contenido.
	...
	**Tenga en cuenta lo siguiente:**
	...
	- **folderPath** especifica el contenedor o carpeta de blobs para los datos de entrada.
	- **frequency=Day** e **interval=1** son los segmentos que están disponibles diariamente.
	- **waitOnExternal** significa que estos datos no se producen por otra canalización, sino que se producen externamente para la factoría de datos.
	

	Vea la [Referencia para desarrolladores de la Factoría de datos][developer-reference] para ver las descripciones de las propiedades de JSON.  
...
6. Ejecute el siguiente comando para crear la tabla de entrada en **ADFTutorialDataFactory**.
	...
	Vea la [Referencia de cmdlets de la Factoría de datos][cmdlet-reference] para obtener una visión general detallada de los cmdlets de la factoría de datos. 
...
3. Programe la canalización.
	...
	> [WACOM.NOTE] Reemplace el valor **StartDateTime** por los tres días antes del día actual y el valor **EndDateTime** por el día actual. Tanto StartDateTime como EndDateTime son horas UTC y deben estar en [formato ISO](http://en.wikipedia.org/wiki/ISO_8601). Por ejemplo: 2014-10-14T16:32:41Z. 
...
4. Vea la sección [Supervisión de los conjuntos de datos y la canalización][adfgetstartedmonitoring] del artículo [Introducción a la Factoría de datos][adfgetstarted].   
