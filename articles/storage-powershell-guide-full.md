<properties 
	pageTitle="Usar Azure PowerShell con Almacenamiento de Azure" 
	description="Aprenda a usar Azure PowerShell para Almacenamiento de Azure" 
	services="storage" 
...
Bienvenido a la guía de Azure PowerShell para el almacenamiento de Azure.

Puede usar varias herramientas de desarrollo y administración para llevar a cabo una serie de tareas en Almacenamiento de Azure; por ejemplo, podrá utilizar el [Portal de administración de Azure](, la http://manage.windowsazure.com/)[Biblioteca cliente de almacenamiento](http://msdn.microsoft.com/library/azure/dn261237.aspx), la [API de REST](http://msdn.microsoft.com/library/azure/dd179355.aspx), y [Azure PowerShell](http://msdn.microsoft.com/library/azure/jj156055.aspx). 

En esta guía, aprenderá a usar Azure PowerShell para realizar tareas comunes en Almacenamiento de Azure. Azure PowerShell es un módulo que ofrece cmdlets para administrar Azure mediante Windows PowerShell. Es un Shell de línea de comandos y un lenguaje de scripting basados en tareas y diseñados especialmente para la administración del sistema. Con PowerShell, podrá controlar y automatizar fácilmente la administración de los servicios y aplicaciones de Azure. Por ejemplo, puede usar los cmdlets para realizar las mismas tareas que podría realizar a través del Portal de administración de Azure. 
...
La primera sección de esta guía le permitirá echar un rápido vistazo a Almacenamiento de Azure y a PowerShell. Para obtener información detallada e instrucciones, puede comenzar por los [requisitos previos para utilizar Azure PowerShell con Almacenamiento de Azure](#pre).
...
## Tabla de contenido
 ...
 - [Cómo administrar las colas de Azure y la cola de mensajes][]
	 ...
	 - [Cómo administrar la cola de mensajes][]
...
## <a name="getstart"></a>Introducción de 5 minutos a Almacenamiento de Azure y PowerShell
En esta sección, aprenderá en 5 minutos a acceder a Almacenamiento de Azure a través de PowerShell. 

**Nuevo en Azure:** obtenga una suscripción de Microsoft Azure y una cuenta de Microsoft asociada a esa suscripción. Para obtener información sobre las opciones de compra de Azure, consulte [Prueba gratuita](, http://azure.microsoft.com/pricing/free-trial/)[Opciones de compra](http://azure.microsoft.com/pricing/purchase-options/) y [Ofertas para miembros](http://azure.microsoft.com/pricing/member-offers/) (para miembros de MSDN, Microsoft Partner Network, BizSpark y otros programas de Microsoft).

**¿Le resulta difícil administrar su cuenta de Microsoft Azure o su suscripción?** Consulte [Administrar cuentas, suscripciones y roles administrativos](https://msdn.microsoft.com/library/azure/hh531793.aspx).

**Después de crear una suscripción de Microsoft Azure y una cuenta, deberá:** 

1.	Descargar e instalar [Azure PowerShell](http://go.microsoft.com/?linkid=9811175&clcid=0x409) en el equipo local.
2.	Iniciar el entorno de scripting integrado (ISE) de Windows PowerShell: En el equipo local, vaya al menú **Inicio**. Escriba **Herramientas administrativas** para localizar la opción y haga clic en ella para ejecutarla. En la ventana **Herramientas administrativas**, haga clic derecho en **Windows PowerShell ISE** y haga clic en **Ejecutar como administrador**. 
...
5.	En **Windows PowerShell ISE**, presione CTRL+V para copiar el script. Haga clic en **Archivo** > **Guardar**. En el cuadro de diálogo **Guardar como**, escriba el nombre del archivo de script; por ejemplo, "mystoragescript". Haga clic en **Save**.
...
6.	Ahora deberá actualizar las variables del script basadas en la configuración. Debe actualizar la variable **$SubscriptionName** con su propia suscripción. Puede mantener las demás variables tal y como se especifican en el script o actualizarlas a su gusto.
	...
	- **$SubscriptionName:** debe actualizar esta variable con su propio nombre de suscripción. Escoja uno de los tres métodos que se ofrecen para buscar el nombre de su suscripción: 
		...
		b. Actualmente Azure admite dos portales: el [Portal de administración de Azure](https://manage.windowsazure.com/) y el [Portal de vista previa de Azure](https://portal.azure.com/). Si inicia sesión en el [Portal de administración de Azure](https://portal.azure.com/), desplácese hacia abajo y haga clic en **Configuración**, a la izquierda del portal. Haga clic en **Suscripciones**. Copie el nombre de la suscripción que desee usar para ejecutar los scripts que se proporcionan en esta guía. Puede revisar la siguiente captura de pantalla a modo de ejemplo:

		![Azure Management Portal][Image1]

		c. Si inicia sesión en el [Portal de vista previa de Azure](https://portal.azure.com/), en el menú Concentrador que tiene a la izquierda, haga clic en **EXAMINAR**. A continuación, haga clic en **Todo** y en **Suscripciones**. Copie el nombre de la suscripción que desee usar para ejecutar los scripts que se proporcionan en esta guía. Puede revisar la siguiente captura de pantalla a modo de ejemplo:

		![Azure Preview Portal][Image2]
 

	- **$StorageAccountName:** use el nombre proporcionado en el script o escriba un nombre nuevo para su cuenta de almacenamiento. **Importante:** el nombre de la cuenta de almacenamiento ha de ser único en Azure y ha de escribirlo en minúsculas.
	 
	- **$Location:** use el comando "Oeste de EE. UU." proporcionado en el script o escoja otra localización de Azure como "Este de EE. UU", "Norte de Europa", etc.
...
## <a name="pre"></a>Requisitos previos para usar Azure PowerShell con Almacenamiento de Azure
Necesita una suscripción de Azure y una cuenta para poder ejecutar los cmdlets de PowerShell que se proporcionan en esta guía. Para ver qué opciones de suscripción de Azure tiene disponibles, visite [Introducción a Azure](http://azure.microsoft.com/pricing/free-trial/).

Azure PowerShell es un módulo que ofrece cmdlets para administrar Azure mediante Windows PowerShell. Para obtener más información acerca de la instalación y configuración de Azure PowerShell, consulte [Cómo instalar y configurar Azure PowerShell](http://azure.microsoft.com/documentation/articles/install-configure-powershell/). Le recomendamos que descargue e instale o que actualice el módulo de Azure PowerShell para tener la versión más reciente, antes de comenzar a usar esta guía. 

Puede ejecutar los cmdlets en la consola de PowerShell de Azure, la consola de Windows PowerShell estándar o en el entorno de scripting integrado (ISE) de Windows PowerShell. Por ejemplo, para abrir la **consola de Azure PowerShell**, vaya al menú Inicio, escriba Microsoft Azure PowerShell, haga clic derecho en él y ejecútelo como administrador. Para abrir **Windows PowerShell ISE**, vaya al menú Inicio, escriba Herramientas administrativas y haga clic en la aplicación para ejecutarla. En la ventana Herramientas administrativas, haga clic derecho en Windows PowerShell ISE y haga clic en Ejecutar como administrador. Para obtener más detalles acerca de la instalación y la configuración, consulte la sección Cómo instalar Azure PowerShell en el tutorial [Cómo instalar y configurar Azure PowerShell](http://azure.microsoft.com/documentation/articles/install-configure-powershell/).

## <a name="manageaccount"></a>Cómo administrar cuentas de almacenamiento en Azure
### <a name="setdefsub"></a>Cómo establecer una suscripción predeterminada de Azure
Para administrar Almacenamiento de Azure con Azure PowerShell, necesita autenticar su entorno de cliente con Azure a través de una autenticación basada en certificados o la autenticación de Azure Active Directory. Para obtener más información, consulte la sección [Cómo conectarse a su suscripción](http://azure.microsoft.com/documentation/articles/install-configure-powershell/#Connect) que se encuentra en el tutorial [Cómo instalar y configurar Azure PowerShell](http://azure.microsoft.com/documentation/articles/install-configure-powershell/). Esta guía usa la autenticación de Azure Active Directory.
...
### <a name="createaccount"></a>Cómo crear una nueva cuenta de almacenamiento de Azure
...
Para obtener más información, consulte [Acerca de las cuentas de almacenamiento de Azure](http://azure.microsoft.com/documentation/articles/storage-whatis-account/).

> [AZURE.IMPORTANT] El nombre de la cuenta de almacenamiento es único en Azure y ha de estar en minúscula. Para las convenciones de nomenclatura y otras restricciones, consulte [Acerca de las cuentas de almacenamiento de Azure](, http://azure.microsoft.com/documentation/articles/storage-whatis-account/)[Convenciones de nomenclatura y referencia de contenedores, blobs y metadatos](http://msdn.microsoft.com/library/azure/dd135715.aspx) y [Crear una cuenta de almacenamiento](http://msdn.microsoft.com/library/azure/hh264518.aspx).

### <a name="defaultaccount"></a>Cómo configurar una cuenta de almacenamiento de Azure predeterminada
...
### <a name="listaccounts"></a>Cómo enumerar todas las cuentas de almacenamiento de Azure en una suscripción
Cada suscripción de Azure puede tener hasta 100 cuentas de almacenamiento. Para obtener la última información sobre los límites, consulte [Suscripción de Azure y límites de servicio, cuotas y restricciones](http://azure.microsoft.com/documentation/articles/azure-subscription-service-limits/).
...
### <a name="createctx"></a>Cómo crear un contexto de almacenamiento de Azure
...
- Ejecute el cmdlet [Get-AzureStorageKey](http://msdn.microsoft.com/library/azure/dn495235.aspx) cmdlet para conseguir la clave de acceso de almacenamiento principal de la cuenta de almacenamiento de Azure. A continuación, llame a la cmdlet [New-AzureStorageContext](http://msdn.microsoft.com/library/azure/dn806380.aspx) para crear un contexto de almacenamiento:
...
- Genere un token de firma de acceso compartido para un contenedor de almacenamiento de Azure y úselo para crear un contexto de almacenamiento:
	...
	Para obtener más información, consulte [New-AzureStorageContainerSASToken](http://msdn.microsoft.com/library/azure/dn806416.aspx) y [Firmas de acceso compartido, 1ª parte: Descripción del modelo SAS ](http://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/).
...
Para obtener más información sobre cómo configurar una cadena de conexión de almacenamiento, consulte [Configurar cadenas de conexión](http://msdn.microsoft.com/library/azure/ee758697.aspx).
...
## <a name="manageblobs"></a>Cómo administrar los blobs de Azure y las instantáneas de blob
El almacenamiento de blobs de Azure es un servicio para almacenar grandes cantidades de datos no estructurados, como texto o datos binarios, a los que puede acceder desde cualquier lugar del mundo a través de HTTP o HTTPS. Para realizar esta sección, supondremos que ya está familiarizado con los conceptos del Servicio de almacenamiento de blobs de Azure. Para obtener más información, consulte [Cómo usar el almacenamiento de blobs en .NET](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-blobs) y [Conceptos del servicio blob](http://msdn.microsoft.com/library/azure/dd179376.aspx).

### <a name="container"></a>Cómo crear un contenedor
...
> [AZURE.NOTE] Existen tres niveles de acceso de lectura anónimo: **Desactivado**, **Blob** y **Contenedor**. Para evitar el acceso anónimo a los blobs, establezca el parámetro de permiso en **Desactivado**. El nuevo contenedor es privado por defecto y solo puede acceder a él el propietario de la cuenta. Para permitir un acceso de lectura público y anónimo a los recursos de blob, pero no a los metadatos del contenedor o a la lista de blobs que existen en el contenedor, establezca el parámetro de permiso en **Blob**. Para hacer que el acceso de lectura a los recursos de blob, los metadatos del contenedor y a la lista de blobs que existen en el contenedor sea totalmente público, deberá establecer el parámetro de permiso en **Contenedor**. Para obtener más información, consulte la enumeración [BlobContainerPublicAccessType](http://msdn.microsoft.com/library/azure/microsoft.windowsazure.storageclient.blobcontainerpublicaccesstype(v=azure.95).aspx).

### <a name="uploadblob"></a>Cómo cargar un blob en un contenedor
...
Para cargar blobs en un contenedor, puede utilizar el cmdlet [Set-AzureStorageBlobContent](http://msdn.microsoft.com/library/azure/dn806379.aspx). Este comando carga de forma predeterminada los archivos locales a un blob en bloque. Para especificar el tipo de blob, puede usar el parámetro -BlobType. 
...
### <a name="downblob"></a>Cómo descargar blobs de un contenedor
...
### <a name="copyblob"></a>Cómo copiar blobs de un contenedor de almacenamiento a otro
...
### <a name="deleteblob"></a>Cómo eliminar un blob
...
## <a name="manageshots"></a>Cómo administrar las instantáneas de blob de Azure
...
### <a name="createshot"></a>Cómo crear una instantánea de blob
Para crear una instantánea de un blob, deberá conseguir una referencia de blob y después llamar al método ICloudBlob.CreateSnapshot que se encuentra en ella. El siguiente ejemplo primero establece las variables de una cuenta de almacenamiento y después crea un contexto de almacenamiento. A continuación, recupera una referencia de blob mediante el cmdlet [Get-AzureStorageBlob](http://msdn.microsoft.com/library/azure/dn806392.aspx) y ejecuta el método [ICloudBlob.CreateSnapshot](http://msdn.microsoft.com/library/azure/microsoft.windowsazure.storage.blob.icloudblob.aspx) para crear una instantánea. 
...
### <a name="listshot"></a>Cómo enumerar las instantáneas de un blob
...
### <a name="copyshot"></a>Cómo copiar una instantánea de un blob
Puede copiar una instantánea de un blob para restaurar la instantánea. Para obtener más información y restricciones, consulte [Crear una instantánea de un blob](http://msdn.microsoft.com/library/azure/hh488361.aspx). El siguiente ejemplo primero establece las variables de una cuenta de almacenamiento y después crea un contexto de almacenamiento. A continuación precisa las variables del contenedor y del nombre del blob. Recupera una referencia de blob mediante el cmdlet [Get-AzureStorageBlob](http://msdn.microsoft.com/library/azure/dn806392.aspx) y ejecuta el método [ICloudBlob.CreateSnapshot](http://msdn.microsoft.com/library/azure/microsoft.windowsazure.storage.blob.icloudblob.aspx) para crear una instantánea. Después, el ejemplo ejecuta el cmdlet [Start-AzureStorageBlobCopy](http://msdn.microsoft.com/library/azure/dn806394.aspx) para copiar la instantánea de un blob mediante el objeto ICloudBlob del blob de origen. Asegúrese de actualizar las variables según su configuración antes de ejecutar el ejemplo. Tenga en cuenta que en siguiente ejemplo se supone que los contenedores de origen y destino, así como el blob de origen, ya existen. 

    #Definir la cuenta de almacenamiento y el contexto.
    $StorageAccountName = "yourstorageaccount"
    $StorageAccountKey = "Storage key for yourstorageaccount ends with =="
    $Ctx = New-AzureStorageContext -StorageAccountName $StorageAccountName -StorageAccountKey $StorageAccountKey
...
### <a name="createtable"></a>Cómo crear una tabla
...
### <a name="gettable"></a>Cómo recuperar una tabla
Puede consultar y recuperar una o todas las tablas de una cuenta de almacenamiento. En el siguiente ejemplo le indicaremos cómo recuperar una tabla que mediante el cmdlet [Get-AzureStorageTable](http://msdn.microsoft.com/library/azure/dn806411.aspx).
...
### <a name="remtable"></a>Cómo eliminar una tabla
...
### <a name="mngentity"></a>Cómo administrar las entidades de tabla
...
#### <a name="addentity"></a>Cómo agregar entidades de tabla
Para agregar una entidad a una tabla, primero deberá crear un objeto que defina las propiedades de la entidad. Una entidad puede tener hasta 255 propiedades, incluyendo 3 propiedades de sistema: **PartitionKey**, **RowKey** y **Timestamp**. Usted será el que deba encargarse de insertar y actualizar los valores de **PartitionKey** y **RowKey**. El servidor se encarga de administrar el valor de **Timestamp**, así que no podrá modificarlo. Tanto **PartitionKey** como **RowKey**, identifican de forma exclusiva todas las entidades de una tabla. 

-	**PartitionKey:** Determina la partición en la que se almacena la entidad.
-	**RowKey:** identifica de forma exclusiva la entidad que se encuentra dentro de la partición.

Puede definir hasta 252 propiedades personalizadas para una entidad. Para obtener más información, consulte [Descripción del modelo de datos del servicio de tabla](http://msdn.microsoft.com/library/azure/dd179338.aspx).
...
#### <a name="queryentity"></a>Cómo consultar entidades de tabla
Para consultar una tabla, debe usar la clase [Microsoft.WindowsAzure.Storage.Table.TableQuery](http://msdn.microsoft.com/library/azure/microsoft.windowsazure.storage.table.tablequery.aspx). Para realizar el siguiente ejemplo, tendrá que tener ejecutado el script que se le proporcionó en la sección Cómo agregar entidades de esta guía. Primero, el ejemplo establece una conexión a Almacenamiento de Azure mediante el contexto de almacenamiento, en el cual se incluyen el nombre de la cuenta y su clave de acceso. A continuación, intenta recuperar la tabla "Empleados" que se creó antes mediante el cmdlet [Get-AzureStorageTable](http://msdn.microsoft.com/library/azure/dn806411.aspx). Al llamar al cmdlet [New-Object](http://technet.microsoft.com/library/hh849885.aspx) que se encuentra en la clase Microsoft.WindowsAzure.Storage.Table.TableQuery, se crea un nuevo objeto de consulta. En el ejemplo, buscaremos las entidades que tienen una columna "ID" cuyo valor es 1, tal y como se especificó en un filtro de cadena. Para obtener más información, consulte la sección Construir cadenas de filtro en [Consultar tablas y entidades](http://msdn.microsoft.com/library/azure/dd894031.aspx). Al ejecutar esta consulta, devolverá todas las entidades que coinciden con los criterios del filtro.    
...
#### <a name="deleteentity"></a>Cómo eliminar entidades de tabla
...
## <a name="managequeues"></a>Cómo administrar las colas de Azure y la cola de mensajes
El almacenamiento en cola de Azure es un servicio para almacenar grandes cantidades de mensajes a los que puede obtenerse acceso desde cualquier lugar del mundo a través de llamadas autenticadas con HTTP o HTTPS. Para realizar esta sección supondremos que ya está familiarizado con los conceptos del Servicio de almacenamiento de colas de Azure. Para obtener información detallada, consulte [Cómo usar el almacenamiento de colasen .NET](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-queues/) y [Conceptos del servicio de cola](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-queues/#concepts).

En esta sección le mostraremos cómo administrar el servicio de almacenamiento de cola de Azure con Azure PowerShell. Entre los escenarios descritos se incluyen la **inserción** y la **eliminación** de mensajes de cola, así como la **creación**, **eliminación** y **recuperación de colas**.
 
### <a name="createqueue"></a>Cómo crear una cola
...
Para obtener información sobre las convenciones de nomenclatura del Servicio de cola de Azure consulte [Nomenclatura de colas y metadatos](http://msdn.microsoft.com/library/azure/dd179349.aspx).

### <a name="getqueue"></a>Cómo recuperar una cola
...
Si llama al cmdlet [Get-AzureStorageQueue](http://msdn.microsoft.com/library/azure/dn806377.aspx) sin parámetros, éste obtendrá una lista de todas las colas.

### <a name="remqueue"></a>Como eliminar una cola
...
### <a name="mngqueuemsg"></a>Cómo administrar la cola de mensajes
Actualmente, Azure PowerShell no proporciona cmdlets para administrar directamente los mensajes en cola. Para realizar operaciones en mensajes en cola, puede usar las clases que se proporcionan en la [Biblioteca de cliente de almacenamiento de Azure para .NET](http://msdn.microsoft.com/library/azure/wa_storage_30_reference_home.aspx). 

#### <a name="addqueuemsg"></a>Cómo insertar un mensaje en una cola
Para insertar un mensaje en una cola ya existente, primero deberá crear una instancia nueva de la clase [Microsoft.WindowsAzure.Storage.Queue.CloudQueueMessage](http://msdn.microsoft.com/library/azure/jj732474.aspx). A continuación, llame al método [AddMessage](http://msdn.microsoft.com/library/azure/microsoft.windowsazure.storage.queue.cloudqueue.addmessage.aspx). Un CloudQueueMessage se puede crear a partir de una cadena (en formato UTF-8) o de una matriz de bytes.
 
En el siguiente ejemplo le mostraremos cómo agregar un mensaje a una cola. Primero, el ejemplo establece una conexión a Almacenamiento de Azure mediante el contexto de cuenta de almacenamiento, en el cual se incluyen el nombre de la cuenta y su clave de acceso. A continuación, éste recupera la cola especificada usando el cmdlet [Get-AzureStorageQueue](https://msdn.microsoft.com/library/azure/dn806377.aspx). Si la cola existe, se usa el cmdlet [New-Object](http://technet.microsoft.com/library/hh849885.aspx) para crear una instancia de la clase [Microsoft.WindowsAzure.Storage.Queue.CloudQueueMessage](http://msdn.microsoft.com/library/azure/jj732474.aspx). Después, el ejemplo llama al método [AddMessage](http://msdn.microsoft.com/library/azure/microsoft.windowsazure.storage.queue.cloudqueue.addmessage.aspx) en este objeto de mensaje para agregarlo a la cola. Aquí tiene el código que recupera una cola e inserta el mensaje 'MessageInfo':
...
## <a name="files"></a>Cómo administrar archivos y recursos compartidos de archivos de Azure
...
Para obtener más información sobre el almacenamiento de archivos de Azure, consulte [Cómo usar el almacenamiento de archivos de Azure](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-files/) y [API de REST del servicio de archivos](http://msdn.microsoft.com/library/azure/dn167006.aspx).

Para obtener información sobre cómo administrar el almacenamiento de archivos de Azure mediante Azure PowerShell, consulte [Usar PowerShell para crear un recurso compartido de archivos](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-files/#use-cmdlets).

## <a name="stganalytics"></a>Cómo establecer y consultar el análisis de almacenamiento
El [Análisis de almacenamiento de Azure](http://msdn.microsoft.com/library/azure/hh343270.aspx), le permite recopilar métricas (Métricas de almacenamiento) de sus cuentas de almacenamiento de Azure y sus datos de registro (Registros de almacenamiento), sobre aquellas solicitudes enviadas a la cuenta de almacenamiento. Puede usar las Métricas de almacenamiento para supervisar el estado de una cuenta de almacenamiento y los Registros de almacenamiento para diagnosticar y solucionar los problemas que surjan con su cuenta de almacenamiento.
...
Para obtener más información sobre cómo habilitar y ver los datos de las Métricas de almacenamiento con PowerShell, consulte [Cómo habilitar las Métricas de almacenamiento mediante PowerShell](http://msdn.microsoft.com/library/azure/dn782843.aspx#HowtoenableStorageMetricsusingPowerShell).

Para obtener más información sobre cómo habilitar y recuperar los datos de los Registros de almacenamiento con PowerShell, consulte 
[Cómo habilitar los Registros de almacenamiento mediante PowerShell](http://msdn.microsoft.com/library/azure/dn782840.aspx#HowtoenableStorageLoggingusingPowerShell) y [Buscar los datos de registro de los Registros de almacenamiento](http://msdn.microsoft.com/library/azure/dn782840.aspx#FindingyourStorageLogginglogdata)
...
## <a name="sas"></a>Cómo administrar una firma de acceso compartido (SAS) y la directiva de acceso almacenada
...
- **SAS ad-hoc**: cuando se crea una SAS ad-hoc, la hora de inicio, la hora de finalización y los permisos para la firma de acceso compartido se especifican en el URI de SAS. Este tipo de SAS puede crearse en un contenedor, blob, tabla o cola y no se puede revocar.
- **SAS con directiva de acceso almacenada**: una directiva de acceso almacenada se define en un contenedor de recursos (un contenedor de blobs, tabla o cola) y se puede usar para administrar las restricciones de una o varias firmas de acceso compartido. Cuando asocia una SAS a una directiva de acceso almacenada, la SAS hereda las restricciones (hora de inicio, hora de expiración y permisos) definidas para la directiva de acceso almacenada. Este tipo de SAS se puede revocar.

Para obtener más información, consulte [Firmas de acceso compartido](./storage-dotnet-shared-access-signature-part-1.md/), [parte 1: Descripción del modelo de firmas de acceso compartido](./storage-dotnet-shared-access-signature-part-1.md/)y [Administración del acceso a los recursos de almacenamiento de Azure](http://msdn.microsoft.com/library/azure/ee393343.aspx).

En las siguientes secciones, obtendrá información sobre cómo crear un token de firma de acceso compartido y una directiva de acceso almacenada para las tablas de Azure. Azure PowerShell también proporciona cmdlets similares para contenedores, blobs y colas. Para ejecutar los scripts de esta sección, descargue [Azure PowerShell versión 0.8.14](http://go.microsoft.com/?linkid=9811175&clcid=0x409) o posterior.

### <a name="sub1"></a>Cómo crear una directiva basada en el token de firma de acceso compartido
Use el cmdlet New-AzureStorageTableStoredAccessPolicy para crear una nueva directiva de acceso almacenada. A continuación, llame al cmdlet [New-AzureStorageTableSASToken](http://msdn.microsoft.com/library/azure/dn806400.aspx) para crear un nuevo token de firma de acceso compartido basado en las nuevas directivas para una tabla de almacenamiento de Azure.

    $policy = "policy1"
    New-AzureStorageTableStoredAccessPolicy -Name $tableName -Policy $policy -Permission "rd" -StartTime "2015-01-01" -ExpiryTime "2016-01-01" -Context $Ctx
    New-AzureStorageTableSASToken -Name $tableName -Policy $policy -Context $Ctx

### <a name="sub2"></a>Cómo crear un token de firma de acceso compartido ad hoc (no revocable)
...
### <a name="sub3"></a>Cómo crear una directiva de acceso almacenada
...
### <a name="sub4"></a>Cómo actualizar una directiva de acceso almacenada
...
### <a name="sub5"></a>Cómo eliminar una directiva de acceso almacenada
...
## <a name="gov"></a>Cómo usar el servicio Almacenamiento de Azure desarrollado para el gobierno de Estados Unidos y Azure desarrollado para China
...
Para usar Almacenamiento de Azure con [Azure Government para EE. UU.](http://azure.microsoft.com/features/gov/), deberá precisar un nuevo entorno y crear un nuevo contexto de almacenamiento con este entorno:
...
- [Guía para desarrolladores de Microsoft Azure Government](../azure-government-developer-guide/). 
...
[Introducción de 5 minutos a Almacenamiento de Azure y PowerShell]: #getstart
[Requisitos previos para usar Azure PowerShell con Almacenamiento de Azure]: #pre
[Cómo administrar cuentas de almacenamiento en Azure]: #manageaccount
[Cómo establecer una suscripción predeterminada de Azure]: #setdefsub
[Cómo crear una nueva cuenta de almacenamiento de Azure]: #createaccount
[Cómo configurar una cuenta de almacenamiento de Azure predeterminada]: #defaultaccount
[Cómo enumerar todas las cuentas de almacenamiento de Azure en una suscripción]: #listaccounts
[Cómo crear un contexto de almacenamiento de Azure]: #createctx
[Cómo administrar los blobs de Azure y las instantáneas de blob]: #manageblobs
[Cómo crear un contenedor]: #container
[Cómo cargar un blob en un contenedor]: #uploadblob
[Cómo descargar blobs de un contenedor]: #downblob
[Cómo copiar blobs de un contenedor de almacenamiento a otro]: #copyblob
[Cómo eliminar un blob]: #deleteblob
[Cómo administrar las instantáneas de blob de Azure]: #manageshots
[Cómo crear una instantánea de blob]: #createshot
[Cómo enumerar las instantáneas de un blob]: #listshot
[Cómo copiar una instantánea de un blob]: #copyshot
[Cómo administrar las entidades de tabla y las tablas de Azure]: #managetables
[Cómo crear una tabla]: #createtable
[Cómo recuperar una tabla]: #gettable
[Cómo eliminar una tabla]: #remtable
[Cómo administrar las entidades de tabla]: #mngentity
[Cómo agregar entidades de tabla]: #addentity
[Cómo consultar entidades de tabla]: #queryentity
[Cómo eliminar entidades de tabla]: #deleteentity
[Cómo administrar las colas de Azure y la cola de mensajes]: #managequeues
[Cómo crear una cola]: #createqueue
[Cómo recuperar una cola]: #getqueue
[Como eliminar una cola]: #remqueue
[Cómo administrar la cola de mensajes]: #mngqueuemsg
[Cómo insertar un mensaje en una cola]: #addqueuemsg
[Cómo extraer el siguiente mensaje de la cola]: #dequeuemsg
[Cómo administrar archivos y recursos compartidos de archivos de Azure]: #files
[Cómo establecer y consultar el análisis de almacenamiento]: #stganalytics
[Cómo administrar una firma de acceso compartido (SAS) y la directiva de acceso almacenada]: #sas
[Cómo usar el servicio Almacenamiento de Azure desarrollado para el gobierno de Estados Unidos y Azure desarrollado para China]: #gov
