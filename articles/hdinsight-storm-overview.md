<properties title="Informaci&oacute;n general de Storm en HDInsight" pageTitle="Obtenga informaci&oacute;n acerca de Apache Storm en HDInsight (hadoop)" description="Aprenda a usar Apache Storm en HDInsight (Hadoop)" metaKeywords="Azure hdinsight storm, Azure hdinsight realtime, azure hadoop storm, azure hadoop realtime, azure hdinsight real-time, azure hadoop storm real-time, aure hadoop real-time" services="hdinsight" solutions="" documentationCenter="big-data" authors="larryfr" videoId="" scriptId="" manager="paulettm" editor="cgronlun"/>

<tags ms.service="hdinsight" ms.workload="big-data" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="09/30/2014" ms.author="larryfr" />

# Información general de Storm en HDinsight

## ¿Qué es Storm?

[Apache Storm][Apache Storm] es un sistema de cálculo de código abierto, distribuido y con tolerancia a errores que le permite procesar los datos en tiempo real. Las soluciones de Storm pueden proporcionar también procesamiento de datos garantizado, con la posibilidad de reproducir los datos que no se han procesado correctamente la primera vez.

## ¿Qué es Storm en HDInsight de Azure?

Storm en HDInsight se ofrece como un clúster administrado integrado en el entorno de Azure, donde se puede usar como parte de una solución de Azure más amplia. Por ejemplo, Storm puede consumir datos de servicios como colas de bus de servicio o centros de eventos, y usar Sitios web o Servicios en la nube para proporcionar visualización de los datos. Los clústeres de Storm en HDInsight se pueden configurar también en una red virtual de Azure, lo cual reduce la latencia en la comunicación con otros recursos de la misma red virtual y puede permitir también la comunicación segura con los recursos de un centro de datos privado.

Para introducirse en el uso de Storm, consulte [Introducción a Storm en HDInsight][Introducción a Storm en HDInsight].
...
-   **Tupla**: una lista con nombre de valores tipados de forma dinámica.

-   **Spout**: consume datos de un origen de datos y emite una o varias **secuencias**.

    > [WACOM.NOTE] En muchos casos, los datos se leen de una cola como Kafka, las colas de bus de servicio de Azure o los centros de eventos. La cola garantiza que los datos persistan en caso de una interrupción.
...
-   **Thrift**: Apache Thrift es un marco de software para el desarrollo de servicios escalables en distintos lenguajes. Así, permite crear servicios que funcionan entre C++, Java, Python, PHP, Ruby, Erlang, Perl, Haskell, C#, Cocoa, JavaScript, Node.js, Smalltalk y otros lenguajes.
    ...
    -   **Nimbus** es un servicio de Thrift, y una **topología** es una definición de Thrift, luego es posible desarrollar topologías mediante diversos lenguajes de programación.

Para obtener más información sobre los componentes de Storm, consulte el [tutorial de Storm][tutorial de Storm] en apache.org.

## Escenarios: ¿Cuáles son los casos de uso de Storm?

A continuación se indican algunos de los supuestos comunes en los que podría usar Apache Storm. Para obtener información sobre supuestos del mundo real, lea [cómo las empresas usan Storm][cómo las empresas usan Storm].
...
Para obtener más información sobre RPC distribuido y el cliente DRPC que se proporciona con Storm, consulte [RPC distribuido][RPC distribuido].

### Aprendizaje automático en línea

Storm se puede usar con soluciones de aprendizaje automático que previamente se han entrenado mediante procesamiento por lotes, como puede ser una solución basada en Mahout. Sin embargo, su modelo de cálculo distribuido genérico también abre la puerta a soluciones de aprendizaje automático basadas en secuencias. Por ejemplo, el [proyecto Scalable Advanced Massive Online Analysis (SAMOA)][proyecto Scalable Advanced Massive Online Analysis (SAMOA)] es una biblioteca de aprendizaje automático que emplea el procesamiento por secuencias, y puede funcionar con Storm.

## ¿Qué lenguajes de programación se pueden usar?

El clúster de Storm en HDInsight proporciona compatibilidad inmediata con .NET, Java y Python. Si bien Storm [es compatible con otos lenguajes][es compatible con otos lenguajes], para muchos de ellos será necesario instalar un lenguaje de programación adicional en el clúster de HDInsight además de otros cambios en la configuración.

### .NET

SCP es un proyecto que permite a los desarrolladores de .NET diseñar e implementar una topología (lo que incluye spouts y bolts). La compatibilidad con SCP se proporciona de forma predeterminada con los clústeres de Storm en HDInsight.
...
Para obtener más información sobre Trident, consulte el [tutorial de Trident][tutorial de Trident] en apache.org.
...
Para obtener más información, consulte [Garantías en el procesamiento de los datos][Garantías en el procesamiento de los datos] en apache.org

### BasicBolt

El patrón de leer una tupla de entrada, emitir cero o más tuplas y luego confirmar la tupla de entrada inmediatamente al final del método ejecutado es tan común que Storm proporciona la interfaz [IBasicBolt][IBasicBolt] para automatizarlo.

### Combinaciones

La unión de dos secuencias de datos variará según la aplicación. Por ejemplo, puede unir cada tupla de varias secuencias en una nueva, o unir únicamente lotes de tuplas de una ventana específica. De cualquier modo, la unión se puede efectuar mediante [fieldsGrouping][fieldsGrouping], que es una forma de definir cómo las tuplas se enrutan a los bolts.
...
Para ver un ejemplo de uso de tuplas de señalización, consulte [Análisis de los datos de sensor con Storm y HDInsight][Análisis de los datos de sensor con Storm y HDInsight]

Si está usando Trident, se basa en el procesamiento por lotes de tuplas.

### Almacenamiento en caché

El almacenamiento en caché de memoria interna se usa a menudo como mecanismo para acelerar el procesamiento, dado que mantiene en memoria los activos usados con mayor frecuencia. Dado que una topología se distribuye entre varios nodos, y varios procesos dentro de cada nodo, debe considerar el uso de [fieldsGrouping][fieldsGrouping] para garantizar que las tuplas que contienen los campos que se usan para la búsqueda en caché se enruten siempre al mismo proceso. De esta forma se evita la duplicación de las entradas de la caché entre los procesos.

### Transmisión por secuencias del N principal

Cuando la topología depende del cálculo de un valor N 'principal', como los 5 principales trending topics en Twitter, debe calcular el valor N principal en paralelo y luego combinar el resultado de esos cálculos en un valor global. Esto se puede hacer mediante [fieldsGrouping][fieldsGrouping] para enrutar por campo a los bolts paralelos, lo cual particiona los datos por el valor de campo y finalmente enruta a un bolt que determina de forma global el valor N principal.

Para ver un ejemplo de este procedimiento, consulte el ejemplo [RollingTopWords][RollingTopWords].

## Pasos siguientes

-   [Introducción a Storm en HDInsight][Introducción a Storm en HDInsight]

-   [Análisis de los datos de sensor con Storm y HDInsight][1]

-   [Desarrollo de aplicaciones de procesamiento de datos de streaming con SCP.NET y C# en Storm de HDInsight][Desarrollo de aplicaciones de procesamiento de datos de transmisión por secuencias con SCP.NET y C# en Storm en HDInsight]

  [Apache Storm]: https://storm.incubator.apache.org
  [Introducción a Storm en HDInsight]: /es-es/documentation/articles/hdinsight-storm-getting-started
  [tutorial de Storm]: https://storm.incubator.apache.org/documentation/Tutorial.html
  [cómo las empresas usan Storm]: https://storm.incubator.apache.org/documentation/Powered-By.html
  [RPC distribuido]: https://storm.incubator.apache.org/documentation/Distributed-RPC.html
  [proyecto Scalable Advanced Massive Online Analysis (SAMOA)]: http://yahooeng.tumblr.com/post/65453012905/introducing-samoa-an-open-source-platform-for-mining
  [es compatible con otos lenguajes]: https://storm.incubator.apache.org/about/multi-language.html
  ...
  [tutorial de Trident]: https://storm.incubator.apache.org/documentation/Trident-tutorial.html
  [Garantías en el procesamiento de los datos]: https://storm.apache.org/about/guarantees-data-processing.html
  ...
  [Análisis de los datos de sensor con Storm y HDInsight]: /es-es/documentation/articles/hdinsight-storm-sensor-data-analysis.md
  [RollingTopWords]: https://github.com/nathanmarz/storm-starter/blob/master/src/jvm/storm/starter/RollingTopWords.java
  [1]: /es-es/documentation/articles/hdinsight-storm-sensor-data-analysis
