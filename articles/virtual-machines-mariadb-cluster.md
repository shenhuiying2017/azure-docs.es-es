<properties pageTitle="Ejecución de un clúster MariaDB (MySQL) en Azure" description="Creación de un clúster MariaDB + Galera MySQL en máquinas virtuales de Azure" services="virtual-machines" documentationCenter="" authors="sabbour" manager="timlt" editor=""/>
...
> [AZURE.NOTE]  Este tema usa las herramientas [Azure CLI], asegúrese de descargarlas y conectarlas a su suscripción de Azure siguiendo las instrucciones. Si necesita una referencia a los comandos disponibles en Azure CLI, visite este vínculo para obtener la [referencia de comandos de Azure CLI]. También necesitará [crear una clave SSH para la autenticación] y anotar la **ubicación del archivo .pem**.
...
esto devolverá un resultado similar a `5112500ae3b842c8b9c604889f8753c3__OpenLogic-CentOS-70-20140926`. Use el nombre del siguiente paso.

4. Cree la plantilla de máquina virtual reemplazando **/path/to/key.pem** por la ruta de acceso donde se almacena la clave SSH .pem generada.
...
9.  Optimice el sistema para el rendimiento. Consulte este artículo sobre [estrategia de optimización del rendimiento] para obtener más detalles.

	- Editar de nuevo el archivo de configuración de MySQL
	
			vi /etc/my.cnf.d/server.cnf
		
	- Editar la sección **[mariadb]** y anexar la siguiente
	
	> [AZURE.NOTE] Se recomienda que **innodb\_buffer\_pool_size** sea el 70 % de la memoria de su máquina virtual. Aquí se ha establecido a 2,45 GB aquí para la máquina virtual media de Azure, con 3,5 GB de RAM.
...
11. Capture la máquina virtual a través del portal. (Actualmente, el [problema #1268 en las herramientas Azure CLI] describe el hecho de que las imágenes capturadas por las herramientas de xplat-cli no capturan los discos de datos adjuntos).

	- Apagar la máquina a través del portal
    - Haga clic en la captura y especifique el nombre de imagen como **mariadb-galera-image** y proporcione una descripción y active "He ejecutado waagent".
...
1. Cree la primera VM de CentOS 7 desde la imagen **mariadb-galera-image** que creó, proporcionando el nombre de red virtual **mariadbvnet** y la subred **mariadb**, tamaño de máquina **medio**, pasando el nombre del servicio de nube que **mariadbha** (o cualquier nombre al que desee tener acceso a través de mariadbha.cloudapp.net), estableciendo el nombre del equipo para **mariadb1** y el nombre de usuario como **azureuser**, habilitando el acceso SSH y pasando el archivo .pem de certificado SSH y reemplazando **/path/to/key.pem** por la ruta de acceso en la que almacenó la clave SSH .pem generada.
...
2. Cree dos máquinas virtuales más _conectándolas_ al servicio de nube **mariadbha** creado actualmente, cambiando el **nombre de VM** así como el **puerto SSH** a un único puerto que no entre en conflicto con otras máquinas virtuales en el mismo servicio de nube.
...
## Equilibrio de carga del clúster
Al crear las máquinas virtuales en clúster, las agregó al conjunto de disponibilidad denominado **clusteravset** para asegurarse de que se colocan en diferentes dominios de error y de actualización y que Azure nunca realice el mantenimiento en todos los equipos a la vez. Esta configuración cumple los requisitos para ser compatible por ese contrato de nivel de servicio (SLA) de Azure.
...
El trabajo más duro ya está terminado. Ahora, el clúster debe estar accesible en `mariadbha.cloudapp.net:3306`, lo que presionará el equilibrador de carga y enrutará las solicitudes entre las tres máquinas virtuales de forma perfecta y eficaz.
...
<!--Link references-->
[Azure CLI]: http://azure.microsoft.com/es-es/documentation/articles/xplat-cli/
[referencia de comandos de Azure CLI]: http://azure.microsoft.com/es-es/documentation/articles/command-line-tools/
...
[optimizar y probar el rendimiento de MySQL en máquinas virtuales de Linux de Azure]:http://azure.microsoft.com/sv-se/documentation/articles/virtual-machines-linux-optimize-mysql-perf/
[problema #1268 en las herramientas Azure CLI]:https://github.com/Azure/azure-xplat-cli/issues/1268
[otro modo para el clúster MySQL en Linux]: http://azure.microsoft.com/es-es/documentation/articles/virtual-machines-linux-mysql-cluster/
