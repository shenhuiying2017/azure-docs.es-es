...
10. Agregue un certificado de confianza a Azure. Puede agregar hasta 20 certificados. Para obtener instrucciones sobre cómo crear un certificado raíz autofirmado mediante *makecert*, consulte [Uso de certificados raíz autofirmados para las configuraciones de punto a sitio](vpn-gateway-certificates-point-to-site.md). Al agregar un archivo X.509 con codificación Base64 (.cer) a Azure, se está indicando que se confíe en el certificado raíz que representa el archivo.
	...
	Para obtener la clave pública, exporte el certificado como archivo X.509 con codificación Base64 (.cer). Apunte la ruta de acceso donde exportó el archivo .cer. A continuación, se muestra un ejemplo de cómo obtener la representación de cadena Base64 del certificado. En este paso, tendrá que utilizar la ruta de su archivo .cer.
...
Los certificados se usan para autenticar a los clientes VPN para VPN de punto a sitio. Los siguientes pasos le enseñarán a agregar y quitar certificados raíz. Al agregar un archivo X.509 con codificación Base64 (.cer) a Azure, se está indicando que se confíe en el certificado raíz que representa el archivo.

### Adición de un certificado raíz de confianza

Puede agregar hasta 20 certificados raíz de confianza a Azure. Siga los siguientes pasos para agregar un certificado raíz.

1. Cree y prepare el nuevo certificado raíz para agregarlo a Azure.
...
2. Agregue el nuevo certificado raíz. Tenga en cuenta que solo puede agregar un certificado raíz a la vez.
...
Puede quitar un certificado raíz de confianza de Azure. Al quitar un certificado raíz de confianza, los certificados de cliente que se generaron a partir del certificado no podrán volver a conectarse a Azure a través de una conexión de punto a sitio hasta que se instale un certificado cliente que se genere a partir de un certificado que sea válido en Azure.
