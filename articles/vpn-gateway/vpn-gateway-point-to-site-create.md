...
Los certificados se utilizan para autenticar a los clientes VPN para VPN de punto a sitio. Este procedimiento consta de varios pasos. Use los vínculos siguientes para completar cada paso, por orden.

1. [Generar un certificado raíz autofirmado](#generate-a-self-signed-root-certificate) (de momento solo se admiten los certificados raíz autofirmados).
2. [Cargar el archivo de certificado raíz en el portal de Azure](#upload-the-root-certificate-file-to-the-management-portal).
3. [Generar un certificado de cliente](#generate-a-client-certificate).
4. [Exportar e instalar el certificado de cliente](#export-and-install-the-client-certificate).
...
1. Cuando generó un certificado raíz autofirmado en el procedimiento anterior, también se crea un archivo *.cer*. Ahora podrá cargar este archivo en el portal de Azure. Tenga en cuenta que el archivo .cer no contiene la clave privada del certificado raíz.
...
1. En el portal de Azure, en la página **Panel** correspondiente a la red virtual, vaya al menú de vista rápida, en la esquina derecha, y haga clic en el paquete VPN que pertenece al cliente que desea conectar a la red virtual. Se admiten los siguientes sistemas operativos de cliente:
