---
title: "Configuración de BGP en Azure VPN Gateway: Resource Manager: CLI | Microsoft Docs"
...
---
# <a name="how-to-configure-bgp-on-azure-vpn-gateways-using-cli"></a>Configuración de BGP para Azure VPN Gateway con la CLI

Este artículo lo ayudará a habilitar BGP en una conexión de VPN de sitio a sitio (S2S) entre locales y una conexión de red virtual a red virtual mediante el modelo de implementación de Resource Manager y la CLI.
...
* [Habilitar BGP para VPN Gateway](#enablebgp) (obligatorio)
  ...
  Después, puede configurar cualquiera de las siguientes secciones, o ambas:
...
Puede combinar estas secciones juntas para crear una red de tránsito más compleja y de saltos múltiples que satisfaga sus necesidades.
...
Cree la puerta de enlace de red virtual para TestVNet1. BGP requiere una instancia de VPN Gateway basada en rutas. También necesita el parámetro adicional "-Asn" para establecer el ASN (número de AS) para TestVNet1. Se tardan unos 45 minutos o algo más en crear una puerta de enlace. 

Si este comando se ejecuta con el parámetro '--no-wait', no se verán los comentarios o resultados. El parámetro "--no-wait" permite que la puerta de enlace se cree en segundo plano. No significa que se termine de crear la puerta de enlace de VPN de inmediato.
...
Una vez creada la puerta de enlace, debe obtener la dirección IP del par BGP en Azure VPN Gateway. Esta dirección es necesaria para configurar VPN Gateway como par BGP para los dispositivos de VPN local.
...
En este ejemplo, la puerta de enlace de red virtual y la puerta de enlace de red local están presentes en distintos grupos de recursos. Cuando las puertas de enlace están en distintos grupos de recursos, debe especificar el identificador de recurso completo de las dos puertas de enlace para establecer una conexión entre las redes virtuales.
...
#### <a name="3-create-the-testvnet1-to-site5-connection"></a>3. Cree la conexión de TestVNet1 a Site5

En este paso, creará la conexión de TestVNet1 a Site5. Como se explicó anteriormente, es posible tener conexiones BGP y no BGP para la misma instancia de Azure VPN Gateway. A menos que BGP esté habilitado en la propiedad de conexión, Azure no habilitará BGP para esta conexión, aunque los parámetros BGP ya estén configurados en ambas puertas de enlace. Asegúrese de reemplazar los identificadores de suscripción por los suyos propios.
...
Después de unos minutos, la conexión debería haberse establecido. Se inicia la sesión de emparejamiento de BGP una vez establecida la conexión IPsec.
...
Las siguientes instrucciones son la continuación de los pasos anteriores de las secciones previas. Debe completar la sección [Habilitar BGP para VPN Gateway](#enablebgp) para crear y configurar TestVNet1 y VPN Gateway con BGP.
...
En este paso, creará la conexión de TestVNet1 a Site5. Para habilitar BGP para esta conexión, debe especificar el parámetro "--enable-bgp".
...
Cree la conexión de TestVNet1 a TestVNet2 y viceversa. Asegúrese de reemplazar los identificadores de suscripción por los suyos propios.
...
Después de completar estos pasos, se establecerá la conexión en cuestión de minutos. La sesión de emparejamiento BGP funcionará una vez completada la conexión de red virtual a red virtual.

Si completa las tres secciones de este ejercicio, establecerá la topología de red mostrada en el siguiente ejemplo:
