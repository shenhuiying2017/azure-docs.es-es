<properties title="Get started using Azure Data Factory" pageTitle="Introducción al uso de la Factoría de datos de Azure" description="En este tutorial se muestra cómo crear una canalización de datos de ejemplo que copia datos de un blob a una instancia de base de datos de SQL de Azure." metaKeywords=""  services="data-factory" solutions=""  documentationCenter="" authors="spelluru" manager="jhubbard" editor="monicar" />
...
4.	Crear **canalizaciones**. Una canalización consiste en una o más actividades que consumen datos de entrada y generan datos de salida. Una actividad de copia copia los datos de un almacén de datos de origen en un almacén de datos de destino y una actividad HDInsight procesa los datos de entrada mediante scripts de Hive y Pig.  
5.	Especifique el **período activo** para las canalizaciones (para el procesamiento de los datos). El período activo define la duración de tiempo en que se producirán segmentos de datos. Puede especificar la fecha y hora de inicio y final de una canalización (o) puede hacer que se ejecute todo el tiempo.
...
- Una suscripción de Azure. Para obtener más información acerca de la obtención de una suscripción, consulte [Opciones de compra] [azure-purchase-options], [Ofertas para miembros][azure-member-offers] o [Evaluación gratuita][azure-free-trial].
...
- Cuenta de almacenamiento de Azure. En este tutorial usará el almacenamiento de blobs como almacén de datos de origen.
- Base de datos SQL de Azure. En este tutorial creará una base de datos de ejemplo y la usará como almacén de datos de destino.

##Apartados de este tutorial

Paso | Descripción
-----| -----------
[Paso 1: Crear una factoría de datos de Azure](#CreateDataFactory) | En este paso, creará una factoría de datos de Azure llamada **ADFTutorialDataFactory**. 
...
[Paso 3: Crear conjuntos de datos de entrada y salida](#CreateInputAndOutputDataSets) | En este paso, definirá dos conjuntos de datos (**EmpTableFromBlob** y **EmpSQLTable**) que se usan como entrada y salida para la **actividad de copia** en el elemento ADFTutorialPipeline que creará en el siguiente paso.
[Paso 4: Crear y ejecutar una canalización](#CreateAndRunAPipeline) | En este paso, creará una canalización llamada **ADFTutorialPipeline**. La canalización dispondrá de una **actividad de copia** que copia datos de un blob de Azure en una tabla de base de datos de Azure de salida.
[Paso 5: Supervisar los conjuntos de datos y la canalización](#MonitorDataSetsAndPipeline) | En este paso, supervisará los conjuntos de datos y la canalización con Azure Management Studio.
...
6. En la hoja **Nueva factoría de datos**:
	...
	2. Haga clic en **NOMBRE DE GRUPO DE RECURSOS** y haga lo siguiente:
		...
		2. En la hoja **Crear grupo de recursos**, escriba **ADFTutorialResourceGroup** como **nombre** del grupo de recursos y haga clic en **Aceptar**. 

			![Create Resource Group][image-data-factory-create-resource-group]

		Se supone que en algunos de los pasos de este tutorial se usa el grupo de recursos denominado **ADFTutorialResourceGroup**. Si utiliza un grupo de recursos diferentes, deberá usar el grupo de recursos que seleccione aquí en lugar de ADFTutorialResourceGroup.  
...
9. Haga clic en el **Centro de notificaciones** a la izquierda y busque las notificaciones del proceso de creación.
...
2. En la hoja Nuevo almacén de datos:
	1. Escriba un **nombre** para el almacén de datos. Para los fines de este tutorial, escriba **MyAzureSQLStore** como **NOMBRE**. 
	2. Escriba una**descripción (opcional)** para el almacén.
	3. Haga clic en **Tipo** y seleccione **Base de datos SQL de Azure**.
3. Especifique **Servidor**, **Base de datos**, **Nombre de usuario** y **Contraseña** para la base de datos SQL de Azure y haga clic en **Aceptar**.
...
* Crear y cargar un archivo de texto, **emp.txt**, como un blob en la carpeta **input** en el contenedor **adftutorial** (**input/emp.txt**). 
...
2. Use herramientas como el [Explorador de almacenamiento de Azure](https://azurestorageexplorer.codeplex.com/) para crear el contenedor **adftutorial** y para cargar el archivo **emp.txt** en la carpeta **input** dentro del contenedor (**input/emp.txt**).

    ![Azure Storage Explorer][image-data-factory-get-started-storage-explorer]
3. Use el siguiente script SQL para crear la tabla **emp** en la base de datos SQL de Azure. Puede utilizar SQL Server Management Studio para conectarse a una base de datos SQL de Azure y para ejecutar el script de SQL.
...
1. Cree un archivo JSON para una tabla de factoría de datos que representa los datos de emp.txt en el Bloc de notas blob.Launch, copie el siguiente script JSON y guárdelo como EmpBlobTable.json en la carpeta C:\ADFGetStarted.
	...
	- **folderPath** está establecido en la carpeta **input** en el contenedor **adftutorial**. También puede especificar el nombre de un blob en la carpeta. Puesto que no se especifica el nombre del blob, los datos de todos los blobs del contenedor se consideran datos de entrada.  
	- format **type** está establecido en **TextFormat**
	- - Hay dos campos en el archivo de texto, **FirstName** y **ColumnName**, separados por un carácter de coma (columDelimiter)	
	- La **disponibilidad** está establecida en **hourly** (**frequency** está establecido en **hour** y **interval** está establecido en **1**). El servicio Factoría de datos buscará los datos de entrada cada hora en la carpeta **input** del contenedor de blobs (**adftutorial**) que ha especificado.

	Si no especifica un **nombre de archivo** para una **tabla de entrada**, todos los archivos o blobs de esta carpeta (**folderPath**) se consideran entradas. Si especifica un nombre de archivo en JSON, solo el archivo o blob especificado se consideran una entrada. Consulte los archivos de muestra del [tutorial][adf-tutorial] para ver algunos ejemplos.
	...
	Para obtener más información acerca de las propiedades JSON, consulte [Referencia de scripting JSON](http://go.microsoft.com/fwlink/?LinkId=516971).
...
3. Use el cmdlet **New-AzureDataFactoryTable** para crear la tabla de entrada mediante el archivo **EmpBlobTable.json**.
	...
	Para obtener más información sobre este cmdlet y otros cmdlets de factoría de datos, consulte [Referencia de cmdlets de factoría de datos][cmdlet-reference].
...
1. Cree un archivo JSON para una tabla de factoría de datos que representa los datos de la base de datos SQL de Azure. Inicie el Bloc de notas, copie el siguiente script JSON y guárdelo como **EmpSQLTable.json** en la carpeta **C:\ADFGetStarted**.
	...
	* location **type** está establecido en **AzureSQLTableLocation**.
	...
	* Hay tres columnas, **ID**, **FirstName** y **LastName**, en la tabla emp de la base de datos, pero ID es una columna de identidad, así que solo debe especificar aquí **FirstName** y **LastName**.
	* La **disponibilidad** está establecida en **hourly** (frequency está establecido en hour e interval en 1).  El servicio Factoría de datos generará un segmento de datos de salida cada hora en la tabla **emp** de la base de datos SQL de Azure.
...
1. Cree un archivo JSON para la canalización. Inicie el Bloc de notas, copie el siguiente script JSON y guárdelo como **ADFTutorialPipeline.json** en la carpeta **C:\ADFGetStarted**.
	...
	- En la sección de actividades, hay solo una actividad cuyo **tipo** está establecido en **CopyActivity**.
...
3. Una vez creadas las canalizaciones, puede especificar la duración en la que se producirá el procesamiento de datos. Al especificar el período activo de una canalización, se está definiendo la duración de tiempo en el que se procesarán los segmentos de datos basándose en las propiedades de disponibilidad que se han definido para cada tabla de la factoría de datos de Azure.  Ejecute el siguiente comando de PowerShell para establecer el período activo en la canalización y escriba Y para confirmar. 
	...
	> Si no especifica **EndDateTime**, se calcula como "**StartDateTime + 48 horas**". Para ejecutar la canalización indefinidamente, especifique **9/9/9999** como el valor de **EndDateTime**.  
...
5. Observe que ya se han producido los segmentos de datos hasta la hora actual y que están **listos** porque el archivo **emp.txt** existe todo el tiempo en el contenedor de blobs: **adftutorial\input**. Confirme que no aparecen segmentos en la sección de **segmentos problemáticos** en la parte inferior.
...
6. Aparecerá la hoja **EmpSQLTable** como se muestra a continuación:

	![table blade][image-data-factory-get-started-table-blade]
 
7. Observe que ya se han producido los segmentos de datos hasta la hora actual y que están **listos**. No se muestra ningún segmento en la sección de **segmentos con problemas** de la parte inferior.
...
14. (opcional) Haga clic en **Canalizaciones** en la página principal de **ADFTutorialDataFactory**, haga clic en **ADFTutorialPipeline** en la hoja **Canalizaciones** y profundice en las tablas de entrada (**Consumidas**) o en las tablas de salida (**Producidas**).
...
[Uso de actividades personalizadas en una factoría de datos][use-custom-activities] | Este artículo proporciona un tutorial con instrucciones paso a paso para la creación de una **actividad personalizada** y usarla en una canalización. 
...
[Solución de problemas de la factoría de datos][troubleshoot] | Este artículo describe cómo **solucionar los problemas** de la Factoría de datos de Azure. Puede probar el tutorial de este artículo en ADFTutorialDataFactory introduciendo un error (eliminación de la tabla en la Base de datos SQL de Azure). 
