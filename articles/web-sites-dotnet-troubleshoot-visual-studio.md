...
2.  [Configuración y administración de sitios][]
3.  [Vista remota][]
4.  [Depuración remota][]
5.  [Información general de registros de diagnóstico][]
6.  [Creación y visualización de registros de seguimiento de aplicación][]
7.  [Visualización de registros de servidor web][]
8.  [Visualización de registros de mensajes de error detallados][]
9.  [Descarga de registros de sistema de archivos][]
10. [Visualización de registros de almacenamiento][]
11. [Visualización de registros de solicitudes con error][]
12. [Pasos siguientes][]

## <a name="prerequisites"></a>Requisitos previos

Este tutorial funciona con el entorno de desarrollo, proyecto web y sitio web de Azure que configuró en [Introducción a Azure y ASP.NET][]. Los ejemplos de código que aparecen en este tutorial son para una aplicación web C# MVC, pero los procedimientos de solución de problemas son los mismos para las aplicaciones de Web Forms y Visual Basic.
...
1.  Si todavía no ha iniciado sesión en Azure en Visual Studio, haga clic en el botón **Conectar a Azure** en el **Explorador de servidores**.
    ...
    Para obtener más información acerca de la conexión a recursos de Azure desde Visual Studio, consulte [Administrar cuentas, suscripciones y roles administrativos][].

2.  En el **Explorador de servidores**, expanda **Azure** y, a continuación, **Sitios web**.

3.  Haga clic con el botón secundario en el nodo del sitio web que creó en [Introducción a Azure y ASP.NET][] y, a continuación, haga clic en **View Settings**.

    ![Ver configuración en el Explorador de servidores][]
    ...
    Para obtener información acerca de los cuadros App Settings y Connection Strings de esta ventana, consulte[Azure Web Sites: funcionamiento de las cadenas de aplicación y de las cadenas de conexión][].

    Si desea realizar una tarea de administración de sitios que no se puede hacer en esta ventana, puede hacer clic en **Full Web Site Settings** para abrir una ventana del explorador con el portal de administración. Para obtener más información, consulte [Configuración de Sitios web][].
...
Esta sección muestra cómo depurar remotamente con el proyecto que crea en [Introducción a Azure y ASP.NET][].

1.  Abra el proyecto web que creó en [Introducción a Azure y ASP.NET][].
...
6.  En la lista desplegable **Perfil**, seleccione el mismo perfil que utilizó en [Introducción a Azure y ASP.NET][].

7.  Haga clic en la pestaña **Configuración**, cambie la **Configuración** a **Depurar** y, a continuación, haga clic en **Publicar**.

    ![Publicar en modo de depuración][]
...
10. Para Visual Studio 2012 con actualización 4:<span id="vs2012"></span></a>
    ...
    ![Definición de la depuración remota en el portal de administración][]
...
12. Mantenga el mouse sobre la variable `currentTime` para ver el valor de tiempo.
    ...
    ![Ver la variable en ejecución en modo de depuración en Azure][]
...
14. Presione F5 para continuar la ejecución.
    ...
    ![Página About con el valor nuevo][]
...
-   Si encuentra que el depurador no entra en el código que desea depurar, es posible que deba cambiar la configuración de Solo mi código. Para obtener más información, consulte [Restringir la ejecución paso a paso a Solo mi código][].
...
-   Puede asociar manualmente el depurador a cualquier proceso. No solo puede depurar el proceso del sitio web (w3wp.exe), sino que también puede depurar otros procesos, como [WebJobs][]. Para obtener más información acerca de cómo utilizar el modo de depuración en Visual Studio, consulte [Depurar en Visual Studio][].
...
1.  Presione F5 para ejecutar la aplicación en modo de depuración.
    ...
    ![Ventana de seguimiento en depuración][]
...
3.  Agregue un [elemento de seguimiento][] bajo `<system.web>` en el archivo Web.config, tal como en el siguiente ejemplo:
...
3.  En el **Explorador de servidores**, haga clic con el botón secundario en su sitio web y seleccione **View Streaming Logs in Output Window**.
    ...
    ![Ver registros de streaming en el menú contextual][]
...
4.  En la ventana del explorador que muestra la página principal de su aplicación, haga clic en **Contact**.
    ...
    ![Seguimiento de errores en la ventana Output][]
    ...
    ![Registro de aplicación desactivado][]
...
6.  Cambie **Application Logging (File System)** a **Verbose** y haga clic en **Save**.
    ...
    ![Definir el nivel de seguimiento como Verbose][]
...
7.  En la ventana del explorador que ahora muestra su página **Contact**, haga clic en **Inicio**, en **About** y, a continuación, haga clic en **Contact**.
    ...
    ![Resultados detallados del seguimiento][]
...
![Botones de la pestaña de registros][]
...
4.  En la barra de direcciones de la ventana del explorador, agregue un carácter adicional a la dirección URL para generar un error 404 (por ejemplo, `http://localhost:53370/Home/Contactx`) y presione Entrar.
    ...
    ![Registro de error detallado en la ventana Resultados][]

    Presione Ctrl y haga clic en el vínculo para ver el resultado del registro formateado en un explorador:

    ![Registro de error detallado en la ventana del explorador][]
...
2.  Extraiga el archivo *.zip* y verá la siguiente estructura de carpeta:
    ...
    -   Los registros de servidor web están en los archivos *.log* de la carpeta *LogFiles\\http\\RawLogs*. Puede utilizar una herramienta como [Analizador del registro][] para ver y manipular estos archivos.
...
Las tablas facilitan la visualización de los detalles de registros en línea y son compatibles con streaming; puede consultar los registros en las tablas y ver nuevos registros a medida que se van creando. Los blobs facilitan la descarga de registros en archivos y su análisis con HDInsight, porque HDInsight sabe cómo trabajar con el almacenamiento de blobs. Para obtener más información, consulte **Hadoop and MapReduce** en [Data Storage Options (Building Real-World Cloud Apps with Azure)][].
...
7.  Haga clic en **Manage Table Storage**.
    ...
    ![Hacer clic en Manage Table Storage][]
    ...
    ![Administrar el almacenamiento de tablas][]
...
12. Haga clic en **View all application logs**.
    ...
    ![Tabla de seguimiento en el Explorador de servidores][]

    ![Registros de almacenamiento en vista de tabla][]
...
1.  En la pestaña **Configuration** de la ventana **Azure Web Site** que abrió desde el **Explorador de servidores**, cambie **Failed Request Tracing** a **On** y haga clic en **Save**.
    ...
    ![Habilitar el seguimiento de solicitudes con error][]
...
4.  En el portal de administración, haga clic en **Panel** y luego en **Reset your deployment credentials** en la sección **Quick Glance**.
    ...
    ![Restablecer el vínculo a las credenciales de FTP en el panel][]

5.  Escriba un nuevo nombre de usuario y contraseña.

    ![Nuevo nombre de usuario y contraseña de FTP][]
...
11. Haga clic en el archivo XML de la solicitud con error para la que desea ver información de seguimiento.
    ...
    ![Seguimiento de solicitudes con error en un explorador][]
...
-   [Supervisión de sitios web][]
     La sección de [configuración de diagnóstico y descarga de registros][] contiene información valiosa que no está incluida en los documentos para la solución de problemas.
-   [Investigación de fugas de memoria en Sitios web de Azure con Visual Studio 2013][]. Artículo del blog ALM de Microsoft sobre las características de Visual Studio para el análisis de problemas de memoria administrada.
-   [Herramientas de Sitios web de Windows que debe conocer][]. Publicación en el blog de Amit Apple.
...
-   [El foro sobre Azure en el sitio de ASP.NET][] (en inglés).
-   [El foro de Azure en MSDN][] (en inglés).
-   [StackOverflow.com][].

### Depuración en Visual Studio

Para obtener más información acerca del uso del modo de depuración en Visual Studio, consulte el tema [Depurar en Visual Studio][] de MSDN y [Debugging Tips with Visual Studio 2010][].

Para obtener más información acerca de depuración de Sitios web Azure, consulte los siguientes recursos:

-   [Introducción a la depuración remota en Sitios web de Azure][].
-   [Introducción a la depuración remota en Sitios web de Azure, 2ª parte: detalles de la depuración remota][]
-   [Introducción a la depuración remota en Sitios web de Azure, 3ª parte: entorno de varias instancias y GIT][]

Si su sitio web utiliza una API web de Azure o el back-end de Servicios móviles y necesita depurarlo, consulte [Debugging .NET Backend in Visual Studio][].
...
-   [Agentes de escucha de seguimiento][]
     Información acerca de las escuchas de seguimiento, pero no menciona [WebPageTraceListener][].
...
-   [Streaming Diagnostics Trace Logging from the Azure Command Line (plus Glimpse!)][]
     Uso de la línea de comandos para hacer lo que muestra este tutorial en Visual Studio. [Glimpse][] es una herramienta de depuración de aplicaciones ASP.NET.
-   [Using Azure Web Site Logging and Diagnostics, con David Ebbo][] y [Streaming Logs from Azure Web Sites, con David Ebbo][]
     Vídeos de Scott Hanselman y David Ebbo.

Para el registro de errores, una alternativa a escribir su propio código de seguimiento es utilizar un marco de registro de código abierto, como [ELMAH][]. Para obtener más información, consulte [Scott Hanselman's blog posts about ELMAH][].
...
-   [Publicaciones en el blog de Robert McMurray sobre el uso de LogParser][]
-   [El código de estado HTTP en IIS 7.0, IIS 7.5 y IIS 8.0][]
...
Si desea depurar un servicio en la nube de Azure en lugar de un sitio web, consulte [Depurar servicio en la nube][].
