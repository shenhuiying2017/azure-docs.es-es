<properties 
	pageTitle="Proceso de ciencia de datos de Azure en acción: uso de SQL Server | Azure" 
	description="Proceso de ciencia de datos de Azure en acción" metaKeywords="" 
...
# Proceso de ciencia de datos de Azure en acción: uso de SQL Server

En este tutorial, se seguirá el mapa completo de proceso de ciencia de datos de Azure para crear e implementar un modelo con un conjunto de datos disponible públicamente, el conjunto de datos [NYC Taxi Trips](http://www.andresmh.com/nyctaxitrips/). 


## <a name="dataset"></a>Descripción del conjunto de datos de carreras de taxi de Nueva York

El conjunto de datos NYC Taxi Trips consiste en aproximadamente 20 GB de archivos de valores separados por comas (CSV) comprimidos (~48 GB sin comprimir), que incluyen más de 173 millones de carreras individuales y las tarifas pagadas por cada carrera. Cada registro de carrera incluye la hora y la ubicación de recogida y de entrega, el número de licencia de (del conductor) anónimo y el número de ida y vuelta incluye la ubicación de entrega y recogida y el tiempo, la número de licencia y el número de identificador único del taxi. Los datos cubren todos los viajes del año 2013 y se proporcionan en los dos conjuntos de datos siguientes para cada mes:
...
2. El archivo CSV 'trip_fare' contiene información detallada de la carrera pagada para cada viaje, como el tipo de pago, el importe de la tarifa, los suplementos e impuestos, las propinas y los peajes, y el importe total pagado. Estos son algunos registros de ejemplo:
...
## <a name="setup"></a>Configuración del entorno de ciencia de datos de Azure

Como puede ver en la guía [Planificar su entorno](machine-learning-data-science-plan-your-environment.md), existen varias opciones para trabajar con el conjunto de datos NYC Taxi Trips en Azure:
...
En función del tamaño del conjunto de datos, la ubicación del origen de datos y el entorno de destino de Azure seleccionado, este escenario es similar al [Escenario n.° 5: conjunto de datos grande en un archivo local, con SQL Server en una máquina virtual de Azure como destino](machine-learning-data-science-plan-sample-scenarios.md#largelocaltodb).
...
4. Compruebe o cambie las carpetas de registro y las bases de datos de SQL Server predeterminadas para asegurarse de que las bases de datos recién creadas se almacenarán en un disco de datos. La imagen de la máquina virtual de SQL Server que está optimizada para cargas de almacén de datos está configurada previamente con discos de registros y datos. Si la máquina virtual no incluía un disco de datos y agregó nuevos discos duros virtuales durante el proceso de instalación de la máquina virtual, cambie las carpetas predeterminadas de la manera siguiente:
	...
	- Haga clic con el botón secundario en el nombre del SQL Server en el panel izquierdo y haga clic en **Propiedades**.

		![SQL Server Properties][14]

	- Seleccione **Configuración de base de datos** en la lista **Seleccionar una página** de la izquierda.

	- Compruebe o cambie las **Ubicaciones de base de datos predeterminadas** a las ubicaciones de **Disco de datos** que prefiera. Aquí es donde se almacenarán las nuevas bases de datos si se crean con la configuración de ubicación predeterminada.
...
1. Guardar la consulta SQL final para extraer y muestrear los datos, y copiar y pegar la consulta directamente en un módulo de lector de Aprendizaje automático de Azure; o bien 
2. conservar los datos muestreados y de ingeniería que pretende usar para la generación de modelos de una nueva tabla de base de datos y usar la nueva tabla en el módulo de Lector de Aprendizaje automático de Azure.

En esta sección se guardará la consulta final para extraer y muestrear los datos. El segundo método se muestra en la sección [Exploración de datos y diseño de características en Blocs de notas de IPython](#ipnb).
...
Para una comprobación rápida del número de filas y columnas en las tablas que se rellenaron anteriormente mediante la importación masiva paralela,
	...
	-- Informe con el número de filas en la tabla nyctaxi_trip sin recorrido de tabla
	SELECT SUM(rows) FROM sys.partitions WHERE object_id = OBJECT_ID('nyctaxi_trip')

	-- Informe con el número de columnas en la tabla nyctaxi_trip
...
Las consultas de exploración de conversión geográfica y la generación de etiquetas pueden usarse también para generar etiquetas y características mediante la eliminación de la parte de recuento. Se ofrecen ejemplos SQL de diseño de características adicionales en la sección [Exploración de datos y diseño de características en Blocs de notas de IPython](#ipnb). Resulta más eficaz ejecutar consultas de generación de características sobre el conjunto de datos completo o un subconjunto grande mediante consultas SQL que se ejecuten directamente en la instancia de base de datos de SQL Server. Las consultas se pueden ejecutar en **SQL Server Management Studio**, Blocs de notas de IPython o cualquier herramienta o entorno de desarrollo que tenga acceso a la base de datos local o remotamente.
...
## <a name="ipnb"></a>Exploración de datos y diseño de características en Blocs de notas de IPython

En esta sección, se realizará la exploración de datos y la generación de características
mediante consultas SQL y Python sobre la base de datos de SQL Server que se creó anteriormente. Se proporciona un IPython notebook llamado **machine-Learning-data-science-process-sql-story.ipynb** en la carpeta **Blocs de notas de IPython de prueba**. Este notebook también está disponible en [GitHub](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/iPythonNotebooks).
...
1. Guardar la consulta SQL final para extraer y muestrear los datos, y copiar y pegar la consulta directamente en un módulo de lector de Aprendizaje automático de Azure. Este método se muestra en la sección [Generación de modelos en Aprendizaje automático de Azure](#mlmodel).    
2. Conservar los datos muestreados y de ingeniería que pretende usar para la generación de modelos de una nueva tabla de base de datos y usar la nueva tabla en el módulo del Lector.
...
#### Informe con el número de filas y columnas en la tabla nyctaxi_trip
...
#### Visualización: Ejemplo de caja

A continuación, observaremos la caja de la distancia de la carrera para ver los cuantiles

    df1.boxplot(column='trip_distance',return_type='dict')

![Plot #1][1]

#### Visualización: Ejemplo de gráfico de distribución
...
#### Visualización: Gráfico de barras y líneas
...
#### Diseño de características: Características de recuento de columnas de categorías
...
#### Diseño de características: Características de discretización de columnas numéricas
...
#### Diseño de características: Extraer las características de ubicación a partir de una latitud o longitud decimal

En este ejemplo se divide la representación decimal de un campo de longitud o latitud en varios campos de región de granularidad diferente, como país, provincia, localidad, bloque, etc. Tenga en cuenta que los nuevos campos geográficos no están asignados a ubicaciones reales. Para obtener información sobre la asignación de ubicaciones de códigos geográficos, consulte [Servicios de REST de Mapas de Bing](https://msdn.microsoft.com/library/ff701710.aspx). 
...
2. Clasificación de multiclase: para predecir el intervalo de la propina pagada, según las clases definidas anteriormente.
...
1. Proporcionar los datos a Aprendizaje automático de Azure con el módulo **Lector**, disponible en la sección **Entrada y salida de datos**. Para obtener más información, consulte la página de referencia del módulo [Lector](https://msdn.microsoft.com/library/azure/4e1b0fe6-aded-4b3f-a36f-39b8862b9004).

	![Azure ML Reader][17]

2. Seleccionar **Base de datos SQL de Azure** como el **Origen de datos** en el panel **Propiedades**.

3. Escribir el nombre DNS de la base de datos en el campo **Nombre del servidor de la base de datos**. Formato: `tcp:<nombre_DNS_de_su_máquina_virtual>,1433`

4. Escriba el **Nombre de la base de datos** en el campo correspondiente.

5. Escriba el **nombre de usuario de SQL** en **Nombre de la cuenta de usuario del servidor y la contraseña en **Contraseña de la cuenta de usuario del servidor**.

6. Activar la opción **Aceptar cualquier certificado de servidor**.

7. En el área de texto editable **Consulta de base de datos**, pegar la consulta que extrae los campos de la base de datos necesarios (incluidos los campos calculados, como las etiquetas) y reduce la muestra al tamaño de muestra deseado.
...
Cuando el modelo esté listo, podrá implementarlo fácilmente como un servicio web directamente desde el experimento. Para obtener más información sobre la publicación de servicios web de Aprendizaje automático de Azure, consulte [Publicación del servicio web de Aprendizaje automático de Azure](machine-learning-publish-a-machine-learning-web-service.md).
...
Cuando se crea el experimento de puntuación, revíselo y ajústelo según sea necesario. Un ajuste común consiste en reemplazar la consulta o el conjunto de datos de entrada por uno que excluya los campos de etiqueta, ya que estos no estarán disponibles cuando se llame al servicio. También es una buena práctica reducir el tamaño de la consulta o del conjunto de datos de entrada a unos pocos registros, los necesarios para indicar el esquema de entrada. Para el puerto de salida, es habitual excluir todos los campos de entrada y solo incluir las **Etiquetas puntuadas** y las **Probabilidades puntuadas** en la salida mediante el módulo **Proyectar columnas**.
...
Para recapitular, en este tutorial paso a paso, se ha creado un entorno de ciencia de datos de Azure, se ha trabajado con un conjunto grande de datos público de principio a fin, desde la adquisición de los datos al entrenamiento del modelo, y la publicación de un servicio web de Aprendizaje automático de Azure.
