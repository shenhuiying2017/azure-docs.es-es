...
La aplicación complementaria de ejemplo de este tutorial se puede [descargar aquí](http://go.microsoft.com/fwlink/?LinkId=271213). El ejemplo se puede ejecutar sin cambios, aunque puede que tenga que modificar la [asignación de puertos en Visual Studio](http://msdn.microsoft.com/es-es/library/ms178109(v=vs.100).aspx) para que use https. Siga las instrucciones del vínculo y configure el protocolo de enlace como "https" en la sección enlaces del archivo ApplicationHost.config. Todos los fragmentos de código de los pasos que aparecen a continuación se han extraído del ejemplo.

> [WACOM.NOTE] La aplicación de directorio multiempresa de ejemplo tiene fines ilustrativos únicamente. Este ejemplo (incluyendo sus clases de biblioteca auxiliar) no debe usarse en producción.
...
1.  En el nodo **appSettings**, reemplace los valores de "clientId" y "SymmetricKey" por el Id. de cliente, el secreto de cliente y el dominio de la aplicación:
         ...
         <appSettings>
             ...
             <add key="SymmetricKey" value="(el valor del secreto de cliente)"/>
             <add key="AppHostname" value="(el dominio de la aplicación)"/>
		 </appSettings>
...
2.  En el nodo **audienceUris** de **system.identityModel**, inserte el Id. de cliente después de "spn:":
         ...
         <system.identityModel>
                ...
                <add value="spn:(el valor de Id. de cliente)" />
	    </audienceUris\>
...
**RequestedPermissions**: (opcional) Permisos que el inquilino debe conceder a la aplicación. Durante el desarrollo, estos permisos se usan para probar las aplicaciones no publicadas. Este parámetro se ignora en el caso de las aplicaciones publicadas. Y en vez de lo anterior, se usarán los permisos solicitados en la lista de aplicaciones. Consulte la parte 5 para obtener más información acerca de esta lista. Hay tres valores posibles para este parámetro:
...
A continuación, encontrará un ejemplo de dirección URL de solicitud de consentimiento válida: *https://activedirectory.windowsazure.com/Consent/AuthorizeApplication.aspx?ApplicationId=33E48BD5-1C3E-4862-BA79-1C0D2B51FB26&RequestedPermissions=DirectoryReader*
...
A continuación encontrará un ejemplo de respuesta válida a la solicitud de consentimiento que indica que la aplicación se ha autorizado: *https://app.litware.com/redirect.aspx&TenantId=7F3CE253-66DB-4AEF-980A-D8312D76FDC2&Consent=Granted*
...
Una vez concedido el consentimiento, Azure AD puede tardar unos instantes antes de que se aprovisionen el acceso a Graph y el SSO. El primer usuario de cada organización que inicie sesión en la aplicación podría ver errores de inicio de sesión hasta que termine el aprovisionamiento.
...
Una vez que el cliente haya concedido su consentimiento a la aplicación, es importante asociar y almacenar el inquilino recién creado en la aplicación con el TenantId devuelto por la respuesta de consentimiento. La aplicación de ejemplo contiene un *HttpModule* en el espacio de nombres *Microsoft.IdentityModel.WAAD.Preview.Consent* que registra automáticamente el TenantId en un almacén de datos customer/TenantId de todas las respuestas de consentimiento correctas. El código se incluye más abajo y el método *TrustedIssuers.Add* realiza el registro de TenantId en un almacén de datos customer/TenantId:
             ...
             EventArgs e)
    {
        ...
        { 
            ...
            {
                // Para este ejemplo almacenamos los inquilinos que conceden el consentimiento en
                // un archivo XML. Recomendamos encarecidamente cambiarlo
                // para usar el almacén de datos.
...
Si los clientes inician sesión en un nuevo inquilino de Azure AD, el aprovisionamiento del inquilino podría tardar unos instantes. Puede que los usuarios vean errores en la página de consentimiento hasta que termine el aprovisionamiento.
...
En la aplicación de ejemplo, podrá encontrar el código original debajo del espacio de nombres *Microsoft.IdentityModel.WAAD.Preview.WebSSO*. El controlador de token también usa el método Contains de la clase *Microsoft.IdentityModel.WAAD.Preview.WebSSO.TrustedIssuers*, que comprueba que el TenantID esté guardado en el almacén de datos customer/TenantId.
    ...
    /// <summary>
    /// Extiende la salida del controlador de token SAML2 del cuadro asegurándose de que
    /// los inquilinos registrados emitan los tokens entrantes. 
    /// </summary>
...
La biblioteca auxiliar de la aplicación de ejemplo Microsoft.IdentityModel.WAAD.Preview ya contiene una clase de proxy generada automáticamente (creada agregando una referencia de servicio a https://graph.windows.net/your-domain-name llamada GraphService). La aplicación usará esta clase de proxy para llamar al servicio Azure AD Graph.
...
En la aplicación de ejemplo, el método GraphInterface de la clase *Microsoft.IdentityModel.WAAD.Preview.Graph.GraphInterface* controla estas llamadas a la API, como se indica en el código siguiente.
    ...
    {
        // 1a: Cuando el cliente inició sesión, obtuvimos un token de seguridad 
        // que contiene un Id. de inquilino. Extráigalo aquí.
        TenantDomainName = ClaimsPrincipal.Current.FindFirst("http://schemas.microsoft.com/ws/2012/10/identity/claims/tenantid").Value;

        // 1b: Generamos una dirección URL (https://graph.windows.net/<CustomerDomainName>)
        // para el acceso al extremo de la API Graph de Azure AD para el inquilino. 
        connectionUri = new Uri(string.Format(@"https://{0}/{1}", TenantUtils.Globals.endpoint, TenantDomainName));

        // 2: Crear una instancia del proxy de servicio AzureAD con la dirección URL de la conexión
        dataService = new DirectoryDataService(connectionUri);

        // Este indicador ignora la excepción de recurso no encontrado
        // Si el servicio de AzureAD inicia esta excepción, devuelve nulo.
        ...
        // 3: Así se agregan los encabezados necesarios predeterminados a cada solicitud
...
AAL está disponible como paquete de NuGet y se puede instalar dentro de Visual Studio.

    /// <summary>
    /// Método para obtener el encabezado de autorización de Oauth2 desde ACS
    /// </summary>
...
La aplicación es responsable de administrar las relaciones de facturación con los clientes. Azure Marketplace solo proporciona vínculos al sitio web de la aplicación e información acerca de ella.
