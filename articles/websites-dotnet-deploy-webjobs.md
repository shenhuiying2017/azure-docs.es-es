<properties 
	pageTitle="Implementación de WebJobs de Azure en Sitios web Azure" 
	description="Obtenga información acerca de cómo implementar WebJobs de Azure en Sitios web de Azure con Visual Studio." 
	services="web-sites" 
	documentationCenter="" 
	authors="tdykstra" 
	manager="wpickett" 
...
* Copia los archivos en tiempo de ejecución a la carpeta correspondiente en el Sitio web Azure (*App_Data/jobs/continuous* para trabajos web continuos, *App_Data/jobs/triggered* para WebJobs programados y a petición).
* Configura [trabajos del programador de Azure](#scheduler) para WebJobs que estén programados para ejecutarse en momentos concretos. (Esto no se necesita para WebJobs continuos.)
...
* Un archivo [webjob-publish-settings.json](#publishsettings) que contiene la configuración de implementación y del programador. 
...
Puede implementar un proyecto como un WebJob por sí mismo o vincularlo a un proyecto web que se implemente automáticamente siempre que implemente el proyecto web. Para vincular proyectos, Visual Studio incluye el nombre del proyecto con WebJobs habilitados en un archivo [webjobs-list.json](#webjobslist) en el proyecto web.
...
### <a id="convertlink"></a>Activación de la implementación automática de WebJobs con un proyecto web
...
1. Haga clic con el botón secundario en el proyecto de aplicación de consola en el **Explorador de soluciones** y, después, haga clic en **Publicar como WebJob de Azure**. 
	...
	El cuadro de diálogo [Agregar WebJob de Azure](#configure) aparece, con el proyecto seleccionado en el cuadro **nombre de proyecto**.
...
1. Haga clic con el botón secundario en el proyecto web en el **Explorador de soluciones** y, después, haga clic en **Agregar** > **Nuevo proyecto WebJob de Azure**.
	...
	El cuadro de diálogo [Agregar WebJob de Azure](#configure) aparece.
...
Para un WebJob programado (no para WebJobs continuos), Visual Studio crea una colección de trabajos de [Programador de Azure](/es-es/services/scheduler/) si todavía no existe y, a su vez, crea un trabajo en la colección:

* La colección de trabajos de programador se denomina *WebJobs-{regionname}*, donde *{regionname}* hace referencia a la región en que se hospeda el sitio web. Por ejemplo: WebJobs-WestUS.
* El trabajo del programador se llama *{websitename}-{webjobname}*. Por ejemplo: MyWebSite-MyWebJob. 
...
y almacena la información de la programación en un archivo *webjob-publish-settings.json* en la carpeta de proyecto *Properties* del proyecto WebJobs. A continuación se muestra un ejemplo de ese archivo:
...
Puede editar este archivo directamente y Visual Studio proporciona IntelliSense. El esquema de archivo se almacena en [http://schemastore.org](http://schemastore.org/schemas/json/webjob-publish-settings.json) y se puede ver allí.  
