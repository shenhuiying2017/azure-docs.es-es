---
title: Incorporación de notificaciones push a la aplicación de Servicios móviles (Xamarin.Forms) - Servicios móviles
...
3. [Incorporación de notificaciones push a la aplicación de Xamarin.Forms.iOS](#iOS)
4. [Incorporación de notificaciones push a la aplicación de Xamarin.Forms.Android](#Android)
5. [Incorporación de notificaciones push a la aplicación de Xamarin.Forms.Windows](#Windows)
6. [Actualizar el script de inserción de la tabla de Azure para enviar notificaciones push a todas las aplicaciones](#all-apps)
...
3. En **Nombre de tabla** escriba *TodoItem* y haga clic en el botón de comprobación.
...
> [!IMPORTANT]
> Debido a los requisitos del Servicio de notificaciones push de Apple (APNS), tiene que implementar y realizar una prueba de las notificaciones push en un dispositivo compatible con iOS (iPhone o iPad) en lugar de hacerlo en el emulador.
> 
> 
...
1. En **AppDelegate**, reemplace el evento **RegisteredForRemoteNotifications**:
...
2. En **AppDelegate**, reemplace el evento **ReceivedRemoteNotification**:
...
1. En **todoitem**, haga clic en la pestaña **Script** y seleccione **Insertar**.
...
3. Abra el archivo de proyecto MainActivity.cs y agregue la siguiente instrucción using a la clase:
...
1. En el proyecto ToDoAzure.Droid, cree una nueva clase en el proyecto denominado `GcmService`.
2. Agregue las siguientes instrucciones using a la clase **GcmService**:
...
4. En el archivo de proyecto **GcmService.cs** agregue la siguiente clase:
...
5. En el archivo de proyecto GcmService.cs, agregue el código siguiente que define la clase **GcmService**:
...
1. Agregue el código siguiente a la clase **GcmService** que reemplaza al controlador de eventos **OnRegistered**.
...
2. Reemplace el método** OnMessage** en **GcmService** con el código siguiente:
...
3. Al agregar el siguiente método se invalida **OnUnRegistered()** y **OnError()**, que son necesarios para compilar el proyecto.
...
> [!IMPORTANT]
> Para recibir notificaciones de inserción, debe configurar la cuenta de Google en el dispositivo virtual de Android (en el emulador, diríjase a **Settings** (Configuración) y haga clic en **Add Account** [Agregar cuenta]). Además, asegúrese de que el emulador esté conectado a Internet.
> 
> 
...
4. En la aplicación, agregue una nueva tarea.
5. Deslice el dedo hacia abajo desde la parte superior de la pantalla para abrir el Centro de notificaciones del dispositivo y visualizar la notificación.
...
2. Agregue lo siguiente a App.xaml.cs:
   ...
   > [!NOTE]
   > En este tutorial, el servicio móvil envía una notificación del sistema al dispositivo. Cuando envía una notificación de icono, debe llamar al método **BindToShellTile** en el canal.
   > 
   > 
...
5. En el Explorador de soluciones, expanda **Propiedades**, abra el archivo WMAppManifest.xml, haga clic en la pestaña **Funcionalidades** y asegúrese de que la funcionalidad **ID\_CAP\_PUSH\_NOTIFICATION** esté activada.
...
1. Haga clic en la pestaña **Inserción**, marque **Habilite las notificaciones de inserción no autenticadas** y, a continuación, haga clic en **Guardar**.
...
1. En Visual Studio, presione la tecla F5 para ejecutar la aplicación.
   ...
   > 
   > 
2. En la aplicación, cree una nueva tarea con el título **Hello push** y luego haga clic inmediatamente en el botón Inicio o Atrás para abandonar la aplicación.
