---
title: Introducción a la autenticación personalizada | Microsoft Docs
...
4. En la carpeta **Models**, encontrará una clase **DbContext** derivada con el nombre de su servicio móvil. Abra el contexto e incluya el código siguiente para agregar la tabla de cuentas al modelo de datos:
   
        public DbSet<Account> Accounts { get; set; }
   
   > [!NOTE]
   > Los fragmentos de código de este tutorial usan `todoContext` como nombre del contexto. Debe actualizar los fragmentos de código para el contexto del proyecto. A continuación, va a configurar las funciones de seguridad para trabajar con estos datos.
   > 
   > 
...
> [!IMPORTANT]
> A este extremo de registro puede tener acceso cualquier cliente a través de HTTP. Antes de publicar este servicio en un entorno de producción, debe implementar algún tipo de esquema para validar los registros, como una comprobación por correo electrónico o SMS. Esto puede ayudar a evitar que un usuario malintencionado cree registros fraudulentos.
> 
> 
...
2. sustituya la definición de clase **CustomLoginProvider** por el código siguiente:
...
3. Cree una nueva clase denominada `CustomLoginProviderCredentials` en el mismo archivo de código.
...
4. Agregue la siguiente implementación del método abstracto `ConfigureMiddleware` a **CustomLoginProvider**.
...
5. Agregue la siguiente implementación del método abstracto `ParseCredentials` a **CustomLoginProvider**.
...
6. Agregue la siguiente implementación del método abstracto `CreateCredentials` a **CustomLoginProvider**.
...
7. Abra el archivo de proyecto WebApiConfig.cs en la carpeta App\_Start y se crea la siguiente línea de código después de **ConfigOptions**:
...
3. Haga clic con el botón derecho en **Controladores**, haga clic en **Agregar** y **Controlador**, cree un nuevo **Controlador de Servicios móviles de Microsoft Azure personalizado** denominado `CustomLoginController` y, a continuación, agregue las siguientes instrucciones `using`:
   
        using Microsoft.WindowsAzure.Mobile.Service.Security;
        using System.Security.Claims;
        using <my_project_namespace>.DataObjects;
        using <my_project_namespace>.Models;
4. Sustituya la definición de clase **CustomLoginController** por el código siguiente:
...
> [!IMPORTANT]
> El que utilice `CustomLoginController` en producción debe contener también una estrategia de detección de ataques por fuerza bruta. De lo contrario, la solución de inicio de sesión puede ser vulnerable a ataques.
> 
> 
...
> [!NOTE]
> Las bibliotecas de los clientes de Servicios móviles se comunican con el servicio mediante HTTPS. Dado que esta solución requiere que envíe contraseñas como texto sin formato, asegúrese de usar HTTPS cuando llame a estos extremos mediante solicitudes REST directas.
> 
> 
