---
title: Trabajo con la biblioteca de cliente Android de Servicios móviles
...
2. En el método **onCreate** de la clase de actividad, agregue la siguiente línea de código después del código que crea el objeto `MobileServiceClient`: se supone que la referencia al objeto `MobileServiceClient` es *mClient*.
   ...
   > [!NOTE]
   > Si usa un proveedor de identidades que no sea Google, cambie el valor pasado al método **login** anterior a uno de los siguientes: *MicrosoftAccount*, *Facebook*, *Twitter* o *WindowsAzureActiveDirectory*.
   > 
   > 
