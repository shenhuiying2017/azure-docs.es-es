---
title: Introducción a la autenticación (Xamarin.iOS) - Servicios móviles
...
1. En Xcode, abra el proyecto que creó al completar el tutorial [Introducción a los Servicios móviles].
2. Presione el botón **Run** (Ejecutar) para crear el proyecto e iniciar la aplicación en el emulador de iPhone; compruebe que se produce una excepción no controlada con el código de estado 401 (No autorizado) después de iniciarse la aplicación.
...
2. A continuación, agregue un método nuevo con nombre **Authenticate** a **ToDoService** definido como:
   ...
   > [!NOTE]
   > Si usa un proveedor de identidades que no sea una cuenta Microsoft, cambie el valor que pasó anteriormente a **LoginAsync** por uno de los siguientes: *Facebook*, *Twitter*, *Google* o *WindowsAzureActiveDirectory*.
   > 
   > 
...
1. Quite la llamada original a **RefreshAsync** de **TodoListViewController.ViewDidLoad**.
2. Presione el botón **Run** (Ejecutar) para compilar el proyecto, inicie la aplicación en el emulador de iPhone e inicie sesión con el proveedor de identidades de su elección.
