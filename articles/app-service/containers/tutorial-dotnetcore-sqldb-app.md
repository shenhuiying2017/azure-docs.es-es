...
El proyecto de ejemplo contiene una aplicación básica CRUD (crear, leer, actualizar, eliminar) que usa [Entity Framework Core](https://docs.microsoft.com/en-us/ef/core/).
...
En Cloud Shell, cree un servidor lógico de SQL Database con el comando [az sql server create](/cli/azure/sql/server#create).
...
Cree una [regla de firewall de nivel de servidor de Azure SQL Database](../../sql-database/sql-database-firewall-configure.md) con el comando [az sql server firewall create](/cli/azure/sql/server#create). Cuando tanto la dirección IP de inicio como final están establecidas en 0.0.0.0., el firewall solo se abre para otros recursos de Azure. 
...
Cree una base de datos con un [nivel de rendimiento S0](../../sql-database/sql-database-service-tiers.md) en el servidor con el comando [az sql db create](/cli/azure/sql/db#create). 
...
En este paso, implementará la aplicación .NET Core conectada a SQL Database en Web Apps for Containers.
...
Para establecer las cadenas de conexión de la aplicación de Azure, use el comando [az webapp config appsettings update](/cli/azure/webapp/config/appsettings#update) en Cloud Shell. En el comando siguiente, reemplace el parámetro *\<app name>* así como *\<connection_string>* por la cadena de conexión que creó anteriormente.

```azurecli-interactive
az webapp config connection-string set --resource-group myResourceGroup --name <app name> --settings MyDbConnection='<connection_string>' --connection-string-type SQLServer
```

A continuación, establezca la configuración de la aplicación `ASPNETCORE_ENVIRONMENT` en _Producción_. Esta configuración permite saber si ejecuta Azure, porque usa SQLLite para el entorno de desarrollo local y SQL Database para el entorno de Azure. 
...
La llamada de `Database.Migration()` le ayuda cuando se ejecuta en Azure, porque crea automáticamente las bases de datos que necesita la aplicación .NET Core según su configuración de migración. 
...
![Aplicación web de Azure después de aplicar Migraciones de Code First](./media/tutorial-dotnetcore-sqldb-app/this-one-is-done.png)
...
![Navegación desde el portal a la aplicación web de Azure](./media/tutorial-dotnetcore-sqldb-app/access-portal.png)
