<properties title="Administraci&oacute;n de la simultaneidad en Almacenamiento de Microsoft Azure" pageTitle="requerido" description="Administraci&oacute;n de la simultaneidad para los servicios BLOB, Cola, Tabla y Archivo" metaKeywords="Optional" services="Optional" solutions="Optional" documentationCenter="Optional" authors="Tamra Myers" videoId="Optional" scriptId="Optional" />
...
# Administración de simultaneidad en el servicio BLOB
...
El siguiente fragmento de código de C# (usando Client Storage Library 4.2.0) muestra un ejemplo sencillo de cómo construir un **If-Match AccessCondition** basándose en el valor ETag al que se accede desde las propiedades de un blob que se recuperó o insertó previamente. A continuación usa el objeto **AccessCondition** cuando actualiza el blob: el objeto **AccessCondition** agrega el encabezado **If-Match** a la solicitud. Si otro proceso ha actualizado el blob, el servicio BLOB devuelve un mensaje de estado HTTP 412 (Error en la condición previa). Puede descargar el ejemplo completo [aquí][aquí].
...
El servicio Almacenamiento también incluye soporte para encabezados condicionales adicionales como **If-Modified-Since**, **If-Unmodified-Since** y **If-None-Match**, así como combinaciones de los mismos. Para obtener más información, consulte [Especificar encabezados condicionales para las operaciones del servicio BLOB][Especificar encabezados condicionales para las operaciones del servicio BLOB] en MSDN.
...
Para bloquear un blob para uso exclusivo, puede adquirir una [concesión][concesión] en él. Cuando adquiere una concesión, especifica cuánto tiempo la necesita: este tiempo puede ser de entre 15 y 60 segundos o infinito, que da lugar a un bloqueo exclusivo. Puede renovar una concesión finita para extenderla y puede liberar cualquier concesión cuando haya terminado con ella. El servicio de blob libera automáticamente concesiones finitas cuando expiran.
...
-   [Especificar encabezados condicionales para las operaciones del servicio BLOB][1]
-   [Lease Container][Lease Container]
-   [Lease Blob][concesión]
...
-   [Operaciones en entidades][Operaciones en entidades]
...
-   [API de REST del servicio Cola][API de REST del servicio Cola]
...
-   [Administración de simultaneidad usando Almacenamiento de Azure - Aplicación de ejemplo][aquí]

Para obtener más información acerca de Almacenamiento de Azure, consulte:

-   [Página principal de Almacenamiento de Microsoft Azure][Página principal de Almacenamiento de Microsoft Azure]
-   [Introducción a Almacenamiento de Azure][Introducción a Almacenamiento de Azure]
-   Introducción al almacenamiento para [Blob][Blob], [Tabla][Tabla] y [Colas][Colas]
-   Arquitectura de almacenamiento – [Almacenamiento de Windows Azure: un servicio de almacenamiento en la nube altamente disponible con coherencia fuerte][Almacenamiento de Windows Azure: un servicio de almacenamiento en la nube altamente disponible con coherencia fuerte]

  [aquí]: http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114
  [Especificar encabezados condicionales para las operaciones del servicio BLOB]: http://msdn.microsoft.com/es-es/library/dd179371.aspx
  [concesión]: http://msdn.microsoft.com/es-es/library/azure/ee691972.aspx
  [1]: http://msdn.microsoft.com/es-es/library/azure/dd179371.aspx
  ...
  [Blob]: http://azure.microsoft.com/es-es/documentation/articles/storage-dotnet-how-to-use-blobs/
  [Tabla]: http://azure.microsoft.com/es-es/documentation/articles/storage-dotnet-how-to-use-tables/
  [Colas]: http://azure.microsoft.com/es-es/documentation/articles/storage-dotnet-how-to-use-queues/
  [Almacenamiento de Windows Azure: un servicio de almacenamiento en la nube altamente disponible con coherencia fuerte]: http://blogs.msdn.com/b/windowsazurestorage/archive/2011/11/20/windows-azure-storage-a-highly-available-cloud-storage-service-with-strong-consistency.aspx
