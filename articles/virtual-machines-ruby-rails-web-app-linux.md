...
2. Abra una nueva línea de comandos o sesión de terminal y escriba el siguiente comando para instalar Ruby on Rails:
	...
	> [AZURE.NOTE]Es posible que este comando requiera privilegios de administrador o raíz en algunos sistemas operativos. Si recibe un error al ejecutar este comando, intente usar 'sudo' como se muestra:
	>
	>````` 
...
2. Cambie de directorio al directorio **blog_app** y, a continuación, use el siguiente comando para crear un scaffolding de blog básico:
...
3. Para crear la base de datos que va a almacenar las publicaciones del blog, use el siguiente comando:
	...
	De esta manera se usará el proveedor de la base de datos predeterminada para Rails, que es la [SQLite3 Database][sqlite3] (Base de datos de SQLite3). Si bien es posible que desee usar una base de datos diferente para una aplicación de producción, SQLite es suficiente para los propósitos de este tutorial.
...
2. Abra el explorador web y navegue a http://localhost:3000/. Debería ver una página similar a la siguiente:

	![Página predeterminada de Rails][default-rails]
...
o para el puerto predeterminado 22
...
2. En la sesión de SSH, use los siguientes comandos para instalar Ruby en la máquina virtual:
	...
	Esto debería devolver la versión de Ruby que está instalada en la máquina virtual, que puede ser distinta de la 1.9.1. Por ejemplo,  **ruby 1.9.3p194 (2012-04-20 revision 35410) [x86_64-linux]**.
...
> [AZURE.NOTE]Si va a usar Windows como entorno de desarrollo, puede usar una utilidad como **pscp** para la funcionalidad de scp. Pscp puede obtenerse desde [PuTTY download page](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html) (Página de descarga de PuTTY).
...
La lista de archivos devuelta debe coincidir con los archivos contenidos en el directorio **blog_app** en su entorno de desarrollo.

##<a id="start"></a>Instalación de gemas e inicio de Rails

1. En la máquina virtual, cambie de directorio al directorio **blog_app** y use el siguiente comando para instalar las gemas especificadas en **Gemfile**:
...
4. En el cuadro de diálogo **ADD ENDPOINT**, haga clic en la flecha de la parte inferior izquierda para continuar en la segunda página y escriba la siguiente información en el formulario:
	...
	* **PRIVATE PORT**: &lt;Información del puerto del paso 3 anterior&gt;
...
6. Debe aparecer un mensaje que muestre **UPDATE IN PROGRESS**. Después de que el mensaje desaparece, el extremo está activo. Ahora puede probar su aplicación dirigiéndose al nombre de DNS de la máquina virtual. El sitio web que aparece debe ser similar al siguiente:
	...
	La anexión de **/posts** a la dirección URL debe mostrar las páginas que se generaron mediante el comando de scaffolding.

	![Página de publicaciones][blog-rails-cloud]
