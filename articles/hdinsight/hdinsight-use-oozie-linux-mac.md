...
> [AZURE.NOTE] Otra opción para definir los flujos de trabajo con HDInsight es Factoría de datos de Azure. Para más información sobre Azure Data Factory, consulte el artículo sobre el [uso de Pig y Hive con Data Factory][azure-data-factory-pig-hive].
...
- **CLI de Azure**: Consulte [Install and Configure the CLI de Azure](../xplat-cli-install.md)
...
2.  Una acción de Sqoop exporta el contenido de la nueva tabla de Hive a una tabla de una base de datos SQL de Azure. Para más información sobre Sqoop, consulte [Uso de Hadoop Sqoop en HDInsight][hdinsight-use-sqoop].

> [AZURE.NOTE] Para ver las versiones de Oozie compatibles con los clústeres de HDInsight, consulte el artículo sobre las [novedades en las versiones de clústeres Hadoop proporcionadas por HDInsight][hdinsight-versions].

##<a name="create-the-working-directory"></a>Creación del directorio de trabajo

Oozie espera los recursos necesarios para que un trabajo se almacene en el mismo directorio. Este ejemplo usa **wasbs: / / / tutoriales/useoozie**. Use el comando siguiente para crear este directorio y el directorio de datos que contendrá la nueva tabla de Hive creada con este flujo de trabajo:

    hdfs dfs -mkdir -p /tutorials/useoozie/data

> [AZURE.NOTE] El directorio `-p` hizo que todos los directorios de la ruta de acceso se crearan si aún no existían. El directorio **data** se usará para almacenar los datos que usa el script **useooziewf.hql**.
...
2. Presione Ctrl-X para salir del editor. Cuando se lo pidan, seleccione **Y** para guardar el archivo y presione **Entrar** para usar el nombre de archivo **useooziewf.hql**.

3. Utilice los comandos siguientes para copiar **useooziewf.hql** a **wasbs:///tutorials/useoozie/useooziewf.hql**:
...
1. Cuando se abra el editor nano, introduzca lo siguiente como contenido del archivo:
    ...
    - **RunSqoopExport**: exporta los datos creados a partir del script de Hive a Base de datos SQL con Sqoop. Solo se ejecutarán si la acción **RunHiveScript** es correcta.
        ...
        > [AZURE.NOTE] Para más información sobre el flujo de trabajo de Oozie y el uso de acciones de flujo de trabajo, consulte la documentación de [Apache Oozie 4.0][apache-oozie-400] (para la versión de HDInsight 3.0) o la [documentación de Apache Oozie 3.3.2][apache-oozie-332] (para la versión de HDInsight 2.1).
...
Siga los pasos del tutorial [Creación de una base de datos SQL](../sql-database/sql-database-get-started.md) para crear una nueva base de datos. Al crear la base de datos, utilice __oozietest__ como nombre de la base de datos. También tome nota del nombre que utiliza para el servidor de base de datos, ya que lo necesitará en la sección siguiente.
...
1. Use el comando siguiente para obtener la dirección completa de WASB para el almacenamiento predeterminado. Se usará en el archivo de configuración en un momento:
    ...
    Guarde el valor **wasbs://mycontainer@mystorageaccount.blob.core.windows.net**, ya que se usará en los pasos siguientes.
...
2. Cuando se abra el editor nano, use lo siguiente como contenido del archivo:
    ...
    * Reemplace todas las instancias de **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** por el valor que recibió anteriormente.

    > [AZURE.WARNING] Debe utilizar la ruta de acceso completa de WASB, con la cuenta de almacenamiento y de contenedor como parte de la ruta de acceso. Con el formato corto (wasbs:///), la acción RunHiveScript generará un error cuando se inicie el trabajo.
...
1. Use lo siguiente para obtener la dirección URL del servicio de Oozie:
    ...
    La parte **http://hn0-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:11000/oozie** es la dirección URL que se usa con el comando Oozie.
...
**Síntomas**: el estado del trabajo cambiará a **SUSPENDED** (SUSPENDIDO). Los detalles del trabajo mostrarán el estado de RunHiveScript como **START_MANUAL**. Si se selecciona la acción, se mostrará el mensaje de error siguiente:
...
###<a name="ja002:-oozie-is-not-allowed-to-impersonate-&lt;user>"></a>JA002: Oozie no tiene permiso para suplantar &lt;USER>
...
- [Uso del coordinadortemporal de Oozie con HDInsight][hdinsight-oozie-coordinator-time]
