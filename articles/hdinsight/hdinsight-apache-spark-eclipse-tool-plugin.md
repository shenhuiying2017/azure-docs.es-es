...
* Clúster de Apache Spark en HDInsight. Para obtener instrucciones, vea [Creación de clústeres Apache Spark en HDInsight de Azure](hdinsight-apache-spark-jupyter-spark-sql.md).
...
![Instalación automática del complemento Scala](./media/hdinsight-apache-spark-eclipse-tool-plugin/auto-install-scala.png)
...
4. En el cuadro de diálogo **New HDInsight Scala Project** (Nuevo proyecto de Scala para HDInsight), proporcione los valores siguientes y luego seleccione **Siguiente**:
   ...
   * Asegúrese de que el SDK de Spark esté establecido en la ubicación donde ha descargado el SDK. El vínculo a la ubicación de descarga figura en la sección [Requisitos previos](#prerequisites) al principio de este artículo. También puede descargar el SDK desde el vínculo del cuadro de diálogo.
...
5. En el siguiente cuadro de diálogo, seleccione la pestaña **Bibliotecas** y deje los valores predeterminados, luego seleccione **Finalizar**. 
...
2. Seleccione el nodo **Trabajos**. Las herramientas de HDInsight detectan automáticamente si se ha instalado el complemento E(fx)clipse. Seleccione **Aceptar** para continuar y luego siga las instrucciones para instalar Marketplace de Eclipse y reiniciar Eclipse.
