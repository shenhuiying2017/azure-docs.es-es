...
2. Seleccione la pestaña **Configs** (Configuraciones).
    ...
    ![Selección de la pestaña Configs (Configuraciones)](media/hdinsight-troubleshoot-spark/update-config-2.png)
...
3. Establezca las siguientes configuraciones de Spark en los valores adecuados, que no deben superar el 90 % de la memoria y los núcleos disponibles. Los valores deben cumplir los requisitos de memoria de la aplicación Spark: 
    ...
    Para obtener la memoria total usada por todos los ejecutores, ejecute el siguiente comando: 
    
    ```apache
    spark.executor.instances * (spark.executor.memory + spark.yarn.executor.memoryOverhead) 
    ```
    Para obtener la memoria total usada por el controlador, ejecute el siguiente comando:
