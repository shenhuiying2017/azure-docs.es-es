<properties
pageTitle="Análisis de registros de telemetría de Application Insights con Spark en HDInsight | Microsoft Azure"
...
* Experiencia en la creación de un clúster de HDInsight basado en Linux. Si no está familiarizado con la creación de un clúster, consulte [Creación de Spark en HDInsight](hdinsight-apache-spark-jupyter-spark-sql.md) para obtener más información.
    ...
    > [AZURE.NOTE] Este documento no es un tutorial para crear un nuevo clúster. En su lugar, hace referencia a otros documentos que proporcionan información sobre cómo crear un clúster que tenga acceso a los datos de telemetría.
...
* __Ubicación__: la cuenta de almacenamiento debe encontrarse en la misma región que HDInsight. Esto reduce la latencia al acceder a los datos y evita los cargos de salida que se producen cuando se mueven datos entre regiones.

* __Tipo de blob__: HDInsight solo es compatible con blobs en bloques. Application Insights usa blobs en bloques de forma predeterminada, por lo que debería funcionar con HDInsight sin ninguna configuración adicional.

* __Permisos de acceso__: si utiliza la misma cuenta de almacenamiento para la exportación continua de Application Insights y el almacenamiento predeterminado de HDInsight, HDInsight tendrá acceso completo a los datos de telemetría de Application Insights. Esto significa que se pueden eliminar los datos de telemetría desde el clúster de HDInsight.
...
1. Desde el [Portal de Azure](https://portal.azure.com), seleccione el clúster de Spark en HDInsight. En la sección __Vínculos rápidos__, seleccione __Paneles de clúster__ y, a continuación, seleccione __Jupyter Notebook__ en la hoja de Paneles de clúster\_\_.

    ![Los paneles de clúster](./media/hdinsight-spark-analyze-application-insight-logs/clusterdashboards.png)

2. En la esquina superior derecha de la página de Jupyter, seleccione __Nuevo__ y, a continuación, __PySpark__. Se abrirá una nueva pestaña en el explorador con un cuaderno de Jupyter Notebook basado en Python.

3. En el primer campo (llamado una __celda__) de la página, escriba lo siguiente:
...
4. Utilice __MAYÚS + INTRO__ para ejecutar el código. En el lado izquierdo de la celda, aparece un asterisco entre corchetes para indicar que se está ejecutando el código en esta celda. Una vez finalizado, el asterisco cambiará a un número y se mostrará un resultado similar al siguiente debajo de la celda:
...
5. Se habrá creado una nueva celda debajo de la primera. Escriba lo siguiente en la nueva celda. Reemplace __CONTAINER__ y __STORAGEACCOUNT__ con el nombre de la cuenta de Almacenamiento de Azure y el nombre del contenedor de blobs utilizado al configurar la exportación continua de Application Insights.

        %%bash
        hdfs dfs -ls wasb://CONTAINER@STORAGEACCOUNT.blob.core.windows.net/
    
    Utilice __MAYÚS + INTRO__ para ejecutar esta celda. Verá un resultado similar al siguiente:

        Found 1 items
        drwxrwxrwx   -          0 1970-01-01 00:00 wasb://appinsights@contosostore.blob.core.windows.net/contosoappinsights_2bededa61bc741fbdee6b556571a4831
    
    La ruta de acceso de WASB devuelta es la ubicación de los datos de telemetría de Application Insights. Cambie la línea `hdfs dfs -ls` en la celda para usar la ruta wasb devuelta y, a continuación, use __MAYÚS + INTRO__ para volver a ejecutar la celda. Esta vez, los resultados deberían mostrar los directorios que contienen datos de telemetría.

    > [AZURE.NOTE] Para el resto de los pasos descritos en esta sección, se ha usado el directorio `wasb://appinsights@contosostore.blob.core.windows.net/contosoappinsights_{ID}/Requests`. Este directorio podría no existir, a menos que los datos de telemetría sean de una aplicación web. Si se usan datos de telemetría que no incluyan un directorio de solicitudes, seleccione otro directorio y ajuste el resto de los pasos para utilizar ese directorio y el esquema para los datos almacenados en él.

6. Escriba lo siguiente en la nueva celda. Reemplace __WASB\_PATH__ con la ruta de acceso del paso anterior.

        jsonFiles = sc.textFile('WASB_PATH')
        jsonData = sqlContext.read.json(jsonFiles)

    Esto crea una nueva trama de datos de los archivos JSON exportados por el proceso de exportación continua. Utilice __MAYÚS + INTRO__ para ejecutar esta celda.
...
8. Para registrar la trama de datos como una tabla temporal y ejecutar una consulta de los datos, utilice lo siguiente:
    ...
    > [AZURE.NOTE] La estructura de contexto está presente en toda la telemetría registrada por Application Insights. Sin embargo, el elemento de la ciudad no se rellena en los registros. Utilice el esquema para identificar otros elementos que se puedan consultar y que puedan contener datos de los registros.
...
2. En la esquina superior derecha de la página de Jupyter, seleccione __Nuevo__ y, después, __Scala__. Se abrirá una nueva pestaña en el explorador con un cuaderno de Jupyter Notebook basado en Scala.
