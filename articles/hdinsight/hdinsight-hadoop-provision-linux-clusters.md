...
Un clúster de Hadoop se compone de varias máquinas virtuales (nodos) que se usan para el procesamiento distribuido de tareas en el clúster. Azure HDInsight abstrae los detalles de implementación de la instalación y configuración de los nodos individuales, por lo que solo tiene que proporcionar información de configuración general. En este artículo, obtendrá información sobre estas opciones de configuración.

## <a name="basic-configurations"></a>Configuraciones básicas

Desde [Azure Portal](https://portal.azure.com), puede crear un clúster de HDInsight con el método de *creación rápida* o *personalizado*. En esta sección se analizan las opciones de configuración básicas utilizadas en la opción de creación rápida. La opción personalizada incluye las configuraciones adicionales siguientes:

- [Aplicaciones](#hdinsight-applications)
...
### <a name="cluster-types"></a> Tipo de clúster
...
| [R Server](hdinsight-hadoop-r-server-overview.md) |Diversas estadísticas de macrodatos, modelado de predicción y funcionalidades de aprendizaje automático. |
...
| Tipo | Nodos | Diagrama |
| --- | --- | --- |
| Hadoop |Nodo principal (2), nodo de datos (más de 1) |![Nodos de clúster de Hadoop en HDInsight](./media/hdinsight-hadoop-provision-linux-clusters/HDInsight-Hadoop-roles.png) |
| HBase |Servidor principal (2), servidor de región (más de 1), nodo maestro/ZooKeeper (3) |![Nodos de clúster de HBase en HDInsight](./media/hdinsight-hadoop-provision-linux-clusters/HDInsight-HBase-roles.png) |
| Storm |Nodo Nimbus (2), servidor de supervisor (más de 1), nodo ZooKeeper (3) |![Nodos de clúster de Storm en HDInsight](./media/hdinsight-hadoop-provision-linux-clusters/HDInsight-Storm-roles.png) |
| Spark |Nodo principal (2), nodo de trabajo (más de 1), nodo ZooKeeper (3) (gratis para el tamaño de máquina virtual ZooKeeper A1) |![Nodos de clúster de Spark en HDInsight](./media/hdinsight-hadoop-provision-linux-clusters/HDInsight-Spark-roles.png) |
...
Puede agregar otros componentes, como Hue, a estos tipos básicos usando [acciones de script](#customize-clusters-using-script-action).
...
Puede crear clústeres de HDInsight en Linux o Windows.  Para más información sobre las versiones de SO, consulte [Versiones compatibles de HDInsight](hdinsight-component-versioning.md#supported-hdinsight-versions). HDInsight 3.4 y versiones posteriores usan la distribución de Ubuntu Linux como el SO subyacente para el clúster. 

### <a name="version"></a>Versión
Esta opción se usa para determinar la versión de HDInsight necesaria para este clúster. Para más información, consulte [Versiones compatibles de HDInsight](hdinsight-component-versioning.md#supported-hdinsight-versions).

### <a name="cluster-tiers"></a>Nivel de clúster

HDInsight de Azure proporciona las ofertas de la nube de macrodatos en dos categorías: Estándar y Premium.  Para más información, consulte [HDInsight Standard and HDInsight Premium]](hdinsight-component-versioning.md#hdinsight-standard-and-hdinsight-premium).
...
La ubicación de clúster no tiene que especificarse explícitamente. El clúster comparte la misma ubicación del almacenamiento predeterminado. Si desea obtener una lista de las regiones admitidas, haga clic en la lista desplegable **Región** en [Precios de HDInsight](https://go.microsoft.com/fwLink/?LinkID=282635&clcid=0x409).


### <a name="storage"></a>Storage

El sistema de archivos distribuido de Hadoop (HDFS) original usa muchos discos locales en el clúster. HDInsight usa blobs en [Azure Storage](hdinsight-hadoop-provision-linux-clusters.md#azure-storage) o [Azure Data Lake Store](hdinsight-hadoop-provision-linux-clusters.md#azure-data-lake-store). Existen algunos requisitos específicos sobre el uso de almacenes de Data Lake para HDInsight. Para más información consulte la sección de introducción de [Creación de clústeres de HDInsight con Data Lake Store mediante Azure Portal](../data-lake-store/data-lake-store-hdinsight-hadoop-use-portal.md).

![Almacenamiento de HDInsight](./media/hdinsight-hadoop-provision-linux-clusters/hdinsight-cluster-creation-storage.png)


#### <a name="azure-storage"></a>Azure Storage
Azure Storage es una solución de almacenamiento sólida y de uso general, que se integra sin problemas con HDInsight. Mediante una interfaz HDFS, el conjunto completo de componentes de HDInsight puede operar directamente en datos estructurados o no estructurados almacenados en blobs. Almacenar los datos en Azure Storage ayuda a eliminar de forma segura los clústeres de HDInsight que se usan para el cálculo sin perder los datos del usuario. HDInsight solo admite cuentas de Azure Storage de __uso general__. No admite actualmente el tipo de cuenta de __Blob Storage__.

Durante la configuración, debe especificar una cuenta de Azure Storage y un contenedor de blobs en la cuenta de Azure Storage. El clúster usa el contenedor de blobs como ubicación de almacenamiento predeterminada. De manera opcional, puede especificar más cuentas de Azure Storage (almacenamiento vinculado) a las que el clúster pueda acceder. El clúster también puede acceder a cualquier contenedor de blobs que esté configurado con acceso de lectura público completo o con acceso de lectura público solo para blobs.
...
Para más información sobre el uso de una cuenta de Azure Storage, consulte [Uso de almacenamiento compatible con HDFS con Hadoop en HDInsight](hdinsight-hadoop-use-blob-storage.md).
...
#### <a name="use-additional-storage"></a>Almacenamiento adicional
...
Una aplicación de HDInsight es una aplicación que los usuarios pueden instalar en un clúster de HDInsight basado en Linux. Estas aplicaciones puede desarrollarlas Microsoft, fabricantes de software independientes (ISV) o el propio usuario. Para más información, consulte [Instalación de aplicaciones de Hadoop de terceros en Azure HDInsight](hdinsight-apps-install-applications.md).
...
## <a name="use-virtual-network"></a>Uso de la red virtual
Con [Azure Virtual Network](https://azure.microsoft.com/documentation/services/virtual-network/), puede crear una red segura y persistente que contiene los recursos necesarios para una solución. Para más información sobre el uso de HDInsight con una red virtual, incluidos los requisitos de configuración específicos de la red virtual, consulte [Extensión de las funcionalidades de HDInsight con Azure Virtual Network](hdinsight-extend-hadoop-virtual-network.md).
...
- [¿Qué es HDInsight?](hdinsight-hadoop-introduction.md).
- [Tutorial de Hadoop: introducción al uso de Hadoop en HDInsight](hdinsight-hadoop-linux-tutorial-get-started.md).
