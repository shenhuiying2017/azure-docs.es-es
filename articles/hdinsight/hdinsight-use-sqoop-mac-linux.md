...
* **Un clúster de Hadoop en HDInsight** y una **base de datos de SQL de Azure**: los pasos descritos en este documento se basan en el clúster y la base de datos creados mediante el documento [Creación del clúster y la base de datos SQL](hdinsight-use-sqoop.md#create-cluster-and-sql-database). Si ya tiene un clúster de HDInsight y una base de datos SQL, puede sustituir los valores utilizados en este documento por aquellos.
...
## Creación de la tabla en la base de datos SQL
> [!IMPORTANT]
...
3. Para salir de la utilidad tsql, escriba `exit` en el símbolo del sistema `1>`.
...
2. Utilice el siguiente comando para exportar los datos de **hivesampletable** a la tabla **mobiledata**:
   
        sqoop export --connect 'jdbc:sqlserver://<serverName>.database.windows.net:1433;database=sqooptest' --username <adminLogin> --password <adminPassword> --table 'mobiledata' --export-dir 'wasbs:///hive/warehouse/hivesampletable' --fields-terminated-by '\t' -m 1
   
    Esto indica a Sqoop que debe conectarse a la base de datos SQL, la base de datos **sqooptest**, y exportar los datos de **wasbs:///hive/warehouse/hivesampletable** (archivos físicos para *hivesampletable*) en la tabla **mobiledata**.
...
1. Use lo siguiente para importar datos desde la tabla **mobiledata** de la base de datos SQL al directorio **wasbs:///tutorials/usesqoop/importeddata** en HDInsight:
...
* HDInsight y SQL Server deben estar en la misma red virtual de Azure
  ...
  > [!NOTE]
  > En el caso d las redes virtuales de **punto a sitio**, SQL Server debe ejecutarse en la aplicación de configuración de clientes VPN, que se encuentra disponible en el **Panel** de la configuración de red virtual de Azure.
  > 
  > 
  
    Para obtener más información sobre la creación y la configuración de una red virtual, consulte [Tareas de configuración de la red virtual](../services/virtual-machines.md).
