...
También puede crear notificaciones de alerta en el menú **Actions** . Esto le permite crear desencadenadores que envían notificaciones a través de **CORREO ELECTRÓNICO** o **SNMP** cuando se produce una combinación específica de alerta y gravedad. Por ejemplo, puede enviar una alerta cuando alguna de las alertas del grupo **YARN Default** (Predeterminado de YARN) está definida en **Critical** (Crítica).
...
> [!WARNING]
> No cambie la contraseña del guardián Ambari (hdinsightwatchdog) en el clúster de HDInsight basado en Linux. Cambiar la contraseña impedirá usar acciones de script o realizar operaciones de escalado con el clúster.
> 
> 
