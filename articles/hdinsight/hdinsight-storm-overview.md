...
Apache Storm en HDInsight es un clúster administrado integrado en el entorno de Azure. Storm y otros componentes de Hadoop de HDInsight se basan en Hortonworks Data Platform (HDP), mientras que el sistema operativo del clúster es Ubuntu (una distribución de Linux). Se esta forma se logra una plataforma que es muy compatible con herramientas y servicios muy usados del ecosistema de Hadoop.
...
* Integración con otros servicios de Azure, entre los que se incluyen Event Hub, Azure Virtual Network, SQL Database, Blob Storage y DocumentDB.
...
* __Conectividad de Secure Shell__: puede acceder a los nodos principales de un clúster de HDInsight a través de Internet mediante SSH. Esto le permite ejecutar comandos directamente en el clúster.

  Para más información, consulte [Uso SSH con HDInsight](hdinsight-hadoop-linux-use-ssh-unix.md).

* __Conectividad web__: los clústeres de HDInsight proporcionan la interfaz de usuario web de Ambari. Esto le permite supervisar, configurar y administrar servicios fácilmente en un clúster. Storm en HDInsight también proporciona la interfaz de usuario de Storm, lo que permite supervisar y administrar desde el explorador las topologías de Storm en ejecución.
...
* __Integración de Visual Studio__: Data Lake Tools para Visual Studio (que anteriormente se denominaba Herramientas de HDInsight para Visual Studio) incluye plantillas de proyecto para crear topologías de Storm en C#, así como herramientas para Storm en HDInsight. Las topologías de C# se pueden crear, implementar, supervisar y administrar desde Visual Studio.
...
* __Integración con otros servicios de Azure__
  ...
  * Para el desarrollo de __Java__, Microsoft saca provecho de los componentes de Storm existentes para integrarlos con otros servicios de Azure, siempre que sea posible. En algunos casos, pueden ser necesarios una solución o un componente específicos del servicio.
    ...
    * __Azure Blob Storage__ (cuando se usa como almacenamiento para HDInsight): las topologías basadas en Java pueden acceder a la instancia de Azure Blob Storage asociada con el clúster mediante el bolt Storm-HDFS con el esquema de identificador URI `wasb://`.
  ...
  * En el caso del desarrollo en __C#__, normalmente puede utilizar el SDK de .NET para el servicio de Azure. En algunos casos, el SDK puede usar marcos de trabajo que no estén disponibles en Linux (el sistema operativo host de HDInsight 3.4, y las versiones posteriores). En este caso, puede usar componentes Java desde dentro de la solución de C#.

    * Algunos ejemplos para trabajar con __SQL DB__, __DocumentDB__, __EventHub__ y __HBase__ se incluyen como plantillas en Azure Data Lake Tools para Visual Studio. Para más información, consulte [Desarrollo de topologías de C# para Apache Storm en HDInsight con herramientas de Hadoop para Visual Studio](hdinsight-storm-develop-csharp-visual-studio-topology.md).
...
> [!NOTE]
> Para aprovechar de los nodos nuevos que se agregan mediante el escalado, deberá reequilibrar las topologías iniciadas antes de que se aumentara el tamaño del clúster.
...
* **Spout**: consume datos de un origen de datos y emite una o varias **transmisiones**.
  ...
  > [!NOTE]
  > En muchos casos, los datos se leen de una cola, como Kafka o Azure Event Hubs. La cola garantiza que los datos persistan en caso de una interrupción.
