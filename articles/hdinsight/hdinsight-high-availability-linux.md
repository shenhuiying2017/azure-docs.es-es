...
También se puede acceder al nodo perimetral proporcionado con el servidor de R en clústeres de HDInsight directamente mediante SSH a través del puerto 22. Por ejemplo, `ssh username@RServer.mycluster.ssh.azurehdinsight.net` se conectará al nodo perimetral para un servidor de R en un clúster de HDInsight denominado **mycluster**.
...
* **Red virtual de Azure**: si el clúster de HDInsight forma parte de una Red virtual de Azure, cualquier recurso en la misma Red virtual puede acceder directamente a todos los nodos del clúster.
