...
* Una instancia de Apache Storm en un clúster de HDInsight, versión 3.4. Use uno de los siguientes artículos de introducción para crear un clúster:
  
  * Un clúster de HDInsight. Para información sobre la creación de un clúster, consulte [Introducción a Storm en un clúster de HDInsight](hdinsight-apache-storm-tutorial-get-started-linux.md).
...
* Un editor de texto o un entorno de desarrollo integrado (IDE) de Java.
  ...
  * Un cliente SSH. Para obtener más información sobre el uso de SSH con HDInsight, consulte uno de los siguientes artículos:
    ...
    * [Utilización de SSH con Hadoop en HDInsight basado en Linux desde Linux, Unix, OS X o Bash en Windows 10](hdinsight-hadoop-linux-use-ssh-unix.md).
...
**com.microsoft.example.EventHubReader** lee los datos del Centro de eventos (es decir, los datos escritos por EventHubWriter) y los guarda en el almacenamiento predeterminado de clústeres en el directorio /devicedata.
...
> [!NOTE]
> Al trabajar con una versión anterior de HDInsight, como la 3.2, debe registrar manualmente estos componentes. Para obtener un ejemplo de este proceso, junto con los bits personalizados necesarios para clústeres de HDInsight antiguos, consulte la rama de [Storm v0.9.3](https://github.com/Azure-Samples/hdinsight-java-storm-eventhub/tree/Storm_v0.9.3) del repositorio de ejemplos.
...
* Cambiar el nombre de los archivos de licencia para las dependencias: si no se hace, se puede producir un error en tiempo de ejecución.
...
Tiene que rellenar el archivo `EventHubs.properties` con información sobre el clúster del Centro de eventos y HDInsight.
...
5. Seleccione **Configurar**y cree dos directivas de acceso con la información siguiente:
    ...
    Después de crear los permisos, seleccione el icono **Guardar** en la parte inferior de la página. Con esto se crean las directivas de acceso compartido que se usan para enviar (escritor) y escuchar (lector) a este Centro de eventos.
   
    ![directivas](./media/hdinsight-storm-develop-csharp-event-hub-topology/policy.png)

6. Después de guardar las directivas, use el **generador de claves de acceso compartido** que se encuentra en la parte inferior de la página para recuperar la clave para las directivas de **escritor** y **lector**. Guárdelas, ya que se usan después.
...
2. Para compilar y empaquetar el proyecto, use lo siguiente:
    ...
    Esto descarga las dependencias requeridas, compilaciones y, luego, los paquetes del proyecto. La salida se almacena en el directorio **/target** como **EventHubExample-1.0-SNAPSHOT.jar**.
...
3. Use el comando siguiente para iniciar las topologías.
    ...
    Esto inicia las topologías y les da un nombre descriptivo de "lectura" y "escritura".

4. Espere un minuto o dos para que las topologías escriban y lean los eventos desde Centro de eventos y, a continuación, use el siguiente comando para comprobar que EventHubReader está almacenando los datos en el almacenamiento de HDInsight:
   ...
   > Algunos archivos pueden mostrar un tamaño de 0, ya que EventHubReader los creó pero aún no tienen datos almacenados.
    ...
    Esta acción devuelve datos similares a lo siguiente:
...
### <a name="if-using-a-windows-based-hdinsight-cluster"></a>Si se usa un clúster de HDInsight basado en Windows

> [!IMPORTANT]
> Linux es el único sistema operativo que se usa en la versión 3.4 de HDInsight, o en las superiores. Para más información, consulte [El contrato de nivel de servicio para las versiones de clúster de HDInsight](hdinsight-component-versioning.md#hdi-version-32-and-33-nearing-deprecation-date).

1. Abra https://CLUSTERNAME.azurehdinsight.net en el explorador. Cuando se le pida, escriba las credenciales de administrador para el clúster de HDInsight. Llega al Panel de Storm.

2. Use la lista desplegable **Jar File** (Archivo Jar) para examinar y seleccionar el archivo EventHubExample-1.0-SNAPSHOT.jar desde el entorno de compilación.

3. Para **Nombre de clase**, escriba `com.mirosoft.example.EventHubWriter`.

4. Para **Parámetros adicionales**, escriba `writer`. Por último, haga clic en **Submit** (Enviar) para cargar el archivo jar e iniciar la topología de EventHubWriter.

5. Una vez que se inicie la topología, use el formulario para iniciar EventHubReader:
   
   * **Jar File**(Archivo Jar): seleccione el archivo EventHubExample-1.0-SNAPSHOT.jar que se cargó previamente

   * **Nombre de clase**: escriba `com.microsoft.example.EventHubReader`.

   * **Parámetros adicionales**: escriba `reader`
     
     Haga clic en Submit (Enviar) para iniciar la topología de EventHubReader.

6. Espere unos minutos para permitir a las topologías generar eventos y almacenarlos Azure Storage; después, seleccione la pestaña **Hadoop Query Console** (Consola de consultas de Hadoop) situada en la parte superior de la página del **Panel de Storm**.

7. En **Consola de consultas**, seleccione **Hive Editor** (Editor Hive) y reemplace el valor predeterminado `select * from hivesampletable` por lo siguiente:
    ...
    Haga clic en **Select** (Seleccionar) para ejecutar la consulta. Esto devuelve 10 filas de los datos que EventHubReader escribe en el almacenamiento de clúster Azure. Una vez terminada la consulta, debería ver datos similares a los siguientes:
...
8. Seleccione el **Panel de Storm** en la parte superior de la página y, después, **Storm UI** (IU de Storm). Desde **Storm UI** (IU de Storm), seleccione el vínculo de la topología de **lector** y use el botón **Terminar** para detener la topología. Repita el proceso para la topología de **escritura** .
...
También puede exportar e importar los puntos de control persistentes a WASB (Azure Storage que usan los clústeres de HDInsight basados en Windows). Los scripts para hacerlo se encuentran en el clúster de HDInsight en Storm, en **c:\apps\dist\storm-0.9.3.2.2.1.0-2340\zkdatatool-1.0\bin**.
