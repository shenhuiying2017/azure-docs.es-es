<properties 
   ...
   description="Guía del desarrollador para la autorización con la API de Azure Resource Manager y Active Directory a fin de integrar una aplicación con Azure."
...
# Guía del desarrollador para la autorización con la API de Azure Resource Manager
...
En este tema se proporciona una descripción detallada de cómo crear una aplicación que emplea ambos métodos de autorización.
...
Este es, de principio a fin, el flujo de la aplicación web que va a escribir.
...
- Permiso delegado **Acceder a Azure Service Management** para **Azure Service Management API**. Deje el valor predeterminado **Enable single sign-on and read user's profile** (Habilitar inicio de sesión único y leer el perfil del usuario) en **Azure Active Directory**.
...
1. **Nombre de dominio de directorio**: el nombre de dominio de la instancia de Azure Active Directory asociada a la suscripción de Azure del usuario. La solicitud de autorización de OAuth 2.0 se debe enviar a Azure AD. Para averiguar el nombre de dominio de Azure AD, el usuario debe navegar hasta el Portal de Azure y seleccionar la cuenta de la esquina superior derecha. Puede proporcionar instrucciones visuales al usuario como:
...
| redirect\_uri | URL de respuesta con codificación URL de la aplicación. Por ejemplo: http://www.vipswapper.com/cloudsense/Account/SignIn |
| resource | Identificador con codificación URL de Azure Service Management API: https://management.core.windows.net/ |
| ámbito | openid+perfil
| nonce | Fragmento de datos para enlazar la solicitud de autorización al id\_token devuelto para asegurarse de que se solicita la respuesta de la autorización y no se repite.
...
Antes de que la aplicación inicie la sesión del usuario debe validar el id\_token. La validación de tokens es un tema complejo, y se recomienda usar una biblioteca estándar de controlador de JSON Web Token para la plataforma de desarrollo (consulte el [código fuente del controlador de JWT de Azure AD para .NET](https://github.com/AzureAD/azure-activedirectory-identitymodel-extensions-for-dotnet/blob/master/src/System.IdentityModel.Tokens.Jwt/JwtSecurityTokenHandler.cs)). Dicho esto, la seguridad de la aplicación es responsabilidad suya, así que asegúrese de que la biblioteca que utiliza para controlar el id\_token correctamente valida los siguientes aspectos del token:

- **Tiempo del token**: compruebe las notificaciones nbf y exp para asegurarse de que el token no es demasiado nuevo ni demasiado viejo. Es habitual tener cierta demora (5 minutos) para acomodar los desfases temporales.
...
- **Nonce**: compruebe la notificación nonce y compárela con los datos del valor nonce que envió en la solicitud de autorización, con el fin de asegurarse de que su aplicación ha solicitado la respuesta y de que el token no se repite.
- **Firma**: la aplicación debe comprobar que Azure Active Directory ha firmado el token. Las claves de firma de Azure AD cambian con frecuencia, por lo que la aplicación debe sondear las claves actualizadas a diario o buscar errores en las claves actualizadas si no se puede realizar la validación de la firma. Para obtener más información, consulte [Información importante acerca de la sustitución de la clave de firma en Azure AD](active-directory/active-directory-signing-key-rollover.md).

Una vez que se valide **id\_token**, utilice el valor de notificación oid como identificador inmutable y no reutilizable del usuario. Use la notificación **unique\_name** o la notificación de upn o correo electrónico como nombre para mostrar en lenguaje natural del usuario. También puede usar las notificaciones opcionales given\_name/family\_name con fines de presentación.
...
| client\_id | Id. de cliente de la aplicación
| resource | Identificador del recurso con codificación URL para el que se solicita el token de acceso. En este caso, el identificador de la API de Azure AD Graph: **https://graph.windows.net/**
| client\_secret o client\_assertion\_type + client\_assertion | Si la aplicación utiliza credencial de contraseña, utilice client\_secret. Si la aplicación usa credencial de certificado, utilice client\_assertion.
...
- Si la aplicación administra Azure, la suscripción, y se ocupa de la creación, modificación o eliminación de entidades, requerirá uno de los permisos de colaborador. Asigne el rol **Lector**.
...
La asignación de roles de la aplicación será visible para los usuarios, así que seleccione el mínimo privilegio necesario.
