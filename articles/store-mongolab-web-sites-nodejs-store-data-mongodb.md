...
¡Saludos, aventurero! Bienvenido a MongoDB como servicio. En este tutorial, aprenderá a:

1.  [Aprovisionar la base de datos](#provision): El complemento [MongoLab](http://mongolab.com) de la Tienda de Azure proporcionará una base de datos MongoDB hospedada en la nube de Azure y administrada por la plataforma de base de datos en la nube de MongoLab.
2.  [Crear la aplicación](#create): Será una aplicación Node.js sencilla para mantener una lista de tareas.
3.  [Implementar la aplicación](#deploy): Al unir unos pocos enlaces de configuración, haremos la implementación de nuestro código en un abrir y cerrar de ojos.
4.  [Administrar la base de datos](#manage): Finalmente, le mostraremos el portal de administración de base de datos basado en web de MongoLab donde puede buscar, visualizar y modificar los datos con facilidad.
...
-   [Node.js](http://nodejs.org) versión 0.8.14+
...
Si tiene algún conocimiento de la Tienda de Azure, use esta sección para comenzar rápidamente. De lo contrario, vaya a la sección [Aprovisionar la base de datos](#provision) a continuación.
...
![ConnectionInfoScreen](./media/store-mongolab-web-sites-nodejs-store-data-mongodb/dialog-mongolab_connectioninfo.png)  
     **Este URI contiene el nombre de usuario y la contraseña de su base de datos. Dele el mismo tratamiento que a la información confidencial y no lo comparta.**
...
En esta sección podrá configurar su entorno de desarrollo y poner el código para una aplicación web de listas de tareas básicas usando Node.js, Express y MongoDB. [Express](http://expressjs.com) proporciona un marco de controlador de vista para Node, mientras que [Mongoose](http://mongoosejs.com) es un controlador para comunicarse con MongoDB en Node.
...
2.  Seleccione **Proceso \> Web Site \> Quick Create**.  ![CrearSitio](./media/store-mongolab-web-sites-nodejs-store-data-mongodb/screen-mongolab-newwebsite.png)
...
5.  Cuando se complete la creación del sitio web, haga clic en el nombre del sitio web en la lista de sitios web. Aparece el panel del sitio web.  
     ![PanelDelSitioWeb](./media/store-mongolab-web-sites-nodejs-store-data-mongodb/screen-mongolab-websitedashboard.png)
...
A. un [Git remoto](http://git-scm.com/docs/git-remote) llamado 'azure', que se usará para publicar la aplicación en Azure. 
A. un archivo [iisnode.yml](https://github.com/WindowsAzure/iisnode/blob/master/src/samples/configuration/iisnode.yml), que contiene la configuración usada por Azure para hospedar aplicaciones de Node. A. un archivo .gitignore para evitar que la carpeta de módulos de Node se publique en .git.
...
#### Obtener la cadena de conexión de MongoLab.

[WACOM.INCLUDE [howto-get-connectioninfo-mongolab](../includes/howto-get-connectioninfo-mongolab.md)]

#### Agregar la cadena de conexión a las variables de entorno del sitio web
