...
Los conjuntos de registros se crean con el comando `azure network record-set create`. Deberá especificar el nombre del conjunto de registros, la zona, el período de vida (TTL) y el tipo de registro.
...
Para recuperar un conjunto de registros existente, use `azure network dns-record-set show` y especifique el nombre relativo del conjunto de registros, el grupo de recursos, el tipo de registro y el nombre de zona:
...
Para crear un conjunto de registros, use `azure network dns-record-set create` y especifique el nombre relativo del conjunto de registros, el grupo de recursos, el tipo de registro, el nombre de zona y el período de vida (TTL):
...
Los registros se pueden eliminar de un conjunto de registros mediante `azure network dns-record-set delete-record`. Tenga en cuenta que el registro que se va a eliminar debe coincidir exactamente con todos los parámetros de un registro existente.
