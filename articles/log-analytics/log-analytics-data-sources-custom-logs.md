<properties 
   pageTitle="Registros personalizados en Log Analytics | Microsoft Azure"
...
2.	Haga clic en **Data** (Datos) y en **Custom logs** (Registros personalizados).
...
>[AZURE.NOTE]Actualmente, Log Analytics trata la fecha y hora recopilada en un registro con un delimitador de marca de tiempo como una hora UTC. Esto se cambiará pronto a utilizar la zona horaria del agente.
...
1.	Escriba un nombre para el registro. El sufijo **\_CL** se proporciona automáticamente.
...
>[AZURE.NOTE] Si falta la propiedad RawData en la búsqueda, puede ser necesario cerrar y volver a abrir el explorador.
...
## Búsquedas de registros con entradas de registros personalizados
...
|:--|:--|
| Type=MyApp\_CL | Todos los eventos de un registro personalizado llamado MyApp\_CL. |
| Type=MyApp\_CL Severity\_CF=error | Todos los eventos de un registro personalizado llamado MyApp\_CL con el valor *error* en un campo personalizado denominado *Severity\_CF*. |
...
Use el nombre *MyApp\_CL* y escriba una descripción en el campo **Description** (Descripción).
...
Utilice una consulta *Type=MyApp\_CL* para devolver todos los registros desde el registro recopilado.
...
Los campos personalizados se usan para definir los campos *EventTime*, *Code* (Código), *Status* (Estado) y *Message* (Mensaje) y poder ver la diferencia en los registros devueltos por la consulta.
