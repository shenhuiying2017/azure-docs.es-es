<properties
	pageTitle="Búsquedas de registros en Log Analytics | Microsoft Azure"
...
# Búsquedas de registros en Log Analytics
...
1. En el portal de OMS, haga clic en **Log Search** (Búsqueda de registros). ![search tile](./media/log-analytics-log-searches/oms-overview-log-search.png)
2. En el campo de consulta, escriba `error` y luego haga clic en **Buscar**. ![search error](./media/log-analytics-log-searches/oms-search-error.png) Por ejemplo, la consulta de `error` en la siguiente imagen devolvió 100 000 registros de **Event** (recopilados por la administración de registros), 18 registros de **ConfigurationAlert** (generados por la evaluación de la configuración) y 12 registros de **ConfigurationChange** (capturados por el seguimiento de cambios). ![search results](./media/log-analytics-log-searches/oms-search-results01.png)  
...
- En el campo de consulta de búsqueda, escriba `EventLog=System TimeGenerated>NOW-24HOURS"` ![search with boolean](./media/log-analytics-log-searches/oms-search-boolean.png)
...
- En el campo de consulta de búsqueda, escriba `Type=Event EventID=2110 | Top 1` ![search top](./media/log-analytics-log-searches/oms-search-top.png)
...
3. Seleccione algunas de ellas explícitamente y la consulta cambiará a `Type=Event | Select Computer,EventID,RenderedDescription`. ![search select](./media/log-analytics-log-searches/oms-search-select.png)
...
Esta función es muy adecuada para números, pero también para campos de fecha y hora. Resulta útil para comprobar la última marca de tiempo o la más reciente en cualquier parte de los datos indexados para cada equipo. Por ejemplo, ¿cuándo fue el cambio de configuración más reciente notificado por la solución de seguimiento de cambios para cada equipo?
...
Lo primero que observará es que Log Analytics muestra dos perspectivas. Una es la perspectiva Métricas, que muestra los gráficos de los contadores de rendimiento. La otra es la perspectiva Registros, que muestra los registros reales tras los gráficos.
...
- **CounterValue** es el valor real del contador.
...
Puede usar fácilmente esto en un panel. Para aprender más acerca del uso de paneles, consulte [Create a custom dashboard in Log Analytics](log-analytics-dashboards.md) (Creación de un panel personalizado en Log Analytics).
...
### Uso de la función percentile con el comando measure

La función percentile se parece a Avg() y Sum() en que solo se puede usar para campos numéricos. Puede utilizar cualquier percentil entre 1 y 99 en un campo numérico. También puede usar ambos comandos, **percentil** y **pct**. Estos son algunos ejemplos:
...
## Uso de la función countdistinct con el comando measure
...
## Uso del comando measure interval
...
### Uso de varios agregados en una consulta
