...
En este tutorial se da por supuesto que ha instalado y configurado la [CLI de Azure 2.0](/cli/azure/install-az-cli2). También debe tener una clave pública RSA de SSH en `~/.ssh/id_rsa.pub`. Si no tiene una, consulte los pasos para [OS X y Linux](../virtual-machines/linux/mac-create-ssh-keys.md) o [Windows](../virtual-machines/linux/ssh-from-windows.md).
...
kubectl proxy
```
Esta opción ejecuta un proxy autenticado simple en localhost, que puede usar para ver la interfaz de usuario web de Kubernetes que se ejecuta en [http://localhost:8001/ui](http://localhost:8001/ui). Para más información, consulte [Uso de la interfaz de usuario web de Kubernetes con Azure Container Service](container-service-kubernetes-ui.md).
