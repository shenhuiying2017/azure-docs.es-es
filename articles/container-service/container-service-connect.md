...
* Un clúster de Kubernetes, DC/OS o Docker Swarm [implementado en Azure Container Service](container-service-deployment.md).
* Archivo de clave privada RSA de SSH, correspondiente a la clave pública agregada al clúster durante la implementación. Estos comandos asumen que la clave privada SSH está en `$HOME/.ssh/id_rsa` en el equipo. Para más información, consulte estas instrucciones para [macOS y Linux](../virtual-machines/linux/mac-create-ssh-keys.md) o [Windows](../virtual-machines/linux/ssh-from-windows.md). Si la conexión SSH no funciona, es posible que tenga que [restablecer las claves SSH](../virtual-machines/linux/troubleshoot-ssh-connection.md).
