...
- **Mejorar la disponibilidad de las aplicaciones críticas**: el Administrador de tráfico permite mejorar la disponibilidad de las aplicaciones críticas supervisando los extremos de Azure y ofreciendo capacidades de conmutación por error automática cuando un servicio en la nube de Azure, el sitio web de Azure u otra ubicación deja de funcionar.
...
- **Distribución del tráfico en implementaciones grandes y complejas**: con perfiles anidados del Administrador de tráfico, en los que un perfil del Administrador de tráfico puede tener otro perfil del Administrador de tráfico como extremo, puede crear configuraciones para optimizar el rendimiento y la distribución en implementaciones más grandes y complejas. Para obtener más información, consulte [Perfiles anidados](#nested-profiles).
...
*La figura 1* muestra cómo el Administrador de tráfico dirige a los usuarios a uno de los extremos de un conjunto de extremos. Los números de la figura 1 corresponden a las siguientes descripciones numeradas:

![Cómo funciona el Administrador de tráfico](./media/traffic-manager-overview/IC740854.jpg)

**Figura 1**

1. **Tráfico de usuarios hacia el nombre de dominio de la compañía**: el cliente solicita información con el nombre de dominio de la compañía. El objetivo es resolver un nombre de DNS en una dirección IP. Los dominios de la compañía deben reservarse a través de registros normales de nombres de dominio de Internet de la compañía que se mantienen fuera del Administrador de tráfico. En la figura 1, el nombre de dominio de la compañía es *www.contoso.com*.
2. **Nombre de dominio de la compañía al nombre de dominio del Administrador de tráfico**: el registro de recursos DNS del dominio de la compañía apunta al nombre de dominio del Administrador de tráfico que se mantiene en el Administrador de tráfico. Esto se consigue con un registro de recursos CNAME que asigna el nombre de dominio de la compañía al nombre de dominio del Administrador de tráfico. En el ejemplo, el nombre de dominio del Administrador de tráfico es *contoso.trafficmanager.net*.
3. **Perfil y nombre de dominio del Administrador de tráfico**: el nombre de dominio del Administrador de tráfico forma parte del perfil del Administrador de tráfico. El servidor DNS del usuario envía una nueva consulta DNS para el nombre de dominio del Administrador de tráfico (en nuestro ejemplo, *contoso.trafficmanager.net*), que los servidores de nombre de dominio del Administrador de tráfico reciben.
4. **Reglas de perfil del Administrador de tráfico procesadas**: el Administrador de tráfico usa el método de equilibrio de carga y el estado de supervisión especificados para determinar qué extremo de Azure o de otros debe atender la solicitud.
5. **Nombre de dominio de extremo enviado al usuario**: el Administrador de tráfico devuelve un registro CNAME que asigna el nombre de dominio del Administrador de tráfico al nombre de dominio del extremo. El servidor DNS del usuario resuelve el nombre de dominio de extremo en su dirección IP y la envía al usuario.
6. **Usuario llama al extremo**: el usuario llama al extremo devuelto directamente con su dirección IP.
...
La *figure 2* muestra por orden los pasos que son necesarios para implementar el Administrador de tráfico. Estos pasos pueden realizarse en un orden ligeramente diferente una vez que se tenga un conocimiento sólido de la configuración del Administrador de tráfico y las prácticas recomendadas. Los números de la figura 2 corresponden a las siguientes descripciones numeradas:
...
2. **Decidir un nombre para el dominio del Administrador de tráfico**. Considerar la posibilidad de usar un nombre para el dominio con un prefijo único. La última parte del dominio, trafficmanager.net, es fija. Para obtener más información, consulte [Prácticas recomendadas](#best-practices).
...
5. **Crear el perfil y configurar las opciones**. Puede usar las API de REST, Windows PowerShell o el Portal de administración para crear el perfil del Administrador de tráfico y configurar los valores. Para obtener más información, consulte [Cómo configurar opciones del Administrador de tráfico](#how-to-configure-traffic-manager-settings). En los siguientes pasos se supone que va a usar **Creación rápida** en el Portal de administración. 
   - **Crear el perfil del Administrador de tráfico**: para crear un perfil mediante Creación rápida en el Portal de administración, consulte [Administrar perfiles del Administrador de tráfico](traffic-manager-manage-profiles.md).
   - **Configurar los valores del método de equilibrio de carga**: en Creación rápida, debe seleccionar el método de equilibrio de carga para el perfil. Esta configuración puede cambiarse en cualquier momento después de completar los pasos de Creación rápida. Para obtener los pasos de configuración, consulte el tema correspondiente al método de equilibrio de carga: [Configurar Equilibrio de carga de rendimiento](traffic-manager-configure-performance-load-balancing.md),[Configurar Equilibrio de carga de conmutación por error](traffic-manager-configure-failover-load-balancing.md), [Configurar Equilibrio de carga de Round Robin](traffic-manager-configure-round-robin-load-balancing.md).
...
6. **Probar el perfil del Administrador de tráfico**. Compruebe que el perfil y el dominio funcionan según lo esperado. Para obtener información sobre cómo hacerlo, consulte [Pruebas de configuración del Administrador de tráfico](traffic-manager-testing-settings.md).
7. **Hacer que el registro de recursos DNS del nombre de dominio de la compañía apunte al perfil para activarlo**. Para obtener más información, consulte [Hacer que un dominio de Internet de la compañía apunte a un dominio del Administrador de tráfico](traffic-manager-point-internet-domain.md).

En el ejemplo en la figura 1, cambiaría el registro de recursos DNS en los servidores de la manera siguiente para que el nombre de dominio de la compañía apunte al nombre de dominio del Administrador de tráfico:
...
Para obtener más información sobre cmdlets de Windows PowerShell para el Administrador de tráfico, consulte [Cmdlets del Administrador de tráfico de Azure ](http://go.microsoft.com/fwlink/p/?LinkId=400769).
...
En el Portal de administración, puede crear su perfil del Administrador de tráfico mediante Creación rápida. Creación rápida permite crear un perfil básico. Después de crear el perfil, puede configurar opciones adicionales o modificar las opciones que configuró previamente. Para obtener más información sobre cómo crear el perfil del Administrador de tráfico mediante Creación rápida, consulte [Administrar perfiles del Administrador de tráfico](traffic-manager-manage-profiles.md).
...
- **TTL de DNS**: el valor de Período de vida (TTL) de DNS controla la frecuencia con que el servidor de nombres de almacenamiento en caché local del cliente consultará el sistema DNS del Administrador de tráfico de Azure para actualizar las entradas DNS.
...
- **Método de equilibrio de carga**: forma en que quiere que el Administrador de tráfico controle el equilibrio de carga.
- **Orden de conmutación por error**: cuando se emplea el método de equilibrio de carga de conmutación por error, orden de los extremos.
...
- **Perfil**: un perfil contiene un prefijo de nombre de dominio que el usuario crea. Cada perfil se corresponde con la suscripción. Puede crear varios perfiles por suscripción. El nombre del perfil se ve en el Portal de administración. El nombre que cree, que se incluye en el perfil, se conoce como el dominio del Administrador de tráfico.
- **Definición**: una definición contiene la configuración de directiva y la configuración de supervisión. Una definición corresponde a un perfil. Únicamente puede tener una sola definición por perfil. La propia definición no se ve en el Portal de administración, aunque muchos de los valores contenidos en la definición se ven y pueden configurarse en el Portal de administración.
...
- **Utilizar puntos para que los nombres de dominio sean únicos o legibles**: también puede usar puntos para separar las partes del prefijo del nombre de dominio.  Si piensa crear varias directivas en el Administrador de tráfico, use una jerarquía coherente para distinguir un servicio de otro. Por ejemplo, Contoso tiene servicios globales para web, facturación y administración de la utilidad. Las tres directivas serían *web.contoso.trafficmanager.net*, *bill.contoso.trafficmanager.net* y *util.contoso.trafficmanager.net*. Al configurar servicios en la nube o sitios web, use nombres que incluyan la ubicación. Por ejemplo, *web-us-contoso.cloudapp.net* y *web-asia-contoso.cloudapp.net*. Las limitaciones son las impuestas por el DNS. Suponga que un nombre de dominio es una secuencia de etiquetas separadas por puntos (label.label.label.label.etc.).: en el momento de realizar esta documentación, los límites para los nombres de dominio en el Administrador de tráfico son los siguientes:
...
- **Todos los servicios de nube de un perfil deben usar la misma configuración de supervisión**: solo puede elegir una ruta de acceso y un archivo para supervisar todos los extremos de una definición determinada. Puede especificar "/" en el cuadro de texto **Ruta de acceso relativa y nombre de archivo** para que la supervisión intente acceder a la ruta y nombre de archivo predeterminados.
...
- **Almacenamiento**: cómo se diseña la ubicación y distribución del almacenamiento es una consideración importante al usar el Administrador de tráfico. Al diseñar e implementar las aplicaciones en el Administrador de tráfico, piense en la transacción de un extremo a otro y en cómo fluirán sus datos.
- **SQL Azure**: igual que con el diseño de almacenamiento, analice los requisitos de datos y el estado de la aplicación al extender los extremos a varias regiones geográficas.
...
Esto permite configurar el Administrador de tráfico para que las consultas de nombres de DNS entrantes se analicen en un conjunto de niveles a fin de asegurarse de que el cliente solicitante se dirige al conjunto correcto de extremos. *Figure 3* muestra un ejemplo.

![Ejemplo de perfiles del Administrador de tráfico anidados](./media/traffic-manager-overview/IC751072.png)
...
![Ejemplo de perfiles del Administrador de tráfico de varios niveles](./media/traffic-manager-overview/IC751073.png)

**Figura 4**

En *Figure 4*, el perfil del Administrador de tráfico del nivel superior es un perfil principal y los perfiles del Administrador de tráfico del nivel intermedio son perfiles secundarios.
