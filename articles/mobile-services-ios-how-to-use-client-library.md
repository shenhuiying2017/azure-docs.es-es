...
Si va a crear la aplicación de iOS desde el principio, asegúrese de agregar `WindowsAzureMobileServices.framework` al valor **Link Binary With Libraries** de la aplicación.
...
En el ejemplo siguiente, una sola función solicita 20 registros del servidor y, a continuación, los anexa a la colección local de registros previamente cargados:
    ...
    - (bool) loadResults() {
        ...
        [query readWithCompletion:(NSArray *items, NSInteger totalCount, NSError *error) {
            if(!error) {
                //Agregar los elementos a la copia local
                [self.loadedItems addObjectsFromArray:items];       

                //Establecer una marca para hacer un seguimiento en caso de haber registros adicionales que deban cargarse
...
Para insertar una nueva fila en la tabla, debe crear un [objeto NSDictionary](http://go.microsoft.com/fwlink/p/?LinkId=301965) y pasarlo a la función de inserción. El código siguiente inserta un elemento "ToDo" nuevo en la tabla:

    NSDictionary *newItem = @{@"text": @"my new item", @"complete" : @NO};
    [table insert:newItem completion:^(NSDictionary *result, NSError *error) {
        // El resultado contiene el nuevo elemento insertado que,
        // dependiendo de los scripts del servidor, puede tener datos adicionales o modificados 
        // en comparación con los datos pasados al servidor.
    }]; 
...
Para actualizar un objeto existente, modifique un elemento devuelto de una consulta anterior y, a continuación, llame a la función **update**.
    ...
    [table update:item completion:^(NSDictionary *item, NSError *error) {
        //Gestionar errores o cualquier lógica adicional según sea necesario
    }];
...
En el ejemplo siguiente se usa el [SDK de Live Connect](http://go.microsoft.com/fwlink/p/?LinkId=301960) a fin de habilitar el inicio de sesión único para las aplicaciones iOS.
...
Para evitar que los usuarios tengan que autenticarse cada vez que ejecuten la aplicación, puede almacenar la identidad del usuario actual en caché una vez que haya iniciado sesión. Esa información podrá utilizarse posteriormente para crear el usuario directamente y omitir el proceso de inicio de sesión. Para ello, debe almacenar el identificador de usuario y el token de autenticación localmente. En el ejemplo siguiente, el token se almacena en caché de forma segura en [KeyChain]:
    ...
    - (NSMutableDictionary *) createKeyChainQueryWithClient:(MSClient *)client andIsSearch:(bool)isSearch
    {
        ...
        if(isSearch) {
            // Usar las constantes de búsqueda adecuadas, devolver solamente los atributos de la primera coincidencia.
    ...
    - (IBAction)loginUser:(id)sender {
        ...
        if (status == noErr) {
            ...
            //Tenemos que hacer que el usuario inicie sesión
            [self.todoService.client loginWithProvider:@"MicrosoftAccount" controller:self animated:YES
                completion:^(MSUser *user, NSError *error) {
                    ...
                    //Guardar el identificador y el token del usuario en KeyChain
                    ...
                    if(status != errSecSuccess) {
                        //Gestionar el error según sea necesario
...
En caso de haberse producido un error, incluya el archivo MSError.h en el código si desea obtener más información. Este archivo define las siguientes constantes, que se pueden usar para obtener acceso a datos adicionales de `[error userInfo]`:
