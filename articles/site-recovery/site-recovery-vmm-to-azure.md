<properties
	pageTitle="Configuración de la protección entre un sitio de VMM local y Azure | Microsoft Azure"
...
En el siguiente diagrama se muestran los distintos canales y puertos de comunicación usados por Azure Site Recovery para la orquestación y la replicación
...
1. Inicie sesión en el [portal de Azure](https://portal.azure.com) desde el servidor VMM que desee registrar.
...
2. En el menú contextual, seleccione **Entre un sitio Hyper-V local y Microsoft Azure**.
...
9. En **Cifrado de datos para máquinas virtuales protegidas para Azure**, especifique una ubicación para guardar un certificado SSL que se genera automáticamente para el cifrado de datos. Este certificado se usa si habilita el cifrado de datos para una nube protegida por Azure en el portal de Azure Site Recovery. Mantenga el certificado en un lugar seguro. Cuando ejecute una conmutación por error en Azure, la seleccionará para descifrar los datos cifrados.

	![Registro de servidor](./media/site-recovery-vmm-to-azure/ASRE2AVMM_ProviderSyncEncrypt.png)

8. Haga clic en **Register** para finalizar el proceso. Después del registro, la Recuperación del sitio de Azure recupera los metadatos del servidor VMM. El servidor se muestra en la pestaña **Recursos** de la página **Servidores** del almacén.
...
5. Establezca <b>Cifrar datos almacenados</b> en <b>Desactivado</b>. Este valor especifica que los datos de deben cifrar entre el sitio local y Azure.
...
4. Seleccione la red de VM de origen y luego haga clic en **Asignar**.
...
4. En la pestaña de configuración de la máquina virtual, se pueden modificar las siguientes propiedades de red.
...
2. Guarde el nombre de las direcciones IP que se asignaron a los errores en la máquina virtual de DNS.
...
1. Después de la conmutación por error, podrá ver la réplica de prueba de la máquina virtual en el Portal de Azure. Si está configurando para acceder a máquinas virtuales desde la red local puede iniciar una conexión de Escritorio remoto a la máquina virtual.

    a. Compruebe que las máquinas virtuales se inician correctamente.
    ...
    c. Después de conmutación por error, si usa una dirección IP pública para conectarse a la máquina virtual en Azure mediante Escritorio remoto, asegúrese de no tener directivas de dominio que le impidan conectarse a una máquina virtual con una dirección pública.
