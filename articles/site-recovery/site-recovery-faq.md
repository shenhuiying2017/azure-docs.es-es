...
En este artículo se incluyen las preguntas más frecuentes sobre Site Recovery.
...
Site Recovery contribuye a su estrategia de continuidad empresarial y recuperación ante desastres (BCDR) mediante la garantía de que las cargas de trabajo y las aplicaciones permanecerán disponibles durante las interrupciones. Puede usar Site Recovery para organizar y automatizar la replicación de máquinas virtuales locales y servidores físicos en Azure, o en un centro de datos secundario. [Más información](site-recovery-overview.md).
...
- **Servidores físicos**: Site Recovery puede proteger cargas de trabajo que se ejecutan en servidores físicos con Windows o Linux.
...
### ¿Qué son los requisitos de Hyper-V?

Los requisitos para el servidor de host de Hyper-V dependen del escenario de implementación. Compruebe los requisitos previos en:
...
No, las máquinas virtuales deben estar ubicadas en un servidor de host de Hyper-V en el que se esté ejecutando una máquina de servidor de Windows compatible. Si necesita proteger un equipo cliente podría replicarlo como una máquina física [en Azure](site-recovery-vmware-to-azure-classic.md) o en un [Centro de datos secundario](site-recovery-vmware-to-vmware.md).


### ¿Qué cargas de trabajo se pueden proteger con Site Recovery?

Site Recovery puede replicar la mayoría de las cargas de trabajo que se ejecutan en una máquina virtual compatible o servidor físico. Site Recovery también proporciona compatibilidad para la replicación con reconocimiento de aplicaciones para que estas se puedan recuperar a un estado inteligente. Se integra con aplicaciones de Microsoft, incluido SharePoint, Exchange, Dynamics, SQL Server y Active Directory, y colabora estrechamente con los principales proveedores, como Oracle, SAP, IBM y Red Hat. [Obtenga más información](site-recovery-workload.md) acerca de la protección de la carga de trabajo.


### ¿Se necesita siempre un servidor de System Center VMM para proteger las máquinas virtuales de Hyper-V? 

Si desea replicar a un centro de datos secundario, las máquinas virtuales de Hyper-V deben administrarse en servidores de host de Hyper-V en una nube de VMM. Pero si desea replicar a Azure puede replicar las máquinas virtuales en los servidores de host de Hyper-V con o sin nubes de VMM. [Más información](site-recovery-hyper-v-site-to-azure.md).
...
Puede proteger servidores físicos con Windows y Linux, en Azure o en un sitio secundario. [Aprenda](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment) acerca de los requisitos del sistema operativo. Se aplican las mismas limitaciones si está replicando servidores físicos a Azure o a un sitio secundario.
...
Para proteger las máquinas virtuales de VMWare, necesitará un hipervisor vSphere y máquinas virtuales que ejecuten herramientas de VMware. También recomendamos que tenga un servidor VMware vCenter para administrar los hipervisores. [Más información](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment) sobre los requisitos exactos de los servidores VMware y las máquinas virtuales. Se aplican las mismas limitaciones si está replicando máquinas virtuales de VMware a Azure o a un sitio secundario.
...
Sí. Puede automatizar los flujos de trabajo de Site Recovery mediante la API de Rest, PowerShell o el SDK de Azure. Más información acerca de [la implementación de Azure Site Recovery con PowerShell y Azure Resource Manager](site-recovery-deploy-with-powershell-resource-manager.md).
...
La conmutación por error no es automática. Puede iniciar las conmutaciones por error con solo un clic en el portal o bien puede utilizar los [cmdlets de Site Recovery PowerShell](https://msdn.microsoft.com/library/dn850420.aspx) para desencadenar una conmutación por error. La conmutación por recuperación es una serie de acciones del portal de Site Recovery.
...
- [Más información](https://support.microsoft.com/es-ES/kb/3056159) acerca de la limitación de tráfico de Hyper-V.
- [Más información](site-recovery-vmware-to-azure-classic.md#capacity-planning) acerca de la limitación de replicación de VMware a Azure.
...
Se admite Azure Pack, el sistema de plataforma en la nube e implementaciones basadas en System Center (2012 y superiores). Obtenga más información acerca de la integración de Azure Pack en [Configurar la protección para máquinas virtuales](https://technet.microsoft.com/library/dn850370.aspx).
