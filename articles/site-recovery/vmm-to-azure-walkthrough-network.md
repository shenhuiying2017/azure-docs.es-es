---
title: "Planeamiento de las redes para la replicación de Hyper-V (con System Center VMM) en Azure con Azure Site Recovery | Microsoft Docs"
description: "Este artículo describe el planeamiento de las redes necesarias al replicar máquinas virtuales de Hyper-V (con VMM) en Azure"
...
---
# <a name="step-4-plan-networking-for-hyper-v-with-vmm-to-azure-replication"></a>Paso 4: Planeamiento de las redes para la replicación de Hyper-V en Azure (con VMM)

Una vez que haya llevado a cabo el [planeamiento de capacidades](vmm-to-azure-walkthrough-capacity.md) (en el caso de que esté efectuando una implementación completa), lea este artículo para obtener información sobre las consideraciones del planeamiento al replicar máquinas virtuales locales de Hyper-V en nubes de System Center Virtual Machine Manager (VMM) a Azure mediante el servicio [Azure Site Recovery](site-recovery-overview.md).
...
- **Conexión de red**: después de la conmutación por error, todas las máquinas virtuales replicadas de Azure se conectan a la red de Azure asignada. Todas las máquinas con conmutación por error en la misma red pueden conectarse entre sí, aunque lo hagan en planes de recuperación diferentes.
- **Puerta de enlace de red**: si se configura una puerta de enlace de red en la red de Azure de destino, las máquinas virtuales se pueden conectar a otras máquinas virtuales locales en la red local asignada.
...
1. Si no tiene ninguna, prepare una [red lógica de VMM](https://docs.microsoft.com/system-center/vmm/network-logical) que esté asociada a la nube en la que se encuentran los hosts de Hyper-V.
2. Si no tiene ninguna, cree una [red de máquina virtual](https://docs.microsoft.com/system-center/vmm/network-virtual) vinculada a la red lógica que acaba de preparar.
3. Conecte a la red de máquina virtual las máquinas virtuales del servidor de host de Hyper-V o del clúster de la nube VMM.
...
- Se prepara una red de Azure y se configuran las asignaciones de redes a medida que se implementa el escenario.

## <a name="connecting-to-azure-vms-after-failover"></a>Conexión a máquinas virtuales de Azure después de la conmutación por error
