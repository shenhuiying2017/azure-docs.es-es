...
**Requisito previo** | **Detalles**
--- | ---
**vSphere**| Necesita uno o varios hipervisores de VMware vSphere.<br/><br/>Los hipervisores deben ejecutar las versiones 6.0, 5.5 o 5.1 de vSphere con las últimas actualizaciones.<br/><br/>Se recomienda que los hosts vSphere y los servidores vCenter se encuentren en la misma red que el servidor de procesos (se trata de la red en que está ubicado el servidor de configuración a menos que se haya configurado un servidor de procesos dedicado).
...
1. Para usar una cuenta dedicada para la detección automática, cree un rol (por ejemplo, Azure\_Site\_Recovery) a nivel de vCenter con los [permisos necesarios](#vmware-account-permissions).
...
2. Haga clic en **Nueva** > **Integración híbrida** > **Servicios de recuperación**. También puede hacer clic en **Examinar** > **Almacenes de servicios de recuperación** > **Agregar**.
...
2. En la hoja **Agregar servidor**, compruebe que **Servidor de configuración** aparece en **Tipo de servidor**.
...
8. En **Ubicación de instalación**, seleccione dónde quiere instalar los archivos binarios y almacenar la memoria caché. Puede seleccionar una unidad que tenga al menos 5 GB de almacenamiento disponible, pero se recomienda usar una unidad de memoria caché con 600 GB o más de espacio libre.
...
6. En **Frecuencia de instantánea coherente con la aplicación** especifique la frecuencia (en minutos) con que se crearán puntos de recuperación que contengan las instantáneas coherentes con la aplicación.
...
2.	Una vez que haya terminado, seleccione **Yes** (Sí) en **Have you completed capacity planning**? (¿Ha completado el planeamiento de capacidad?).
...
1.	Asegúrese de que la máquina de Linux que quiere proteger sea compatible, tal como se describe en [Requisitos previos de equipos protegidos](#protected-machine-prerequisites). Asegúrese de que haya conectividad de red entre la máquina de Linux y el servidor de procesos. 
...
- Puede consultar la hora de la detección más reciente de las máquinas virtuales de VMware en el campo **Último contacto a las** correspondiente al servidor vCenter/host vSphere en la hoja **Servidores de configuración**.
...
1. Haga clic en **Step 2: Replicate application** (Paso 2: Replicar aplicación) > **Origen**. Después de habilitar la replicación por primera vez, haga clic en **+Replicar** en el almacén para habilitar la replicación de máquinas adicionales.
...
- Para acceder a través de Internet, habilite el protocolo RDP, asegúrese de que se agregan las reglas de TCP y UDP para el **Público**, y asegúrese de que el protocolo RDP está permitido en el **Firewall de Windows** -> **Aplicaciones y características permitidas** para todos los perfiles.
- Para acceder a través de una conexión de sitio a sitio, habilite el protocolo RDP en la máquina y asegúrese de que este está permitido en el **Firewall de Windows** -> **Aplicaciones y características permitidas** para redes de **Dominio** y **Privadas**.
...
5. Cuando la conmutación por error alcance el estado **Completar pruebas**, haga lo siguiente:
...
Rol de usuario de vCenter | Detección/conmutación por error de VMware sin apagar la máquina virtual de origen | Asigne estos privilegios para el servidor vCenter:<br/><br/>Objeto de centro de datos –> Propagar a objeto secundario, rol=solo lectura <br/><br/>El usuario se asigna en el nivel de centro de datos y, por lo tanto, tiene acceso a todos los objetos del centro de datos. Si quiere restringir el acceso, asigne el rol **Sin acceso** con **Propagar a objeto secundario** a los objetos secundarios (hosts vSphere, almacenes de datos, máquinas virtuales y redes). 
...
- [Obtenga más información sobre la conmutación por recuperación](site-recovery-failback-azure-to-vmware.md) para que las máquinas a las que se realizó la conmutación por error y que se ejecutan en Azure vuelvan al entorno local.
