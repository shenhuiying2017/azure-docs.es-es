---
title: "Replicación de máquinas virtuales de VMware y servidores físicos en Azure con Azure Site Recovery en Azure Portal | Microsoft Docs"
...
---
# <a name="replicate-vmware-virtual-machines-and-physical-machines-to-azure-with-azure-site-recovery-using-the-azure-portal"></a>Replicación en Azure de máquinas físicas y máquinas virtuales de VMware con Azure Site Recovery con la utilización del Portal de Azure
...
En este artículo se describe cómo replicar en Azure máquinas virtuales de VMware locales o servidores físicos de Windows/Linux con Azure Site Recovery en Azure Portal.
...
| **Ámbito** | **Detalles** |
| --- | --- |
| **Escenario de implementación** |Replicación de máquinas virtuales de VMware o servidores físicos (Windows/Linux) en Azure mediante Azure Portal. |
| **Requisitos locales** |Máquina local que ejecute un servidor de configuración, servidor de procesos y servidor de destino maestro.<br/><br/> El servidor de configuración precisa una conexión a Internet y acceso (de forma directa o mediante proxy) a direcciones URL específicas. [Detalles completos](#configuration-server-or-additional-process-server-prerequisites). |
| **Requisitos de Azure** |Cuenta de Azure<br/><br/> Almacén de Servicios de recuperación <br/><br/> Cuenta de almacenamiento LRS o GRS en una región de almacenes<br/><br/> Cuenta de almacenamiento estándar o premium<br/><br/> Red virtual de Azure en una región de almacenes. [Detalles completos](#azure-prerequisites). |
| **Limitaciones de Azure** |Si usa GRS, necesita otra cuenta LRS para el registro<br/><br/> Las cuentas de almacenamiento creadas en Azure Portal no pueden moverse entre grupos de recursos.<br/><br/> No se admite la replicación en las cuentas de Premium Storage en el Centro de la India y el India del Sur. |
| **Replicación de Windows** |Windows de 64 bits en servidores físicos o máquinas virtuales de VMware:<br/><br/> Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2 con al menos SP1. [Detalles completos](#replicated-machine-prerequisites). |
| **Replicación de Linux** |Linux en servidores físicos o máquinas virtuales de VMware:<br/><br/>Red Hat Enterprise Linux 6.7, 7.1 y 7.2<br/><br/> CentOS 6.5, 6.6, 6.7, 7.0, 7.1 y 7.2<br/><br/> Oracle Enterprise Linux 6.4 y 6.5, en ejecución en el kernel compatible de Red Hat o Unbreakable Enterprise Kernel Release 3 (UEK3)<br/><br/> SUSE Linux Enterprise Server 11 SP3. [Detalles completos](#replicated-machine-prerequisites). |
| **Agent** |El agente de Mobility Service se instala en cada equipo replicado.<br/><br/> Instálelo manualmente o realice una instalación de inserción desde el servidor de procesos. [Detalles completos](#install-the-mobility-service). |
| **Requisitos de replicación** |Las máquinas replicadas deben cumplir los [requisitos previos de Azure](site-recovery-best-practices.md#azure-virtual-machine-requirements).<br/><br/> No es posible replicar máquinas virtuales con discos replicados.<br/><br/> No se admiten clústeres de invitados de disco compartido.<br/><br/> Puede excluir discos básicos específicos de la replicación, pero no los que sean dinámicos o de sistema operativo.<br/><br/> En el caso de las máquinas con Windows, el disco de sistema operativo debe encontrarse en la unidad C y no ser dinámico. [Más información](#replicated-machine-prerequisites). |
| **Requisitos de VMware** | Uno o más servidores de VMware vSphere (6.0, 5.5 o 5.1 con las últimas actualizaciones). Se recomienda que se encuentren en la misma red que el servidor de configuración (o que el servidor de procesos si lo configura de forma dedicada).<br/><br/> Se recomienda un servidor de vCenter para administrar los hosts de vSphere (6.0 o 5.5 con las últimas actualizaciones). |
| **Limitaciones de VMware** |Site Recovery no admite las nuevas características de vCenter y vSphere 6.0, como Cross vCenter vMotion, volúmenes virtuales y DRS de almacenamiento. La compatibilidad está limitada a las características que también estaban presentes en la versión 5.5. |
| **Pasos de implementación** | **1)** Preparar Azure (suscripción, almacenamiento y red) -> **2)** Preparar los componentes locales (máquina del servidor de configuración y cuenta de VMware) -> **3)** Crear almacén de Recovery Services -> **4)** Preparar el servidor de configuración -> **5)** Configurar las opciones de replicación -> **6)** Prepararse para implementar el agente de Mobility Service -> **7)** Habilitar la replicación -> **8)** Probar la replicación y la conmutación por error. |
| **Conmutación por recuperación** | La conmutación por recuperación solo está destinada a VMware, aunque replique los servidores físicos.<br/><br/> Se necesita una VPN o Azure ExpressRoute entre Azure y el sitio primario.<br/><br/> Necesitará un servidor de procesos temporal configurado como una máquina virtual de Azure. Puede crearlo cuando esté listo para realizar la conmutación por recuperación y eliminarlo después de completar dicha conmutación. |
...
* Proporcionar una protección externa para cargas de trabajo empresariales que se ejecuten en servidores físicos y máquinas virtuales de VMware.
...
* Detectar automáticamente las máquinas virtuales de VMware agregadas a los hosts de vSphere.
* Conmutaciones por error simples desde su infraestructura local a Azure y conmutaciones por recuperación (restauración) desde Azure a servidores de máquina virtual de VMware en su sitio local.
* Habilitar la replicación y conmutación por error, para que las cargas de trabajo de la aplicación repartidas en niveles por varias máquinas se repliquen a la vez. Puede reunir varias máquinas en planes de recuperación para que las cargas de trabajo de la aplicación divididas en niveles conmuten por error al mismo tiempo.

## <a name="scenario-architecture"></a>Arquitectura del escenario
Estos son los componentes del escenario:

- **Servidor de configuración**: una máquina local que coordina la comunicación y administra los procesos de replicación y recuperación de datos. Tendrá que ejecutar la instalación unificada para instalar estos componentes adicionales y el servidor de configuración:
...
- **Azure**: no es necesario crear ninguna máquina virtual de Azure para controlar la replicación y la conmutación por error en Azure.  Necesita una suscripción, una cuenta de Azure Storage para almacenar los datos replicados y una instancia de Azure Virtual Network a la que las máquinas virtuales de Azure se puedan conectar tras la conmutación por error. La red y la cuenta de almacenamiento deben estar en la misma región que el almacén de Servicios de recuperación.
- **Conmutación por recuperación**: se necesitan diversos componentes para realizar la conmutación por recuperación:
 - Servidor de procesos temporal: precisa una máquina virtual de Azure que actúe como servidor de procesos temporal. Puede eliminarlo una vez finalizada la conmutación por recuperación.
 - VPN: necesita una conexión VPN (o Azure ExpressRoute) entre el sitio local y la red de Azure donde se encuentran las máquinas virtuales de Azure.
 - Servidor de destino maestro: si el tráfico de conmutación por recuperación es intenso, es posible que necesite configurar una máquina local del servidor de destino maestro dedicado. En el caso de un tráfico menos intenso, se puede utilizar el servidor de destino maestro predeterminado que se ejecuta en el servidor de configuración.
...
**Figura 1: VMware/physical to Azure**
...
| **Almacenamiento de Azure** |Los datos replicados se almacenan en almacenamiento de Azure y las máquinas virtuales de Azure se crean cuando se produce la conmutación por error. <br/><br/>Para almacenar datos, necesita una cuenta de almacenamiento estándar o premium en la misma región que el almacén de Recovery Services.<br/><br/>Puede usar una cuenta de almacenamiento LRS o GRS. Se recomienda GRS para que los datos sean resistentes si se produce una interrupción regional o si no se puede recuperar la región principal. [Más información](../storage/storage-redundancy.md).<br/><br/> [Premium Storage](../storage/storage-premium-storage.md) se usa normalmente para las máquinas virtuales que necesitan un alto rendimiento constante de E/S y latencia baja para hospedar cargas de trabajo intensivas de E/S.<br/><br/> Si desea utilizar una cuenta premium para almacenar los datos replicados, también necesita una cuenta de almacenamiento estándar para almacenar los registros de replicación que capturan los cambios continuos de los datos locales.<br/><br/> **Limitación:** las cuentas de almacenamiento creadas en Azure Portal no pueden moverse entre grupos de replicación.<br/><br/> **Limitación**: no se admite la replicación en las cuentas de Premium Storage en el Centro de la India y el India del Sur.<br/><br/> [Más información](../storage/storage-introduction.md) sobre Azure Storage. |
| **Red de Azure** |Necesita una instancia de Azure Virtual Network a la que se conectarán las máquinas virtuales de Azure cuando se produzca la conmutación por error. La red virtual de Azure debe estar en la misma región que el almacén de Servicios de recuperación. |
| **Conmutación por recuperación desde Azure** |Necesitará un servidor de procesos temporal configurado como una máquina virtual de Azure. Puede crearlo cuando esté listo para realizar la conmutación por recuperación y eliminarlo después de completar dicha conmutación.<br/><br/> Para la conmutación por recuperación, necesita una conexión VPN (o Azure ExpressRoute) desde la red de Azure al sitio local. |

## <a name="configuration-server-or-additional-process-server-prerequisites"></a>Requisitos previos del servidor de configuración o de servidores de procesos adicionales
Configura una máquina local como el servidor de configuración.

> [!NOTE]
> Si desea escalar horizontalmente los servidores de procesos para disfrutar de una mayor capacidad, los requisitos previos de los servidores de procesos adicionales son idénticos a los requisitos del servidor de configuración.
>

| **Componente** | **Requisito** |
| --- | --- |
| **Servidor de configuración** |Una máquina virtual o física local que ejecute Windows Server 2012 R2. Todos los componentes de Site Recovery local deben estar instalados en esta máquina.<br/><br/>Para la replicación de máquinas virtuales de VMware, se recomienda implementar el servidor como una máquina virtual de VMware de alta disponibilidad. Para la replicación de máquina física, esta puede ser un servidor físico.<br/><br/> La conmutación por recuperación de Azure siempre se efectúa a máquinas virtuales de VMware, aunque replicara un servidor físico. Si no implementa el servidor de configuración como una máquina virtual de VMware, no será necesario configurar un servidor de destino maestro independiente como una máquina virtual de VMware para recibir el tráfico de conmutación por recuperación antes de realizar dicha recuperación.<br/><br/>Si el servidor es una máquina virtual de VMware, el tipo de adaptador de red debe ser VMXNET3. Si usa otro tipo de adaptador de red, instale una [actualización de VMware](https://kb.vmware.com/selfservice/microsites/search.do?cmd=displayKC&docType=kc&externalId=2110245&sliceId=1&docTypeID=DT_KB_1_1&dialogID=26228401&stateId=1) en el servidor de vSphere 5.5.<br/><br/>El servidor debe tener una dirección IP estática.<br/><br/>El servidor no debe ser un controlador de dominio.<br/><br/>El nombre de host del servidor debe contener 15 caracteres o menos.<br/><br/>El sistema operativo deben estar inglés únicamente.<br/><br/> Instale VMware vSphere PowerCLI 6.0. Server.<br/><br/>El servidor de configuración necesita acceso a Internet. Se requiere el acceso de salida de la manera siguiente:<br/><br/>Acceso temporal en HTTP 80 durante la instalación de los componentes de Site Recovery (para descargar MySQL)<br/><br/>Acceso de salida continuo en HTTPS 443 para la administración de replicación<br/><br/>Acceso de salida continuo en HTTPS 9443 para el tráfico de replicación (este puerto se puede modificar)<br/><br/>El servidor también debe poder acceder a las siguientes direcciones URL para que pueda conectarse a Azure: ``*.accesscontrol.windows.net``<br/><br/> ``*.backup.windowsazure.com``<br/><br/> ``*.hypervrecoverymanager.windowsazure.com``<br/><br/> ``*.store.core.windows.net``<br/><br/> ``*.blob.core.windows.net``<br/><br/> ``https://www.msftncsi.com/ncsi.txt``<br/><br/> ``time.windows.com``<br/><br/> ``time.nist.gov``<br/><br/> Si tiene reglas de firewall basadas en direcciones IP en el servidor, compruebe que las reglas permitan la comunicación con Azure.<br/><br/> Permita los [intervalos de IP del centro de datos de Azure](https://www.microsoft.com/download/confirmation.aspx?id=41653) y el protocolo HTTPS (443).<br/><br/>Permita los intervalos de direcciones IP correspondientes a la región de Azure de su suscripción y del oeste de EE. UU.<br/><br/>Permita esta dirección URL para la descarga de MySQL: ``http://cdn.mysql.com/archives/mysql-5.5/mysql-5.5.37-win32.msi`` |

## <a name="vmware-vcentervsphere-host-prerequisites"></a>Requisitos previos de host vCenter/vSphere de VMware
...
* La red debe estar en la misma región de Azure que aquella en la que va a implementar el almacén de Recovery Services.
...
* [Más información](../vpn-gateway/vpn-gateway-site-to-site-create.md) sobre los modelos de implementación compatibles para conexiones VPN de sitio a sitio y sobre la [configuración de una conexión](../vpn-gateway/vpn-gateway-site-to-site-create.md#CreateVNet).
...
1. Para usar una cuenta dedicada para la detección automática, cree un rol (por ejemplo, Azure_Site_Recovery) en el nivel de vCenter con los [permisos necesarios](#vmware-account-permissions).
...
2. Haga clic en **Nuevo** > **Administración** > **Backup y Site Recovery (OMS)**. También puede hacer clic en **Examinar** > **Almacén de Recovery Services** > **Agregar**.
...
2. Después de finalizar el registro, el servidor aparecerá en **Configuración** > **Servidores** del almacén.
...
### <a name="add-the-vmware-account-used-for-automatic-discovery"></a>Agregar la cuenta de VMware utilizada para la detección automática
...
### <a name="connect-to-vsphere-hosts-and-vcenter-servers"></a>Conexión a hosts de vSphere y servidores vCenter
...
3. En **Add vCenter** (Agregar vCenter), especifique un nombre descriptivo para el host de vSphere o el servidor vCenter y especifique la dirección IP o el FQDN del servidor. Deje el puerto 443 a menos que los servidores de VMware estén configurados para escuchar las solicitudes en un puerto diferente. A continuación, seleccione la cuenta que se usará para conectarse al servidor VMware. Haga clic en **Aceptar**.
   ...
   > [!NOTE]
   > Si agrega el servidor vCenter o el host vSphere con una cuenta que no tiene privilegios de administrador en el servidor vCenter o el servidor host, asegúrese de que la cuenta tiene habilitados los siguientes privilegios: Centro de datos, Almacén de datos, Carpeta, Host, Red, Recurso, Máquina virtual, Conmutador distribuido de vSphere. Además, el servidor vCenter necesita el privilegio Vistas de almacenamiento.
   >
   >
...
4. Si no ha creado una cuenta de almacenamiento y desea crear una con Resource Manager, haga clic en **+Cuenta de almacenamiento** para hacerlo directamente.  En la hoja **Crear cuenta de almacenamiento**, especifique el nombre, el tipo, la suscripción y la ubicación de la cuenta. La cuenta debe estar en la misma región que el almacén de Servicios de recuperación.
   ...
   * Si utiliza una cuenta de almacenamiento premium para los datos replicados, deberá configurar una cuenta de almacenamiento estándar adicional para los registros de replicación del almacén que capturan los cambios continuos de los datos locales.
   ...
   > [!NOTE]
   > Actualmente, no se admite la protección en las cuentas de almacenamiento premium en el Centro de la India y el India del Sur.
   >
   >
5. Seleccione una red de Azure. Si no ha creado una red y desea crear una con Resource Manager, haga clic en **+Red** para hacerlo directamente. En la hoja **Crear red virtual** , especifique el nombre, el intervalo de direcciones, los detalles de subred, la suscripción y la ubicación de la red. La red debe estar en la misma ubicación que el almacén de Servicios de recuperación.
...
4. En **Retención de punto de recuperación**, especifique, en horas, el tiempo que estará disponible el período de retención para cada punto de recuperación. Los equipos protegidos se pueden recuperar en cualquier punto dentro de un período. Se admite una retención de hasta 24 horas para máquinas replicadas en almacenamiento premium.
5. En **Frecuencia de instantánea coherente con la aplicación**especifique la frecuencia (en minutos) con la que se crearán puntos de recuperación que contengan las instantáneas coherentes con la aplicación.
6. Cuando se crea una directiva de replicación, de forma predeterminada una directiva de conciliación se crea automáticamente para la conmutación por recuperación. Por ejemplo, si la directiva de replicación es **rep-policy**, la directiva de conmutación por recuperación será **rep-policy-failback**. Esta directiva no se usa hasta que inicie una conmutación por recuperación.  
7. Haga clic en **Aceptar** para crear la directiva.

    ![Directiva de replicación](./media/site-recovery-vmware-to-azure/gs-replication2.png)
8. Cuando se crea una nueva directiva, esta se asocia automáticamente con el servidor de configuración. Haga clic en **Aceptar**.
...
2. Cuando haya terminado, seleccione **Sí** in **¿Completó el planeamiento de capacidad?**

   ![planeamiento de capacidad](./media/site-recovery-vmware-to-azure/gs-capacity-planning.png)

La tabla siguiente lo ayuda a planear la capacidad.
...
## <a name="step-6-replicate-applications"></a>Paso 6: Replicación de aplicaciones
...
Cuando se habilita la replicación, de forma predeterminada se replican todos los discos en una máquina. Puede excluir discos de la replicación. Por ejemplo, es posible que no desee replicar los discos con datos temporales, o los datos que se actualizan cada vez que una máquina o aplicación se reinicia (por ejemplo, pagefile.sys o tempdb de SQL Server). Observe lo siguiente:

* Solo puede excluir discos que ya tienen instalado Mobility Service. Tendrá que [instalar manualmente Mobility Service](#install-the-mobility-service-manually), porque este servicio solo se instala con el mecanismo de inserción después de habilitar la replicación.
...
5. Seleccione el servidor de procesos. Si no ha creado ningún servidor de procesos adicional, este será el nombre del servidor de configuración. y, a continuación, haga clic en **Aceptar**.
...
7. Seleccione la cuenta de Azure Storage que desea usar para los datos de replicación. Observe lo siguiente:
   ...
   * Si desea utilizar una cuenta de almacenamiento diferente de las que tiene, puede [crear una](#set-up-an-azure-storage-account). Para crear una cuenta de almacenamiento mediante Resource Manager, haga clic en **Crear nueva**. Si desea crear una cuenta de almacenamiento mediante el modelo clásico, lo hará [en el Portal de Azure](../storage/storage-create-storage-account-classic-portal.md).
...
3. En **Proceso y red** > **Propiedades de proceso**, puede especificar el nombre y el tamaño de destino de la máquina virtual de Azure. Modifique el nombre para que cumpla con los requisitos de Azure si es necesario.
   También puede ver y agregar la información sobre la red, la subred y la dirección IP de destino que se asignarán a la máquina virtual de Azure. Tenga en cuenta lo siguiente:
...
1. Realice la conmutación por error no planeada tal y como se mencionó [aquí](site-recovery-failover.md#run-an-unplanned-failover).
...
* [Instale manualmente Mobility Service](#install-the-mobility-service-manually) en las máquinas, en lugar de usar el servidor de procesos para insertar el servicio automáticamente. Este paso es necesario porque la instalación de inserción solo se produce después de habilitar la máquina para la replicación.
...
Si tiene un grupo de seguridad de red asociado a la máquina virtual o a la subred de la máquina virtual, asegúrese de que dicho grupo cuente con una regla de salida para permitir los protocolos HTTP y HTTPS. Además, asegúrese de que el DNS de la red a la que la máquina virtual conmuta por error esté configurado correctamente. De lo contrario, la conmutación por error podría agotar el tiempo de espera con el error -'PreFailoverWorkflow task WaitForScriptExecutionTask timed out'. [Más información](site-recovery-monitoring-and-troubleshooting.md#recovery).
...
1. Para conmutar por error una sola máquina, en **Configuración** > **Elementos replicados**, haga clic en el icono Máquina virtual > **+Probar conmutación por error**.
    ...
    ![Probar conmutación por error](./media/site-recovery-vmware-to-azure/test-failover1.png)
...
5. Cuando la conmutación por error alcance el estado **Completar prueba** , haga lo siguiente:
   ...
   3. Haga clic en **Completar prueba** para terminarla.
       ...
       ![Probar conmutación por error](./media/site-recovery-vmware-to-azure/test-failover6.png)
...
1. Haga clic en el nombre del almacén para acceder al panel **Essentials** . En este panel puede ver los trabajos de Site Recovery, el estado de la replicación, los planes de recuperación, el estado del servidor y los eventos.  Puede personalizar la configuración para mostrar los iconos y diseños que más útiles le resulten, incluido el estado de los almacenes.<br>
   ![Información esencial](./media/site-recovery-vmware-to-azure/essentials.png)
...
### <a name="install-an-additional-process-server"></a>Instalación de un servidor de procesos adicional
...
Site Recovery tiene que acceder a las cuentas de VMware para la conmutación por error y recuperación, así como con la finalidad de que el servidor de procesos pueda detectar automáticamente las máquinas virtuales de un servidor vCenter. En la tabla siguiente se resumen los permisos de rol necesarios.

* Si solo desea detectar máquinas virtuales y efectuar la conmutación por error a Azure sin realizar la conmutación por recuperación (migración), puede emplear una cuenta de solo lectura.
* En el caso de desear aplicar ambas conmutaciones, se recomienda crear un rol de VMware (Azure_Site_Recovery) con los permisos necesarios y, después, asignarlo a un usuario o grupo de VMware.

| **Task** | **Tipo de rol** | **permisos** | **Detalles** |
| --- | --- | --- | --- |
| Detección de VMware<br/><br/> Conmutación por error a Azure sin apagar la máquina virtual de origen (útil para la migración sin conmutación por recuperación) |Usuario de VMware de solo lectura |Objeto de centro de datos  –> Propagar al objeto secundario, rol = solo lectura |El usuario se asigna en el nivel de centro de datos y, por lo tanto, tiene acceso a todos los objetos de este.<br/><br/> Si quiere restringir el acceso, asigne el rol Sin acceso con propagación a objetos secundarios a los objetos secundarios (hosts de vSphere, almacenes de datos, máquinas virtuales y redes). |
| Conmutación por error y conmutación por recuperación |Usuario de VMware<br/><br/> Este usuario deberá poder realizar operaciones como crear y quitar discos, encender máquinas virtuales, etc.<br/><br/> Se recomienda crear un rol (Azure_Site_Recovery) con los permisos necesarios y, después, asignarlo a un grupo o usuario de VMware. |Objeto de centro de datos –> Propagar a objeto secundario, rol = Azure_Site_Recovery<br/><br/> Almacén de datos -> Asignar espacio, examinar almacén de datos, operaciones de archivo de bajo nivel, quitar archivo, actualizar archivos de máquina virtual<br/><br/> Red -> Asignación de red<br/><br/> Recursos-> Asignar máquina virtual al grupo de recursos, migrar apagado de máquina virtual, migrar máquinas virtuales encendidas<br/><br/> Tareas -> Crear tarea, actualizar tarea<br/><br/> Máquina virtual -> Configuración<br/><br/> Máquina virtual -> Interactuar -> Responder a pregunta, conexión de dispositivos, configurar soporte de CD, Configurar soporte de disquete, apagar, encender, instalación de herramientas de VMware<br/><br/> Máquina virtual -> Inventario -> Crear, registrar, anular registro<br/><br/> Máquina virtual -> Aprovisionamiento -> Permitir descarga de máquina virtual, permitir carga de archivos de máquina virtual<br/><br/> Máquina virtual -> Instantáneas -> Quitar instantáneas |El usuario se asigna en el nivel de centro de datos y, por lo tanto, tiene acceso a todos los objetos de este.<br/><br/> Si quiere restringir el acceso, asigne el rol Sin acceso con propagación a objetos secundarios a los objetos secundarios (hosts de vSphere, almacenes de datos, máquinas virtuales y redes). |

## <a name="next-steps"></a>Pasos siguientes
* [Más información](site-recovery-failover.md) sobre los diferentes tipos de conmutación por error.
* [Más información sobre la conmutación por recuperación](site-recovery-failback-azure-to-vmware.md), para realizar la conmutación por recuperación y empezar a replicar las máquinas virtuales de Azure en el sitio local primario.
