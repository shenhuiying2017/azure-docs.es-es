---
title: "Planeación de red para la replicación de servidor físico en Azure | Microsoft Docs"
description: "En este artículo se describe el planeamiento de red necesario cuando se replican servidores físicos en Azure"
...
# <a name="step-4-plan-networking-for-physical-server-replication-to-azure"></a>Paso 4: Planeación de red para la replicación de servidor físico en Azure

En este artículo se resumen las consideraciones de planeamiento de red al replicar servidores físicos locales en Azure con el servicio [Azure Site Recovery](site-recovery-overview.md).
...
## <a name="connect-to-replica-azure-vms"></a>Conexión a máquinas virtuales de Azure de réplica
...
Para que Woodgrove pueda replicar sus servidores en Azure y conservar las direcciones IP, la empresa debe hacer lo siguiente:
...
3. Al configurar la conexión de sitio a sitio en la red de Azure, solo puede enrutar el tráfico a la ubicación local (red local) si el intervalo de direcciones IP es distinto del intervalo de direcciones IP local.
    ...
    - Esto es normal, ya que Azure no sabe que no hay ninguna máquina activa en la subred y que se está creando la subred solo para fines de recuperación ante desastres.
...
2. Para asegurarse de que la dirección IP de una máquina se conserva después de una conmutación por error, en las Propiedades de la máquina > **Configurar**, especifique la misma dirección IP que el servidor tiene localmente y haga clic en **Guardar**.
3. Cuando se produzca la conmutación por error de la máquina, Azure Site Recovery le asignará la dirección IP proporcionada.
...
Vaya al [Paso 5: Preparación de Azure](physical-walkthrough-prepare-azure.md).
