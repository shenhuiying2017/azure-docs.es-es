<properties 
	...
	description="Cómo usar el ShardMapManager, la API de escala elástica para .NET" 
...
En un **mapa de particiones de intervalo**, el intervalo de claves se describe mediante un par **[valor bajo, valor alto)** donde *Low Value* es la clave mínima en el intervalo y *High Value* es el primer valor más alto que el intervalo. 
...
En las API de Escalado elástico, el Administrador de mapas de particiones es una colección de mapas de particiones. Los datos administrados por un objeto **ShardMapManager** .Net se mantienen en tres lugares: 
...
Los métodos usados para rellenar o cambiar datos de **ShardMapManager** no modifican los datos de usuario almacenados en las propias particiones. Por ejemplo, los métodos como **CreateShard**, **DeleteShard**, **UpdateMapping**, etc., afectan solo a los metadatos del mapa de particiones, no eliminan, agregan ni modifican los datos de usuario contenidos en las particiones. En su lugar, estos métodos están diseñados para usarse conjuntamente con operaciones independientes que se lleven a cabo para crear o quitar bases de datos reales, o para mover filas de una partición a otra con el fin de reequilibrar un entorno particionado.  (El servicio **División y combinación** incluido en la vista previa de Escalado elástico aprovecha estas API junto con la coordinación del movimiento de datos real entre particiones). 
...
* Para poner las asignaciones en línea y fuera de conexión, use **MarkMappingOffline** y **MarkMappingOnline** para controlar el estado en línea de una asignación. 
