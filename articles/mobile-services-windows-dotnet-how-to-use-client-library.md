...
- [Uso de Llamada a una API personalizada]
- [Uso de simultaneidad optimista]
- [Uso de datos a la interfaz de usuario en un servicio móvil]
...
Cuando está habilitado el esquema dinámico, Servicios móviles de Azure genera automáticamente columnas nuevas basadas en el objeto en las solicitudes de inserción o actualización. Para obtener más información, vea [Esquema dinámico](http://go.microsoft.com/fwlink/?LinkId=296271).
...
<h2><a name="querying"></a>Consulta de datos desde un servicio móvil</h2>
...
Puede especificar qué conjunto de propiedades se incluirá en los resultados agregando una cláusula  `Select` a su consulta. Por ejemplo, el siguiente código muestra cómo seleccionar solo un campo y también cómo seleccionar varios campos y darle formato:
...
<a name="inserting"></a>Consulta de datos en un servicio móvil
...
Para eliminar datos sin tipo, puede aprovechar Json.NET. Tenga en cuenta que cuando realice una solicitud de eliminación, debe especificarse un identificador, ya que de esa forma el servicio móvil identifica qué instancia eliminar. Una solicitud de eliminación precisa solo el identificador; las demás propiedades no se pasan al servicio, y si se pasa alguna, se ignoran en el servicio. El resultado de una llamada  `DeleteAsync` es normalmente también  `null` El identificador puede obtenerse a partir del resultado de la llamada  `InsertAsync`.
...
##<a name="#custom-api"></a>Búsqueda de  Llamada a una API personalizada
...
##<a name="optimisticconcurrency"></a>Búsqueda de  simultaneidad optimista
...
<h2><a name="binding"></a>Uso de de datos a la interfaz de usuario en un servicio móvil</h2>
...
Para finalizar, imagine que la tabla contiene muchos campos, pero solo desea que se muestren algunos en el control. Puede usar la guía de la sección <a href="#selecting">"Selección de columnas específicas anterior para seleccionar las columnas específicas"</a> que mostrar en la interfaz de usuario.

<h2><a name="authentication"></a>Consulta de usuarios</h2>
...
Para que los Servicios móviles administren el proceso de autenticación en la aplicación de la Tienda Windows o Windows Phone,
...
Para obtener un ejemplo de cómo utilizar una cuenta Microsoft para proporcionar una experiencia de inicio de sesión único, vea el tutorial "Autenticación de la aplicación con el inicio de sesión único" ([Tienda Windows ](/es-es/develop/mobile/tutorials/single-sign-on-windows-8-dotnet/)/[Windows Phone](/es-es/develop/mobile/tutorials/single-sign-on-wp8/)).
...
<h2><a name="untyped"></a>Búsqueda de  datos sin tipo</h2>
...
<h2><a name="unit-testing"></a>Diseño de pruebas unitarias</h2>
...
[Uso de datos a la interfaz de usuario en un servicio móvil]: #binding
...
[Uso de Llamada a una API personalizada]: #custom-api
