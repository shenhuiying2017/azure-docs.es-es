...
Consulte también [Pausar recursos de proceso][Pausar recursos de proceso], [Reanudar recursos de proceso][Reanudar recursos de proceso], [Escalar los recursos de proceso][Escalar los recursos de proceso]
...
Consulte también [Información general sobre transacciones][Información general sobre transacciones], [Optimización de transacciones][Optimización de transacciones]
...
Consulte también [Administración de estadísticas de tabla][Administración de estadísticas de tabla], [CREATE STATISTICS][CREATE STATISTICS], [UPDATE STATISTICS][UPDATE STATISTICS]
...
Consulte también [Carga de datos][Carga de datos], [Guía para el uso de PolyBase][Guía para el uso de PolyBase], [Patrones y estrategias de carga en Azure SQL Data Warehouse][Patrones y estrategias de carga en Azure SQL Data Warehouse], [Carga de datos con Azure Data Factory][Carga de datos con Azure Data Factory], [Traslado de datos con Azure Data Factory][Traslado de datos con Azure Data Factory], [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT], [Create table as select (CTAS)][Create table as select (CTAS)]
...
Consulte también [Guía para el uso de PolyBase][Guía para el uso de PolyBase]
...
Consulte también [Introducción a las tablas][Introducción a las tablas], [Distribución de tablas][Distribución de tablas], [Selección de la distribución de tablas][Selección de la distribución de tablas], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]

## <a name="do-not-overpartition"></a>Sin particiones excesivas
...
Consulte también [Partición de tabla][Partición de tabla]
...
Consulte también [Información general sobre transacciones][Información general sobre transacciones], [Optimización de transacciones][Optimización de transacciones], [Partición de tabla][Partición de tabla], [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE], [Create table as select (CTAS)][Create table as select (CTAS)]
...
Consulte también [Introducción a las tablas][Introducción a las tablas], [Tipos de datos de tabla][Tipos de datos de tabla], [CREATE TABLE][CREATE TABLE]
...
Consulte también [Índices de tablas][Índices de tablas], [Guía de índices de almacén de columnas][Guía de índices de almacén de columnas], [Regeneración de índices de almacén de columnas][Regeneración de índices de almacén de columnas]
...
Consulte también [Simultaneidad y administración de cargas de trabajo en Almacenamiento de datos SQL][Simultaneidad y administración de cargas de trabajo en Almacenamiento de datos SQL]
...
Consulte también [Simultaneidad y administración de cargas de trabajo en Almacenamiento de datos SQL][Simultaneidad y administración de cargas de trabajo en Almacenamiento de datos SQL], [sys.dm_pdw_waits][sys.dm_pdw_waits]
...
Consulte también [Supervisión de la carga de trabajo mediante DMV][Supervisión de la carga de trabajo mediante DMV], [LABEL][LABEL], [OPTION][OPTION], [sys.dm_exec_sessions][sys.dm_exec_sessions], [sys.dm_pdw_exec_requests][sys.dm_pdw_exec_requests], [sys.dm_pdw_request_steps][sys.dm_pdw_request_steps], [sys.dm_pdw_sql_requests][sys.dm_pdw_sql_requests], [sys.dm_pdw_dms_workers], [DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN], [sys.dm_pdw_waits][sys.dm_pdw_waits]

## <a name="other-resources"></a>Otros recursos:
Consulte también el artículo [Solución de problemas][Solución de problemas] para más información sobre problemas comunes y soluciones.

Si no encuentra lo que buscaba en este artículo, pruebe a usar la "búsqueda de documentos" en la parte izquierda de esta página para buscar todos los documentos de Almacenamiento de datos SQL de Azure.  El [foro de MSDN de Azure SQL Data Warehouse][foro de MSDN de Azure SQL Data Warehouse] se creó como un lugar para formular preguntas a otros usuarios y al grupo de productos de SQL Data Warehouse.  Supervisamos continuamente este foro para garantizar que sus preguntas las responde otro usuario o alguno de nosotros.  Si prefiere formular sus preguntas en Stack Overflow, también tenemos un [foro de Stack Overflow sobre Azure SQL Data Warehouse][foro de Stack Overflow sobre Azure SQL Data Warehouse].

Por último, use la página [de comentarios de Azure SQL Data Warehouse][de comentarios de Azure SQL Data Warehouse] para realizar solicitudes de características.  Añadir sus solicitudes o valoraciones positivas sobre otras solicitudes realmente nos ayuda a priorizar las características.
...
[Tipos de datos de tabla]: ./sql-data-warehouse-tables-data-types.md
...
[Administración de estadísticas de tabla]: ./sql-data-warehouse-tables-statistics.md
...
[Carga de datos]: ./sql-data-warehouse-overview-load.md
[Movimiento de datos con Azure Data Factory]: ../data-factory/data-factory-azure-sql-data-warehouse-connector.md
[Carga de datos con Azure Data Factory]: ./sql-data-warehouse-get-started-load-with-azure-data-factory.md
...
[Pausar recursos de proceso]: ./sql-data-warehouse-manage-compute-overview.md#pause-compute-bk
[Reanudar recursos de proceso]: ./sql-data-warehouse-manage-compute-overview.md#resume-compute-bk
...
[Azure SQL Data Warehouse loading patterns and strategies]: https://blogs.msdn.microsoft.com/sqlcat/2016/02/06/azure-sql-data-warehouse-loading-patterns-and-strategies (Patrones y estrategias de carga en Azure SQL Data Warehouse)
