...
Consulte también [Table overview][] \(Introducción a las tablas), [Table distribution][] \(Distribución de tablas), [Selecting table distribution][] \(Selección de la distribución de tablas), [CREATE TABLE][], [CREATE TABLE AS SELECT][]
...
Consulte también [Table partitioning][] \(Creación de particiones de tablas).
...
Consulte también [Transacciones en el Almacenamiento de datos SQL][], [Optimización de transacciones para Almacenamiento de datos SQL][], [Table partitioning][] \(Creación de particiones de tablas), [TRUNCATE TABLE][], [ALTER TABLE][], [Create Table As Select (CTAS) en Almacenamiento de datos SQL][]
...
Consulte también [Table overview][] \(Introducción a las tablas), [Table data types][] \(Tipos de datos para tablas), [CREATE TABLE][]
...
Consulte también [Simultaneidad y administración de cargas de trabajo en Almacenamiento de datos SQL][], [sys.dm\_pdw\_waits][]
...
## Otros recursos:
Consulte también el artículo [Solución de problemas de Almacenamiento de datos SQL de Azure][] para más información sobre problemas comunes y soluciones.

Si no encuentra lo que buscaba en este artículo, pruebe a usar la "búsqueda de documentos" en la parte izquierda de esta página para buscar todos los documentos de Almacenamiento de datos SQL de Azure. El [foro de MSDN de Almacenamiento de datos SQL de Azure][] se creó como un lugar para formular preguntas a otros usuarios y al grupo de productos de Almacenamiento de datos SQL. Supervisamos este foro para garantizar de que sus preguntas las responde otro usuario o alguno de nosotros. Si prefiere preguntar acerca de Stack Overflow, también tenemos un [foro de Stack Overflow para Almacenamiento de datos SQL de Azure][].

Por último, use la página [ de comentarios sobre Almacenamiento de datos SQL de Azure][] para realizar solicitudes de características. Añadir sus solicitudes o valoraciones positivas sobre otras solicitudes realmente nos ayuda a priorizar las características.
...
[Solución de problemas de Almacenamiento de datos SQL de Azure]: ./sql-data-warehouse-troubleshoot.md
...
[ de comentarios sobre Almacenamiento de datos SQL de Azure]: https://feedback.azure.com/forums/307516-sql-data-warehouse
