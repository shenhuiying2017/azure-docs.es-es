<properties
   pageTitle="Carga de Contoso Retail Data Warehouse en Almacenamiento de datos SQL | Microsoft Azure"
   description="Obtenga información acerca de cómo cargar Contoso Retail Data Warehouse en Almacenamiento de datos SQL mediante PolyBase para lograr el máximo rendimiento"
...
# Carga de Contoso Retail Data Warehouse

En este artículo aprenderá a cargar rápidamente datos en el esquema Contoso Retail Data Warehouse.

Este conjunto de datos proporciona un origen de datos alternativo que puede usar para aprender todo sobre Almacenamiento de datos SQL Azure.

El conjunto de datos de Contoso tiene un tamaño de GB. Los datos sin procesar se guardan en un almacenamiento de blobs de Azure, al que accederemos directamente para cargarlo en Almacenamiento de datos SQL.

En este tutorial, aprenderá lo siguiente:

1. Configurar PolyBase para cargar desde un contenedor de almacenamiento de blobs público
2. Importar datos mediante [CTAS][]
3. [RECOMPILAR][] las tablas para garantizar que los índices del almacén de columnas están optimizados
4. Crear estadísticas en columnas clave

## Configuración de PolyBase

El primer paso al cargar los datos es configurar los scripts de requisitos previos de PolyBase:

### Creación del origen de datos externo

Como los datos se almacenan en un contenedor público, la primera tarea es crear el origen de datos externo:
...
No es necesario crear una clave maestra o unas credenciales con ámbito de base de datos en este caso porque el contenedor de Almacenamiento de blobs está configurado como "público", lo que significa que cualquiera puede leer y enumerar este contenedor.

> [AZURE.IMPORTANT] Si decide que sus contenedores de Almacenamiento de blobs de Azure serán públicos, tenga en cuenta que se aplicarán cargos de salida de datos cuando los datos dejen el centro de datos.

### Creación de un objeto de formato de archivo externo

El formato de archivo proporciona la información necesaria para analizar los archivos de texto almacenados en el almacenamiento de blobs.
...
En nuestro caso, los datos no están comprimidos y están delimitados por canalización.

## Creación de la tablas externas

Ahora que la conectividad con Almacenamiento de blobs de Azure está configurado, podemos crear las tablas externas.

Para que todo esté bien organizado, vamos a crear un esquema:
...
Ahora, crearemos las tablas externas. Tenga en cuenta que lo que estamos haciendo aquí es proporcionar la asignación de los tipos de datos, enlazar esta definición a los metadatos para localizar los archivos.

Le advertimos que es un script largo.
...
## Carga de los datos con CTAS

Para cargar los datos es mejor aprovechar las tablas externas fuertemente tipadas que acabamos de crear. De este modo, serán suficientes unas simples instrucciones [CTAS][].

Sin embargo, en primer lugar crearemos un nuevo esquema para cargar las tablas.
...
Para cargar los datos,usamos la instrucción [CTAS][]. Observe que hay una [CTAS][] por tabla.
...
Puede seguir el progreso de la carga con la vista de administración dinámica (DMV) `[sys].[dm_pdw_exec_requests]`. Como las instrucciones CTAS anteriores tienen un valor de `label`, también puede realizar el seguimiento mediante `label`. Para más detalles, consulte el artículo [label][].

## Recompilación de los índices

Por último, para garantizar que las tablas tengan el formato comprimido de almacén de columnas, podemos ejecutar una operación de recompilación de la tabla. Esto convierte los grupos de filas abiertos o cerrados en un formato comprimido de almacén de columnas.
...
Para más información sobre cómo comprobar el estado del almacén de columnas agrupado, consulte el artículo [manage columnstore indexes][].

## Creación de algunos objetos de estadísticas

Para crear las estadísticas en todas las columnas, puede aprovechar el código del procedimiento almacenado `prc_sqldw_create_stats` que se describe en el artículo [statistics][].

El siguiente es un subconjunto de la salida que incluye las dimensiones en su totalidad, y todas las columnas de combinación de Contoso Retail Data Warehouse. Este es un buen punto de partida.

Más adelante podrá agregar más estadísticas a otras columnas de hechos.

Recuerde que este script es bastante largo.
...
Ahora tiene todos los datos cargados en Almacenamiento de datos SQL de Azure. Buen trabajo.

Ya puede empezar a consultar las tablas mediante consultas como la siguiente:
...
Disfrute explorando con Almacenamiento de datos SQL.

## Pasos siguientes
Para obtener información general sobre la carga, vea [Carga de datos en Almacenamiento de datos SQL][]. Para más información sobre cómo buscar índices de almacén de columnas, consulte [manage columnstore indexes][].
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/es-ES/library/mt203887.aspx
[RECOMPILAR]: https://msdn.microsoft.com/es-ES/library/ms188388.aspx
