...
Cuando se distribuyen los datos de la tabla mediante el método de distribución hash, es probable que la distribución de los datos esté "sesgada".

Si identifica que algunas distribuciones tienen muchos más datos que otras, se dice que la tabla está sesgada. En función de la importancia del sesgo, podrá solucionarlo. Un exceso en el sesgo de los datos tendrá un impacto en el resultado de la consulta, ya que los recursos de procesos distribuidos no se consumirán uniformemente.

Este artículo se ha creado para ayudarle a identificar los sesgos en los datos de las tablas hash distribuidas.

## Detección del sesgo de la distribución
...
## Resolución del sesgo de los datos
En algunas ocasiones, vale la pena conservar el sesgo. Normalmente, es útil cuando se está uniendo la tabla en una clave de distribución compartida.

Sin embargo, para resolver un sesgo de datos sería necesario elegir una columna diferente. También es posible utilizar ROUND\_ROBIN en lugar de HASH.

Consulte la sección de recomendaciones del artículo sobre la [distribución hash][] para obtener más información sobre cómo seleccionar una columna diferente.

## Pasos siguientes
Para obtener más detalles sobre la distribución de las tablas, consulte los siguientes artículos:
