<properties
   pageTitle="Carga de datos de Almacenamiento de blobs de Azure en Almacenamiento de datos SQL (PolyBase) | Microsoft Azure"
...
## Antes de empezar
Para ejecutar este tutorial, necesita una cuenta de Azure que cuente ya con una base de datos de Almacenamiento de datos SQL. Si todavía no la tiene, consulte [Creación de Almacenamiento de datos SQL][].
...
### 1\.1. Creación de una credencial
...
> [AZURE.IMPORTANT] Si decide que sus contenedores de Almacenamiento de blobs de Azure serán públicos, recuerde que, como propietario de los datos, se le aplicarán cargos de salida de datos cuando los datos dejen el centro de datos.
...
### 3\.2. Creación de la tablas externas 
...
El parámetro **LOCATION** es la carpeta situada bajo la carpeta raíz en el Blob de almacenamiento de Azure. Cada tabla está en una carpeta diferente.
...
### 4\.1. Creación de un nuevo esquema
...
### 4\.3. Seguimiento del progreso de carga

Puede seguir el progreso de la carga con la vista de administración dinámica (DMV) `[sys].[dm_pdw_exec_requests]`.
...
Si decide crear estadísticas de columna única en todas las columnas de cada una de las tablas, puede usar el ejemplo de código de procedimiento almacenado `prc_sqldw_create_stats` en el artículo sobre [estadísticas][].
...
## Pasos siguientes
Para cargar todos los datos de Contoso Retail Data Warehouse, use el script que aparece en. Para más sugerencias sobre desarrollo, consulte la [información general sobre desarrollo de Almacenamiento de datos SQL][].
