<properties
 pageTitle="Conceptos, terminología y jerarquía de entidades de Programador"
 description=""
 services="scheduler"
...
Hay dos tipos de trabajos: los trabajos HTTP (incluidas trabajos HTTPS que admiten SSL) y los trabajos de cola de almacenamiento. Los trabajos HTTP son perfectos si dispone de un extremo de una carga de trabajo o servicio existente. Los trabajos de la cola de almacenamiento le permiten enviar mensajes a colas de almacenamiento, por lo que dichos trabajos son ideales para cargas de trabajo que utilizan colas de almacenamiento.

## La entidad "Trabajo" en detalle

En un nivel básico, un trabajo programado cuenta con varias partes:

1.  La acción que se va a realizar cuando se activa el temporizador de trabajo  

2.  (Opcional) El tiempo para ejecutar el trabajo

3.  (Opcional) Cuándo y con qué frecuencia se debe repetir el trabajo

4.  (Opcional) Una acción que se desencadena si se produce un error en la acción principal
...
A continuación se muestra un trabajo de programador de ejemplo completo. En las secciones siguientes se proporcionan los detalles.
...
Como se muestra en el trabajo del Programador de ejemplo anterior, una definición de trabajo tiene varias partes:

1.  Hora de inicio ("startTime")  

2.  Acción ("action"), que incluye la acción del error ("errorAction")
...
"action" es la acción que se invoca en cada repetición y describe un tipo de invocación del servicio. La acción es lo que se ejecutará en la programación especificada. Actualmente Programador admite acciones HTTP y cola de almacenamiento.
...
"errorAction" es el controlador de errores, la acción que se invoca cuando se produce un error en la acción primaria. Puede utilizar esta variable para llamar a un extremo de control de errores o para enviar una notificación al usuario. Esto puede utilizarse para llegar a un extremo secundario en caso de que el principal no esté disponible (por ejemplo, en caso de un desastre en el sitio del extremo) o se puede usar para notificar a un extremo de control de errores. Igual que la acción primaria, la acción de error puede ser una lógica simple o compuesta basada en otras acciones. Para obtener información sobre cómo crear un token de SAS, consulte [Crear y usar una firma de acceso compartido](https://msdn.microsoft.com/library/azure/jj721951.aspx).

## recurrence

La periodicidad tiene varias partes:

1.  Frecuencia: un minuto, hora, día, semana, mes, año  

2.  Intervalo: el intervalo de frecuencia definido para la periodicidad

3.  Programación prescrita: especifique minutos, horas, días de la semana, meses y días de mes para de la periodicidad

4.  Recuento: número de repeticiones

5.  Hora de finalización: ningún trabajo se ejecutará después de la hora de finalización especificada
...
Para establecer una directiva de reintentos, se puede especificar una configuración adicional de dos valores: un intervalo de reintento (**retryInterval**) y el número de reintentos (**retryCount**).
