<properties 
	pageTitle="Uso del coordinador de Oozie de tiempo con Hadoop en HDInsight | Azure" 
	description="Uso del coordinador de Oozie de tiempo con Hadoop en HDInsight, una solución para grandes volúmenes de datos Aprenda a definir flujos de trabajo y coordinadores de Oozie, y envíe trabajos del coordinador." 
	services="hdinsight" 
	documentationCenter="" 
	authors="mumian" 
	manager="paulettm" 
...
> [AZURE.NOTE] Para ver las versiones de Oozie compatibles con los clústeres de HDInsight, consulte [Novedades en las versiones de clústeres proporcionadas por HDInsight][hdinsight-versions].
...
- **Un clúster de HDInsight**. Para obtener información sobre la creación de un clúster de HDInsight, consulte [Aprovisionamiento de clústeres de HDInsight][hdinsight-provision] o [Introducción a HDInsight][hdinsight-get-started]. Para completar el tutorial, necesitará los datos siguientes:
...
Las definiciones de los flujos de trabajo de Oozie se escriben en hPDL (un lenguaje de definición de proceso XML). El nombre de archivo del flujo de trabajo predeterminado es  *workflow.xml*.  Guardará el archivo de flujo de trabajo localmente y lo implementará en el clúster de HDInsight con Azure PowerShell posteriormente en este tutorial.
...
Hay un problema conocido de la ruta de acceso de Hive. Se producirá este problema cuando envíe un trabajo de Oozie. Las instrucciones para corregir el problema pueden encontrarse en [Wiki de TechNet][technetwiki-hive-error].
...
1. Cree un archivo de texto con el siguiente contenido:
	...
	<tr><td>${jobTracker}</td><td>Especifique la dirección URL del seguimiento de trabajo de Hadoop. Use <strong>jobtrackerhost:9010</strong> en las versiones 2.0 y de 3.0 de clúster de HDInsight.</td></tr>
...
1. Cree un archivo de texto con el siguiente contenido:
    ...
    | ${coordTimezone}  | Oozie procesa los trabajos del coordinador en una zona horaria sin horario de verano (normalmente UTC), esta zona horaria se denomina  'Oozie processing timezone'. |
...
HDInsight usa el almacenamiento de blobs de Azure para el almacenamiento de datos.  Se denomina *WASB* o *Windows Azure Storage - Blob*. WASB es la implementación del sistema de archivos distribuido de Hadoop (HDFS) de Microsoft en el almacenamiento de blobs de Azure. Para obtener más información, consulte [Uso del almacenamiento de blobs de Azure con HDInsight][hdinsight-storage]. 
...
- El comando CREATE TABLE mueve el archivo de datos a la carpeta /hive/warehouse/<NombreTabla> en el contenedor predeterminado.
...
el cmdlet de PowerShell Invoke-RestMethod para invocar los servicios web de Oozie. La API de servicios web de Oozie es una API HTTP REST JSON. Para obtener más información sobre la API de servicios Web de Oozie, consulte la [ocumentación de Apache Oozie 4.0][apache-oozie-400] (para la versión de clústeres de HDInsight 3.0) o la [documentación de Apache Oozie 3.3.2][apache-oozie-332] (para la versión de clústeres de HDInsight 2.1).

**Para enviar un trabajo de Oozie**

1. Abra Windows PowerShell ISE (en la pantalla Inicio de Windows 8, escriba **PowerShell_ISE** y, a continuación, haga clic en **Windows PowerShell ISE**. Consulte [Inicio de Windows PowerShell en Windows 8 y Windows][powershell-start]).

3. Copie el siguiente script en el panel de scripts y, a continuación, configure las catorce primeras variables (omita la sexta, $storageUri). 
...
Para solucionar los problemas de un flujo de trabajo, puede encontrar el archivo de registro de Oozie en C:\apps\dist\oozie-3.3.2.1.3.2.0-05\oozie-win-distro\logs\Oozie.log en el nodo principal del clúster. Para obtener información acerca de RDP, consulte [Administración de los clústeres de HDInsight mediante el Portal de administración][hdinsight-admin-portal].
