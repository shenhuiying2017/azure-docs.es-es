<properties 
   pageTitle="Preguntas más frecuentes sobre Base de datos SQL de Azure" 
...
# Preguntas más frecuentes sobre Base de datos SQL

## ¿Cómo aparece el uso de Base de datos SQL en la factura? 
Base de datos SQL se factura con tarifas por horas predecibles basadas en el nivel de servicio + nivel de rendimiento para bases de datos únicas o en eDTU por grupo de bases de datos elásticas. El uso real se procesa y se prorratea por horas por lo que la factura podría mostrar fracciones de una hora. Por ejemplo, si una base de datos existe durante 12 horas al mes, la factura mostrará ese uso de 0,5 días. Además, los niveles de servicio + nivel de rendimiento y las eDTU por grupo se desglosan en la factura para que pueda ver más fácilmente el número de días de base de datos que usó para cada uno en un mes.
...
- Por ejemplo, si actualiza una base de datos de Básica a Premium a las 22:00 h y la actualización termina a la 01:35 h del día siguiente, se le cobrará la tarifa Premium a partir de la 01:00 h.
...
- Si una base de datos única se beneficiaría de estar en un grupo, el motor inteligente de Azure recomendará un grupo de bases de datos elásticas si ve un patrón de uso histórico que lo garantiza. Para obtener más información, consulte [Grupos de bases de datos elásticas recomendados](sql-database-elastic-pool-portal.md#recommended-elastic-database-pools). Consulte [Consideraciones de precio y rendimiento para un grupo de bases de datos elásticas para realizar los cálculos matemáticos por su cuenta](sql-database-elastic-pool-guidance.md).
...
Puede cambiar el nivel de servicio (entre Básica, Estándar y Premium) o el nivel de rendimiento dentro de un nivel de servicio (por ejemplo, S1 a S2) un total de cuatro veces en un período de 24 horas.
...
Para cambiar el nivel de servicio de una base de datos e incorporarla y retirarla de un grupo es necesario copiar la base de datos en la plataforma como operación en segundo plano. Puede tardar desde solo unos minutos hasta varias horas, según el tamaño de las bases de datos. En ambos casos, las bases de datos permanecen en línea y disponibles durante el traslado. Para obtener información acerca de cómo cambiar las bases de datos únicas, consulte [Cambiar el nivel de servicio de una base de datos](sql-database-scale-up.md). Para las bases de datos elásticas, consulte [Referencia de grupos elásticos](sql-database-elastic-pool-reference.md#latency-of-elastic-pool-operations).

##¿Cuándo se deben usar bases de datos elásticas y cuándo una base de datos única? 
En general, los grupos de bases de datos elásticas están diseñados para un patrón de aplicación de software como servicio (SaaS) típico, donde hay una base de datos por cliente o inquilino. Adquirir bases de datos individuales y aprovisionarlas en exceso para cubrir todas las variaciones y la demanda máxima no resulta rentable. Con los grupos, el rendimiento colectivo del grupo y el escalado horizontal y vertical de las bases de datos se administra automáticamente.

El motor inteligente de Azure recomendará un grupo para las bases de datos si ve un patrón de uso que lo garantiza. Para obtener más información, consulte [Grupos de bases de datos elásticas recomendados](sql-database-elastic-pool-portal.md#recommended-elastic-database-pools). Para obtener instrucciones detalladas acerca de cómo elegir entre bases de datos únicas y elásticas, consulte [Consideraciones de precio y rendimiento para grupos de bases de datos elásticas](sql-database-elastic-pool-guidance.md).

## ¿Qué significa tener hasta un 200% del almacenamiento de base de datos aprovisionado máximo para almacenamiento de copias de seguridad? 
El almacenamiento de copia de seguridad es el almacenamiento asociado a las copias de seguridad de bases de datos automatizadas que se usan para la restauración a un momento dado y la restauración geográfica. Base de datos SQL de Microsoft Azure proporciona hasta un 200% de su almacenamiento de base de datos aprovisionado máximo para almacenamiento de copias de seguridad sin costo adicional. Por ejemplo, si tiene una instancia de base de datos Estándar con un tamaño de base de datos aprovisionado de 250 GB, se le proporcionarán 500 GB de almacenamiento de copias de seguridad sin costo adicional. Si su base de datos excede el almacenamiento de copias de seguridad suministrado, puede elegir reducir el período de retención poniéndose en contacto con el Servicio técnico de Azure o pagar almacenamiento de copias de seguridad extra, que se facturará a la tarifa de almacenamiento con redundancia geográfica de acceso de lectura (RA-GRS) estándar. Para obtener más información sobre la facturación RA-GRS, consulte Detalles de precios de almacenamiento.

## Me cambio de Web/Business a los nuevos niveles de servicio, ¿qué necesito saber?
Las bases de datos Web y Business de Azure se han retirado. Los niveles Básica, Estándar, Premium y Elástica son los que reemplazan las bases de datos Web y Business. Tenemos Preguntas más frecuentes adicionales que le ayudarán en este período de transición. [Preguntas frecuentes relacionadas con la retirada de las versiones Web y Business](sql-database-web-business-faq.md)
