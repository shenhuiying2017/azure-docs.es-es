...
La conexión por Internet es menos estable que a través de una red privada. Por lo tanto se recomienda que en la cadena de conexión: - establezca el parámetro **tiempo de espera de conexión** en **30** segundos (en lugar de 15 segundos).
...
Si el programa usa clases ADO.NET como **System.Data.SqlClient.SqlConnection** para conectarse a la Base de datos SQL de Azure, le recomendamos que use .NET Framework 4.5 o una versión posterior.
...
Si está usando ADO.NET 4.0 o versiones anteriores, se recomienda que lo actualice al ADO.NET más reciente. -Desde julio de 2015, puede [descargar ADO.NET 4.6](http://blogs.msdn.com/b/dotnet/archive/2015/07/20/announcing-net-framework-4-6.aspx).
...
En cualquier equipo con Windows, puede probar estas utilidades: - SQL Server Management Studio (ssms.exe), que se conecta mediante ADO.NET. - sqlcmd.exe, que se conecta mediante [ODBC](http://msdn.microsoft.com/library/jj730308.aspx).
...
En Linux, resultan útiles las utilidades siguientes:- `netstat -nap` - `nmap -sS -O 127.0.0.1` -(cambie el valor del ejemplo por su dirección IP).
...
A veces es mejor es diagnosticar un problema intermitente mediante la detección de un patrón general durante varios días o semanas.
...
La lógica de reintento para tratar errores transitorios es un área en la que puede ayudar EntLib60:- [4 - Perseverancia, el secreto de todos los triunfos: uso del bloque de aplicación de control de errores transitorios](http://msdn.microsoft.com/library/dn440719%28v=pandp.60%29.aspx)


Un breve ejemplo de código C# que usa EntLib60 en su lógica de reintento está disponible en: - [Ejemplo de código: lógica de reintento de Enterprise Library 6, en C# para conectarse a la Base de datos SQL](sql-database-develop-entlib-csharp-retry-windows.md)
...
- Procedimientos recomendados: [Orientación general sobre reintentos](best-practices-retry-general.md) tiene una excelente explicación detallada de lógica de reintento.
...
Se han quitado de esta copia numerosas líneas **//comment** para potenciar la legibilidad.
...
No es necesario descargar el código fuente para EntLib60. Pero si desea descargarlo, visite el siguiente tema y haga clic en el botón [Descargar código](http://go.microsoft.com/fwlink/p/?LinkID=290898):- [Enterprise Library 6: abril de 2013](http://msdn.microsoft.com/library/dn169621%28v=pandp.60%29.aspx)
