<properties 
   pageTitle="Migración mediante el Asistente para migración a SQL" 
...
# Migración mediante el Asistente para migración a SQL de Azure
...
![texto alternativo](./media/sql-database-migration-wizard/06MigrationWizard.png) -Dependiendo de la evaluación de los errores, puede volver atrás en el asistente y excluir los objetos que están causando problemas y volver a generar el script. Cuando piense en cómo solucionar los errores, considere el impacto en otros objetos de la base de datos, así como de las aplicaciones que usan la base de datos. - Si el script tiene errores que deban corregirse, puede guardar un archivo de script solo de esquema desde la pestaña Script SQL y abrir y editar el script en su editor favorito para corregir los errores antes de ejecutarlo fuera de SAMW en la nueva base de datos que creó en el paso 1. Si el script es grande o hay un gran número de errores, debe utilizar la opción 3. Tenga en cuenta que aunque puede importar el archivo de scripts generado en Visual Studio, la importación desde un archivo SQL puede tardar mucho tiempo ya que la importación de una base de datos, tal como se describe en la opción 3.

7.	Si no hay ningún error o se ha eliminado del origen de los errores resultantes de la generación, puede hacer clic en **Siguiente** y en la página siguiente haga clic en **Conectar con el servidor** para conectarse al servidor de Base de datos SQL de Azure que creó en el paso 1.

![texto alternativo](./media/sql-database-migration-wizard/07MigrationWizard.png)

8.	El siguiente paso consiste en seleccionar la base de datos en la que se ejecutará el script. Se enumerarán todas las bases de datos existentes en el servidor. Seleccione la base de datos que creó en el paso 1. La base de datos debe estar vacía y tener nivel de precios adecuado para esta operación. 
- Si lo prefiere, puede crear una base de datos en este momento mediante el asistente. Para ello, haga clic en Crear base de datos para configurarla y crearla. Consulte la sección de introducción para obtener instrucciones acerca de cómo elegir el nivel de rendimiento que se usará durante el proceso de migración.
- Una vez creada la base de datos, selecciónela y continúe. 

9.	Con una base de datos vacía seleccionada, haga clic en **Siguiente** y confirme que desea ejecutar el script para completar la migración.
