<properties
	pageTitle="Consulta de la base de datos SQL con C# | Microsoft Azure"
	description="Detalles acerca de las direcciones IP, las cadenas de conexión, el archivo de configuración para asegurar un inicio de sesión seguro y Visual Studio gratuito para permitir al programa de C# conectarse a la base de datos de Base de datos SQL de Azure en la nube, mediante ADO.NET."
...
Este tema describe cada paso para las personas que son nuevas en la Base de datos SQL de Azure y C#. Otras personas que tienen experiencia con C# y Microsoft SQL Server pueden omitir algunos pasos y centrarse en los que son específicos de Base de datos SQL.
...
- Una base de datos de demostración **AdventureWorksLT** en el servicio Base de datos SQL de Azure.
 - [Cree la base de datos de ejemplo](sql-database-get-started.md) en minutos.
...
- Visual Studio 2013, actualización 4 o posterior. Microsoft ahora ofrece Visual Studio Community *gratis*.
 ...
 - O bien, use un [paso](#InstallVSForFree) más adelante en este tema en el que se describe cómo el [Portal de vista previa de Azure](http://portal.azure.com/) le guía para instalar Visual Studio.
...
3. En el cuadro de texto de filtro, cerca de la parte superior, empiece a escribir el nombre de su base de datos **AdventureWorksLT**.
...
En Visual Studio, cree un nuevo proyecto basado en la plantilla de inicio para C# > Windows > **Aplicación de consola**.
...
3. Desplácese y haga clic para resaltar **System.Configuration**. Asegúrese de que la casilla está seleccionada.
...
Use el [Portal de vista previa de Azure](http://portal.azure.com/) para copiar la cadena de conexión para la base de datos.
...
2. Agregue un elemento **&#x3c;configuration&#x3e; &#x3c;/configuration&#x3e;**, como se muestra en el siguiente ejemplo de código de ejemplo App.config.
...
3. Usa la cadena de conexión y las clases de ADO.NET para conectarse a la base de datos de demostración **AdventureWorksLT** en Base de datos SQL de Azure.

4. Emite una instrucción **SELECT** de SQL para leer desde la tabla **SalesLT**.
...
Su programa cliente de C# no se puede conectar a Base de datos SQL de Azure hasta que se ha agregado la dirección IP del equipo cliente en el firewall de la base de datos SQL. El programa fallará con un mensaje de error útil que indica la dirección IP necesaria.


Puede usar el [Portal de vista previa de Azure](http://portal.azure.com/) para agregar la dirección IP.
...
1. En Visual Studio, ejecute el programa mediante el menú **DEPURAR** > **Iniciar depuración**. Se muestra una ventana de consola.
