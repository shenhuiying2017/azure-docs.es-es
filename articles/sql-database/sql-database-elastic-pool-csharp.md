<properties 
    pageTitle="Creación de un grupo de bases de datos elásticas de Base de datos SQL con C# | Microsoft Azure" 
    description="En este artículo se muestra cómo crear un grupo de bases de datos elásticas de Base de datos SQL con C# (mediante la Biblioteca de Base de datos SQL de Azure para .NET)." 
...
En este artículo se muestra cómo crear un [grupo de bases de datos elásticas](sql-database-elastic-pool.md) desde una aplicación mediante C#.
...
2. Mantenga el puntero sobre su nombre en la esquina superior derecha y anote el dominio que aparece en la ventana emergente. Reemplace **domain.onmicrosoft.com** en el fragmento de código siguiente por el valor de su cuenta.
...
De forma predeterminada, un servidor no tiene ninguna regla de firewall, por lo que no se puede conectar con él desde cualquier ubicación. Para conectarse a un servidor o a cualquier base de datos del servidor, se debe definir una [regla de firewall](sql-database-firewall-configure.md) que permita el acceso desde la dirección IP del cliente.

En el ejemplo siguiente se crea una regla de firewall de servidor que abre el acceso al servidor desde cualquier dirección IP. Se recomienda crear inicios de sesión y contraseñas de SQL adecuados para proteger la base de datos y no depender de reglas de firewall como defensa principal frente a las intrusiones. Para obtener más información, consulte [Administrar bases de datos e inicios de sesión en Base de datos SQL de Azure](sql-database-manage-logins.md).
...
*Después de crear un grupo también puede usar Transact-SQL para mover bases de datos existentes dentro y fuera de un grupo. Para obtener información detallada, consulte [Material de referencia del grupo de bases de datos elásticas: Transact-SQL](sql-database-elastic-pool-reference.md#Transact-SQL).*
