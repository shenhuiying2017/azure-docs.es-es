...
La base de datos se implementa en la base de datos SQL de Azure mediante SSMS. La base de datos se puede implementar directamente o mediante la exportación a un BACPAC que, a continuación, se importa para crear una nueva base de datos SQL de Azure. Use este método cuando la base de datos de origen sea totalmente compatible con la Base de datos SQL de Azure.

### Opción n.º 2:
***Migración de una base de datos casi compatible mediante el Asistente para migración a SQL de Azure (SAMW)***

La base de datos se procesa con el SAMW para generar un script de migración que contiene el esquema o el esquema con los datos en formato BCP. Use este método cuando el esquema de la base de datos requiera actualización y cuando se puedan controlar los cambios mediante el asistente.

### Opción n.º 3:
***Actualización del esquema de la base de datos sin conexión con Visual Studio (VS) y SAMW e implementación con SSMS***

La base de datos de origen se importa en un proyecto de base de datos de Visual Studio para el procesamiento sin conexión. A continuación, SAMW se ejecuta en todos los scripts del proyecto para aplicar una serie de transformaciones y correcciones. El proyecto se adapta y se crea para la Base de datos SQL V12 y se notifican los errores restantes. Dichos errores, a continuación, se resuelven manualmente mediante SQL Server Data Tools (SSDT) en Visual Studio. Una vez compilado correctamente el proyecto, este se publica en una copia de la base de datos de origen. Dicha base de datos actualizada se implementa en la base de datos SQL de Azure mediante la opción n.º 1. Si solo es necesaria la migración del esquema, es posible publicar el esquema directamente desde Visual Studio a la base de datos SQL de Azure. Use este método cuando el esquema de la base de datos requiera más cambios de los que se pueden controlar mediante el asistente para la migración por sí solo.
...
- Si se producen errores durante la exportación de la opción n.º 1, use el Asistente para la migración a SQL Azure para procesar la base de datos en modo de solo esquema, tal como se describe en la opción n.º 2. Si el asistente para la migración no notifica errores, use la opción nº2. 
- Si el asistente SAMW informa de que el esquema requiere trabajo adicional; a no ser que solo necesite pequeñas correcciones, se recomienda usar la opción n.º 3 y corregir el esquema de la base de datos sin conexión en Visual Studio mediante la combinación del asistente para la migración y los cambios de esquema de aplicación manual. Con este proceso, se actualizará in situ una copia de la base de datos de origen que, a continuación, se migrará a Azure mediante la opción n.º 1.
...
| No permite cambiar el resultado si hay errores, por lo que el esquema de origen debe ser compatible. | La edición de la secuencia de comandos monolítica generada puede resultar compleja en caso de ser necesaria. El script se puede abrir y editar en SSMS o Visual Studio mediante SSDT. Todos los errores deben corregirse para poder implementar la secuencia de comandos en la base de datos SQL de Azure.| Estarán disponibles todas las características de SSDT en Visual Studio. El esquema se cambia sin conexión. |
