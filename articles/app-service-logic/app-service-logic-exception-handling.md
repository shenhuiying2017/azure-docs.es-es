<properties
   pageTitle="Control de excepciones de Logic Apps | Microsoft Azure"
...
# Control de errores y excepciones de Logic Apps
...
Para más información sobre la sintaxis admitida, consulte la [sección sobre las directivas de reintentos en MSDN][retryPolicyMSDN].

## Propiedad RunAfter para detectar errores

Cada acción de una aplicación lógica declara las acciones que se deben realizar antes de que se inicie la acción. Se puede considerar esto como el orden de los pasos del flujo de trabajo. Esta ordenación se conoce como la propiedad `runAfter` en la definición de acción. Es un objeto que describe qué acciones y estados de acciones ejecutarían la acción. De forma predeterminada, todas las acciones que se agregan mediante el diseñador se establecen en `runAfter` el paso anterior, si el paso anterior era `Succeeded`. Sin embargo, puede personalizar este valor para desencadenar acciones cuando las acciones anteriores son `Failed`, `Skipped`, o un posible conjunto de estos valores. Si quisiera agregar un elemento a un tema de Bus de servicio designado después de que una acción específica `Insert_Row` produce error, usaría la siguiente configuración `runAfter`:
...
>[AZURE.TIP] Las acciones que se ejecutan después de que una acción anterior ha producido error, y que finalizan correctamente, se marcarán como `Succeeded`. Este comportamiento significa que si detectar correctamente todos los errores de un flujo de trabajo, la propia ejecución se marca como `Succeeded`.

## Ámbitos y resultados para evaluar acciones
...
### Obtención del contexto de errores con resultados
...
1. Acción **Filter Array** para filtrar el valor de `@result('My_Scope')` y obtener el resultado de todas las acciones dentro de `My_Scope`
1. Condición de **Filter Array** si algún elemento `@result()` con el estado es igual a `Failed`. Esto filtra la matriz de todos los resultados de acción de `My_Scope` por solo una matriz de resultados de acción con error.
1. Realización de una acción **For Each** en las salidas **Filtered Array**. Se lleva a cabo una acción *for each* para cada resultado de acción con error que hemos filtrado anteriormente.
...
1. Envío de una solicitud HTTP POST en el cuero de respuesta del elemento `foreach`, o `@item()['outputs']['body']`. La forma del elemento `@result()` es la misma que la forma de `@actions()`, y se puede analizar del mismo modo.
1. También se incluyen dos encabezados personalizados con el nombre de la acción con errores `@item()['name']` y el id. de seguimiento de cliente de la ejecución con errores `@item()['clientTrackingId']`.
...
## Diagnósticos de Azure y datos de telemetría
