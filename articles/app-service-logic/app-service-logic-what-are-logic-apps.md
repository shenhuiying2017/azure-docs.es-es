...
Y lo mejor de todo: Aplicaciones lógicas se puede combinar con las [API administradas][managedapis] para ayudar a resolver incluso escenarios de integración complicados de forma fácil:
...
Además, con nuestras [API administradas de BizTalk][biztalk], puede realizar el escalado a escenarios de integración maduros con la eficacia de un [motor de reglas][rules], la [administración de socios comerciales][tpm] y mucho más.
...
- **Desencadenadores**: algunas API administradas también pueden actuar como desencadenador. Un desencadenador inicia una nueva instancia de un flujo de trabajo de acuerdo con un evento específico, como la llegada de un correo electrónico o un cambio en la cuenta de almacenamiento de Azure.
