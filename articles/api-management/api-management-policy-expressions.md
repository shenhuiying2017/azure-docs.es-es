...
|IMessageBody|As<T\>(preserveContent: bool = false): donde T: cadena, JObject, JToken, JArray, XNode, XElement, XDocument<br /><br /> Los métodos `context.Request.Body.As<T>` y `context.Request.Body.As<T>` se usan para leer un cuerpo de mensaje de solicitud o respuesta en uno de los formatos especificados. Tenga en cuenta que, cuando accede al cuerpo del mensaje, de forma predeterminada se pierde el cuerpo del mensaje original, y se debe establecer devolviendo el cuerpo de la expresión que lo lee. Para conservar el contenido del cuerpo, establezca el parámetro `preserveContent` en `true` al acceder al mensaje. El valor predeterminado de `preserveContent` es `false`. Si `preserveContent` está establecido en `true` y la expresión devuelve un cuerpo, se utiliza este último. Para obtener más información, consulte la directiva [Establecer cuerpo](api-management-transformation-policies.md#SetBody).|  
