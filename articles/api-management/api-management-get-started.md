<properties
	pageTitle="Administración de su primera API en Administración de API de Azure"
...
* **Habilitación de los ecosistemas de asociados de fabricantes de software independiente** mediante la oferta de una incorporación rápida de asociados a través del portal para desarrolladores y la creación de una fachada de API para desacoplar desde una implementación interna que no es adecuada para el consumo de los asociados.
...
* La **puerta de enlace de la API** es el extremo que:
  ...
  * almacena en caché las respuestas de back-end donde la instalación
  * registra los metadatos de llamada para fines de análisis

* El **portal para editores** es la interfaz administrativa donde se configura el programa de API, con acciones como las siguientes:
...
Escriba **Contoso Ltd.** en **Nombre de la organización**, y especifique su dirección de correo electrónico en el campo de correo electrónico del administrador.
...
>Microsoft Azure ofrece tres niveles en los que puede ejecutar el servicio Administración de API: Desarrollador, Estándar y Premium. El nivel de desarrollador está orientado al desarrollo, las pruebas y los programas de API piloto para los que la disponibilidad no es un problema. En el nivel Estándar y Premium, puede escalar el número de unidades reservadas para administrar más tráfico. Los niveles Estándar y Premium proporcionan al servicio Administración de API la mayor potencia de procesamiento y rendimiento. Este tutorial se puede completar con cualquier nivel. Para obtener más información acerca de los niveles de Administración de API, consulte [Precios de Administración de API][].
...
Es posible crear las API y luego agregar las operaciones manualmente, o bien importarlas. En este tutorial se importará una API para un servicio web de calculadora de ejemplo proporcionado por Microsoft y hospedado en Azure.
...
Las API se configuran desde el portal para editores de la API, al que se obtiene acceso a través del Portal de administración de Azure. Para obtener acceso al portal para editores, haga clic en **Administrar** en el Portal de Azure para el servicio Administración de API.
...
1. Haga clic en **Desde dirección URL**, escriba ****http://calcapi.cloudapp.net/calcapi.json** en el cuadro de texto the **Dirección URL del documento de especificación** y haga clic en el botón de radio **Swagger**.
...
La sección de API tiene varias pestañas. La pestaña **Resumen** muestra información y métricas básicas acerca de la API. La pestaña [Configuración](api-management-howto-create-apis.md#configure-api-settings) se utiliza para ver y editar la configuración de una API. La pestaña [Operaciones](api-management-howto-add-operations.md) se utiliza para administrar las operaciones de la API. La pestaña **Seguridad** puede utilizarse para configurar la autenticación de proxy para el servidor back-end mediante la autenticación básica o la [autenticación mutua de certificados](api-management-howto-mutual-certificates.md) y para configurar la [autorización de usuario mediante OAuth 2.0](api-management-howto-oauth2.md). La pestaña **Problemas** se utiliza para ver los problemas notificados por los desarrolladores que usan las API, y la pestaña **Productos** se utiliza para configurar los productos que contienen esta API.
...
Haga clic en **API** en el menú superior y, a continuación, haga clic en **Basic Calculator** para ver las operaciones disponibles.
...
Puede escribir algunos valores para los parámetros o conservar los valores predeterminados; a continuación, haga clic en **Enviar**.
...
[Configuración de notificaciones]: api-management-howto-configure-notifications.md
