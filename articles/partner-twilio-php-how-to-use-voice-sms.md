<properties 
	pageTitle="Cómo usar Twilio para voz y SMS (PHP) - Azure" 
...
* [Cómo proporcionar respuestas TwiML desde su propio sitio web](#howto_provide_twiml_responses)
...
Los clientes de Azure conseguirán una [oferta especial](http://www.twilio.com/azure): 10 $ de regalo en crédito Twilio al actualizar su cuenta de Twilio. Este crédito Twilio se puede aplicar a cualquier uso de Twilio (10 $ de crédito equivalen al envío de aproximadamente 1.000 mensajes SMS o recibir hasta 1.000 minutos de voz entrante, según la ubicación del número de teléfono y el destino del mensaje o de la llamada). Canjee este crédito Twilio y comience en: [http://ahoy.twilio.com/azure](http://ahoy.twilio.com/azure).

Twilio es un servicio de pago por uso. No hay comisiones establecidas y puede cerrar su cuenta en cualquier momento. Puede conseguir más información acerca de los precios de Twilio en [Precios de Twilio ][twilio_pricing].
...
A continuación se presenta una lista de verbos de Twilio. Obtenga información sobre otros verbos y capacidades en [Documentación de lenguaje de marcado de Twilio](http://www.twilio.com/docs/api/twiml).
...
Para obtener más información acerca de los verbos de Twilio, sus atributos y TwiML, consulte [TwiML][twiml]. Para obtener información adicional acerca de la API de Twilio, consulte la [API de Twilio][twilio_api] (en inglés).
...
Una aplicación PHP que usa el servicio Twilio y que se ejecuta en Azure no es distinta a otra aplicación PHP que use el servicio Twilio. Mientras que los servicios de Twilio se basan en la API REST y pueden llamarse desde una aplicación PHP de varias formas, este artículo se centrará en cómo usar los servicios de Twilio con la [biblioteca de Twilio para PHP desde Github][twilio_php] (en inglés). Para obtener más información acerca del uso de la biblioteca de Twilio para PHP, consulte [http://readthedocs.org/docs/twilio-php/en/latest/index.html][twilio_lib_docs].
...
Como se mencionó, este código usa el sitio proporcionado por Twilio para devolver la respuesta de TwiML. A parte, también puede usar su propio sitio web para proporcionar la respuesta de TwiML; si desea obtener más información, consulte [Cómo proporcionar respuestas de TwiML desde su propio sitio web](#howto_provide_twiml_responses).
...
<h2><a id="howto_send_sms"></a>Cómo enviar un mensaje SMS</h2>
...
Para obtener más información acerca de TwiML, consulte [https://www.twilio.com/docs/api/twiml][twiml_reference]. 

Una vez tenga configurada su página de PHP para proporcionar respuestas de TwiML, use la URL de la página PHP como la URL que se transmitió al método `Services_Twilio->account->calls->create`. Por ejemplo, si tiene una aplicación web llamada **MyTwiML** implementada en un servicio hospedado en Azure y el nombre de la página PHP es **mytwiml.php**, la URL se puede transmitir a  **Services_Twilio->cuenta->llamadas->crear** tal y como se muestra en el siguiente ejemplo:
...
Si desea obtener más información acerca de cómo usar Twilio en Azure con PHP, consulte [Cómo realizar una llamada de teléfono con Twilio en una aplicación PHP en Azure][howto_phonecall_php].
