---
title: Entrega continua para Servicios en la nube con TFS en Azure | Microsoft Docs
...
Para hacerlo de manera más fácil, puede también usar Visual Studio Team Services, una versión de TFS que se hospeda en Azure. Para obtener más información, consulte [Entrega continua a Azure con Visual Studio Team Services][Entrega continua a Azure con Visual Studio Team Services].
...
## 1: Configuración del servidor de compilación
...
No es necesario instalar Visual Studio en el servidor de compilación. Si desea usar el servicio de Team Foundation Build para administrar el servidor de compilación, siga las instrucciones que aparecen en el documento [Configurar el Servicio de Team Foundation Build.][Configurar el Servicio de Team Foundation Build.]

1. En el servidor de compilación, instale [.NET Framework 4.5.2][.NET Framework 4.5.2], que incluye MSBuild.
...
## 2 Compilación de un paquete con comandos de MSBuild
...
3. Ejecute MSBuild con la opción /target:Publish como se muestra en el ejemplo siguiente:
   ...
   De manera opcional, puede especificar el nombre del proyecto como un parámetro de MSBuild. Si no se especifica, se utiliza el directorio actual. Para obtener más información acerca de las opciones de línea de comandos de MSBuild, consulte [Referencia de la línea de comandos de MSBuild1](1.md).
...
5. Guarde la plantilla de script que aparece al final de este artículo en la carpeta de scripts como c:\\scripts\\WindowsAzure\\**PublishCloudService.ps1**.
...
8. Llame al script directamente desde Azure PowerShell, o conecte este script a su automatización de compilación de host que se produce después de la compilación del paquete.
   ...
   > [!IMPORTANT]
   > El script siempre eliminará o reemplazará sus implementaciones actuales de manera predeterminada si se detectan. Esto es necesario para habilitar la entrega continua desde la automatización donde no es posible una solicitud del usuario.
   > 
   > 
   ...
   **Escritorio remoto:**
   ...
   Si lo prefiere, puede exportar el archivo de certificado PFX con una clave privada y cargar los certificados en cada servicio en la nube de destino mediante el [Portal de Azure clásico](http://go.microsoft.com/fwlink/?LinkID=213885). Lea el siguiente artículo para obtener más información: [http://msdn.microsoft.com/es-es/library/windowsazure/gg443832.aspx][].
   ...
   **Actualizar implementación frente a Eliminar implementación -> Nueva implementación**
   ...
   > [!IMPORTANT]
   > El script siempre eliminará o reemplazará sus implementaciones actuales de manera predeterminada si se detectan. Esto es necesario para habilitar la entrega continua desde la automatización donde no es posible una solicitud del usuario/operador.
   > 
   > 

## 5: Publicación de un paquete con TFS Team Build
...
4. Vuelva a la pestaña **Proceso** y use **Mostrar detalles** para mostrar una lista de las plantillas de proceso de compilación disponibles. Elija el botón **Nuevo...**, vaya al proyecto que acaba de agregar y protéjalo. Localice la plantilla que acaba de crear y haga clic en **Aceptar**.
5. Abra la Plantilla de proceso seleccionada para editar. Puede abrirla directamente en el diseñador de flujo de trabajo o en el editor XML para trabajar con el archivo XAML.
...
7. Agregue una secuencia nueva al final de Ejecutar en el agente:
...
8. Guarde el flujo de trabajo de la plantilla del proceso de compilación y proteja este archivo.
9. Edite la definición de compilación (ciérrela si estuviera abierta) y seleccione el botón **Nuevo** si todavía no ve la nueva plantilla en la lista de plantillas del proceso.
10. Establezca los valores de propiedad del parámetro en la sección Misc como a continuación:
...
11. Guarde los cambios en la Definición de compilación.
12. Ponga una compilación en cola para ejecutar la compilación del paquete y la publicación. Si tiene un desencadenador establecido en Integración continua, ejecutará este comportamiento en cada protección.
