...
Tenga en cuenta que si solo desea que Engagement informe de las sesiones, las actividades, bloqueos e información técnica de la aplicación, a continuación, la manera más sencilla es hacer que todos los objetos personalizados `UIViewController` hereden de la clase `EngagementViewController` correspondiente.
...
La API de Engagement la proporciona la clase `EngagementAgent`. Una instancia de esta clase se puede recuperar mediante una llamada al `[EngagementAgent shared]` método estático (tenga en cuenta que el `EngagementAgent` objeto devuelto es un singleton).

Antes de las llamadas a una API, el `EngagementAgent` objeto debe inicializarse llamando al método `[EngagementAgent init:@"Endpoint={YOUR_APP_COLLECTION.DOMAIN};SdkKey={YOUR_SDK_KEY};AppId={YOUR_APPID}"];`

## Conceptos de Engagement

Las siguientes partes refinan los [conceptos de Mobile Engagement comunes](mobile-engagement-concepts.md) para la plataforma de iOS.
