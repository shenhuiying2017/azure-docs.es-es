<properties pageTitle="Uso del backend .NET de Migraciones de Code First (Servicios móviles)" metaKeywords="" description="" metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="How to make data model changes to a .NET backend mobile service" authors="glenga" solutions="mobile" writer="glenga" manager="dwrede" editor="" />
...
Servicios móviles admite dos clases base de inicializador de modelo de datos en un proyecto de servicio móvil de backend .NET. Estos inicializadores eliminan y vuelven a crear tablas en la base de datos cuando Entity Framework detecta un cambio de modelo de datos en el [DbContext]. Estos inicializadores están diseñados para funcionar tanto si el servicio móvil se ejecuta en un equipo local y como si se hospeda en Azure. Ambas clases base de inicializador eliminan de la base de datos todas las tablas, vistas, funciones y procedimientos del esquema que usa el servicio móvil. 

Los objetos de esquema + **ClearDatabaseSchemaIfModelChanges** <br/> solo se eliminan cuando Code First detecta un cambio en un modelo de datos. El inicializador predeterminado de un proyecto de backend .NET descargado del [Portal de administración de Azure ] hereda elementos de esta clase base.
...
1. En el Explorador de soluciones de Visual Studio, haga clic con el botón secundario en el proyecto de servicio móvil y haga clic en **Establecer como proyecto de inicio**.
 
2. En el menú **Herramientas**, expanda **Administrador de paquetes NuGet** y haga clic en **Consola del Administrador de paquetes**.
...
13. (Opcional) En el [Portal de administración de Azure], seleccione el servicio móvil, haga clic en la pestaña **Configurar** y, por último, haga clic en el vínculo **Base de datos SQL**. 

	![][0]

	De este modo, se abre la página de la base de datos SQL del servicio móvil.

14. (Opcional) Haga clic en **Manage**, inicie sesión en el servidor de la base de datos SQL, haga clic en **Design** y compruebe que los cambios en el esquema se hayan efectuado en Azure. 
...
<!-- URLs -->
[DropCreateDatabaseIfModelChanges]: http://msdn.microsoft.com/es-es/library/gg679604(v=vs.113).aspx
