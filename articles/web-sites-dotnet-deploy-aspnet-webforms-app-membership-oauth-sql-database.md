<properties pageTitle="Implementación de una aplicación de ASP.NET Web Forms segura con suscripción, OAuth y Base de datos SQL en un sitio web de Azure" metaKeywords="Azure WebForms Hello World Tutorial, Azure Getting Started with WebForms Tutorial, Azure C# WebForms Hello World Tutorial, SQL Azure C# WebForms Tutorial" description="Este tutorial muestra cómo compilar una aplicación de ASP.NET Web Forms 4.5 segura que incorpora una base de datos SQL e implementar la aplicación en Azure." metaCanonical="" services="web-sites" documentationCenter=".NET" title="Deploy a secure ASP.NET 4.5 Web Forms app to an Azure Website" authors="erikre"  solutions="" manager="wpickett" editor="mollybos"  />
...
>[WACOM.NOTE] 
Para completar este tutorial, deberá tener una cuenta de Azure. Si aún no la tiene, puede <a href="/es-es/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F" target="_blank">activar los beneficios de suscripción a MSDN</a>, o bien <a href="/es-es/pricing/free-trial/?WT.mc_id=A261C142F" target="_blank">regístrese para obtener una evaluación gratuita</a>.
...
4. En la ventana del **instalador de la plataforma web**, haga clic en **Install** (Instalar) y continúe con la instalación.  
...
5. Si aún no tiene **Visual Studio Update 2**, descargue e instale **[Visual Studio 2013 Update 2](http://www.microsoft.com/download/details.aspx?id=42666)** o una versión superior.  
...
1. En el [Portal de administración de Azure](https://manage.windowsazure.com/), haga clic en **Sitios web** en la pestaña izquierda y luego haga clic en **Nuevo**.  
	![Web Platform Installer](./media/web-sites-dotnet-web-forms-secure/Intro-SecureWebForms-02.png)
2. Haga clic en **Sitio web** y luego en **Creación personalizada**.  
...
3. En el paso **Crear sitio web** del asistente, escriba una cadena en el cuadro **URL** para usarla como dirección URL única de la aplicación. La URL completa consistirá en el valor que escriba más el sufijo que aparece junto al cuadro de texto. La ilustración muestra la dirección URL que se toma con toda probabilidad, así que **usted debe elegir una diferente**.  
...
6. En el cuadro **Nombre de la cadena de conexión de BD**, deje el valor predeterminado de *DefaultConnection*.
7. Haga clic en la flecha que aparece en la parte inferior del cuadro de diálogo. 
El asistente avanza hasta el paso para **especificar la configuración de la base de datos**.
...
11. Complete los campos de **nombre de usuario** y **contraseña** con los datos de un administrador. 
Si ha seleccionado **Nuevo servidor de bases de datos SQL**, no debe escribir un nombre y una contraseña existentes en estos campos, sino definir unos nuevos que volverá a utilizar más adelante para obtener acceso a la base de datos. Si seleccionó un SQL Server creado anteriormente, se le pedirá la contraseña de la cuenta de SQL Server que abrió para crearlo. Para este tutorial, no active el cuadro **Avanzadas**.
...
1. Seleccione **Nuevo proyecto** en el menú **Archivo** de Visual Studio.  
...
4. Nombre al proyecto *ContactManager* y haga clic en **Aceptar**.  
...
3. Si aún no ha iniciado sesión, haga clic en el botón **Iniciar sesión** del cuadro de diálogo **Seleccionar sitio web existente**. Cuando haya iniciado sesión, seleccione el sitio web que creó en la primera parte de este tutorial. Haga clic en **Aceptar** para continuar.  
...
Verá el estado general de publicación en la ventana de **actividad de publicación web** en Visual Studio:  
...
>[WACOM.NOTE] 
Si se encuentra con un error al publicar en un sitio web ya se ha establecido, puede borrar la ubicación antes de agregar los nuevos archivos.  
Publique de nuevo su aplicación, pero esta vez en el cuadro de diálogo **Publicación web**, seleccione la pestaña **Configuración**. A continuación, establezca la configuración en **Depurar** y seleccione la opción **Quitar archivos adicionales en destino**. Seleccione **Publicar** para implementar de nuevo la aplicación.  
...
Comience por crear un modelo de datos sencillo con código. Este modelo de datos se incluirá en una clase denominada `Contacts`. El nombre de clase `Contacts` se eligió para evitar un conflicto de nombres de clase con la clase `Contacts` contenida en el archivo Contact.aspx.cs creado por la plantilla de formularios Web Forms.  

1. En el **Explorador de soluciones**, haga clic con el botón secundario en la carpeta *Models* y luego seleccione **Agregar** -> **Clase**.  
...
2. Nombre a esta nueva clase como *Contacts.cs*.  
...
1. En Visual Studio, en la barra de menús, seleccione **Herramientas** -> **Extensiones y actualizaciones**.  
	Se muestra el cuadro de diálogo **Extensiones y actualizaciones**.
...
6. En el **Explorador de soluciones**, haga clic con el botón secundario en el *proyecto* y luego seleccione **Agregar** -> **New Scaffolded Item** (Nuevo elemento de Scaffolding).  
	Se muestra el cuadro de diálogo **Add Scaffold** (Agregar Scaffolding).
7. Seleccione **Web Forms** en el panel izquierdo y seleccione **Web Forms Pages using Entity Framework** (Páginas de Web Forms que usan Entity Framework). A continuación, haga clic en **Add**.  
	![Add Scaffold dialog box](./media/web-sites-dotnet-web-forms-secure/SecureWebForms13a.png)  
	Se muestra el cuadro de diálogo **Add Web Forms Pages** (Agregar páginas de Web Forms).  

8. En el cuadro de diálogo **Add Web Forms Pages** (Agregar páginas de Web Forms), establezca la **clase de modelo** en `Contact (ContactManager.Models)`. Establezca la **clase de contexto de datos** en `ApplicationDbContext (ContactManager.Models)`. A continuación, haga clic en **Add**. 
	![Add Web Forms Pages dialog box](./media/web-sites-dotnet-web-forms-secure/SecureWebForms13b.png)  

Web Forms Scaffolder agrega una nueva carpeta que contiene las páginas *Default.aspx*, *Delete.aspx*, *Edit.aspx* y *Insert.aspx*. Web Forms Scaffolder también crea una carpeta *DynamicData* que contiene una carpeta *EntityTemplates* y una carpeta *FieldTemplates*. `ApplicationDbContext` se usará tanto para la base de datos de suscripciones como para los datos de contactos.
...
6. Reemplace el método `Seed` por el siguiente código:  
	<pre class="prettyprint">
...
7. En la **Consola del Administrador de paquetes**, escriba el comando:  
	<pre class="prettyprint">
	update-database
	</pre>  
El comando `update-database` ejecuta la primera migración que crea la base de datos. De manera predeterminada, la base de datos que se crea es una base de datos LocalDB de SQL Server Express.  
...
3. Seleccione el vínculo **Contact Demo** (Demostración de contactos) en la parte superior de la página para mostrar la página *Contact List*.  
...
1. En el **Explorador de soluciones**, haga clic en el proyecto **ContactManager** y luego presione **F4** para mostrar la ventana **Propiedades**. 
2. Cambie **SSL habilitado** a `true`. 
3. Copie la **URL de SSL** para que pueda usarla más adelante.  
	La URL de SSL será https://localhost:44300/, a menos que anteriormente creara sitios web SSL (como se muestra a continuación).
	![Project Properties](./media/web-sites-dotnet-web-forms-secure/SecureWebForms18.png)  
4. En el **Explorador de soluciones**, haga clic con el botón secundario en el proyecto **Contact Manager** y haga clic en **Propiedades**.
5. En el panel izquierdo, haga clic en **Web**.
6. Cambie la **URL del proyecto** por la **URL de SSL** que creó anteriormente.  
...
8. Presione **Ctrl+F5** para ejecutar la aplicación. Visual Studio mostrará una opción que le permite evitar las advertencias de SSL.  
9. Haga clic en **Sí** para confiar en el certificado IIS Express de SSL y continuar.  
...
10. Haga clic en **Sí** para instalar el certificado en su host local.  
...
Además de la autenticación, este tutorial también utiliza roles para implementar la autorización. Únicamente los usuarios que agreguen al rol `canEdit` podrán cambiar datos (crear, editar o eliminar contactos). 
...
3. Vaya a la [consola de Google Developers](https://console.developers.google.com/). Debe iniciar sesión también con su cuenta de correo electrónico de desarrollador de Google (gmail.com). Si no tiene una cuenta de Google, seleccione el vínculo **Create an account** (Crear una cuenta).  
...
4. Haga clic en el botón **Create Project** (Crear proyecto) y escriba un nombre de proyecto y un identificador (puede usar los valores predeterminados). A continuación, haga clic en la **casilla del contrato** y en el botón **Create** (Crear).  
...
5. En la pestaña izquierda, haga clic en **APIs & auth** (APIs y autenticación) y después en **Credentials** (Credenciales).
6. Haga clic en **Create New Client ID** (Crear un nuevo Id. de cliente) en **OAuth**.  
	Se mostrará el cuadro de diálogo **Create Client ID** (Crear Id. de cliente).
	![Google - Create Client ID](./media/web-sites-dotnet-web-forms-secure/SecureWebForms21c.png)  
7. En el cuadro de diálogo **Create Client ID** (Crear Id. de cliente) mantenga la **aplicación web** predeterminada para el tipo de aplicación.  
8. Establezca los **orígenes autorizados de JavaScript** en la URL de SSL que ha usado anteriormente en este tutorial (**https://localhost:44300/** a no ser que haya creado otros proyectos de SSL).  
...
10. Haga clic en el botón **Create Client ID** (Crear Id. de cliente).
11. En Visual Studio, actualice el método `UseGoogleAuthentication` de la página *Startup.Auth.cs* copiando y pegando el **Id. de aplicación** y el **secreto de la aplicación** en el método. Los valores de **Id. de aplicación** y **secreto de aplicación** que se muestran a continuación son ejemplos y no funcionan.  
	<pre class="prettyprint">  
	using System;
...
13. En **Use another service to log in** (Usar otro servicio para iniciar sesión), haga clic en **Google**.  
...
16. Haga clic en **Accept** (Aceptar). Ahora se le redirigirá de nuevo a la página **Register** (Registro) de la aplicación **ContactManager** donde puede registrar su cuenta de Google.
...
3. Agregue el siguiente método 'AddUserAndRole' a la clase 'Configuración' después del método 'Seed':  
	<pre class="prettyprint">
...
5. Tras guardar todos los cambios, ejecute el siguiente comando en la ventana **Consola del Administrador de paquetes**:  
	...
	Este código crea un nuevo rol llamado `canEdit` y crea un nuevo usuario local con el correo electrónico de canEditUser@wideworldimporters.com. Luego, el código agrega canEditUser@wideworldimporters.com al rol `canEdit`. Para obtener más información, consulte la página de recursos de [ASP.NET Identity](http://www.asp.net/identity).  
...
1. En el **Explorador de soluciones**, agregue una subcarpeta a la carpeta *Contacts* y nómbrela *Admin*.
...
3. Actualice las referencias de vínculo en *Contacts/Default.aspx*; para ello, agregue "Admin/" delante de las referencias de páginas que se vinculan con *Insert.aspx*, *Edit.aspx* y *Delete.aspx*:  
	<pre class="prettyprint">
...
6. En la lista de plantillas web de Visual C#, seleccione **Archivo de configuración web** en la lista central, acepte el nombre predeterminado *Web.config* y luego seleccione **Agregar**.
...
8. Guarde el archivo *Web.config*. 
	El archivo *Web.config* especifica que solo los usuarios asignados al rol "canEdit" pueden acceder a las páginas contenidas en la carpeta *Admin*. 

Cuando un usuario que no forma parte del rol "canEdit" intenta modificar los datos, se le redirige a la página *Log in*.
...
3. En la pestaña **Perfil**, seleccione **Sitios web Azure** como destino de la publicación si aún no se ha seleccionado.  
	![Publish Web dialog box](./media/web-sites-dotnet-web-forms-secure/SecureWebForms23.png)  
4. Haga clic en **Iniciar sesión** si aún no la ha iniciado.
5. Seleccione el sitio web existente que creó anteriormente en este tutorial en la lista desplegable **Sitios web existentes** y haga clic en el botón **Aceptar**.  
	![Select Existing Web Site dialog box](./media/web-sites-dotnet-web-forms-secure/SecureWebForms25.png)  
	Si se deben guardar los cambios realizados en el perfil, seleccione **Sí**.
...
7. Establezca el cuadro desplegable **Configuración** en **Depurar**.
8. Haga clic en el icono de **flecha abajo** situado junto a **ApplicationDbContext** y establézcalo en **ContactDB**.
...
10. Luego haga clic en **Publicar**.  
...
1. En el explorador, haga clic en el vínculo **Demostración de contactos**.  
	Se muestra la lista de contactos.
	![Contacts listed in Browser](./media/web-sites-dotnet-web-forms-secure/SecureWebForms27.png)  

2. Seleccione **Create new** (Crear nuevo) en la página **Contacts List** (Lista de contactos).  
	![Contacts listed in Browser](./media/web-sites-dotnet-web-forms-secure/SecureWebForms29.png)  
	Se le redirige a la página de **inicio de sesión** porque no ha iniciado sesión con una cuenta que pueda modificar contactos.  
3. Cuando haya proporcionado la dirección de correo electrónico y la contraseña que se indican a continuación, haga clic en el botón **Log in** (Iniciar sesión).  
...
4. Escriba los nuevos datos para cada campo y, a continuación, presione el botón **Insertar**.  
...
5. Seleccione el vínculo de **cierre de sesión**.
...
1. En Visual Studio, en el **menú de vista**, seleccione **Explorador de servidores**. 
2. En el **Explorador de servidores**, vaya a **Sitios web**.
...
3. Si se muestra el cuadro de diálogo **Add Firewall Rule** (Agregar regla de firewall), seleccione **Add Firewall Rule** (Agregar regla de firewall).  

	>[WACOM.NOTE]  
	Si no puede expandir **Bases de datos SQL** y no puede ver **ContactDB** en Visual Studio, puede seguir las instrucciones para abrir un puerto o una serie de puertos de firewall. Para ello, siga las instrucciones que se proporcionan en **Configuración de reglas de firewall de Azure** casi al final del [Tutorial de MVC](http://azure.microsoft.com/es-es/documentation/articles/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/). También puede revisar los datos de la base de datos local compilando, ejecutando y agregando datos a la aplicación en modo local (**CTRL + F5** desde Visual Studio).  

4. Si se muestra el cuadro de diálogo **Conectar con el servidor**, escriba la **contraseña** que creó al comienzo de este tutorial y presione el botón **Conectar**.  

	>[WACOM.NOTE]  
	Si no recuerda la contraseña, puede buscarla en el archivo de proyecto local. En el **Explorador de soluciones**, expanda la carpeta *Propiedades* y luego la carpeta *PublishProfiles*. Abra el archivo *contactmanager.pubxml* (su archivo puede tener otro nombre). Busque su contraseña de publicación en el archivo.
5. Expanda la base de datos **contactDB** y luego expanda **Tablas**.
...
2. Copie el valor de *RoleId* y péguelo en la fila vacía (nueva).  
	![AspNetUserRoles data](./media/web-sites-dotnet-web-forms-secure/SecureWebForms37.png)  
3. En la tabla **dbo.AspNetUsers**, busque el usuario que desea poner en el rol y copie su Id.
4. Pegue el *Id.* copiado en el campo **UserId** del nuevo rol de la tabla **AspNetUserRoles**.  

>[WACOM.NOTE]  
Estamos trabajando en una herramienta que facilitará mucho la tarea de administrar los usuarios y roles.  

##Pasos siguientes
Para obtener más información acerca de ASP.NET Web Forms, consulte [Acerca de ASP.NET Web Forms](http://www.asp.net/web-forms) en el sitio web de ASP.NET y los [Tutoriales y guías de Microsoft Azure](http://azure.microsoft.com/es-es/documentation/services/web-sites/#net).

Este tutorial está basado en el tutorial para MVC [Implementación de una aplicación ASP.NET MVC 5 segura con suscripción, OAuth y Base de datos SQL en un sitio web de Azure](http://azure.microsoft.com/es-es/documentation/articles/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/) escrito por Rick Anderson (Twitter [@RickAndMSFT](https://twitter.com/RickAndMSFT)) con ayuda de Tom Dykstra y Barry Dorrans (Twitter [@blowdart](https://twitter.com/blowdart)). 
