...
La integración de servicios de Azure en una red virtual de Azure (VNet) permite que los servicios sean de acceso privado para las instancias implementadas en esa red virtual.

Puede integrar los servicios de Azure con la red virtual con las siguientes opciones:
- Implementando directamente instancias dedicadas del servicio en una red virtual. Las instancias de estos servicios dedicadas pueden tener acceso de forma privada dentro de la red virtual y localmente.
- Mediante la extensión de una red virtual al servicio, a través de puntos de conexión de servicio. Esto permite que los recursos de servicio individuales se protejan para la red virtual.
...
- Las instancias de servicio se implementan en una subred dedicada en la red virtual de un cliente. El acceso entrante y saliente se debe abrir a través de grupos de seguridad de red (NSG) para la subred, siguiendo las instrucciones proporcionadas por los servicios.


### <a name="list-of-services-that-can-be-deployed-into-a-virtual-network"></a>Lista de servicios que se pueden implementar en una red virtual

Cada servicio implementado directamente en la red virtual tiene requisitos específicos para el enrutamiento y los tipos de tráfico que deben permitirse dentro y fuera de subredes. Consulte los vínculos siguientes para obtener más detalles: 
...
- [Entorno de App Service](../app-service-web/web-sites-integrate-with-vnet.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
...
- Azure Storage: [protección de cuentas de Azure Storage para las redes virtuales](https://docs.microsoft.com/azure/storage/common/storage-network-security)

- Azure SQL Database: [protección Azure SQL Database para redes virtuales](https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview)
...
Puede implementar un servicio de Azure en una subred de una red virtual y proteger recursos de servicio críticos para esa subred. 

Por ejemplo, puede implementar HDInsight en la red virtual y proteger una cuenta de almacenamiento para la subred de HDInsight.
