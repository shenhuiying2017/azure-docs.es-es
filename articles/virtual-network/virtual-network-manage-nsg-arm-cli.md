<properties 
   pageTitle="Administración de los NSG con la CLI de Azure en Resource Manager | Microsoft Azure"
...
>[AZURE.NOTE] También puede usar `azure network nsg rule list --resource-group RG-NSG --nsg-name NSG-FrontEnd` para enumerar las reglas del NSG **NSG-FrontEnd**.
...
>[AZURE.NOTE] El parámetro **--quiet** le asegura que no necesita confirmar la eliminación.
...
>[AZURE.NOTE] Observe el valor "" (vacío) del parámetro **network-security-group-id**. Esta es la manera en que se elimina una asociación a un NSG. No puede hacer lo mismo con el parámetro **network-security-group-name**.
...
>[AZURE.NOTE] El comando anterior solo funciona porque el NSG **NSG-FrontEnd** se encuentra en el mismo grupo de recursos que la red virtual **TestVNet**. Si el NSG está en un grupo de recursos distinto, deberá usar a cambio el parámetro **--network-security-group-id** y proporcionar el id. completo del NSG. Para recuperar el id. ejecute **azure network nsg show --resource-group RG-NSG --name NSG-FrontEnd --json** y busque la propiedad **id**.
