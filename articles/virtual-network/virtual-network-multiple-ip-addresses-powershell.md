...
# Asignación de varias direcciones IP a máquinas virtuales
...
2. Cambiar los "valores" de los siguientes elementos $Variables a lo que desee llamar la NIC, el [grupo de recursos](../resource-group-overview.md#resource-groups) al que desea asignarla y la [ubicación](https://azure.microsoft.com/regions) de Azure donde desea crearla.
...
3. <a name="subnet"></a>La NIC debe estar conectada a una subred dentro de una instancia de Azure Virtual Network (VNet) existente en la misma ubicación y [suscripción](../azure-glossary-cloud-terminology.md#subscription) que la NIC. Si desconoce las redes virtuales, lea el artículo [Información general sobre redes virtuales](virtual-networks-overview.md) para aprender más sobre ellas o lea el artículo sobre cómo [crear una máquina virtual](virtual-networks-create-vnet-arm-ps.md) para aprender a crear una. Cambie los siguientes "valores" de $Variables al nombre de la red virtual y la subred a las que desee conectar la NIC y el nombre del grupo de recursos donde esté la red virtual.
	...
	Escriba los comandos siguientes para obtener el nombre de las subredes de la red virtual y reemplace *Subnet1* arriba por el nombre de una subred:
...
5. <a name="ipconfigs"></a>Defina las configuraciones de dirección IP que desea asignar a la NIC. Cada configuración puede tener una dirección IP privada estática o dinámica y un recurso de dirección IP pública asociado con una dirección estática o dinámica.
	...
	Las siguientes configuraciones de ejemplo se crearán y asignarán a una NIC que tendrá tres direcciones IP privadas y una pública asignadas.
...
9. <a name="os"></a>Agregue manualmente todas las direcciones IP privadas secundarias (direcciones IP con *False* en la columna **Primary** en la salida del paso anterior) a la configuración de TCP/IP en el sistema operativo. La dirección IP privada asignada a *IPConfig-1* en el paso 5 se asigna automáticamente al sistema operativo por medio de Azure DHCP, porque tiene la configuración *Principal*.
	...
	**Windows**
	...
	2. Abra las propiedades del adaptador de red.
	3. Abra las propiedades de **Protocolo de Internet versión 4**.
		- Haga clic en **Usar la siguiente dirección IP** y escriba los valores siguientes:
			...
			- **Puerta de enlace predeterminada**: la primera dirección IP de la subred. Si la subred es 10.0.0.0/24m, la dirección IP de la puerta de enlace es 10.0.0.1.
		...
		- Haga clic en **Aceptar** para cerrar la configuración de TCP/IP y en **Aceptar** otra vez para cerrar la configuración del adaptador.
	4. En un símbolo del sistema, escriba *ipconfig /all*. Se muestran todas las direcciones IP que agregó y DHCP está desactivado.
...
## <a name="add"></a>Incorporación de direcciones IP a una máquina virtual existente
...
3. Cree una variable y establézcala en la NIC con el siguiente comando:
