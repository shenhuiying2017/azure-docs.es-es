---
title: Control del enrutamiento y uso de aplicaciones virtuales mediante PowerShell en el modelo de implementación clásico | Microsoft Docs
...
1. Ejecute el cmdlet **`New-AzureRouteTable`** para crear una tabla de rutas para la subred front-end.
...
2. Ejecute el cmdlet **`Set-AzureRoute`** para crear una ruta en la tabla de rutas creada anteriormente para enviar todo el tráfico destinado a la subred back-end (192.168.2.0/24) a la VM **FW1** (192.168.0.4).
...
3. Ejecute el cmdlet **`Set-AzureSubnetRouteTable`** para asociar la tabla de rutas creada anteriormente a la subred **FrontEnd**.
...
1. Ejecute el cmdlet **`New-AzureRouteTable`** para crear una tabla de rutas para la subred back-end.
   
        New-AzureRouteTable -Name UDR-BackEnd `
            -Location uswest `
            -Label "Route table for back end subnet"
2. Ejecute el cmdlet **`Set-AzureRoute`** para crear una ruta en la tabla de rutas creada anteriormente para enviar todo el tráfico destinado a la subred front-end (192.168.1.0/24) a la VM **FW1** (192.168.0.4).
   
        Get-AzureRouteTable UDR-BackEnd `
            |Set-AzureRoute -RouteName RouteToFrontEnd -AddressPrefix 192.168.1.0/24 `
            -NextHopType VirtualAppliance `
            -NextHopIpAddress 192.168.0.4
3. Ejecute el cmdlet **`Set-AzureSubnetRouteTable`** para asociar la tabla de rutas creada anteriormente a la subred **BackEnd**.
