...
El servicio Microsoft Azure Virtual Network permite que los recursos de Azure se comuniquen de manera segura con otros en una red virtual. Una red virtual es una representación de su propia red en la nube. Una red virtual es un aislamiento lógico de la nube de Azure dedicada a su suscripción. Puede conectar redes virtuales a otras redes virtuales o a su red local. La imagen siguiente muestra algunas de las funcionalidades del servicio Azure Virtual Network:
...
- **[Aislamiento:](#isolation)** las redes virtuales están aisladas entre sí. Puede crear redes virtuales independientes para el desarrollo, la prueba y la producción que usan los mismos bloques de direcciones de CIDR (10.0.0.0/0, por ejemplo). Por el contrario, puede crear varias redes virtuales que usan diferentes bloques de direcciones CIDR y que conectan las redes entre sí. También puede segmentar una red virtual en varias subredes. Azure proporciona la resolución de nombres interna para las instancias de rol de Virtual Machines y de Azure Cloud Services que están implementadas en una red virtual. Si lo desea, puede configurar una red virtual para usar sus propios servidores DNS, en lugar de usar la resolución de nombres interna de Azure.
- **[Comunicación con Internet:](#internet)** todas las instancias de rol de Azure Virtual Machines y Cloud Services de una red virtual tienen acceso a Internet, de manera predeterminada. También puede habilitar el acceso entrante a recursos específicos, según sea necesario.
- **[Comunicación de los recursos de Azure:](#within-vnet)** los recursos de Azure como Cloud Services y Virtual Machines se pueden implementar en la misma red virtual. Los recursos pueden comunicarse entre sí mediante direcciones IP privadas, aunque estén en subredes diferentes. Azure proporciona el enrutamiento predeterminado entre subredes, redes virtuales y redes locales, por lo que no tendrá que configurar ni administrar rutas. Sin embargo, puede personalizar el enrutamiento de Azure, si lo desea.
...
- **[Conectividad local:](#connect-on-premises)** una red virtual se puede conectar de manera privada a una red local o mediante el uso de una conexión VPN de sitio a sitio a través de Internet.
...
También puede implementar muchos otros tipos de recursos de Azure a una red virtual, como Azure Virtual Machines, Azure Cloud Services, Azure App Service Environments y los conjuntos de escalado de Azure Virtual Machine. Para una lista completa de los recursos de Azure que puede implementar en una red virtual, consulte el artículo sobre la [integración de servicios de redes virtuales para los servicios de Azure](virtual-network-for-azure-services.md).

Algunos recursos no se pueden implementar en una red virtual, pero le permiten restringir la comunicación desde los recursos para que solo se realicen dentro de una red virtual. Para más información sobre cómo restringir el acceso a los recursos, consulte [Puntos de conexión del servicio de redes virtuales](virtual-network-service-endpoints-overview.md). 
...
Azure crea tablas de rutas que permiten a los recursos conectados a una subred de cualquier red virtual se comuniquen entre sí, de manera predeterminada. Puede implementar una o ambas de las siguientes opciones para reemplazar las rutas predeterminadas que crea Azure:
