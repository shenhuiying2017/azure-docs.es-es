<properties 
   pageTitle="Solución de problemas de los grupos de seguridad de red - Portal | Microsoft Azure"
...
# Solución de problemas de los grupos de seguridad de red utilizando Azure Portal
...
## Uso de las reglas de seguridad vigentes para solucionar problemas de flujo de tráfico de máquinas virtuales
...
### Visualización de las reglas de seguridad vigentes para una máquina virtual
...
2. Haga clic en **More services** (Más servicios), a continuación, haga clic en **Máquinas virtuales** en la lista que aparece.
...
6. Aparece la hoja **Get effective security rules** (Obtener reglas de seguridad vigentes), como se muestra en la siguiente imagen:
	...
	- Regla de entrada **AllowRDP**: esta regla permite las conexiones RDP con la máquina virtual.
7. Haga clic en la pestaña **Subnet1-NSG** para ver las reglas vigentes desde el NSG aplicado a la subred, tal como se muestra en la siguiente imagen:
	...
	>[AZURE.NOTE]Si la máquina virtual asociada a la NIC no está en estado de ejecución, o no se ha aplicado ningún NSG a la NIC o la subred, no se muestra ninguna regla.

8. Para editar las reglas NSG, haga clic en *Subnet1-NSG* en la sección **Associated NSGs** (NSG asociados). Se abrirá la hoja **Subnet1-NSG**. Puede editar directamente las reglas, haciendo clic en **Reglas de seguridad de entrada**.
...
### Visualización de las reglas de seguridad vigentes para una interfaz de red
...
1. Inicie sesión en Azure Portal en https://portal.azure.com.
2. Haga clic en **More services** (Más servicios), a continuación, haga clic en **Interfaces de red** en la lista que aparece.
3. Seleccione una interfaz de red. En la siguiente imagen, se selecciona una NIC llamada*VM1-NIC1*.
	...
	>[AZURE.NOTE] Si se quita un NSG de una interfaz de red, la subred NSG todavía continúa vigente en dicha NIC. En este caso, el resultado mostraría solo las reglas del NSG de la subred. Las reglas solo aparecen si la NIC está conectada a una máquina virtual.
...
## Visualización de las reglas de seguridad vigentes para un grupo de seguridad de red (NSG)
...
1. Inicie sesión en Azure Portal en https://portal.azure.com.
2. Haga clic en **More services** (Más servicios), a continuación, haga clic en **Grupos de seguridad de red** en la lista que aparece.
3. Seleccione un NSG. En la siguiente imagen, se ha seleccionado un NSG llamado VM1-nsg.
	...
	>[AZURE.NOTE] Si un NSG se aplica a solo una subred vacía, no se mostrará ninguna máquina virtual. Si se aplica un NSG a una NIC que no está asociada a una máquina virtual, las NIC tampoco aparecerán.
...
>[AZURE.NOTE] Aunque una subred y una NIC cada una solo puede tener un NSG aplicado, un NSG puede estar asociado a varias NIC y varias subredes.
...
- Si ha emparejado redes virtuales, de forma predeterminada, la etiqueta VIRTUAL\_NETWORK se expandirá automáticamente para incluir prefijos para redes virtuales emparejadas. Puede ver estos prefijos en la lista **ExpandedAddressPrefix** para solucionar los problemas relacionados con la conectividad de emparejamiento de rede virtuales.
