<properties urlDisplayName="Role Based Access Control with Azure Active Directory" pageTitle="Control de acceso basado en roles en Servicios móviles y Azure Active Directory (Tienda Windows) | Centro de desarrollo móvil" metaKeywords="" description="Obtenga información acerca de cómo controlar el acceso basado en roles de Azure Active Directory en su aplicación de la Tienda Windows." metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Role Based Access Control in Mobile Services and Azure Active Directory" authors="wesmc" manager="dwrede" />
...
>[WACOM.NOTE] La finalidad de este tutorial es ampliar sus conocimientos sobre autenticación para incluir prácticas de autorización. Debe realizar primero el tutorial [Introducción a la autenticación] usando el proveedor de autenticación de Azure Active Directory. Este tutorial continúa la actualización de la aplicación TodoItem que se usa en el tutorial [Introducción a la autenticación].

Este tutorial le guiará para llevar a cabo los siguientes pasos:

1. [Creación de un grupo Sales con pertenencia]
2. [Generación de una clave para la aplicación integrada]
3. [Adición de un script compartido que comprueba la suscripción] 
4. [Adición de comprobación de acceso basado en roles a las operaciones de la base de datos]
5. [Prueba de acceso del cliente]

Este tutorial requiere lo siguiente:

* Visual Studio 2013 en Windows 8.1.
* Realización del tutorial [Introducción a la autenticación] usando el proveedor de autenticación de Azure Active Directory.
* Realización del tutorial [Almacenamiento de scripts de servidor] para estar familiarizado con el uso de un repositorio Git para almacenar scripts de servidor.
...
En esta sección utilizará Git para implementar un archivo de script compartido denominado *rbac.js* en el servicio móvil. Este archivo de script compartido continuará las funciones que usan la [API Graph] para comprobar la pertenencia del usuario al grupo. 
...
4. Agregue el siguiente código a *rbac.js* para definir la función `isMemberOf` que llama al extremo [IsMemberOf] de la API REST Graph.

    Esta función es un contenedor delgado alrededor del extremo [IsMemberOf] de la API REST Graph. Usa el token de acceso Graph para comprobar si el identificador de objeto de directorio del usuario tiene suscripción en el grupo basándose en dicho identificador.
...
7. Agregue la siguiente función exportada `checkGroupMembership` al archivo *rbac.js*.  

    Esta función encapsula el uso de las otras funciones de script y se exporta desde el script compartido para que pueda recibir llamadas de otros scripts para realizar las comprobaciones de acceso reales. Dado el objeto de usuario de servicio móvil, y el identificador de grupo, el script recuperará el identificador de objeto de Azure Active Directory para la identidad del usuario y comprobará la pertenencia al grupo.
...
[Documentación de referencia de errores de la API Graph]: http://msdn.microsoft.com/es-es/library/azure/hh974480.aspx
