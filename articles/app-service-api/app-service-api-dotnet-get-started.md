...
Este es el primero de una serie de tutoriales. Los tutoriales siguientes se basan en lo que se crea en este, y en ellos aprenderá a:
...
En el tutorial se da por hecho que está familiarizado con ASP.NET Web API; si necesita una introducción, consulte [Introducción a ASP.NET Web API 2](http://www.asp.net/web-api/overview/getting-started-with-aspnet-web-api/tutorial-your-first-web-api).

En las instrucciones y capturas de pantalla se da por hecho que está usando Visual Studio 2015, pero las mismas instrucciones funcionarán con Visual Studio 2013.

Necesita una cuenta de Azure para completar el tutorial. Puede:

* [Abra una cuenta de Azure gratis](/pricing/free-trial/?WT.mc_id=A261C142F). Obtenga créditos que puede usar para probar los servicios de Azure de pago. Incluso después de que se agoten los créditos, puede mantener la cuenta y usar los servicios y características gratuitos de Azure, como la característica de Aplicaciones web del Servicio de aplicaciones de Azure.
* [Active las ventajas de suscriptor de Visual Studio](/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F). Su suscripción a MSDN le proporciona créditos todos los meses que puede usar para servicios de Azure de pago.
...
El código que se va a implementar en una aplicación de API y en una aplicación web en este tutorial se muestra en el repositorio de GitHub [Azure-Samples/app-service-api-dotnet-contact-list](https://github.com/Azure-Samples/app-service-api-dotnet-contact-list). La solución ContactsList de Visual Studio incluye los siguientes proyectos:
...
* **ContactsList.MVC**: cliente de ASP.NET MVC para la API ContactsList.
* **ContactsList.Angular**: cliente de interfaz de usuario AngularJS simple para la API ContactsList. Muestra cómo llamar a una aplicación de API sin protección (sin autenticación).
* **ContactsList.Angular.AAD**: cliente de AngularJS que muestra cómo usar Azure Active Directory para autenticar usuarios.
* **CompanyContacts.API**: proyecto de ASP.NET Web API que devuelve una lista de contactos codificada de forma rígida en respuesta a una solicitud Get. Lo llama el método Get de **ContactsList.API** para demostrar cómo llamar a una API mediante la autenticación de servicio a servicio (entidad de servicio).
...
## Uso de Swashbuckle y Swagger durante la ejecución local

[Swagger](http://swagger.io/) es un formato JSON para describir los métodos y las respuestas de una API. El Servicio de aplicaciones de Azure integra compatibilidad con Swagger 2.0. Como verá más adelante en el tutorial, si proporciona metadatos de Swagger para una API, Visual Studio puede generar código de cliente que facilita el consumo de la API.

Para proporcionar metadatos de Swagger 2.0 para un proyecto de ASP.NET Web API, puede instalar el paquete de NuGet de [Swashbuckle](https://www.nuget.org/packages/Swashbuckle). Swashbuckle usa la reflexión para generar metadatos dinámicamente. El paquete de NuGet ya está instalado en el proyecto ContactsList.API que descargó y ya está instalado cuando se crea un nuevo proyecto mediante la plantilla de proyecto **Aplicación de API de Azure**.
...
2. Establezca el proyecto ContactsList.API como proyecto de inicio. (En el **Explorador de soluciones**, haga clic con el botón derecho en el proyecto y, después, haga clic en **Establecer como proyecto de inicio**).
...
12. En la barra de direcciones del explorador, agregue `swagger/docs/v1` al final de la línea y presione ENTRAR. (La dirección URL será `http://localhost:51864/swagger/docs/v1`).
	...
	Si usa Chrome, el explorador muestra los datos JSON en la ventana del explorador.
...
3. En el proyecto ContactsList.API, en el **Explorador de soluciones**, abra el archivo *app\_start\\swaggerconfig* y, después, desplácese hacia abajo hasta el siguiente código y quite los comentarios.
...
4. Cuando aparezca la página de la interfaz de usuario de Swagger, haga clic en **Contactos** para ver los métodos disponibles.

	![](./media/app-service-api-dotnet-get-started/contactsmethods.png)

5. Haga clic en **Obtener > Pruébelo**.
...
6. Haga clic en **Post** y, después, haga clic en el cuadro situado bajo **Esquema de modelo**.
...
7. Cambie el valor JSON en el cuadro de entrada del parámetro `contact` para que tenga un aspecto similar al del ejemplo siguiente o sustituya su nombre y su dirección de correo electrónico:
...
Swashbuckle funciona con cualquier proyecto de ASP.NET Web API. Si desea agregar generación de metadatos de Swagger a un proyecto existente, simplemente instale el paquete de Swashbuckle. Si desea crear un nuevo proyecto para implementar como una aplicación de API del Servicio de aplicaciones, use la plantilla de proyecto **Aplicación de API de Azure** de ASP.NET. Dicha plantilla crea un proyecto de API web con Swashbuckle instalado.
...
En esta sección, se usan las herramientas de Azure integradas en el asistente **Publicación web** de Visual Studio para crear una nueva aplicación de API de Azure. Después, se implementa el proyecto ContactsList.API en la nueva aplicación de API y se vuelve a ejecutar la interfaz de usuario de Swagger para llamar a la API, esta vez mientras se ejecuta en la nube.

1. En el **Explorador de soluciones**, haga clic con el botón derecho en el proyecto ContactsList.API y, después, haga clic en **Publicar**.
...
4. En **Nombre de aplicación de API**, escriba un nombre que sea único en el dominio *azurewebsites.net*.
	...
	Visual Studio propone un nombre único anexando una cadena de fecha y hora al nombre del proyecto. Si lo prefiere, puede aceptar ese nombre.
	...
	Azure usará este nombre como prefijo de la dirección URL de la aplicación. La dirección URL completa consistirá en este nombre más *.azurewebsites.net*. Por ejemplo, si el nombre es `ContactsListAPI`, la dirección URL será `contactslistapi.azurewebsites.net`.

6. En la lista desplegable **Grupo de recursos**, escriba "ContactsListGroup" u otro nombre que prefiera.

	Este cuadro permite seleccionar un [grupo de recursos](../azure-preview-portal-using-resource-groups.md) existente o crear uno nuevo escribiendo un nombre de grupo recursos que no exista en su suscripción.
...
4. Haga clic en el botón **Nuevo** situado junto a la lista desplegable **Plan del servicio de aplicaciones**.
...
6. En el cuadro de diálogo **Configurar plan del servicio de aplicación**, haga clic en **Aceptar**.
...
7. En el cuadro de diálogo **Crear servicio de aplicaciones**, haga clic en **Crear**.
	...
	Nota: hay otras maneras de crear aplicaciones de API en el Servicio de aplicaciones de Azure. En Visual Studio están disponibles los mismos cuadros de diálogo mientras se crea un nuevo proyecto. También puede crear aplicaciones de API mediante el portal de Azure, [cmdlets de Azure para Windows PowerShell](../powershell-install-configure.md) o [la interfaz de línea de comandos multiplataforma](../xplat-cli.md).
...
El proyecto ContactsList.MVC ya tiene el código de cliente generado, pero lo eliminará y lo volverá a generar para ver cómo se hace y para hacer que la dirección URL de su aplicación de API sea la dirección URL de destino predeterminada.
...
2. Haga clic con el botón derecho en el proyecto ContactsList.MVC y después haga clic en **Agregar > Cliente de API de REST**.
...
8. En el cuadro de diálogo **Servicio de aplicaciones**, expanda el grupo de recursos **ContactsListGroup** y seleccione la aplicación de API; después, haga clic en **Aceptar**.
...
6. La clase cliente se generará con un nombre diferente según el nombre de la aplicación de API; cambie este código para que el nombre de tipo coincida con lo que se generó en el proyecto.
...
1. Establezca los proyectos ContactsList.API y ContactsList.MVC como proyectos de inicio, con ContactsList.API antes que ContactsList.MVC. (Haga clic con el botón derecho en la solución, haga clic en **Propiedades**, haga clic en **Proyectos de inicio múltiples** y establezca cada proyecto en **Inicio**. Use los iconos de flecha arriba y abajo para asegurarse de que ContactsList.API está primera en la lista). 
...
2. Convierta en comentario la línea que establece la dirección URL base de la API a la dirección URL de localhost; quite el comentario de la línea que no tiene ningún parámetro de constructor. El código ahora tiene un aspecto similar al del ejemplo siguiente, excepto que ambas líneas deben tener el nombre de clase que refleja el nombre de la aplicación de API a partir de la cual generó el código.
...
1. En el **Explorador de soluciones**, haga clic con el botón derecho en el proyecto ContactsList.MVC y, después, haga clic en **Publicar**.

2. En el asistente **Publicación web**, haga clic en la pestaña **Perfil**.

3.  En la pestaña **Perfil** del asistente **Publicación web**, haga clic en **Servicio de aplicaciones de Microsoft Azure**.

5. En el cuadro de diálogo **Servicio de aplicaciones**, haga clic en **Nuevo**.

3. En la pestaña **Hospedaje** del cuadro de diálogo **Crear servicio de aplicaciones**, haga clic en **Cambiar tipo** y asegúrese de que el tipo es **Aplicación de API**.
...
4. En la lista desplegable **Plan del servicio de aplicaciones**, elija el mismo plan que creó anteriormente.
...
3.  En el paso **Conexión** del asistente **Publicación web**, haga clic en **Publicar**.
...
En este tutorial vimos cómo crear aplicaciones de API, implementar código en ellos y consumirlos desde clientes .NET. El siguiente tutorial de la serie de introducción muestra cómo [consumir aplicaciones de API de clientes de JavaScript mediante CORS](app-service-api-cors-consume-javascript.md).
