<properties
   pageTitle="Notificación y comprobación del estado de Service Fabric | Microsoft Azure"
   description="En este artículo se describe cómo se pueden enviar informes de estado desde el código de servicio y comprobar el estado del servicio mediante las herramientas de seguimiento de estado que proporciona Azure Service Fabric."
...
4. Cuando se ejecute la aplicación, inicie el Explorador de Service Fabric haciendo clic con el botón derecho en la aplicación de bandeja del sistema del Administrador de clústeres local y elija Administrar clúster local en el menú contextual. ![Inicio del explorador de Service Fabric desde la bandeja del sistema]()
...
5. Simulemos este error y veamos cómo se muestra en las herramientas de seguimiento de estado. Para simular el error convierta en comentario la primera línea del código de notificación de estado que agregó anteriormente. Después de convertir en comentario la primera línea, el código será como se muestra a continuación. Esto activará el informe de estado cada vez que se ejecute RunAsync. Después de realizar el cambio, ejecute la aplicación presionando F5.
...
7. Si selecciona la réplica principal en la vista de árbol del explorador de Service Fabric, verá que muestra que su estado está en error también y muestra los detalles del informe de estado que se agregaron al parámetro `HealthInformation` en el código. También puede ver los mismos informes de estado en Powershell, así como en el Portal de Azure. ![Estado de réplica del explorador de Service Fabric](./media/service-fabric-diagnostics-how-to-report-and-check-service-health/replica-health-error-report-sfx.png)

Este informe permanecerá en el administrador de estado hasta que se sustituya por otro informe o se elimine esta réplica (*como no se estableció un período de vida para este informe de estado en el objeto HealthInformation, no caducará nunca*). Para que el estado se consulte y se notifique, FabricClient debe estar en un proceso con privilegios de administrador.
