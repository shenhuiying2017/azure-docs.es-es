...
Una instancia de servicio es una instancia con nombre específica de un nombre de aplicación y un nombre de tipo de servicio (consulte el [Ciclo de vida de aplicaciones de Service Fabric](service-fabric-application-lifecycle.md)). Durante la creación del servicio es cuando especifica el esquema de partición ([Crear particiones de los servicios del Servicio de fábrica](service-fabric-concepts-partitioning.md)) que se va a usar.

El primer nivel de ajuste de escala es por nombres de servicio. Puede crear nuevas instancias de un servicio, con un nivel diferente de particiones, a medidas que sus instancias de servicio anteriores pasen a estar ocupadas. Esto permite que los consumidores del nuevo servicio usen menos instancias de servicio ocupadas en favor de las más ocupadas.

Una opción para aumentar la capacidad, o para aumentar o disminuir los recuentos de particiones, es crear una nueva instancia de servicio con un nuevo esquema de particiones. La complejidad es que los clientes de consumo necesitan saber cuándo y cómo usar un servicio con un nombre diferente.
