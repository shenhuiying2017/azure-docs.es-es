...
**Paquete de datos**: un directorio del disco que contiene los archivos de datos de solo lectura estáticos del tipo de servicio (normalmente archivos fotográficos, de sonido y de vídeo). El archivo `ServiceManifest.xml` del tipo de servicio hace referencia a los archivos del directorio del paquete de datos. Cuando se crea un servicio con nombre, el paquete de datos se copia en los nodos seleccionados para ejecutar dicho servicio.  El código empieza a ejecutarse, tras lo cual se habilita el acceso a los archivos de datos.
...
**Servicio de nomenclatura**: servicio presente en cada clúster de Service Fabric, que resuelve los nombres de servicio como una ubicación del clúster. Usted administra las propiedades y los nombres de servicio de manera similar a un servicio de nombres de dominio (DNS) para el clúster. Los clientes pueden comunicarse de forma segura con cualquier nodo del clúster mediante el servicio de nomenclatura para resolver el nombre de un servicio y su ubicación.  Las aplicaciones se mueven dentro del clúster debido a, por ejemplo, errores, equilibrio de los recursos o cambio de tamaño del clúster. Puede desarrollar servicios y clientes que resuelvan la ubicación de red actual. Los clientes obtienen la dirección IP real de la máquina y el puerto en el que se esté ejecutando.
...
**Servicio Administrador de conmutación por error (FM)**: cada clúster de Service Fabric cuenta con un servicio Administrador de conmutación por error que es responsable de la realización de funciones relacionadas con la alta disponibilidad y coherencia de los servicios, así como de la orquestación de actualización de aplicaciones y clústeres, y de la interacción con otros componentes del sistema.
...
**Reliable Services**: una API para generar servicios con y sin estado. Los servicios con estado almacenan su estado en Reliable Collections (como un diccionario o una cola). También le permiten de conectar una amplia variedad de pilas de comunicación, como la API web y Windows Communication Foundation (WCF).
