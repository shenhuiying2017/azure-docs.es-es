...
* En el caso de la pérdida permanente de todo un clúster de Service Fabric o de todos los nodos que están ejecutando una partición determinada (es decir, todos los nodos del clúster). Esto puede ocurrir cuando, por ejemplo, no se cuenta con replicación geográfica, todo el clúster se encuentra en un solo centro de datos y este centro de datos entero deja de funcionar.
* Errores administrativos por los cuales el estado se elimina o daña accidentalmente. Esto puede ocurrir cuando un administrador con suficientes privilegios elimina el servicio por error.
* Errores en el servicio que causan daños en los datos. Esto puede ocurrir cuando, por ejemplo, hubo una actualización de servicio que escribió datos defectuosos en una colección y hay que revertir tanto el servicio como los datos a un estado anterior.
...
1. La partición del servicio perdió datos. Por ejemplo, el disco de dos de las tres réplicas de una partición (incluida la réplica principal) sufre daños o se borra. Los datos de la nueva réplica principal deben restaurarse.
...
El autor del servicio debe realizar lo siguiente para recuperarlos: 1) Invalidar **IReliableStateManager** para devolver un nuevo ReliableStateManager y proporcionar una función de devolución de llamada para llamar en caso de un evento de pérdida de datos; 2) Buscar la última copia de seguridad en la ubicación externa que contiene las copias de seguridad del servicio; 3) Descargar la última copia de seguridad (y descomprimirla en la carpeta de copia de seguridad si estaba comprimida); 4) Llamar a **IReliableStateManager.RestoreAsync** con la ruta de acceso a la carpeta de copia de seguridad; 5) Devolver true si la restauración se realizó correctamente.
...
Una manera de hacerlo es implementar un nuevo clúster de Service Fabric y restaurar las copias de seguridad de las particiones afectadas igual que en el escenario "Eliminación o pérdida del servicio" anterior. Una vez que se encuentra la última copia de seguridad anterior a los daños en los datos, se pueden mover o eliminar todas las copias de seguridad de esta partición después de esta copia de seguridad. Ahora, cuando se llama a **OnDataLossAsync** en la partición del clúster de producción, la última copia de seguridad que se encuentra en el almacén externo será la elegida por el proceso anterior.
...
>[AZURE.NOTE]La cadena que representa la ruta de acceso de la carpeta de copia de seguridad, así como las rutas de acceso de los archivos dentro de la carpeta de copia de seguridad, puede tener más de 255 caracteres, según la ruta de acceso de FabricDataRoot y la longitud del nombre del tipo de aplicación. Esto puede hacer que algunos métodos de .Net, como **Directory.Move**, produzcan una excepción **PathTooLongException**. Como solución alternativa, se puede llamar directamente a las API de kernel32 como **CopyFile**.
...
El Administrador de estado fiable proporciona la capacidad de restaurar desde una copia de seguridad mediante la API IReliableStateManager.RestoreAsync. Solo se puede llamar al método RestoreAsync dentro del método OnDataLossAsync. El valor booleano devuelto por OnDataLossAsync indica si el servicio restauró su estado a partir de un origen externo. Si OnDataLossAsync devuelve true, Service Fabric volverá a generar todas las demás réplicas a partir de esta principal. Las réplicas que reciban OnDataLossAsync primero realizarán la transición al rol principal, pero no se concederá el estado de lectura ni de escritura. Esto supone que para, los implementadores de StatefulService, no se llamará a RunAsync hasta que OnDataLossAsync se complete correctamente. Hasta que un servicio complete esta API correctamente (y devuelva true o false) y finalice la reconfiguración pertinente, se seguirá llamando a la API.
