...
El nombre de la cuenta, el nombre de la tabla y **PartitionKey** juntos identifican la partición dentro del servicio de almacenamiento donde el servicio Tabla almacena la entidad. Además de ser parte del esquema de direccionamiento de las entidades, las particiones definen un ámbito para las transacciones (consulte [Transacciones de grupos de entidades](#entity-group-transactions) a continuación) y conforman el fundamento de cómo se ajusta el servicio Tabla. Para obtener más información sobre las particiones, consulte [Objetivos de rendimiento y escalabilidad de almacenamiento de Azure](http://msdn.microsoft.com/library/azure/dn249410.aspx).  
...
Para obtener más información acerca de los detalles internos del servicio Tabla y saber en particular cómo administra las particiones, consulte el artículo [Almacenamiento de Microsoft Azure: Un servicio de
almacenamiento en la nube de gran disponibilidad y homogeneidad](http://blogs.msdn.com/b/windowsazurestorage/archive/2011/11/20/windows-azure-storage-a-highly-available-cloud-storage-service-with-strong-consistency.aspx).  

### Transacciones de grupos de entidades 
En el servicio Tabla, Transacciones de grupo de entidad (EGT) son el único mecanismo integrado para realizar actualizaciones atómicas en varias entidades. Las EGT también se conocen como *transacciones por lotes* en algunos documentos. Las EGT funcionan únicamente en entidades almacenadas en la misma partición (comparten la misma clave de partición en una tabla determinada), por lo que siempre que necesite un comportamiento transaccional atómico a través de varias entidades, debe asegurarse de que las entidades se encuentren en la misma partición. Este suele ser un motivo para mantener varios tipos de entidad en la misma tabla (y partición) y no utilizar varias tablas para diferentes tipos de entidad. Una sola EGT puede operar en 100 entidades como máximo.  
...
Para obtener más información, consulte [Introducción al modelo de datos del servicio Tabla](http://msdn.microsoft.com/library/azure/dd179338.aspx).  
...
Estas listas resumen algunas de las instrucciones clave que debe tener en cuenta al diseñar las tablas, y esta guía abordará todo con más detalle más adelante. Estas instrucciones son muy diferentes de las directrices que normalmente seguiría para el diseño de la base de datos relacional.  

Diseñe una solución de servicio Tabla cuya *lectura* sea eficaz:

-	***Diseño para realizar consultas en aplicaciones con muchas lecturas.*** Al diseñar las tablas, piense en las consultas (especialmente las sensibles a la latencia) que ejecutará antes de pensar cómo actualizará las entidades. Normalmente esto produce una solución eficiente y eficaz.
-	***Especifique PartitionKey y RowKey en las consultas.*** *Consultas de punto* como estas son las más eficaces del servicio de tabla.  
...
-	***Tenga en cuenta la posibilidad de desnormalizar los datos.*** El almacenamiento de tablas es barato por lo que puede desnormalizar los datos. Por ejemplo, almacenar entidades de resumen para que las consultas de datos agregados solo necesitan tener acceso a una única entidad.
-	***Utilice valores de clave compuestos.*** Las únicas claves que tiene son **PartitionKey** y **RowKey**. Por ejemplo, utilice valores de clave compuestos para activar rutas de acceso clave alternativas a las entidades.
...
Diseñe una solución de servicio Tabla cuya  *escritura* sea eficaz:  
...
|**Cadena RowKey** (Identificación de empleado)|String|
|**Nombre**|String|
|**Apellidos**|String|
|**Edad**|entero|
|**EmailAddress**|String|

En la sección anterior "[Descripción general del servicio Tabla de Azure](#azure-table-service-overview) " se describen algunas de las características clave del servicio Tabla de Azure que tienen influencia directa en el diseño de la consulta. Estos dan como resultado las siguientes directrices generales para diseñar consultas del servicio Tabla. Tenga en cuenta que la sintaxis de filtro utilizada en los ejemplos siguientes es de la API de REST del servicio Tabla. Para obtener más información, consulte [Entidades de consulta](http://msdn.microsoft.com/library/azure/dd179421.aspx) en MSDN.  

-	Una ***consulta de punto*** es la búsqueda más eficaz que puede usar y se recomienda para búsquedas de gran volumen o búsquedas que requieren menor latencia. Este tipo de consulta puede utilizar los índices para localizar una entidad individual con gran eficacia si se especifican los valores **PartitionKey** y **RowKey**. Por ejemplo:
$filter=(PartitionKey eq 'Ventas') y (RowKey eq '2')  
-	La segunda opción más eficaz es una ***Consulta por rango*** que use **PartitionKey** y filtre un rango de valores **RowKey** para devolver más de una entidad. El valor **PartitionKey** identifica una partición específica y los valores **RowKey** identifican un subconjunto de las entidades de esa partición. Por ejemplo:
$filter=PartitionKey eq 'Ventas' y RowKey ge 'S' and RowKey lt 'T'  
-	En tercer lugar, tiene un **Examen de partición*** que usa **PartitionKey** y filtra otra propiedad no clave y que puede devolver más de una entidad. El valor **PartitionKey** identifica una partición específica y los valores de propiedad seleccionan un subconjunto de las entidades de esa partición. Por ejemplo:
$filter=PartitionKey eq 'Ventas' y LastName eq 'Smith'  
-	Un ***Examen de tabla*** no incluye la **PartitionKey** y es bastante ineficaz, ya que busca en todas las particiones que componen la tabla todas las entidades coincidentes. Realizará un recorrido de tabla independientemente de si su filtro usa **RowKey**. Por ejemplo:
...
Tenga en cuenta que si usa un "**o**" para especificar un filtro basado en valores **RowKey**, se obtendrá un recorrido de partición y no se tratará como una consulta de intervalo. Por lo tanto, debe evitar las consultas que utilizan filtros, como:
$filter=PartitionKey eq 'Ventas' y (RowKey eq '121' o RowKey eq '322')  
...
>[AZURE.NOTE]Por ejemplo, en un sistema que almacena información acerca de los usuarios o empleados, el identificador del usuario puede ser un buen **PartitionKey**.  

Existen otros puntos adicionales que se tienen en cuenta al elegir **PartitionKey** y que están relacionados con la forma de insertar, actualizar y eliminar entidades: consulte la sección [Diseño de modificación de datos](#design-for-data-modification) que aparece a continuación.  

### Optimización de consultas para el servicio Tabla  

El servicio Tabla indexará automáticamente las entidades mediante los valores **PartitionKey** y **RowKey** en un índice agrupado único, por lo tanto, este es el motivo por el que las consultas de punto son las más eficaces. Sin embargo, no hay ningún índice distinto del índice agrupado en **PartitionKey** y **RowKey**.   
...
-	[Patrón de índice secundario dentro de la partición](#inter-partition-secondary-index-pattern) - Almacenar varias copias de cada entidad con distintos valores de RowKey diferentes en particiones independientes o en tablas independientes para habilitar la realización de búsquedas rápidas y eficaces y órdenes alternativos utilizando valores **RowKey** diferentes.  
...
Muchas aplicaciones tienen requisitos para utilizar datos ordenados en distintos órdenes: por ejemplo, ordenar los empleados por su nombre o por su fecha de contratación. Los siguientes patrones de la sección [Patrones de diseño de tabla](#table-design-patterns) tratan la manera de alternar ordenaciones para las entidades:  

-	[Patrón de índice secundario dentro de la partición](#intra-partition-secondary-index-pattern) - Almacenar varias copias de cada entidad con diferentes valores RowKey (en la misma partición) para permitir búsquedas rápidas y eficaces y ordenaciones alternativas mediante el uso de diferentes valores RowKey.  
-	[Patrón de índice secundario dentro de la partición](#inter-partition-secondary-index-pattern) - Almacenar varias copias de cada entidad con distintos valores de RowKey en particiones independientes en tablas independientes para permitir la realización de búsquedas rápidas y eficaces y órdenes alternativos utilizando valores RowKey diferentes. 
-	[Patrón de cola de registro](#log-tail-pattern) - Recuperar las *n* entidades agregadas recientemente a una partición utilizando un valor **RowKey** que se ordene en orden de fecha y hora inverso.  
...
Para actualizar o eliminar una entidad, debe poder identificarla mediante el uso de los valores **PartitionKey** y **RowKey**. En este sentido, la elección de **PartitionKey** y **RowKey** para modificar entidades debería seguir criterios similares a su elección para admitir consultas de punto porque desea identificar las entidades de la forma más eficaz posible. No desea utilizar un análisis ineficaz de partición o de tabla para buscar una entidad con el fin de detectar los valores **PartitionKey** y **RowKey** que necesita actualizar o eliminarla.  

Los siguientes patrones de la sección [Patrones de diseño de tabla](#table-design-patterns) tratan la optimización del rendimiento o las operaciones de inserción, actualización y eliminación:  
...
Los siguientes patrones de la sección [Patrones de diseño de tabla](#table-design-patterns) tratan la forma de administrar la coherencia:  
...
Para obtener más información, consulte más adelante el [Patrón de desnormalización](#denormalization-pattern) que encontrará en esta guía.  
...
-	Implementar controles de acceso (para obtener más información, consulte [Control de acceso con las firmas de acceso compartido](#controlling-access-with-shared-access-signatures)).  
...
Para obtener más información acerca de cómo trabajar con varios tipos de entidad en la misma tabla en el código de cliente, consulte la sección [Trabajar con tipos de entidad heterogéneos](#working-with-heterogeneous-entity-types) que encontrará en esta guía. Esto proporciona ejemplos de cómo reconocer el tipo de entidad en el código de cliente.  
...
Almacene varias copias de cada entidad con diferentes valores **RowKey** (en la misma partición) para permitir búsquedas rápidas y eficaces y ordenaciones alternativas mediante el uso de diferentes valores **RowKey**. Las actualizaciones entre copias se pueden mantener coherentes mediante EGT.  

#### Contexto y problema
El servicio Tabla indexa automáticamente entidades mediante los valores **PartitionKey** y **RowKey**. Esto permite que una aplicación cliente recupere una entidad eficazmente con estos valores. Por ejemplo, mediante la estructura de tabla que se muestra a continuación, una aplicación cliente puede utilizar una consulta de punto para recuperar una entidad de empleado individual con el nombre del departamento y el identificador de empleado (los valores **PartitionKey** y **RowKey**). Un cliente también puede recuperar las entidades ordenadas por identificador de empleado dentro de cada departamento.
...
-	Para buscar a todos los empleados del departamento de ventas con un identificador de empleado comprendido dentro del rango de 000100 a 000199, utilice:
$filter=(PartitionKey eq 'Sales') y (RowKey ge 'empid_000100') y (RowKey le 'empid_000199')  
-	Para buscar a todos los empleados del departamento de ventas que dispongan de una dirección de correo electrónico que comienza por la letra 'a' use:
$filter=(PartitionKey eq 'Sales') y (RowKey ge 'email_a') y (RowKey lt 'email_b')  
...
-	Puede mantener la coherencia de las entidades duplicadas utilizando EGT para actualizar las dos copias de la entidad de forma atómica. Esto implica que debe almacenar todas las copias de una entidad en la misma partición. Para obtener más información, consulte la sección [Uso de transacciones de grupos de entidades](#entity-group-transactions).  
-	El valor utilizado para **RowKey** debe ser único para cada entidad. Considere la posibilidad de usar valores de clave compuestos.  
-	Rellenar valores numéricos en **RowKey** (por ejemplo, el identificador de empleado 000223) permite corregir la ordenación y la filtración en función de los límites inferior y superior.  
...
-	[Patrón de índice secundario dentro de la partición](#inter-partition-secondary-index-pattern)
...
-	Para buscar todos los empleados del departamento de ventas con un identificador de empleado en el intervalo de **000100** a **000199** ordenados en orden de Id. de empleado utilice: 
$filter=(PartitionKey eq 'empid_Sales') y (RowKey ge '000100') y (RowKey le '000199')  
-	Para buscar a todos los empleados del departamento de ventas con una dirección de correo electrónico que empiece por 'a' sorted in email address order use:
$filter=(PartitionKey eq 'email_Sales') y (RowKey ge 'a') and (RowKey lt 'b')  
...
-	Puede mantener las entidades duplicadas finalmente de modo que sean coherentes entre sí mediante el [patrón de transacciones coherente](#eventually-consistent-transactions-pattern) para mantener las entidades de índice principal y secundaria.  
-	El almacenamiento de tablas es relativamente barato, por lo que la sobrecarga de costes de almacenamiento de datos duplicados no debe ser una preocupación importante. Sin embargo, debe evaluar siempre el coste del diseño según los requisitos de almacenamiento previstos y solo agregar entidades duplicadas para admitir las consultas que ejecutará la aplicación cliente.  
-	El valor utilizado para **RowKey** debe ser único para cada entidad. Considere la posibilidad de usar valores de clave compuestos.  
-	Rellenar valores numéricos en **RowKey** (por ejemplo, el identificador de empleado 000223) permite corregir la ordenación y la filtración en función de los límites inferior y superior.  
-	No es necesario duplicar todas las propiedades de su entidad. Por ejemplo, si las consultas que realizan búsquedas en las entidades mediante la dirección de correo electrónico de **RowKey** nunca necesitan la edad del empleado, estas entidades podrían tener la siguiente estructura:
...
#### Cuándo usar este patrón  
Utilice este patrón cuando la aplicación cliente necesite recuperar entidades mediante una serie de claves diferentes, cuando el cliente necesite recuperar entidades de diferentes criterios de ordenación y cuando pueda identificar cada entidad mediante una serie de valores únicos. Utilice este patrón cuando desee evitar exceder los límites de escalabilidad de partición al realizar búsquedas de entidad utilizando los diferentes valores **RowKey**.  
...
Para ilustrar este enfoque, suponga que tiene un requisito para poder almacenar entidades de empleado antiguas. Las entidades de empleado antiguas rara vez se consultan y deben excluirse de las actividades relacionadas con los empleados actuales. Para implementar este requisito, almacene empleados activos en la tabla **Actual** y empleados antiguos en la tabla **Archivo**. Archivar un empleado requiere eliminar la entidad de la tabla **Actual** y agregar la entidad a la tabla **Archivo**, pero no se puede usar un EGT para realizar estas dos operaciones. Para evitar el riesgo de que un error provoque la aparición de una entidad en las dos tablas o en ninguna, la operación de almacenamiento debe ser coherente con el tiempo. En el diagrama de secuencia siguiente se describen los pasos de esta operación. En el texto siguiente se proporcionan más detalles para las rutas de excepción.  

![][12]
 
Un cliente inicia la operación de almacenamiento mediante la colocación de un mensaje en una cola de Azure, en este ejemplo para archivar el empleado #456. Un rol de trabajador sondea la cola de mensajes nuevos; si encuentra alguno, lee el mensaje y deja una copia oculta en la cola. El rol de trabajador busca a continuación una copia de la entidad de la tabla **Actual**, inserta una copia en la tabla **Archivo** y, a continuación, elimina el original de la tabla **Actual**. Por último, si no ha habido errores en los pasos anteriores, el rol de trabajador elimina el mensaje oculto de la cola.  
...
Es importante que las operaciones de los pasos **4** y **5** debe realizarse *idempotent* en caso de que el rol de trabajador necesite reiniciar la operación de almacenamiento. Si está utilizando el servicio Tabla para el paso **4** debe utilizar una operación de "Insertar o reemplazar"; en el paso **5** debe usar una operación "eliminar si existe" en la biblioteca de cliente que está utilizando. Si está utilizando otro sistema de almacenamiento, debe utilizar una operación idempotente adecuada.  

Si el rol de trabajador nunca completa el paso **6**, a continuación, después de un tiempo de espera el mensaje vuelve a aparecer en la cola lista para que el rol de trabajo intente volver a procesarla. El rol de trabajador puede comprobar cuántas veces se ha leído un mensaje de la cola y, si es necesario, marcarlo como mensaje "dudoso" para investigarlo mediante el envío a una cola independiente. Para obtener más información acerca de cómo leer mensajes de la cola y comprobar el número de eliminaciones de cola, consulte [Obtener mensajes](https://msdn.microsoft.com/library/azure/dd179474.aspx).  
...
-	Esta solución no permite el aislamiento de las transacciones. Por ejemplo, un cliente puede leer las tablas **Actual** y **Archivo** cuando el rol de trabajo se encontró entre los pasos **4** y **5** y ver una vista incoherente de los datos. Tenga en cuenta que los datos serán coherentes con el tiempo.  
...
El servicio Tabla indexa automáticamente entidades mediante los valores **PartitionKey** y **RowKey**. Esto permite que una aplicación cliente recupere una entidad eficazmente mediante una consulta de punto. Por ejemplo, mediante la estructura de tabla que se muestra a continuación, una aplicación cliente puede recuperar eficazmente una entidad employee individual con el nombre del departamento y el identificador de empleado (los valores **PartitionKey** y **RowKey**).  
...
<u>Opción 1: Usar el almacenamiento de blobs</u>  
...
1.	Recupere la entidad de índice con un valor **PartitionKey** "Ventas" y el valor **RowKey** "Jones". Guarde el valor ETag de esta entidad para usar en el paso 2.  
2.	Cree un EGT que inserte la nueva entidad employee (valor **PartitionKey** "Ventas" y **RowKey** "000152") y actualiza la entidad de índice (valor **PartitionKey** "ventas" y **RowKey** "Jones") agregando el nuevo identificador de empleado a la lista en el campo EmployeeIDs.  
3.	Si el ETG falla debido a un error de simultaneidad optimista (alguien ha modificado la entidad de índice), necesitará comenzar de nuevo en el paso 1.  
...
1.	Recupere la entidad de índice con un valor **PartitionKey** "Ventas" y el valor **RowKey** "Jones".  
...
<u>Opción 3:</u> Crear entidades de índice en una partición o tabla independiente.  
...
Tenga en cuenta los puntos siguientes al decidir cómo implementar este patrón:  
-	Esta solución requiere al menos dos consultas para recuperar las entidades coincidentes: una para consultar las entidades de índice para obtener la lista de valores **RowKey** y, a continuación, las consultas para recuperar cada entidad de la lista.  
...
-	Puede implementar una solución basada en cola que ofrezca coherencia eventual (consulte el [patrón de coherencia final de transacciones](#eventually-consistent-transactions-pattern) para obtener más información).  
...
Utilice valores **RowKey** compuestos para permitir a un cliente buscar datos relacionados con una consulta de punto único.  
...
Observe cómo **RowKey** ahora es una clave compuesta formada por el identificador de empleado y el año de los datos de revisión que le permite recuperar el rendimiento del empleado y revisar los datos con una única solicitud para una entidad única.  
...
-	Para mantener la coherencia eventual entre la entidad en el servicio Tabla y los datos del servicio Blob, utilice el [patrón de transacciones coherentes](#eventually-consistent-transactions-pattern) para mantener las entidades.    
...
Observe con este ejemplo cómo tanto **PartitionKey** como **RowKey** son claves compuestas. **PartitionKey** utiliza el Id. de departamento y de empleados para distribuir el registro en varias particiones.  
...
Un caso de uso común para los datos del registro es recuperar una selección de entradas de registro para un intervalo de fecha y hora específico: por ejemplo, desea buscar todos los mensajes de error y críticos que ha registrado la aplicación entre las 15:04 y las 15:06 en una fecha concreta. No desea utilizar la fecha y hora del mensaje del registro para determinar la partición en la que guarda las entidades de registro: esto da como resultado una partición activa porque en un momento dado, todas las entidades de registro compartirán el mismo valor **PartitionKey** (consulte la sección "[Antipatrón de anteponer/anexar](#prepend-append-anti-pattern)). Por ejemplo, el siguiente esquema de entidad para un mensaje de registro produce una partición activa debido a que la aplicación escribe todos los mensajes de registro en la partición en la fecha y la hora actuales:  
...
Como se describe en la sección "[Diseño de consulta](#design-for-querying)", la consulta más eficaz es una de punto. Sin embargo, en algunos casos puede que necesite recuperar varias entidades. En esta sección se describen algunos enfoques comunes para recuperar entidades mediante la biblioteca de cliente de almacenamiento.  

#### Ejecutar una consulta de punto mediante la biblioteca de cliente de almacenamiento  

La manera más sencilla de ejecutar una consulta de punto es usar la operación de tabla **Recuperar** como se muestra en el siguiente fragmento de código C# que recupera una entidad con un **PartitionKey** del valor "Ventas" y un **RowKey** del valor "212":  
...
Observe cómo la consulta especifica un **RowKey** y un **PartitionKey** para asegurar un rendimiento mejor.  
...
>[AZURE.NOTE] Normalmente, un token de continuación devuelve un segmento que contiene 1.000 entidades, aunque pueden ser menos. Este también es el caso si limita el número de entradas que devuelve una consulta mediante **Take** para devolver las primeras n entidades que cumplen los criterios de búsqueda: el servicio Tabla puede devolver un segmento que contenga menos de n entidades junto con un token de continuación que le permita recuperar las entidades restantes.  
...
Observe cómo el valor **RowKey** está disponible incluso aunque no se incluyó en la lista de propiedades a recuperar.  
...
El método **Reemplazar** de la clase **TableOperation** siempre reemplaza la entidad completa en el servicio Tabla. Si no incluye una propiedad en la solicitud cuando esa propiedad existe en la entidad almacenada, la solicitud quita esa propiedad de la entidad almacenada. A menos que desee quitar una propiedad de forma explícita de entidad almacenada, debe incluir todas las propiedades en la solicitud.  

Puede utilizar el método **Mezcla** de la clase **TableOperation** para reducir la cantidad de datos que envía al servicio Tabla si desea actualizar una entidad. El método **Mezcla** reemplaza cualquier propiedad de la entidad almacenada por valores de propiedad de la entidad que se incluyen en la solicitud, pero deja intactas las propiedades de la entidad almacenada que no están incluidas en la solicitud. Esto es útil si tiene entidades de gran tamaño y solo tiene que actualizar un pequeño número de propiedades en una solicitud.  

>[AZURE.NOTE] Los métodos **Reemplazar** y **Mezcla** producirá un error si la entidad no existe. Como alternativa, puede utilizar los métodos **InsertOrReplace** y **InsertOrMerge** que crean una nueva entidad si todavía no existe.  

### Trabajar con tipos de entidad heterogéneos  

El servicio Tabla es un almacén de *schema-less* tablas que significa que una sola tabla puede almacenar entidades de varios tipos que proporcionen gran flexibilidad en el diseño. En el ejemplo siguiente se muestra una tabla que almacena entidades de empleado y de departamento:  
...
Escriba la primera opción, anteponiendo el tipo de entidad a **RowKey**. Esto resulta útil si hay una posibilidad de que dos entidades de diferentes tipos tengan el mismo valor de clave. También agrupa las entidades del mismo tipo juntas en la partición.  
 
Las técnicas descritas en esta sección son especialmente importantes para el tema [Relaciones de herencia](#inheritance-relationships) que se ha mostrado anteriormente en la sección [Relaciones de modelos] de esta guía(#modelling-relationships).  
...
La segunda opción es usar el tipo **DynamicTableEntity** (un contenedor de propiedades) en lugar de un tipo concreto de entidad POCO (esta opción puede mejorar también el rendimiento porque no es necesario serializar y deserializar la entidad a los tipos. NET). El siguiente código de C# potencialmente recupera varias entidades de tipos diferentes de la tabla, pero devuelve todas las entidades como instancias **DynamicTableEntity**. A continuación, usa la propiedad **EventType** para determinar el tipo de cada entidad:  
...
Tenga en cuenta que para recuperar otras propiedades debe utilizar el método **TryGetValue** en la propiedad **Propiedades** de la clase **DynamicTableEntity**.  

Una tercera opción consiste en combinar con el tipo **DynamicTableEntity** y una instancia **EntityResolver**. Esto le permite resolver en varios tipos POCO en la misma consulta. En este ejemplo, el delegado **EntityResolver** está usando la propiedad **EventType** para distinguir entre los dos tipos de entidad que devuelve la consulta. El método **Resolver** usa el delegado **Resolución** para resolver instancias **DynamicTableEntity** en instancias **TableEntity**.  
...
No es necesario conocer el tipo de una entidad para eliminarla, y siempre sabe el tipo de una entidad al insertarla. Sin embargo, puede usar el tipo **DynamicTableEntity** para actualizar una entidad sin conocer su tipo y sin utilizar una clase de entidad POCO. El ejemplo de código siguiente recupera una entidad única y comprueba que la propiedad **EmployeeCount** existe antes de actualizarla.  
...
-	En lugar de llamar al método **ExecuteSegmented** para recuperar los resultados, ahora el método llama al método **ExecuteSegmentedAsync** y utiliza el modificador **await** para recuperar resultados de forma asincrónica.  

La aplicación cliente puede llamar a este método varias veces (con valores diferentes para el parámetro **departament**), y cada consulta se ejecutará en un subproceso independiente.  
...
-	La firma del método incluye ahora el modificador **async** y devuelve una instancia de **Tarea**.  
-	En lugar de llamar al método **Execute** para actualizar la entidad, ahora el método llama al método **ExecuteAsync** y utiliza el modificador **await** para recuperar resultados de forma asincrónica.  
...
### Créditos
Nos gustaría dar las gracias a los miembros del equipo de Azure por sus contribuciones: Dominic Betts, Jason Hogg, Jean Ghanem, Jai Haridas, Jeff Irwin, Vamshidhar Kommineni, Vinay Shah y Serdar Ozler.  
