# Información general
## [Introducción a Logic Apps](logic-apps-what-are-logic-apps.md)
## [Lista de conectores](../connectors/apis-list.md)
## [Enterprise Integration Pack](logic-apps-enterprise-integration-overview.md)

# Introducción
## [Creación de una aplicación lógica](logic-apps-create-a-logic-app.md)
## [Uso de las características de aplicaciones lógicas](logic-apps-use-logic-app-features.md)
## [Introducción a las plantillas](logic-apps-use-logic-app-templates.md)
## [Creación de una aplicación mediante una plantilla](logic-apps-arm-provision.md)
## [Ejemplos y escenarios](logic-apps-examples-and-scenarios.md)
## [Escenario de Service Bus](logic-apps-scenario-function-sb-trigger.md)
## [Procesamiento B2B](logic-apps-enterprise-integration-b2b.md)
## [Procesamiento XML](logic-apps-enterprise-integration-xml.md)

# Procedimientos
## Compilación
### [Creación de un conector](logic-apps-create-api-app.md)
### [Uso del conector de sistema de archivos](logic-apps-using-file-connector.md)
### [Uso del conector de SAP](logic-apps-using-sap-connector.md)
### [Herramientas de Visual Studio](logic-apps-deploy-from-vs.md)
### [Uso de Azure Functions](logic-apps-azure-functions.md)
### [Aplicaciones lógicas como puntos de conexión invocables](logic-apps-http-endpoint.md)
### [Bucles, ámbitos y desagrupación](logic-apps-loops-and-scopes.md)
### [Seleccionar la API personalizada](logic-apps-custom-hosted-api.md)
### [Control de excepciones](logic-apps-exception-handling.md)
### [Escenario de control de excepciones](logic-apps-scenario-error-and-exception-handling.md)
### [Tipos de contenido de control](logic-apps-content-type.md)
### [Definición del flujo de trabajo](logic-apps-author-definitions.md)
### [Sugerencias para la solución de problemas](logic-apps-diagnosing-failures.md)
### [Límites y configuración](logic-apps-limits-and-config.md)


## Uso de Enterprise Integration Pack (EIP)
### [Introducción a las cuentas de integración](logic-apps-enterprise-integration-create-integration-account.md)
### [Contratos](logic-apps-enterprise-integration-agreements.md)
### [Procesamiento B2B](logic-apps-enterprise-integration-b2b.md)
### [Procesamiento XML](logic-apps-enterprise-integration-xml.md)
### [Procesamiento de archivos planos](logic-apps-enterprise-integration-flatfile.md)
### [Incorporación de mapas XSLT](logic-apps-enterprise-integration-maps.md)
### [Transformación de XML](logic-apps-enterprise-integration-transform.md)
### [Validación de XML](logic-apps-enterprise-integration-xml-validation.md)
### [Uso de certificados](logic-apps-enterprise-integration-certificates.md)
### [Incorporación de asociados](logic-apps-enterprise-integration-partners.md)
### [Incorporación de esquemas](logic-apps-enterprise-integration-schemas.md)
### [Almacenamiento de metadatos](logic-apps-enterprise-integration-metadata.md)
### [Integración de AS2](logic-apps-enterprise-integration-as2.md)
### [Codificación de AS2](logic-apps-enterprise-integration-as2-encode.md)
### [Descodificación de AS2](logic-apps-enterprise-integration-as2-decode.md)
### [Integración de EDIFACT](logic-apps-enterprise-integration-edifact.md)
### [Codificación de EDIFACT](logic-apps-enterprise-integration-edifact-encode.md)
### [Descodificación de EDIFACT](logic-apps-enterprise-integration-edifact-decode.md)
### [Integración de X12](logic-apps-enterprise-integration-x12.md)
### [Codificación de X12](logic-apps-enterprise-integration-x12-encode.md)
### [Descodificación de X12](logic-apps-enterprise-integration-x12-decode.md)


## Uso de puerta de enlace
### [Conexión de local](logic-apps-gateway-connection.md)
### [Instalación](logic-apps-gateway-install.md)

## Automatizar
### [Plantilla de Automation](logic-apps-create-deploy-template.md)
### [Publicación desde VS](logic-apps-deploy-from-vs.md)

## Administrar
### [Uso de Cloud Explorer para Visual Studio](logic-apps-manage-from-vs.md)
### [Protección de la aplicación lógica](logic-apps-securing-a-logic-app.md)
### [Supervisión de aplicaciones lógicas](logic-apps-monitor-your-logic-apps.md)
### [Supervisión de mensajes B2B](logic-apps-monitor-b2b-message.md)
#### [Esquema de seguimiento de AS2](logic-apps-track-integration-account-as2-tracking-schemas.md)
#### [Esquemas de seguimiento de X12](logic-apps-track-integration-account-x12-tracking-schema.md)
#### [Esquema de seguimiento personalizado](logic-apps-track-integration-account-custom-tracking-schema.md)
#### [Seguimiento de mensajes B2B](logic-apps-track-b2b-messages-omsportal.md)
#### [Consulta del portal de OMS](logic-apps-track-b2b-messages-omsportal-query-filter-control-number.md)

### [Cuentas de integración](logic-apps-enterprise-integration-accounts.md)
### [Precios](logic-apps-pricing.md)

# Referencia
## [PowerShell](/powershell/resourcemanager/azurerm.logicapp/v2.3.0/azurerm.logicapp)
## Historial de esquemas
### [Disponibilidad general](logic-apps-schema-2016-04-01.md)
### [Versión preliminar](logic-apps-schema-2015-08-01.md)
## [REST](/rest/api/logic/)

# Recursos
## [Precios](https://azure.microsoft.com/pricing/details/logic-apps/)  
## [Foro de MSDN](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=azurelogicapps)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-logic-apps)
## [Actualizaciones del servicio](https://azure.microsoft.com/updates/?product=logic-apps)


<!--HONumber=Jan17_HO3-->


