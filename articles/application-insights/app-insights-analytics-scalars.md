...
Las expresiones escalares son distintas de las [consultas](app-insights-analytics-queries.md), cuyos resultados son tablas.
...
Una cadena que representa el tipo de almacenamiento subyacente de su único argumento. Esto es especialmente útil cuando se tienen valores de tipo `dynamic`: en este caso, `gettype()` mostrará cómo se codifica un valor.
...
Esta función devuelve el valor de *ifTrue* si *predicate* equivale a `true`, o al valor de *ifFalse* en caso contrario.
...
Redondea los valores hacia abajo hasta un entero múltiplo del tamaño de un intervalo determinado. Se usa mucho en la consulta [`summarize by`](app-insights-analytics-queries.md#summarize-operator). Si tiene un conjunto de valores dispersos, se agruparán en un conjunto más pequeño de valores específicos.
...
Resta un intervalo de tiempo especificado a la hora UTC actual. Al igual que `now()`, esta función puede usarse varias veces en una instrucción, y la hora UTC a la que se hace referencia será la misma para todas las instancias.
...
El `timespan` desde medianoche al principio del domingo anterior, redondeado a la baja a un número entero de días.
...
La hora UTC actual, con una diferencia opcional determinada por un intervalo de tiempo especificado. Esta función puede usarse varias veces en una instrucción, y la hora UTC a la que se hace referencia será la misma para todas las instancias.
...
La barra diagonal inversa (``) se usa para marcar los caracteres de escape como `\t` (tabulación), `\n` (nueva línea) y las instancias del carácter de comillas.
...
* *text:* una cadena.
* *search:* la cadena sin formato o la [expresión regular](app-insights-analytics-reference.md#regular-expressions) que coincide en *text*.
...
Obtenga una coincidencia para una [expresión regular](app-insights-analytics-reference.md#regular-expressions) desde una cadena de texto. Opcionalmente, convierte a continuación la subcadena extraída al tipo indicado.
...
* *regex:* una [expresión regular](app-insights-analytics-reference.md#regular-expressions).
...
Si *regex* encuentra una coincidencia en *text*: la subcadena que coincide con el valor *captureGroup* del grupo de captura indicado, opcionalmente convertido en *typeLiteral*.
...
Este ejemplo es equivalente a `substring(Text, 2, 4)`:
...
* *rewrite:* la expresión regular de reemplazo de cualquier coincidencia encontrada por *matchingRegex*. Use `\0` para hacer referencia a toda la coincidencia, `\1` para el primer grupo de captura, `\2` y valores sucesivos para los grupos de captura posteriores.
* *text:* una cadena.
...
**Casting** En algunos casos es necesario convertir un elemento que se extrae de un objeto, ya que su tipo puede variar. Por ejemplo, `summarize...to` necesita un tipo específico:
...
Observe que `indexer` se usa para marcar el punto en el que debería usar un índice numérico. En el caso de este esquema, algunas rutas válidas podrían las siguientes (suponiendo que estos índices de ejemplo se encuentren en el intervalo):
...
En este ejemplo se crea un valor dinámico y, a continuación, se usan sus campos:
...
|[`mvexpand` listColumn](app-insights-analytics-queries.md#mvexpand-operator) | Replica una fila para cada valor de una lista en una celda especificada.
|[`summarize buildschema(`column`)`](app-insights-analytics-queries.md#summarize-operator) |Deduce el esquema de tipo del contenido de la columna
|[`summarize makelist(`column`)` ](app-insights-analytics-queries.md#summarize-operator)| Acopla grupos de filas y coloca los valores de la columna en una matriz.
|[`summarize makeset(`column`)`](app-insights-analytics-queries.md#summarize-operator) | Acopla grupos de filas y coloca los valores de la columna en una matriz, sin duplicación.

### Objetos dinámicos en cláusulas let


[Las cláusulas Let](app-insights-analytics-queries.md#let-clause) almacenan valores dinámicos como cadenas, por lo que estas dos cláusulas son equivalentes, y ambas requieren `parsejson` (o `todynamic`) antes de que se usen:
...
entonces, el siguiente fragmento recupera el valor de la ranura `duration` del objeto, y desde ahí recupera dos ranuras, `duration.value` y `duration.min` (`118.0` y `110.0` respectivamente).
...
Observe que "[0]" indica la presencia de una matriz, pero no especifica el índice usado por una ruta específica.
...
Cuando [muestreo](app-insights-sampling.md) está en funcionamiento, se recopila solo una fracción de los datos generados por el SDK en el portal de Application Insights. El muestreo se puede aplicar en el SDK o en la recopilación para el punto de conexión del portal.

`itemCount` indica el número de eventos que se generaron en el SDK para cada evento que aparece en la secuencia. Por ejemplo, si la frecuencia de muestreo actual es el 25 %, entonces itemCount == 4.

Cuando desee contar eventos, use `sum(itemCount)` en lugar de `count()` para obtener una estimación precisa de los números reales. Por ejemplo:

* El número de excepciones que se han producido en las últimas dos horas:
