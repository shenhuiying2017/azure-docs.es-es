...
# Procesamiento de la telemetría en el SDK de Application Insights
...
* [Muestreo](#sampling) reduce el volumen de telemetría sin estadísticas con sesgo. Mantiene juntos los puntos de datos relacionados para facilitar las búsquedas.
* [Procesadores de telemetría](#telemetry-processors) inicializan, filtran y realizan muestreo de datos en el SDK antes de enviarlo al servidor. Por ejemplo, puede agregar propiedades calculadas a alguna telemetría o reducir el volumen de telemetría excluyendo las solicitudes de robots. 
* [Inicializadores de telemetría](#telemetry-initializers) agregan propiedades predeterminadas a toda la telemetría que se envía y pueden invalidar el comportamiento de algunos módulos de telemetría estándar. Las propiedades se evalúan para cada llamada de seguimiento, por lo que puede cambiar estos valores.
* [Inicializadores de contexto](#context-initializers) también establecen propiedades globales. Las propiedades se establecen una vez, en el momento del inicio y no se pueden cambiar más adelante. Aunque esto ahorra algún tiempo de CPU (en contraste con los inicializadores de telemetría) también son muy poco flexibles.
* [La API del SDK](app-insights-api-custom-events-metrics.md) se usa para enviar métricas y eventos personalizados.

Hay cierta redundancia entre estos diferentes mecanismos, y es probable que los inicializadores cambien en el futuro.

Antes de comenzar:

* Instale el [SDK de Application Insights](app-insights-asp-net.md) en su aplicación. Los procesadores de telemetría lo necesitan 
...
Para los servidores ASP.NET, inserte esto en una ubicación de inicialización adecuada como Application\_Start:
...
Para las páginas web, ponga una línea extra en el [fragmento de Application Insights](app-insights-javascript.md) insertado (normalmente en una página maestra como \_Layout.cshtml):
...
* Establezca un porcentaje (10 en estos ejemplos) que es igual a 100/N donde N es un número entero: por ejemplo 50 (= 1/2), 33,33 (= 1/3), 25 (= 1/4), 10 (= 1/5).
...
[Más información acerca del muestreo](app-insights-sampling.md).

## Procesadores de telemetría

Escriba procesadores de telemetría para filtrar y preprocesar la telemetría generada por el SDK de Application Insights antes de enviarla al portal de back-end. Esto incluye la telemetría de los módulos estándar como el recopilador de solicitud HTTP y el recolector de dependencia. Por ejemplo, puede filtrar para dejar fuera la telemetría acerca de las solicitudes de robots o las llamadas de dependencia correctas.
...
> 
> En su lugar, considere el uso de [muestreo](#sampling).
...
En general, [se recomienda usar inicializadores de telemetría en lugar de inicializadores de contexto](http://apmtips.com/blog/2015/06/09/do-not-use-context-initializers/).
