...
En este tutorial se muestra cómo crear un sitio web PHP-MySQL de Azure y cómo implementarlo mediante FTP. En este tutorial se presupone que ha instalado [PHP][install-php], [MySQL][install-mysql], un servidor web y un cliente FTP en el equipo. Las instrucciones de este tutorial se pueden seguir en cualquier sistema operativo, incluidos Windows, Mac y Linux. Una vez completada esta guía, tendrá un sitio web PHP/MySQL ejecutándose en Azure.
...
Una vez creado el sitio web, se mostrará el texto **Creación correcta del sitio web '[[NOMBRE DEL SITIO]]'**. Ahora puede habilitarse la publicación FTP.
...
4. Abra un explorador web y diríjase a [http://localhost/registration/createtable.php][localhost-createtable]. De esta forma, se creará la tabla `registration_tbl` en la base de datos.
...
5. Abra el archivo **index.php** en un editor de texto o IDE y agregue el código CSS y HTML básicos para la página (el código PHP se agregará en pasos posteriores).
		...
		<p>Introduzca su nombre y su dirección de correo electrónico y, a continuación, haga clic en <strong>Enviar</strong> para registrar.</p>
...
2. Tome nota de los valores de `Database`, `Data Source`, `User Id` y `Password`.

3. Desde el panel del sitio web, haga clic en el vínculo **Descargar perfil de publicación** que aparece en la esquina inferior derecha de la página:

	![Download publish profile][download-publish-profile]

4. Abra el archivo `.publishsettings` en un editor XML. 

3. Busque el elemento `<publishProfile >` con `publishMethod="FTP"` que tiene un aspecto similar a este:
...
3. Especifique los atributos `userName` y `userPWD` que anotó anteriormente en el cliente FTP.
...
Cuando haya cargado `index.php` and `createtable.php`, diríjase a **http://[nombre de sitio].azurewebsites.net/createtable.php** para crear una tabla MySQL para la aplicación y, a continuación, diríjase a **http://[nombre de sitio].azurewebsites.net/index.php** para comenzar a usar la aplicación.
