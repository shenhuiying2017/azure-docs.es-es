...
| Enabled | Al crearse, el runbook está habilitado de forma predeterminada. Si se establece como Disabled, ningún cliente podrá usarlo. Puede establecer la propiedad **Enabled** al crear el webhook o en cualquier momento una vez creado. |
...
Cuando un cliente inicia un runbook mediante un webhook, no puede reemplazar los valores de parámetros definidos en el webhook. Para recibir datos del cliente, el runbook puede aceptar un único parámetro denominado **$WebhookData** de tipo [object] que contendrá los datos que el cliente incluye en la solicitud POST.
...
| RequestHeader | Los encabezados de la solicitud POST entrante. |
| RequestBody | El cuerpo de la solicitud POST entrante. |
...
>[AZURE.NOTE]Los valores de todos los parámetros de entrada se registran con el trabajo de runbook. Esto significa que cualquier entrada proporcionada por el cliente registrará con $WebhookData y estará disponible para cualquier persona con acceso a la tarea de automatización. Por este motivo, debe tener cuidado en cómo incluir información confidencial en las llamadas de webhook.
...
3. Haga clic en **Parámetros** para proporcionar valores para los parámetros de runbook. <br>
1. Haga clic en **Aceptar** cuando haya terminado de configurar el webhook.


## Uso de un webhook

Para utilizar un webhook, la aplicación cliente debe emitir una solicitud HTTP POST con la dirección URL para el webhook. La sintaxis del webhook estará en el formato siguiente.

	http://<Webhook Server>/token?=<Token Value>


El cliente recibirá uno de los siguientes códigos en respuesta a la solicitud POST.

| Código | Texto | Descripción |
|:---|:----|:---|
| 202 | Accepted | Se aceptó la solicitud y el runbook se inició correctamente. |
...
El cliente no puede determinar cuándo se completa el trabajo de runbook o su estado de finalización a partir de un webhook porque el webhook no devuelve un identificador para el trabajo de runbook. Solo se puede validar que la solicitud se envió correctamente.

### Ejemplo

En el ejemplo siguiente se inicia un runbook mediante un webhook con Windows PowerShell. Este ejemplo incluye datos en el encabezado y el cuerpo que puede utilizar el runbook. Tenga en cuenta que cualquier lenguaje que pueda realizar una solicitud HTTP puede utilizar un webhook.
...
El siguiente runbook de muestra acepta la solicitud anterior y recupera los datos del webhook.
