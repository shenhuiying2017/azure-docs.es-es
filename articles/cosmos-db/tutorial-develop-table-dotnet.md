...
- [Distribución global](../documentdb/documentdb-distribute-data-globally.md) llave en mano con hospedaje múltiple y [conmutaciones por error manuales y automáticas](../documentdb/documentdb-regional-failovers.md)
...
- Compatibilidad con [rendimiento dedicado por tabla](../documentdb/documentdb-request-units.md) que se puede escalar de cientos a millones de solicitudes por segundo
- Compatibilidad con [cinco niveles de coherencia bien definidos](../documentdb/documentdb-consistency-levels.md) para compensar la disponibilidad, la latencia y la coherencia en función de lo que necesita la aplicación
- Disponibilidad del 99,99 % dentro de una única región y la capacidad de agregar más regiones para tener una mayor disponibilidad, además de [Acuerdos de Nivel de Servicio completos líderes en el sector](https://azure.microsoft.com/support/legal/sla/documentdb/v1_1/) sobre la disponibilidad general
...
| TableConsistencyLevel | Puede compensar entre la latencia, la coherencia y la disponibilidad si elige entre cinco niveles de coherencia bien definidos: `Strong`, `Session`, `Bounded-Staleness`, `ConsistentPrefix` y `Eventual`. El valor predeterminado es `Session`. La elección del nivel de coherencia marca una diferencia importante en el rendimiento en las configuraciones con varias regiones. Consulte [Niveles de coherencia](../documentdb/documentdb-consistency-levels.md) para obtener detalles. |
| TableThroughput | Rendimiento reservado para la tabla expresado en unidades de solicitud (RU) por segundo. Las tablas únicas puede admitir cientos de millones de RU/s. Consulte [Unidades de solicitud](../documentdb/documentdb-request-units.md). El valor predeterminado es `400` |
| TableIndexingPolicy | Indexado secundario coherente y automático de todas las columnas dentro de las tablas | Cadena JSON en conformidad con la especificación de la directiva de indexación. Consulte [Directiva de indexación](../documentdb/documentdb-indexing-policies.md) para ver cómo puede cambiar la directiva de indexación para incluir o excluir columnas específicas. | Indexado automático de todas las propiedades (hash para cadenas e intervalo para números) |
...
* El modelo de reserva es más [rentable para las cargas de trabajo de alto uso de rendimiento](../documentdb/documentdb-key-value-store-cost.md)
...
Una lectura de una entidad de 1 KB se normaliza como 1 RU y otras operaciones se normalizan como un valor de RU fijo basado en el consumo de CPU, memoria e IOPS. Más información sobre [Unidades de solicitud en Azure Cosmos DB(../documentdb/documentdb-request-units.md).
...
En la versión preliminar, Azure Cosmos DB admite la misma funcionalidad de consulta que Azure Table Storage para Table API. Azure Cosmos DB también admite la ordenación, los agregados, la consulta geoespacial, la jerarquía y una amplia variedad de funciones integradas. Una futura actualización de servicio de Table API ofrecerá funcionalidades adicionales. En [Consulta de Azure Cosmos DB](../documentdb/documentdb-sql-query.md) puede encontrar información general sobre esas funcionalidades. 
