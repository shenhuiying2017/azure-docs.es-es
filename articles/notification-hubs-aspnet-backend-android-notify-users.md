...
> [AZURE.NOTE] Este tutorial asume que ha creado y configurado el centro de notificaciones tal y como se describe en [Introducción a los Centros de notificaciones (Android)][].
...
1.  Siga el tutorial [Introducción a los Centros de notificaciones (Android)][] para crear y configurar la aplicación para recibir notificaciones de inserción desde GCM.

2.  Abra el archivo res/layout/activity\_main.xml y sustituya el contenido por lo siguiente:
...
3.  Abra el archivo res/values/strings.xml y agregue las líneas siguientes:
...
4.  Ahora cree una clase **RegisterClient** en el mismo paquete que la clase **MainActivity**. Asegúrese de reemplazar `{backend endpoint}` por el extremo back-end obtenido en la sección anterior.
...
5.  En la clase **MainActivity** quite los campos privados para **NotificationHub** y agregue un campo para **RegisterClient**:

        //private NotificationHub hub;
        private RegisterClient registerClient;

6.  Luego, en el método **onCreate**, quite la inicialización del campo **hub** y el método **registerWithNotificationHubs**. Agregue después las siguientes líneas que inicializan una instancia de la clase **RegisterClient**. El método debe contener las siguientes líneas:
...
7.  Después, agregue los siguientes métodos, asegurándose de reemplazar `{backend endpoint}` por el extremo back-end obtenido en la sección anterior.
...
4.  En la interfaz de usuario de la aplicación Android, haga clic en **Log in**. Después, haga clic en **Send push**.
  ...
  [Introducción a los Centros de notificaciones (Android)]: /es-es/documentation/articles/notification-hubs-android-get-started/
