...
-   [¿Qué es Sitios web Azure?][]
-   [Direccionamiento del de la configuración predeterminada de PHP][]
-   [Direccionamiento del de extensiones en el tiempo de ejecución de PHP integrado][]
-   [Direccionamiento del de un tiempo de ejecución de PHP personalizado][]
-   [Pasos siguientes][]
...
1.  Vaya al panel del sitio web en el Portal de Azure y haga clic en **Configure**.
    ...
    ![Pestaña de configuración del panel Sitios web][]
...
Como alternativa al uso del archivo `.user.ini`, puede usar la función [ini\_set()][] en los scripts para definir las opciones de configuración que no son directivas a nivel de sistema.
...
5.  En la sección **app settings**, cree una clave **PHP\_EXTENSIONS** y un valor **bin\\your-ext-file**. Para habilitar varias extensiones, incluya una lista separada por comas de archivos `.dll`.
    ...
    ![Habilitar extensiones en app settings][]
...
2.  Modifique el archivo `php.ini` para el tiempo de ejecución. Tenga en cuenta que Sitios web Azure ignorará la configuración que se corresponda con directivas que sean solo a nivel de sistema. Para obtener información acerca de las directivas solo a nivel de sistema, consulte la [lista de directivas de php.ini][] (en inglés).
...
7.  En la sección **handler mappings**, agregue `*.php` a EXTENSION y agregue la ruta de acceso al archivo ejecutable `php-cgi.exe`. Si coloca el tiempo de ejecución de PHP en el directorio `bin` en la raíz de la aplicación, la ruta de acceso será `D:\home\site\wwwroot\bin\php\php-cgi.exe`.
    ...
    ![Especificar el controlador en hander mappings][]
