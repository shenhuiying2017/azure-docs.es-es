<properties 
	pageTitle="Creación de un WebJob .NET en el Servicio de aplicaciones de Azure" 
	description="Aprenda a crear una aplicación de varios niveles utilizando ASP.NET MVC y Azure. El front-end se ejecuta en una aplicación web del Servicio de aplicaciones de Azure y el back-end se ejecuta como un WebJob. La aplicación usa Entity Framework, base de datos SQL, y colas y blobs de almacenamiento de Azure." 
...
Las instrucciones del tutorial se aplican a Azure SDK para .NET 2.5.1 o posterior. En la sección de creación desde cero donde se crea el proyecto de WebJob, los paquetes de SDK de WebJobs se incluyen automáticamente en el proyecto; con versiones anteriores del SDK debe instalar manualmente los paquetes.
...
1. Para iniciar el front-end web de la aplicación, presione CTRL+F5. 
	...
	El explorador predeterminado se abre en la página principal. (The web project runs because you've made it the startup project.)
...
2. En el explorador, haga clic en *Crear un anuncio**.
...
* Impleméntela en aplicaciones Web. Visual Studio creará automáticamente una aplicación web en una instancia de Base de datos SQL y en Servicio de aplicaciones.
...
2. En el cuadro **Seleccionar aplicación web existente**, haga clic en **Iniciar sesión** y especifique las credenciales si todavía no ha iniciado sesión.
 
5. Después de iniciar sesión, haga clic en **Nuevo**.

	![Click New](./media/websites-dotnet-webjobs-sdk-get-started/clicknew.png)

9. En el cuadro de diálogo **Crear aplicación web en Microsoft Azure**, especifique un nombre único en el cuadro **Nombre de la aplicación web**.

	La dirección URL completa consta de lo que escriba aquí más .azurewebsites.net (como se muestra junto al cuadro de texto **Nombre de la aplicación web**). Por ejemplo, si el nombre de la aplicación web es ContosoAds, la URL será ContosoAds.azurewebsites.net.

9. En la lista desplegable **Región**, elija la misma región que seleccionó para la cuenta de almacenamiento.

	Esta configuración especifica el centro de datos de Azure en el que se ejecutará la aplicación web. Al mantener la aplicación web y la cuenta de almacenamiento en el mismo centro de datos, se minimiza la latencia y los cargos por concepto de salida de los datos.
...
7. En el **Explorador de servidores**, haga clic en la aplicación web en el nodo **Aplicaciones web** y, a continuación, haga clic en **Ver configuración**.
...
10. En el **Explorador de servidores**, haga clic con el botón secundario en la aplicación web y, a continuación, haga clic en **Detener aplicación web**. 

12. Una vez detenida la aplicación web, haga clic de nuevo con el botón secundario en la aplicación web y, a continuación, haga clic en **Iniciar aplicación web**.
...
1. En el Portal de Azure, seleccione la aplicación web.
...
>[AZURE.NOTE] Cuando finalice las pruebas, elimine la aplicación web y la instancia de Base de datos SQL. La aplicación web es gratuita, pero la instancia de Base de datos SQL y la cuenta de almacenamiento tienen un costo (mínimo dado su pequeño tamaño). Asimismo, si deja la aplicación ejecutándose, cualquiera que encuentre su dirección URL puede crear y ver anuncios. En el Portal de administración de Azure, vaya a la pestaña **Panel** correspondiente a su aplicación  web y haga clic en el botón **Eliminar** que se encuentra en la parte inferior de la página. A continuación, puede activar una casilla para eliminar la instancia de la Base de datos SQL al mismo tiempo. Si lo que desea es evitar temporalmente que otros accedan a la aplicación, haga clic en **Detener**. En ese caso, se seguirán acumulando cargos para la cuenta de almacenamiento y la base de datos SQL. Puede seguir un procedimiento similar para eliminar la base de datos SQL y la cuenta de almacenamiento cuando ya no las necesite.

### Habilitación de AlwaysOn para procesos de larga ejecución

Para esta aplicación de ejemplo, la actividad de la aplicación web siempre precede a la creación de un mensaje de la cola, por lo que no hay ningún problema si la aplicación web entra en suspensión y finaliza el WebJob debido a un largo período de inactividad. Cuando llega una solicitud, se activa la aplicación y se reinicia el WebJob.

Para los WebJobs que desee seguir ejecutando incluso cuando la propia aplicación web está inactiva durante un largo período de tiempo, puede utilizar la característica [AlwaysOn](http://weblogs.asp.net/scottgu/archive/2014/01/16/windows-azure-staging-publishing-support-for-web-sites-monitoring-improvements-hyper-v-recovery-manager-ga-and-pci-compliance.aspx).
...
2.  Haga clic en **OK**.
	...
	* Agregue un archivo *webjob-publish-settings.json* a la carpeta Propiedades del proyecto WebJob.
	* Agregue un archivo *webjobs-list.json* a la carpeta Propiedades del proyecto WebJob.
	* Instala el paquete NuGet Microsoft.Web.WebJobs.Publish en el proyecto WebJob.
	 
	Para obtener más información acerca de estos cambios, consulte [Implementación de WebJobs mediante Visual Studio](websites-dotnet-deploy-webjobs.md).
...
3. En el proyecto ContosoAdsWeb, agregue los siguientes archivos desde el proyecto descargado.
	...
	- En la carpeta *Views\Shared*: <em>archivo _Layout.cshtml</em>. 
...
Cuando se inicia el WebJob, el método `Main` llama al SDK de WebJobs `JobHost.RunAndBlock` para comenzar la ejecución de funciones desencadenadas en el subproceso actual.
...
* [Uso del almacenamiento de la cola de Azure con el SDK de WebJobs](websites-dotnet-webjobs-sdk-storage-queues-how-to.md)
...
* [Uso del Bus de servicio de Azure con el SDK de WebJobs](websites-dotnet-webjobs-sdk-service-bus.md)
