<properties urlDisplayName="Handle Conflicts with Offline Data" pageTitle="Control de conflictos con datos sin conexión en Servicios móviles (Windows Phone) | Centro de desarrollo móvil" metaKeywords="" description="Obtenga información acerca de cómo usar Servicios móviles de Azure para gestionar los conflictos que se producen al sincronizar datos sin conexión en su aplicación de Windows Phone" metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Handling conflicts with offline data in Mobile Services" authors="wesmc" manager="dwrede" />
...
<a href="/es-es/documentation/articles/mobile-services-windows-store-dotnet-handling-conflicts-offline-data" title="Windows Store C#">C# para Tienda Windows</a>
...
5. En la aplicación, escriba texto para algunos nuevos elementos todo y, a continuación, haga clic en **Guardar** para guardarlos. También puede modificar la fecha de vencimiento de los elementos todo que agrega.
...
3. En el Explorador de soluciones para Visual Studio, expanda la carpeta **App_Start** y abra el archivo WebApiConfig.cs. 
...
4. En el Explorador de soluciones para Visual Studio, expanda la carpeta **Controladores** y abra ToDoItemController.cs. Cambie el nombre de la clase `TodoItemController` a `TodoWithDateController`. Esto cambiará el extremo de REST para las operaciones de tabla. 
...
7. De nuevo en la aplicación, haga clic en **Insertar**.
...
1. Si la opción **Emulator WVGA 512MB** todavía no está activada, presione **Ctrl+F5** para reiniciarla.
...
3. En la segunda instancia de la aplicación, haga clic en **Extraer** para sincronizar el almacén local con la base de datos del servicio móvil. Ambas instancias de la aplicación deben tener los mismos datos.
 
    ![][6]

4. En la segunda instancia de la aplicación, active la casilla para completar uno de los elementos y luego haga clic en **Insertar** para insertar el cambio en la base de datos remota. En la siguiente captura de pantalla, **Recoger a James** se ha completado, lo que indica que ya se ha recogido a James. La primera instancia de la aplicación tiene ahora un registro obsoleto.

    ![][9]

5. En la primera instancia de la aplicación, intente cambiar la fecha del registro obsoleto y luego haga clic en **Insertar** para intentar actualizar la base de datos remota con el registro obsoleto. En la siguiente captura de pantalla, vamos a intentar programar la recogida de James el **5/10/2014**.

    ![][7]

6. Cuando hace clic en el botón **Insertar** para efectuar el cambio de fecha, verá un cuadro de diálogo que indica que se ha detectado el conflicto. Se le preguntará cómo va a resolver el conflicto. Elija una de las opciones para resolver el conflicto.
...
La clase `SyncHandler` en **MainPage.xaml.cs** implementa `IMobileServiceSyncHandler`. Se llama al método `ExecuteTableOperationAsync` cuando se envía cada operación de inserción al servidor. Si se lanza una excepción del tipo `MobileServicePreconditionFailedException`, significa que hay un conflicto entre las versiones local y remota de un elemento.
