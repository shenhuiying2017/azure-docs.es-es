...
* Registre los proveedores de recursos de Batch IA una vez para su suscripción mediante Azure Cloud Shell o la CLI de Azure. Un registro de proveedor puede tardar hasta 15 minutos.
...
Cree estos parámetros en el script de Python, sustituyendo sus propios valores:
...
Como procedimiento recomendado, todas las credenciales deben almacenarse en un archivo de configuración independiente, lo que no se muestra en este ejemplo. Consulte las [recetas](https://github.com/Azure/BatchAI/tree/master/recipes) para implementar un archivo de configuración. 

## <a name="authenticate-with-batch-ai"></a>Autenticación con Batch AI

Para poder acceder a Azure Batch AI, necesita autenticarse con Azure Active Directory. A continuación se muestra el código para autenticarse con el servicio (cree un objeto `BatchAIManagementClient`) con sus credenciales de entidad de servicio y el identificador de suscripción:
...
1. Cree un recurso compartido de archivos denominado *batchaiquickstart*.
...
2. Cree un directorio en el recurso compartido denominado *mnistcntksample*. 
...
El código anterior imprime información de asignación de clúster básica como la siguiente:
...
Puede inspeccionar el estado del trabajo con el comando siguiente: 
...
Use el comando para enumerar los vínculos a los archivos de registro stdout y stderr:
...
Utilice el comando siguiente para eliminar el trabajo:
```Python
batchai_client.jobs.delete(resource_group_name, job_name) 
```

Utilice el comando siguiente para eliminar el clúster:
```Python
