# Información general
## [¿Qué es Azure Functions?](functions-overview.md)
# Introducción
## [Crear su primera función](functions-create-first-azure-function.md)
## [Crear una función desencadenada de webhook](functions-create-github-webhook-triggered-function.md)
## [Crear una función conectada a Azure](functions-create-an-azure-connected-function.md)
## [Crear una función de procesamiento de eventos](functions-create-an-event-processing-function.md)
# Procedimientos
## Planeamiento y diseño
### [Elección entre Flow, Logic Apps, Functions y WebJobs](functions-compare-logic-apps-ms-flow-webjobs.md)
### [Elección entre planes de hospedaje](functions-scale.md)

## Desarrollo
### [Desarrollo de aplicaciones de función](functions-reference.md)
#### [C#](functions-reference-csharp.md)
#### [F#](functions-reference-fsharp.md)
#### [JavaScript](functions-reference-node.md)
### Desencadenadores y enlaces
#### [Conceptos](functions-triggers-bindings.md)
#### [HTTP y webhooks](functions-bindings-http-webhook.md)
#### [Temporizador](functions-bindings-timer.md)
#### [Queue storage](functions-bindings-storage-queue.md)
#### [Blob storage](functions-bindings-storage-blob.md)
#### [Almacenamiento de tablas](functions-bindings-storage-table.md)
#### [Event Hubs](functions-bindings-event-hubs.md)
#### [Bus de servicio](functions-bindings-service-bus.md)
#### [DocumentDB](functions-bindings-documentdb.md)
#### [SendGrid](functions-bindings-sendgrid.md)
#### [Twilio](functions-bindings-twilio.md)
#### [Centros de notificaciones](functions-bindings-notification-hubs.md)
#### [Aplicaciones móviles](functions-bindings-mobile-apps.md)
#### [Archivo externo](functions-bindings-external-file.md)
#### [Tabla externa](functions-bindings-external-table.md)
### [Definición de proxy de Functions](functions-proxies.md)
### [Creación de una Function App en Azure Portal](functions-create-function-app-portal.md)
### [Realización de una tarea de limpieza programada](functions-scenario-database-table-cleanup.md)
### [Prueba de Azure Functions](functions-test-a-function.md)
### [Desarrollo y depuración locales](functions-run-local.md)
### [Procedimientos recomendados para Azure Functions](functions-best-practices.md)
### [Creación de una definición de OpenAPI](functions-api-definition-getting-started.md)
#### [Referencia de una definición de OpenAPI](functions-api-definition.md)

## Administrar
### [Configuración de opciones para Function App](functions-how-to-use-azure-function-app-settings.md)

## Implementación
### [Implementación continua para Azure Functions](functions-continuous-deployment.md)
### [Implementación de funciones mediante la infraestructura como código](functions-infrastructure-as-code.md)

## Supervisión
### [Supervisión de Azure Functions](functions-monitoring.md)

# Recursos
## [Precios](https://azure.microsoft.com/pricing/details/functions/)  
## [Foro de MSDN](https://social.msdn.microsoft.com/Forums/en-US/home?forum=AzureFunctions)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-functions)
## [Repositorio de GitHub de Azure Functions](https://github.com/Azure/Azure-Functions/)
## [Actualizaciones del servicio](https://azure.microsoft.com/en-us/updates/?product=functions&updatetype=&platform=)
