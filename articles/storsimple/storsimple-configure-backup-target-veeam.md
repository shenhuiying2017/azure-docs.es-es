---
title: "Configuración de Microsoft Azure StorSimple con Veeam | Microsoft Docs"
...
# <a name="configure-storsimple-with-veeam"></a>Configuración de StorSimple con Veeam
...
La audiencia este documento incluye administradores de copia de seguridad, administradores de almacenamiento y arquitectos de almacenamiento con conocimientos de almacenamiento, Windows Server 2012 R2, Ethernet, servicios en la nube y Veeam.

## <a name="supported-versions"></a>Versiones compatibles

-   Veeam 9 y versiones posteriores.

-   [StorSimple Update 3 y versiones posteriores](/storsimple-overview#storsimple-workload-summary).
...
4.  El script de la instantánea desencadena la administración de instantáneas en la nube de StorSimple.
...
5.  El script de la instantánea desencadena la administración de instantáneas en la nube de StorSimple.
...
La implementación de esta solución consta de tres pasos: preparar la infraestructura de red, implementar el dispositivo de StorSimple como destino de copia de seguridad y, finalmente, implementar el software Veeam. En las siguientes secciones se detallan cada uno de estos pasos.
...
Para ver los procedimientos recomendados para la instalación de Veeam, vaya a [Best practices for Veeam](https://bp.veeam.expert/) (Procedimientos recomendados para Veeam) y a la guía de usuario que encontrará en [Veeam Help Center (Technical Documentation)](https://www.veeam.com/documentation-guides-datasheets.html) [Centro de ayuda de Veeam (documentación técnica)]
...
-   Para la recuperación ante desastres, realice una copia de la base de datos de Veeam en un volumen de StorSimple.
...
-   Deshabilite la asignación previa del soporte físico.

-   Asegúrese de que está habilitado el procesamiento en paralelo.

-   Deshabilite la compresión.

-   Deshabilite la desduplicación en el trabajo de copia de seguridad.

-   Establezca la optimización en **LAN Target** (LAN destino).

-   Habilite **Create active full backup** (Crear copia de seguridad completa activa) (cada 2 semanas).

-   En el repositorio de la copia de seguridad, configure **Use per-VM backup files** (Usar archivos de copia de seguridad por VM).

-   En **Use multiple upload streams per job** (Usar varias transmisiones de carga por trabajo), seleccione 8 (se permite un máximo de 16). Suba o baje dicho número en función del uso de la CPU en el dispositivo StorSimple.
...
En el siguiente ejemplo se usa una rotación de GFS. En el ejemplo siguiente se asume lo siguiente:
...
## <a name="configuring-veeam-storage"></a>Configuración del almacenamiento de Veeam

1.  Vaya a la configuración de **Backup Infrastructure** (Infraestructura de copia de seguridad). Seleccione **Backup Repositories** (Repositorios de copia de seguridad), haga clic con el botón derecho y seleccione **Add Backup Repository** (Agregar repositorio de copia de seguridad).
...
1.  Escriba el nombre y la descripción del repositorio en los campos **Name** (Nombre) y **Description** (Descripción), respectivamente. Haga clic en **Next >** (Siguiente).

    ![consola de administración de Veeam, pantalla de nombre y descripción](./media/storsimple-configure-backup-target-using-veeam/veeamimage2.png)

1.  Seleccione **Microsoft Windows Server** como repositorio de copia de seguridad. Elija el servidor de Veeam. Haga clic en **Next >** (Siguiente).
...
1.  Para especificar el valor de **Location** (Ubicación), busque y seleccione el volumen requerido. Establezca el valor de **Limit maximum concurrent tasks to:** (Limitar el número máximo de tareas simultáneas a:) en 4. Esto garantiza que cuando se procesa cada máquina virtual solo se procesan cuatro discos virtuales simultáneamente.
...
1.  Elija **Use per-VM backup files** (Usar archivos de copia de seguridad por VM).
...
1.  Habilite **vPower NFS service on the mount server (recommended)** [Servicio NFS de vPower en el servidor de montaje (se recomienda)].
...
1.  Revise la configuración y pase a la página siguiente.
...
> [!IMPORTANT]
> Si tiene que restaurar los datos de una copia de seguridad que ha sido organizada en niveles en la nube, la restauración se realizará a velocidades de la nube.
...
### <a name="assigning-storsimple-volumes-to-a-veeam-backup-job"></a>Asignación de volúmenes de StorSimple a un trabajo de copia de seguridad de Veeam

1.  Cree un trabajo diario con Veeam StorSimple principal en caso de escenario de destino de copia de seguridad el principal o DAS/NAS/JBOD en caso de escenario de destino de copia de seguridad secundario como destino de copia de seguridad. Vaya a **Backup &#38; Replication &gt; (Copia de seguridad y replicación) Backup** (Copia de seguridad). Haga clic con el botón derecho y luego seleccione VMware o Hyper-V en función del entorno.
...
1.  Escriba el nombre y la descripción en los campos **Name** (Nombre) y **Description** (Descripción) del trabajo de copia de seguridad diaria.

    ![consola de administración de Veeam, pantalla de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage9.png)

1.  Seleccione la máquina virtual en la que se va a realizar la copia de seguridad.

    ![consola de administración de Veeam, pantalla de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage10.png)

1.  Establezca los valores de **Backup proxy** (proxy de copia de seguridad) y **Backup repository** (Repositorio de copia de seguridad). Establezca el valor de **Restore points to keep on disk** (Restaurar puntos que se mantienen en disco) por las definiciones de RPO/RTO de su entorno en el almacenamiento conectado localmente. Haga clic en Avanzada.

    ![consola de administración de Veeam, pantalla de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage11.png)

    En la pestaña **Backup** (Copia de seguridad), elija **Incremental**. Asegúrese de la casilla **Create synthetic full backups periodically** (Crear copias de seguridad sintéticas completas) no está activada. Habilite **Create active full backups periodically** (Crear copias de seguridad activas completas periódicamente) y en **Active full backups** (Copias de seguridad completas activas) active Weekly on selected days (Semanalmente en días seleccionados) en el sábado.

    ![consola de administración de Veeam, pantalla de configuración avanzada de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage12.png)

    En la pestaña Storage (Almacenamiento), asegúrese de que la desduplicación y la compresión están deshabilitadas y de que la eliminación y el intercambio de bloques de archivos están habilitados. En **Storage optimization** (Optimización de almacenamiento), seleccione **LAN Target** (LAN destino) si desea que el rendimiento sea equilibrado y que haya desduplicación.

    ![consola de administración de Veeam, pantalla de configuración avanzada de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage13.png)

    Para obtener detalles sobre la configuración tanto de la desduplicación como de la compresión, consulte [Data Compression and Deduplication](https://helpcenter.veeam.com/backup/vsphere/compression_deduplication.html) (Compresión y desduplicación de datos)

1.  Active **Enable Application Aware Processing** (Habilitar procesamiento compatible con aplicación).

    ![consola de administración de Veeam, pantalla de procesamiento de invitado de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage14.png)

1.  Configure la programación para que se ejecute una vez al día en el momento que prefiera.

    ![consola de administración de Veeam, pantalla de Scheduler del nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage15.png)

## <a name="storsimple-as-a-secondary-backup-target"></a>StorSimple como destino de copia de seguridad secundario

> [!NOTE]
> La restauración se produce a la velocidad de la nube en los datos de una copia de seguridad que se ha organizada en niveles en la nube.
...
### <a name="assigning-storsimple-volumes-to-a-veeam-copy-job"></a>Asignación de volúmenes de StorSimple a un trabajo de copia de Veeam

1.  Inicie al Asistente para nuevo trabajo de copia de seguridad. Vaya a **Jobs > Backup Copy** (Trabajos > Copia de seguridad). Seleccione VMware o Hyper-V en función del entorno.

    ![consola de administración de Veeam, pantalla de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage16.png)

1.  Especifique el nombre y la descripción del trabajo.

    ![consola de administración de Veeam, pantalla de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage17.png)

1.  Seleccione las máquinas virtuales que se van a procesar, seleccione las copias de seguridad y seleccione la copia de seguridad diaria que creó anteriormente.

    ![consola de administración de Veeam, pantalla de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage18.png)

1.  Excluya objetos de la tarea de copia de seguridad si fuera necesario.

1.  Seleccione **Backup repository** (Repositorio de copia de seguridad), defina **Restore points to keep** (Restaurar puntos que se conservan) y asegúrese de que habilita **Keep the following restore points for archival purposes** (Mantener los siguientes puntos de restauración para archivado). Defina la frecuencia de copia de seguridad y haga clic en **Advanced**.

    ![consola de administración de Veeam, pantalla de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage19.png)

1.  Especifique las siguientes opciones avanzadas:

    -   En la pestaña **Maintenance**, deshabilite la protección contra daño en el nivel de almacenamiento.

    -   En la pestaña **Storage**, asegúrese de que la desduplicación y la compresión están deshabilitadas.

    ![consola de administración de Veeam, pantalla de configuración avanzada de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage20.png)

    ![consola de administración de Veeam, pantalla de configuración avanzada de nuevo trabajo de copia de seguridad](./media/storsimple-configure-backup-target-using-veeam/veeamimage21.png)

1.  Especifique que la transferencia de datos es directa.

1.  Defina la programación de la ventana de copia de seguridad según sus necesidades y finalice.


Para más información, vaya a [Create backup copy jobs](https://helpcenter.veeam.com/backup/hyperv/backup_copy_create.html) (Creación de trabajos de copia de seguridad).

## <a name="storsimple-cloud-snapshots"></a>Instantáneas de nube de StorSimple

Las instantáneas de nube de StorSimple protegen los datos que residen en el dispositivo de StorSimple. Es como enviar cintas a unas instalaciones fuera del sitio. Si se utiliza el almacenamiento con redundancia geográfica (GRS) de Azure, es como enviar cintas a varios sitios. Si se necesita que se restaure un dispositivo en caso de desastre, puede poner en línea otro dispositivo de StorSimple y realizar una conmutación por error. Después de dicha conmutación, se puede acceder a los datos (a velocidades de nube) desde la instantánea en la nube más reciente.
...
> [!NOTE] 
> Evalúe cuidadosamente las repercusiones tanto en lo relativo al cumplimiento como a la retención de datos antes de eliminar una instantánea de StorSimple. Para más información acerca de cómo ejecutar un script posterior a la copia de seguridad, consulte la documentación de Veeam.
...
5.  En el Bloc de notas, cree un script de Windows PowerShell y guárdelo en la misma ubicación en la que guardó la configuración de publicación de Azure. Por ejemplo: `C:\\CloudSnapshot\\StorSimpleCloudSnapshot.ps1`.
...
Veeam permite la recuperación rápida pormenorizada a nivel de archivo a través de StorSimple mediante los exploradores integrados de la consola de Veeam. Use los exploradores de Veeam para recuperar elementos individuales, como mensajes de correo electrónico, objetos de Active Directory o elementos de SharePoint de las copias de seguridad. La recuperación puede realizarse sin que se produzca ninguna interrupción en la máquina virtual local. También puede realizar la recuperación a un momento dado de las bases de datos de Microsoft SQL y Oracle. Veeam y StorSimple facilitan y agilizan el proceso de recuperación a nivel de elemento desde Azure. Para obtener información acerca de cómo realizar una restauración, consulte la documentación de Veeam.
