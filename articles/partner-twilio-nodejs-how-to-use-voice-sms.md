...
Twilio permite que los desarrolladores [compren números de teléfono programables](https://www.twilio.com/user/account/phone-numbers/available/local) que se pueden usar para enviar y recibir llamadas y mensajes de texto. Cuando un número de Twilio recibe una llamada o un texto entrante, Twilio le enviará a la aplicación web una solicitud HTTP, POST o GET, pidiéndole instrucciones sobre cómo controlar la llamada o el texto. El servidor responderá la solicitud HTTP de Twilio con [TwiML](https://www.twilio.com/docs/api/twiml), un conjunto simple de etiquetas XML que contienen instrucciones sobre cómo controlar una llamada o texto. Veremos ejemplos de TwiML en un momento.
...
A continuación, debemos crear un archivo package.json para administrar nuestras dependencias de módulo de nodo vía [npm](http://npmjs.org). En el mismo nivel del archivo "server.js" que creó en el tutorial de Azure/Node.js, cree un archivo llamado "package.json". Dentro de este archivo coloque lo siguiente:
...
Con esto, el módulo de Twilio se declara como una dependencia, al igual que el popular [express web framework](http://expressjs.com) y el motor de plantilla EJS. Ahora ya estamos: comencemos a escribir código.
...
to:request.body.number,
          ...
          // Cambiar a un número de Twilio que haya comprado; ver:
          // https://www.twilio.com/user/account/phone-numbers/incoming
          from:'+15558675309',

          // Una dirección URL en nuestra aplicación que genera TwiML
          // Cambiar "CHANGE_ME" por el nombre de su aplicación
...
app.post('/outbound\_call', function(request, response) { var twiml = new twilio.TwimlResponse();

      // Decir un mensaje al receptor de la llamada 
...
Ahora, configuremos una lógica de control de formulario e interfaz de usuario para enviar un mensaje de texto. Abra "server.js" y agregue el siguiente código después de la última llamada a "app.post":
  	  ...
  	  app.post('/sms', function(request, response) {
     	  ...
     	  client.sendSms({
             ...
             // Un número de Twilio que haya comprado; ver:
             // https://www.twilio.com/user/account/phone-numbers/incoming
             from:'+15558675309',
