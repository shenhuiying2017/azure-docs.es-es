<properties title="Quick start guide to R for Azure Machine Learning Studio" pageTitle="Guía de inicio rápido de R para el Estudio de aprendizaje automático de Azure" description="Cómo usar R en el Estudio de aprendizaje automático de Azure" metaKeywords="R azure, r machine learning, azure r machin elearning" services="machine-learning" solutions="" documentationCenter="" authors="larryfr" manager="paulettm" editor="cgronlun"  videoId="" scriptId="" />
...
#Guía de inicio rápido de R para Azure ML

Stephen F Elston, PhD.

##  Introducción

El aprendizaje automático de Azure contiene muchos módulos de manipulación de datos y aprendizaje automático.  El lenguaje R se conoce como la lingua franca del análisis de datos.  Afortunadamente, la manipulación y el análisis de datos en el Aprendizaje automático de Azure se pueden ampliar mediante R. Esta combinación aúna la escalabilidad en la implementación del Aprendizaje automático de Azure con la flexibilidad y el análisis profundo de R.

Este documento le ayudará a comenzar a ampliar el Aprendizaje automático de Azure mediante el lenguaje R. Esta guía contiene la información que necesitará para crear, probar y ejecutar código R en el Aprendizaje automático de Azure. A medida que vaya avanzando en esta guía de inicio rápido, creará una solución completa de previsión completa mediante el lenguaje R en el Aprendizaje automático de Azure.  
...
Los datos de series temporales son datos en los que los valores tienen un índice temporal. El índice temporal puede ser normal, es decir, cada mes o cada minuto; o bien puede ser irregular. Los modelos de serie temporal se basan en datos de series temporales. El código R contiene un marco de trabajo flexible y amplias características de análisis de datos de serie temporales.
...
* [Interacción con el lenguaje R en el estudio de aprendizaje automático](#mlstudio)

* [Obtención de datos dentro y fuera del módulo de ejecución de script R](#scriptmodule)

* [Filtrado y transformación de datos](#filtering)

* [Análisis de correlación y objetos de serie temporal](#timeseries)

* [Ejemplo de serie temporal: Previsión estacional](#seasonalforecasting)

* [Apéndice A: guía de RStudio](#appendixa)

* [Apéndice B: lectura adicional](#appendixb)

##<a id="mlstudio"></a>Interacción con el lenguaje R en el Estudio de aprendizaje automático

Esta sección le guiará por los distintos aspectos básicos de la interacción con el código R en el estudio de aprendizaje automático.  El lenguaje R proporciona una herramienta eficaz para crear módulos de manipulación de datos y de análisis personalizado en entornos de aprendizaje automático de Azure.

Utilizaremos RStudio para desarrollar, probar y depurar el código R a escala reducida.  A continuación, este código se cortará y pegará en un módulo de script de ejecución de código R en el estudio de aprendizaje automático listo para ejecutarse.  

###Módulo de script de ejecución de código R

En el estudio de aprendizaje automático, los scripts de código R se ejecutan en el marco del módulo de script de ejecución de código R.  En la ilustración 1, se muestra un ejemplo del módulo de script de ejecución de código R en el estudio de aprendizaje automático.

 ![The ML Studio environment showing the Execute R Script module selected][1]

*Ilustración 1. Entorno de estudio de aprendizaje automático que muestra el módulo de script de ejecución de código R seleccionado.*

Tomando como referencia la ilustración 1, veamos algunas de las partes principales del entorno de Estudio de aprendizaje automático necesarias para trabajar con el módulo de script de ejecución de código R.
...
- La parte inferior del panel derecho muestra algunas propiedades del script de ejecución de código R. Para ver los registros de error y de salida, haga clic en los puntos correspondientes de este panel.

Por supuesto, analizaremos el script de ejecución de código R con mayor detalle en el resto de este documento.

Cuando se utilicen funciones complejas de R, es recomendable editar, probar y depurar el código en RStudio. Al igual que con cualquier desarrollo de software, amplíe el código de forma incremental y pruébelo en casos de prueba más sencillos. A continuación, corte y pegue las funciones en la ventana de script R del módulo de ejecución de código R.  Este enfoque permite aprovechar el IDE de RStudio y las posibilidades que ofrece el Aprendizaje automático de Azure.  

####Ejecución de código R

El código R del módulo de script de ejecución de código R se ejecutará al ejecutar el experimento haciendo clic en el botón **Ejecutar**.  Cuando haya finalizado la ejecución, aparecerá una marca de verificación en el icono del script de ejecución de código R.

####Codificación R defensiva para el aprendizaje automático de Azure

Si está desarrollando código R para un servicio web utilizando el Aprendizaje automático de Azure, deberá planear cómo tratará el código las entradas de datos inesperadas y las excepciones. Para mantener la claridad, he preferido no entrar en muchos detalles en cuanto al método de comprobación o control de excepciones en la mayoría de los ejemplos de código mostrados.  Sin embargo, conforme avancemos compartiré varios ejemplos de funciones mediante la capacidad de control de excepciones de R.  

Si necesita un tratamiento más completado del control de excepciones de R, recomiendo que lea las secciones correspondientes del manual de Wickham que se detallan en el [Apéndice B: información adicional](#appendixb).


####Depuración y pruebas de código R en el Estudio de aprendizaje automático

Nuevamente, recomiendo probar y depurar el código de R a escala reducida en RStudio.  Sin embargo, hay casos en los que necesitará rastrear problemas de código R en el propio script de ejecución de código R.  Además, se recomienda comprobar los resultados en el Estudio de aprendizaje automático.

Los resultados de la ejecución del código R y de la plataforma de Aprendizaje automático de Azure pueden encontrarse en el archivo output.log. Se mostrará información adicional en el archivo error.log.  

Si se produce un error en el Estudio de aprendizaje automático durante la ejecución del código R, lo primero que debe hacer es consultar el archivo error.log. El archivo error.log puede contener mensajes de error útiles que le ayudarán a comprender y a corregir el error.  Para ver el archivo error.log, haga clic en **Ver registro de errores** en el **panel de propiedades** del script de ejecución de código R que contiene el error.

Por ejemplo, he ejecutado el siguiente código R con una variable y sin definir en un módulo de script de ejecución de código R:
...
Parece que tenemos que consultar el archivo output.log para poder ver el mensaje de error R. Haga clic en el script de ejecución de código R en el elemento **Ver archivo output.log** del **panel de propiedades** situado a la derecha. Se abrirá una nueva ventana del explorador y podrá ver lo siguiente:
...
Imprima estos valores en el archivo output.log para comprobar el valor de los objetos de código R.  Las reglas para examinar los valores de objeto son las mismas que las que se usan en sesiones interactivas de código R.  Por ejemplo, si escribe un nombre de variable en una línea, el valor del objeto se imprimirá al archivo output.log.  

####Paquetes en el Estudio de aprendizaje automático

El Aprendizaje automático de Azure incluye más de 350 paquetes R preinstalados.  Puede utilizar el código siguiente en el módulo de script de ejecución de código R para recuperar una lista de paquetes preinstalados.
...
Siga leyendo si no comprende la última línea de este código.  En el resto del documento se describe con detalle el uso de código R en el entorno de Aprendizaje automático de Azure.

###	Introducción de RStudio

RStudio es un entorno de desarrollo integrado (IDE) para lenguaje R. Utilizaremos RStudio para editar, probar y depurar el código R utilizado en esta guía de inicio rápido.  Una vez comprobado y listo el código R, bastará con cortar y pegar desde el editor de RStudio en un módulo de script de ejecución de código R de Estudio de aprendizaje automático.  

Si no tiene instalado R en su equipo de sobremesa, es recomendable que lo instale ahora. Encontrará descargas gratuitas de código R abierto están disponibles en la red completa de archivos de R o CRAN en http://www.r-project.org/. Este sitio ofrece descargas disponibles para Windows, Mac OS y Linux/UNIX. Elija el espejo más cercano a su ubicación y siga las instrucciones de descarga.   Además, CRAN contiene una gran cantidad de paquetes de manipulación de datos y análisis de utilidad.

Si no está familiarizado con RStudio, descargue e instale la versión de escritorio. Encontrará descargas de RStudio para Windows, MacOS y Linux/UNIX en http://www.rstudio.com/products/RStudio/. Siga las instrucciones proporcionadas para instalar RStudio en su equipo.  

Encontrará un tutorial de introducción a RStudio en https://support.rstudio.com/hc/es-es/sections/200107586-Using-RStudio.

En el [Apéndice A][appendixa], encontrará información adicional sobre el uso de RStudio.  

##<a id="scriptmodule"></a>Obtención de datos dentro y fuera del módulo de script de ejecución de código R

En esta sección, veremos cómo introducir y extraer datos del módulo de script de ejecución de código R.  Revisaremos cómo controlar los distintos tipos de datos leídos dentro y fuera del módulo de script de ejecución de código R.

Encontrará el código completo de esta sección en el archivo zip que descargó anteriormente.

###Carga y comprobación de los datos en el estudio de aprendizaje automático

####<a id="loading"></a>Carga del conjunto de datos

En primer lugar, comience por cargar el archivo **csdairydata.csv** en el Estudio de aprendizaje automático de Azure.

- Inicie el entorno de Estudio de aprendizaje automático de Azure.

- Haga clic en el signo + situado en la esquina inferior izquierda de la pantalla y seleccione **Conjunto de datos**.

- Busque el archivo con la opción **Examinar**.

- Asegúrese de haber seleccionado la opción **Archivo CSV genérico con encabezado (.csv)**.

- Haga clic en la marca de verificación.

- Debería poder ver los nuevos conjuntos de datos haciendo clic en la pestaña **Conjuntos de datos**.  

####Creación de un experimento

Ahora que tenemos algunos datos en el Estudio de aprendizaje automático, debemos crear un experimento para realizar el análisis.  

- Haga clic en el signo + situado la esquina inferior izquierda y seleccione **Experimento**

- Asigne un título al experimento.  Llamaré a mi experimento **Análisis de productos lácteos de CA**.

- Busque el conjunto de datos que acaba de cargar.

- Arrastre y suelte el **conjunto de datos csdairydata.csv** en el experimento.  

- En el cuadro **Búsqueda de elementos de experimento** situado en la parte superior del panel izquierdo, escriba **Ejecutar script de código R**.  El módulo aparecerá en la lista de búsqueda.

- Arrastre y suelte el módulo de script de ejecución de código R en el palet.  

- Conecte el resultado del **conjunto de datos csdairydata.csv** a la entrada situada más izquierda (**Dataset1**) del **script de ejecución de código R**.
...
*Ilustración 3.  Experimento Análisis de productos lácteos de CA con el conjunto de datos y el módulo de script de ejecución de código R.*

####Comprobación de los datos

Echemos un vistazo a los datos que se han cargado en nuestro experimento.  En el experimento, haga doble clic en el resultado del **conjunto de datos cadairydata.csv** y seleccione **Visualizar**.   Debería ver algo parecido a lo que se muestra en la ilustración 4.  

![The summary of the cadairydata.csv data set][4]

*Ilustración 4.  Resumen del conjunto de datos cadairydata.csv.*

En esta vista se puede ver una gran cantidad de información útil.  La fila **Tipo de característica** muestra qué tipos de datos asigna el Estudio de aprendizaje automático de Azure a las columnas en el conjunto de datos.  También se pueden ver varias filas de dicho conjunto de datos.  Echar un vistazo rápido como este es una buena manera de realizar una comprobación antes de empezar a realizar cualquier trabajo más importante.

###	Primer script de código R

A continuación, vamos a crear nuestro primer script de código R para experimentar con él en el Estudio de aprendizaje automático de Azure.  Para ello, he creado y probado el siguiente script en RStudio:  
...
Ahora necesito transferir este script a mi Estudio de aprendizaje automático de Azure.  Basta con cortar y pegar.  Sin embargo, en este caso, transferiré el script de código R mediante un archivo zip.

###	Introducción de datos en el módulo de script de ejecución de código R

Echemos un vistazo a las entradas del módulo de script de ejecución de código R.  En este ejemplo, se leerán los datos de productos lácteos de California en el módulo de script de ejecución de código R.  

Existen tres entradas posibles para el módulo de script de ejecución de código R.  Puede usar cualquiera de estas entradas o todas ellas, dependiendo de la aplicación.  También se puede utilizar un script de código R que no tome ninguna entrada.  

Echemos un vistazo a cada una de las entradas de izquierda a derecha. Para ver los nombres de cada una de las entradas, coloque el cursor sobre la entrada y lea la información sobre herramientas.  

####	Paquete de script

El paquete de script permite pasar el contenido de un archivo zip al módulo de script de ejecución de código R.  Puede utilizar uno de los siguientes comandos para leer el contenido del archivo zip en su código R:
...
> [WACOM.NOTE] El Aprendizaje automático de Azure trata los archivos del archivo zip como si se encontrasen en el directorio src/, por lo que necesitará agregar a los nombres de archivo el nombre del directorio como prefijo.  
...
3.	Agregue el archivo al **conjunto de datos** en el Estudio de aprendizaje automático y especifique el tipo como **zip**. Ahora debería ver el archivo zip en los conjuntos de datos.

4.	Arrastre y suelte el archivo zip de los **conjuntos de datos** al **lienzo de estudio de aprendizaje automático**.

5.	Conecte el resultado del icono **datos del zip** a la entrada **agrupación de script** del módulo de **script de ejecución de código R**.

6.	Escriba la función `source()` con el nombre del archivo zip en la ventana de código para el módulo de **script de ejecución de código R**.  En mi caso, he escrito `source("src/SimplePlot.R")`.  

7.	Asegúrese de hacer clic en **Guardar**.

Una vez completados estos pasos, el módulo de script de ejecución de código R ejecutará el script de código R en el archivo zip cuando se ejecute el experimento. En este punto el experimento debería tener un aspecto similar al que se muestra en la ilustración 5.
...
> [WACOM.NOTE] Este mismo resultado impreso está disponible desde el resultado del dispositivo R del módulo de script de ejecución de código R.  Abordaremos los resultados del módulo de ejecución de código R en la sección siguiente.  
...
Después de ejecutar el experimento, haga clic en el puerto de resultados de Dataset1 y, a continuación, haga clic en **Visualizar**.  Debería ver algo parecido a lo que se muestra en la ilustración 6.

![The visualization of the output of the California dairy data][7]

*Ilustración 6. Visualización del resultado de los datos de productos lácteos de California.*

Este resultado es idéntico a la entrada, justo como se esperaba.  

###	Resultados del dispositivo R

Los resultados del dispositivo del módulo de script de ejecución de código R contienen los mensajes y los gráficos.  Los mensajes de error y de resultados estándar de R se envían al puerto de salida del dispositivo R.  

Para ver el resultado del dispositivo R, haga clic en el puerto y, a continuación, en **Visualizar**.  En la ilustración 7 puede verse el resultado estándar y el error estándar del script de código R.

![Standard output and standard error from the R Device port][8]

*Ilustración 7. Resultado estándar y errores estándar desde el puerto del dispositivo R.*
...
*Ilustración 8. Resultado de gráficos desde el puerto de dispositivo R.*  
...
Examen de los tipos de datos de las columnas introducidas en la sección anterior. Todas las columnas son de tipo numérico, excepto la columna con la etiqueta 'Month' que es del tipo de caracteres.  A continuación, vamos a convertirla en un factor y a probar los resultados.  

He eliminado la línea que creaba la matriz de trazado de dispersión y he agregado una línea para convertir la columna 'Month' en un factor.  En mi experimento solo cortaré y pegaré el código R en la ventana de código del módulo de script de ejecución de código R.  También puede actualizar el archivo zip y cargarlo en el Estudio de aprendizaje automático de Azure; aunque esto implica varios pasos.  
...
A continuación, ejecutaremos este código y analizaremos los resultados. El resultado de **Visualizar** que se muestra en el menú del dispositivo R se muestra en la ilustración 9.

![Summary of the dataframe with a factor variable][10]

*Ilustración 9. Resumen del marco de datos con una variable de factor.*

El tipo de Month no debe ser '**Factor con 14 niveles**'.  Esto es un problema, puesto que un año solo tiene 12 meses. También puede realizar comprobaciones para ver que el tipo de **Visualizar** del puerto del conjunto de datos es '**Categorical**'.

El problema es que la columna 'Month' no se ha codificado de forma sistemática.  En algunos casos, un mes se denomina Abril y en otros se abrevia como abr. Para resolver este problema, basta con acortar la cadena a 3 caracteres.  La línea de código tendrá el aspecto siguiente:
...
Vuelva a ejecutar el experimento y observe los resultados de **Visualizar** en el puerto del dispositivo R con los resultados que se muestran en la ilustración 10.  
...
Las tramas de datos R incluyen capacidades de filtrado eficaces.  Es posible obtener subconjuntos de los conjuntos de datos mediante el uso de filtros lógicos en filas o columnas. En muchos casos, serán necesarios criterios de filtro complejos. Las referencias que se detallan en el [Apéndice B: lectura adicional](#appendixb) contienen ejemplos detallados del filtrado de tramas de datos.  

En nuestro conjunto de datos, es necesario crear un bit de filtrado. Si observamos las columnas de la trama de datos cadariydata, podemos ver que hay dos columnas innecesarias.  La primera columna contiene solo un número de fila, que no es muy útil. La segunda, Year.Month, contiene información redundante.  Estas dos columnas se pueden excluir fácilmente mediante el código R siguiente.

> [WACOM.NOTE] De ahora en adelante en esta sección, solo mostraré el código adicional que voy a agregar en el módulo de script de ejecución de código R.  Voy a agregar cada nueva línea **antes** de la función `str()`. Esta función se utiliza para comprobar los resultados en el Estudio de aprendizaje automático de Azure.

Voy a agregar la siguiente línea a mi código R en el módulo de script de ejecución de código R.
...
Ejecute este código en el experimento y compruebe el resultado con **Visualizar** en el puerto de dispositivo R. Estos resultados se muestran en la ilustración 11.
...
Ahora ejecute el experimento actualizado y utilice **Visualizar** en el puerto de dispositivo R para ver los resultados. Estos resultados se muestran en la ilustración 12.
...
El objetivo de la programación defensiva es evitar que el fallo de una función impida que el procesamiento continúe.  Un error repentino en un análisis cuya ejecución lleva mucho tiempo puede resultar muy frustrante para los usuarios. Para evitar esta situación, se deben elegir valores devueltos predeterminados para limitar los daños en el procesamiento de nivel inferior.  También se genera un mensaje para avisar a los usuarios de que se ha producido un error.

Si no está familiarizado con la programación defensiva en código R, es posible que este código pueda parecerle complejo. Por ello, lo guiaré a través de los pasos principales:

1. Un vector de cuatro mensajes definido se utiliza para comunicar información acerca de algunos de los posibles errores y excepciones que pueden producirse con este código.
...
Ejecute este código R en el experimento y preste atención al resultado impreso en el archivo output.log.  Ahora verá los valores transformados de las cuatro columnas mediante **Visualizar** en el menú del dispositivo R tal como se muestra en la figura 13.
...
En este momento nuestros datos se limpian y estamos preparados el modelado.  Según el resumen de visualización de los resultados del conjunto de datos del módulo de script de ejecución de código R, verá que la columna 'Month' es 'Categorical' con 12 valores únicos, nuevamente, tal como deseaba.

##<a id="timeseries"></a>Análisis de correlación y objetos de serie temporal
...
Comencemos con el ejemplo. Arrastre y suelte un **nuevo módulo de script de ejecución de código R** en el experimento.  Conecte el puerto de salida del conjunto de datos de resultados 1 del módulo ejecutar R Script existente al puerto de entrada 1 del conjunto de datos del nuevo módulo de script de ejecución de código R.
...
Ahora, vamos a ejecutar el experimento.  En el resultado, podemos ver que el puerto del dispositivo R tiene el aspecto que se muestra en la ilustración 14.

![Summary of the dataframe in the Execute R Script module][15]

*Ilustración 14. Resumen de la trama de datos en el módulo de script de ejecución de código R.*
...
Necesitamos agregar un objeto de serie temporal a nuestra trama de datos.  Reemplace el código actual con el siguiente, que agrega una nueva columna de la clase POSIXct.
...
Ahora, compruebe el resultado del dispositivo R. Debería ser similar al que se muestra en la ilustración 15.
...
Exploremos algunas de las variables de este conjunto de datos.  Una matriz de trazado de dispersión es una buena manera de generar un vistazo rápido. Voy a reemplazar la función `str()` del código R anterior con la siguiente línea:
...
Para realizar el análisis de correlación, necesitamos anular las tendencias y estandarizar las variables. Para ello, basta con usar la función R `scale()` que permite central y escalar las variables. Esta función también puede ejecutarse más rápido.  Sin embargo, voy a mostrar un ejemplo de programación defensiva en R.
...
Ya hemos analizado un ejemplo de programación defensiva en [Transformaciones de valores](#valuetransformations). Ambos bloques de cálculo se incluyen en `tryCatch()`.  Para algunos errores, es recomendable devolver el vector de entrada original, mientras que en otros casos es preferible devolver un vector de ceros.  
...
La ejecución de este código genera el resultado mostrado en la ilustración 18.
...
Tenga en cuenta que los nombres de fila están en una columna de la trama de datos. Esto permite conservar los nombres de fila cuando son resultado del script de ejecución de código R.

La ejecución del código genera el resultado se muestra en la ilustración 29 cuando se **visualiza** el resultado en el puerto del conjunto de datos de resultados. Los nombres de fila están en la primera columna según lo previsto.

![Results output from the correlation analysis][20]

*Ilustración 29. Resultados del análisis de correlación.*
...
Nuestro modelo de pronóstico tendrá dos componentes, un componente de tendencia y un componente de temporada.  La previsión completa es el producto de estos dos componentes.  Este tipo de modelo se conoce como un modelo de multiplicación. La alternativa es un modelo de suma. Ya hemos aplicado una transformación de registro a las variables de interés que hacen que este análisis sea manejable.

El código R completo de esta sección está disponible en el archivo zip descargado anteriormente.

###	Creación de la trama de datos para el análisis

Empiece agregando un **nuevo módulo de script de ejecución de código R** al experimento.  Conecte la salida del **Conjunto de datos de resultado** del módulo de script de ejecución de código R existente a la entrada **Dataset1** del módulo nuevo.  El resultado debería ser similar al que se muestra en la ilustración 20.

![The experiment with the new Execute R Script module added][21]

*Ilustración 20. Experimento con el nuevo módulo de script de ejecución de código R agregado.*
...
Ejecute este código y examine el puerto de salida del dispositivo R. El resultado deberá ser similar al que se muestra en la ilustración 21.
...
Con la trama de datos construida, necesitamos crear un conjunto de datos de entrenamiento. Estos datos incluirán todas las observaciones, excepto las últimas 12, que corresponden al año 2013 y que forman nuestro conjunto de datos de prueba. El siguiente código crea subconjuntos de la trama de datos y gráficas de las variables de producción junto con las variables de precios. A continuación, crearé gráficos de las cuatro producciones y de las variables de precios. Se utilizará una función anónima para definir algunos argumentos para la gráfica y para, a continuación, iterar la lista de los otros dos argumentos con la función `Map()`. Si piensa que un bucle for podría haber funcionado para esta operación, está en lo cierto. Sin embargo, puesto que R es un lenguaje funcional, he preferido ofrecer un enfoque funcional.
...
*Ilustración 22. Gráficos de series temporales de la producción de productos lácteos de California y de los datos de precios.*

###	Modelo de tendencia

Una vez creado el objeto de la serie temporal y tras haber analizado los datos, procederemos con la creación de un modelo de tendencias para los datos de producción de leche de California. Para ello, utilizaremos una regresión de serie temporal.  Sin embargo, resulta evidente en la trama, que necesitaremos algo más una pendiente y una intercepción para modelar con precisión la tendencia observada en los datos de entrenamiento.

Dada la pequeña escala de los datos, crearé el modelo de la tendencia en RStudio para, a continuación, cortar y pegar el modelo resultante en el Aprendizaje automático de Azure. RStudio proporciona un entorno interactivo para este tipo de análisis interactivo.
...
Como prueba de validez, vamos a crear un gráfico de serie temporal de los datos de producción de productos lácteos de California con la curva de tendencias.  He agregado el código siguiente al modelo de script de ejecución de código R de Aprendizaje automático de Azure (no RStudio) para crear el modelo y hacer un gráfico.
...
*Ilustración 23. Datos de producción de leche de CA con modelo de tendencias.*
...
A continuación, crearemos otro gráfico de serie temporal con los datos de producción de productos lácteos de California para ver cómo funciona el modelo de temporada.  He agregado el código siguiente en el script de ejecución de código R de Aprendizaje automático de Azure para crear el modelo y hacer un gráfico:
...
La ejecución de este código en el Aprendizaje automático de Azure genera el gráfico que se muestra en la ilustración 24.

![CA milk production with model including seasonal effects](./media/machine-learning-r-quickstart/unnamed-chunk-20.png)

*Ilustración 24. Producción de leche de CA con modelo que incluye los efectos de temporada.*
...
Con el fin de realizar una comprobación adicional en nuestro modelo, veamos los valores residuales. El código siguiente calcula los valores de predicción de los dos modelos, calcula los valores residuales para el modelo de temporada y, a continuación, crea un gráfico de estos valores residuales para los datos de entrenamiento.
...
El gráfico que se muestra en la ilustración 25 es útil para detectar los patrones que dependen del tiempo en los valores residuales. El enfoque explícito de cálculo y gráficos de los valores residuales que he utilizado coloca los valores residuales por orden cronológico en el gráfico.  Por otro lado, si hubiese creado el gráfico con la función `milk.lm$residuals`, el gráfico no estaría ordenado por orden cronológico.

También puede usar la función `plot.lm()` para generar una serie de gráficos de diagnóstico:
...
Este código genera una serie de gráficos de diagnósticos que se muestran en la ilustración 26.
...
*Ilustración 26. Gráficos de diagnóstico para el modelo estacional.*
...
Se utiliza una gran cantidad de métricas para medir el rendimiento de los modelos de serie temporal. En nuestro caso, usaremos el error cuadrático medio o error de RMS. La función siguiente calcula el error RMS entre dos series:  
...
*Ilustración 27. Comparación de error RMS para los modelos.*
...
1.	Creación de proyectos
	...
	Puede organizar y administrar el código de R en proyectos con RStudio.  La documentación que utiliza proyectos puede encontrarse en https://support.rstudio.com/hc/es-es/articles/200526207-Using-Projects.
...
2.	Edición y ejecución de código R
	...
	RStudio proporciona un entorno integrado para editar y ejecutar código R.  La documentación puede encontrarse en https://support.rstudio.com/hc/es-es/articles/200484448-Editing-and-Executing-Code.

3.	Depuración

	RStudio incluye eficaces capacidades de depuración.  Encontrará la documentación de estas características en https://support.rstudio.com/hc/es-es/articles/200713843-Debugging-with-RStudio.

	Las características de solución de problemas en puntos de interrupción se detallan en https://support.rstudio.com/hc/es-es/articles/200534337-Breakpoint-Troubleshooting.

##<a id="appendixb"></a>Apéndice B: lectura adicional

Si no está familiarizado con el código R, encontrará dos introducciones disponibles en CRAN.
...
- An Introduction to R by W. N. Venables et. al. aborda el código R en mayor profundidad. Lo encontrará en http://cran.r-project.org/doc/manuals/R-intro.html.
...
- bacoff es otro libro que puede resultarle muy útil.  El sitio de web complementario 'Quick R' es un recurso que le será de gran utilidad. http://www.statmethods.net/.
...
- Si desea obtener información más detallada sobre temas avanzados de R, le recomiendo el título 'Advanced R' de Hadley Wickham que estará próximamente disponible.  La versión en línea de este libro está disponible de forma gratuita en http://adv-r.had.co.nz/.

Encontrará un catálogo de paquetes de series temporales de R en la vista de tareas CRAN que podrá utilizar para realizar análisis de series temporales.: http://cran.r-project.org/web/views/TimeSeries.html. For information on specific time series object packages, you should refer to the documentation for that package.
