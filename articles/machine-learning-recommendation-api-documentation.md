...
API documentation" pageTitle="Recomendaciones del aprendizaje automático de Azure: 
Documentación de la API" description="Recomendaciones de Aprendizaje automático de Azure - 
Documentación de API" metaKeywords="" services="machine-learning" solutions="" documentationCenter="" authors="AharonGumnik" manager="paulettm" editor="cgronlun" videoId="" scriptId="" />
...
#Recomendaciones del aprendizaje automático de Azure: <br>Documentación de la API

En este documento se describen las API de las recomendaciones de ML de Azure.

##Contenido

- [1. Información general](#1-general-overview)
- [2. Limitaciones](#2-limitations)
- [3. API: información general](#3-apis--general-information)
	- [3.1. Autenticación](#31-authentication)
	- [3,2. URI de servicio](#32-service-uri)
	- [3,3. Versión de API](#33-api-version)
- [4. Modelo básico](#4-model-basic)
	- [4.1. Crear modelo](#41-create-a-model)
	- [4,2. Obtener modelo](#42-get-model)
	- [4,3. Obtener todos los modelos](#43-get-all-models)
	- [4,4. Actualizar modelo](#44-update-model)
	- [4,5. Eliminar modelo](#45-delete-model)
- [5. Modelo avanzado](#5-model-advanced)
	- [5.1. Perspectiva de datos del modelo](#51-model-data-insight)
	- [5,2. Perspectiva de modelo](#52-model-insight)
	- [5,3. Obtener el ejemplo de modelo](#53-get-model-sample)
- [6. Reglas de negocio de modelo](#6-model-business-rules)
	- [6,1. Obtener reglas de modelo](#61-get-model-rules)
	- [6,2. Agregar regla](#62-add-rule)
	- [6,3. Eliminar regla](#63-delete-rule)
	- [6,4. Eliminar todas las reglas](#64-delete-all-rules)
- [7. Catálogo](#7-catalog)
	- [7,1. Importar datos de catálogo](#71-import-catalog-data)
	- [7,2. Obtener catálogo](#72-get-catalog)
	- [7,3. Obtener elementos de catálogo por token](#73-get-catalog-items-by-token)
- [8. Datos de uso](#8-usage-data)
	- [8.1. Importar datos de uso](#81-import-usage-data)
		- [8.1.1. Cargar archivo](#811-uploading-file)
		- [8.1.2. Utilizar la adquisición de datos](#812-using-data-acquisition)
	- [8,2. Archivos de uso del modelo de lista](#82-list-model-usage-files)
	- [8,3. Obtener estadísticas de uso](#83-get-usage-statistics)
	- [8,4. Obtener ejemplo de archivo de uso](#84-get-usage-file-sample)
	- [8,5. Obtener archivo de uso de modelo](#85-get-model-usage-file)
	- [8,6. Eliminar archivo de uso](#86-delete-usage-file)
	- [8,7. Elimine todos los archivos de uso](#87-delete-all-usage-files)
- [9. Compilación](#9-build)
	- [9,1. Crear un modelo](#91-build-a-model)
	- [9,2. Obtener el estado de compilación de un modelo](#92-get-builds-status-of-a-model)
	- [9,3. Obtener el estado de compilación de un usuario](#93-get-builds-status-of-a-user)
	- [9,4. Eliminar compilación](#94-delete-build)
	- [9,5. Cancelar la compilación](#95-cancel-build)
	- [9,6. Obtener parámetros de compilación](#96-get-build-parameters)
- [10. Recomendación](#10-recommendation)
	- [10.1. Obtener recomendaciones](#101-get-recommendations)
- [11. Notificaciones](#11-notifications)
	- [11,1. Obtener notificaciones](#111-get-notifications)
	- [11,2. Eliminar notificaciones de modelo](#112-delete-model-notifications)
	- [11,3. Eliminar notificaciones de usuario](#113-delete-user-notifications)
- [12. Información legal](#12-legal)
...
2.	<ins>Modelo avanzado</ins>: las API que permiten obtener detalles de datos avanzados en el modelo
...
* Número máximo de modelos por suscripción: de
* Número máximo de elementos que puede contener un catálogo: 100.000
* La cantidad máxima de puntos de uso que se mantienen es ~ 5.000.000. Se eliminarán los más antiguos si se cargan o notifican nuevos.
...
|	Nombre del parámetro	|	Valores válidos						|
|:--------			|:--------								|
|	modelName	|	Solo se permiten letras (A-Z, a-z), números (0-9), guiones (-) y carácter de subrayado (_).<br>Longitud máxima: 20 |
|	apiVersion		| 1.0 |
|||
...
###4,4.	Actualizar el modelo

Puede actualizar la descripción del modelo o el identificador de compilación activa.<br>
<ins>Identificador de compilación activa</ins>: todas las compilaciones para cada modelo tienen un "identificador de compilación". El "identificador de compilación" activa es la primera compilación correcta de cada nuevo modelo. Una vez que tiene un id. de compilación activa y realiza compilaciones adicionales para el mismo modelo, tiene que establecerlo de manera explícita como la compilación predeterminada si quiere. Cuando se usan las recomendaciones, si no se especifica el identificador de compilación que desea usar, se utilizará automáticamente el valor predeterminado.<br>
...
|	apiVersion		| 1.0 |
|||
| Cuerpo de la solicitud | <ins>Para agregar la regla BlockList:</ins><br>`<ApiFilter xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><ModelId>24024f7e-b45c-419e-bfa2-dfd947e0d253</ModelId><Type>BlockList</Type><Value>{"ItemsToExclude":["2406E770-769C-4189-89DE-1C9283F93A96"]}</Value></ApiFilter>`<br><br><ins>Para agregar la regla BlockList:</ins><br>`<ApiFilter xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><ModelId>24024f7e-b45c-419e-bfa2-dfd947e0d253</ModelId><Type>Upsale</Type><Value>{"ItemsToUpsale":["2406E770-769C-4189-89DE-1C9283F93A96"]}</Value></ApiFilter>`|
...
|:--------			|:--------								|
|	modelId	|	El identificador único del modelo.  |
| filename | Identificador textual del catálogo.<br>Solo se permiten letras (A-Z, a-z), números (0-9), guiones (-) y caráctere de subrayado (_)<br>Longitud máxima: 50 |
|	apiVersion		| 1.0 |
|||
| Cuerpo de la solicitud | Los datos de uso. Formato:<br>`<User Id>,<Item Id>[,<Time>,<Event>]`<br><br><table><tr><th>Nombre</th><th>Obligatorio</th><th>Tipo</th><th>Descripción</th></tr><tr><td>Id. de usuario</td><td>Sí</td><td>Alfanuméricas</td><td>Identificador único de un usuario</td></tr><tr><td>Id. de elemento</td><td>Sí</td><td>Alfanuméricas, longitud máxima 50</td><td>Identificador único de un elemento</td></tr><tr><td>Hora</td><td>No</td><td>Fecha en formato: AAAA/MM/DDTHH:MM:SS (por ejemplo, 2013/06/20T10:00:00)</td><td>Hora de los datos</td></tr><tr><td>Evento</td><td>No, si se suministra también se debe colocar la fecha</td><td>Una de los siguientes opciones:<br>• Click<br>• RecommendationClick<br>•	AddShopCart<br>• RemoveShopCart<br>• Purchase</td><td></td></tr></table><br>Tamaño de archivo máximo 200 MB<br><br>Ejemplo:<br><pre>149452,1b3d95e2-84e4-414c-bb38-be9cf461c347<br>6360,1b3d95e2-84e4-414c-bb38-be9cf461c347<br>50321,1b3d95e2-84e4-414c-bb38-be9cf461c347<br>71285,1b3d95e2-84e4-414c-bb38-be9cf461c347<br>224450,1b3d95e2-84e4-414c-bb38-be9cf461c347<br>236645,1b3d95e2-84e4-414c-bb38-be9cf461c347<br>107951,1b3d95e2-84e4-414c-bb38-be9cf461c347</pre> |
...
Request body
	...
	Ejemplo de RecommendationClick:
	<Event xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  	<ModelId>2779c063-48fb-46c1-bae3-74acddc8c1d1</ModelId>
  	<SessionId>11112222</SessionId>
  	<EventData>
	...
	Ejemplo de envío de 2 eventos "Click" y "AddShopCart:
	<Event xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  	<ModelId>2779c063-48fb-46c1-bae3-74acddc8c1d1</ModelId>
  	<SessionId>11112222</SessionId>
  	<EventData>
...
| endDate |	Fecha final. Formato: aaaa/MM/ddTHH:mm:ss |
| debe establecer |	Cadena de tipos de eventos separados por comas o null para obtener todos los eventos.  |
| apiVersion | 1.0 |
|||
| Cuerpo de la solicitud | NINGUNO |
...
###8,7.	Elimine todos los archivos de uso
...
|:--------			|:--------								|
| modelId |	El identificador único del modelo.  |
| userDescription | Identifier textual del catálogo. Tenga en cuenta que si usa espacios debe codificarlo con un 20 % en su lugar. Vea el ejemplo anterior.<br>Longitud máxima: 50 |
| apiVersion | 1.0 |
|||
| Cuerpo de la solicitud | Si lo deja en blanco, la compilación se ejecutará con los parámetros de la compilación predeterminados<br><br>Si desea establecer que los parámetros de compilación envíen el siguiente código XML en el cuerpo (vea la sección Obtener parámetros de compilación para consultar la explicación de los parámetros):<br>`<BuildParametersList xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><NumberOfModelIterations>40</NumberOfModelIterations><NumberOfModelDimensions>20</NumberOfModelDimensions><UseFeaturesInModel>false</UseFeaturesInModel><AllowColdItemPlacement>false</AllowColdItemPlacement><MinItemAppearance>5</MinItemAppearance><MinUserAppearance>5</MinUserAppearance><EnableFeatureCorrelation>false</EnableFeatureCorrelation><RefreshFeatureScoreOnBuild>false</RefreshFeatureScoreOnBuild><ComputeUpd>false</ComputeUpd><EnableModelingInsights>true</EnableModelingInsights><ModelingFeatureList /><ReasoningFeatureList /></BuildParametersList>`|
...
NOTA: no puede eliminar una compilación activa. El modelo se debe actualizar a una compilación activa diferente antes de eliminarlo.
...
| includeMetatadata | Uso futuro, coloque siempre falso. |
| apiVersion | 1.0 |
...
##11. Notificaciones
Las recomendaciones de ML de Azure crean notificación cuando se producen errores persistentes en el sistema. Hay 3 tipos de notificaciones:
1.	Error de compilación: esta notificación se desencadena para cada error de compilación.
...
|	Nombre del parámetro	|	Valores válidos						|
|:--------			|:--------								|
| modelId | Parámetro opcional; cuando se omita, obtendrá todas las notificaciones para todos los modelos. <br>Valor válido: El identificador único del modelo. |
| apiVersion | 1.0 |
|||
| Cuerpo de la solicitud | NINGUNO |
