...
2. Si aún no lo ha hecho, haga clic en **Apps** (Aplicaciones), luego, en **Register as a Developer** (Registrarse como desarrollador), acepte la política y siga los pasos de registro.
3. Haga clic en **Apps** (Aplicaciones) y, luego, en **Add a new App** (Agregar una nueva aplicación). A continuación, elija **Website** (Sitio web) como plataforma y haga clic en **Skip and Create App ID** (Omitir y crear un id. de aplicación).
...
8. Copie el valor de **App ID** (Id. de aplicación). Haga clic en **Show** (Mostrar) y copie el valor de **App Secret** (Secreto de la aplicación). Necesitará ambos para configurar Facebook como proveedor de identidades de su directorio.
...
9. Haga clic en la pestaña **Advanced** (Avanzadas) en la parte superior y, luego, escriba [https://login.microsoftonline.com/te/{directory}/oauth2/authresp](https://login.microsoftonline.com/te/{directory}/oauth2/authresp) en el campo **Valid OAuth redirect URIs** (URI de redirección de OAuth válidos) (en la sección **Security** (Seguridad)), donde **{directory}** se sustituirá por el nombre de su directorio (por ejemplo, contosob2c.onmicrosoft.com). Haga clic en **Save Changes** (Guardar cambios) en la parte inferior de la página.

    ![FB - URI de redirección de OAuth](./media/active-directory-b2c-setup-fb-app/fb-oauth-redirect-uri.png)

10. Para que la aplicación Facebook se puede usar con Azure AD B2C, es necesario ponerla a disposición del público. Para ello, haga clic en **Estado y revisión** en el panel de navegación izquierdo y mueva el conmutador en la parte superior de la página a **Sí**. A continuación, haga clic en **Confirmar**.

    ![FB - URI de redirección de OAuth](./media/active-directory-b2c-setup-fb-app/fb-app-public.png)

## Configuración de Facebook como proveedor de identidades del directorio
