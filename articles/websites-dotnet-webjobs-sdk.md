...
El marco también enlaza un objeto `TextWriter` a un blob llamado "nombreblob" en un contenedor llamado "nombrecontenedor":
...
* Puede ver los registros en el Panel, vinculado al Trabajo web específico (registros de aplicación, escritos usando Console.Out, Console.Error, Trace, etc.) o vinculado a la invocación de función determinada que los generó (registros escritos usando un objeto `TextWriter` que el SDK pasa a la función como parámetro). 
