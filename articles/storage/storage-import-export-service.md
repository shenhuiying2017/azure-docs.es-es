...
Para comenzar el proceso de importación o exportación desde el almacenamiento en blobs, cree primero un *job*. Un trabajo puede ser un *import job* o un *export job*:
...
Cuando cree un trabajo de importación o de exportación, deberá tener también el *drive ID*, que es el número de serie que asigna el fabricante de la unidad a un disco duro específico. El identificador de la unidad se muestra en el exterior de la misma. 

<h3>Requisitos y alcance</h3>

1.	**Cuentas de suscripción y almacenamiento:** debe disponer de una suscripción a Azure existente y una o más cuentas de almacenamiento para utilizar el servicio de importación y exportación. Cada trabajo puede usarse para transferir datos hacia o desde solamente una cuenta de almacenamiento. En otras palabras, un trabajo no puede abarcar varias cuentas de almacenamiento. Para obtener más información acerca de cómo crear una nueva cuenta de almacenamiento, consulte [Cómo crear una cuenta de almacenamiento](storage-create-storage-account.md).
2.	**Unidades de disco duro:** solo se admiten las unidades de disco duro de 3,5 pulgadas SATA II/III para su uso con el servicio de importación y exportación. No se admiten las unidades de disco duros mayores de 4 TB. Para los trabajos de importación, se procesará solo el primer volumen de datos de la unidad. El volumen de datos debe tener formato NTFS. Puede conectar un disco SATA II/III de forma externa con la mayoría de equipos mediante un adaptador de USB de SATA II/III.
...
4.	**Destinos de almacenamiento de blobs:** los datos pueden cargarse a o descargarse de blobs en bloques y blobs de página.
5.	**Número de trabajos:** un cliente puede tener hasta 20 trabajos activos por cuenta de almacenamiento.
6.	**Tamaño máximo de un trabajo:** el tamaño de un trabajo viene determinado por la capacidad de los discos duros utilizados y la cantidad máxima de datos que se puede almacenar en una cuenta de almacenamiento. Cada trabajo puede contener hasta 10 unidades de disco duro.
...
La herramienta de importación y exportación de Microsoft Azure genera un archivo *drive journal* para cada unidad mientras se prepara. El archivo de diario de la unidad se almacena en su equipo local, no en la unidad en sí. Cargará el archivo de diario cuando haya creado el trabajo de importación. Un archivo de diario de unidad incluye el identificador de la unidad y la clave de BitLocker, así como otras informaciones acerca de la unidad.  
...
2. 	En el paso 2, proporcione la información de contacto de la persona responsable de este trabajo de exportación. Si desea guardar 	datos de registro detallados del trabajo de exportación, active la opción para <strong>Guardar el registro detallado en mi contenedor de blobs  'waimportexport'</strong>.
