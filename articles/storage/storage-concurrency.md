...
5.	Si el valor ETaf actual del blob es la misma versión que el valor ETag del encabezado condicional **If-Match** de la solicitud, el servicio realiza la operación solicitada y actualiza el valor ETag actual del blob para mostrar que ha creado una nueva versión.  

El siguiente fragmento de código de C# (usando Biblioteca de almacenamiento de cliente 4.2.0) muestra un ejemplo sencillo de cómo constuir una **If-Match AccessCondition** basándose en el valor ETag al que se tiene acceso desde las propiedades de un blob que se recuperó o insertó previamente. Luego usa el objeto **AccessCondition** cuando actualiza el blob: el objeto **AccessCondition** agrega el encabezado **If-Match** a la solicitud. Si otro proceso ha actualizado el blob, el servicio BLOB devuelve un mensaje de estado HTTP 412 (Error en la condición previa). Puede descargar el ejemplo completo [aquí](http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114).  
...
Tenga en cuenta que las operaciones **Insert or Replace Entity** e **Insert or Merge Entity** *not* no realizan ninguna comprobación de simultaneidad porque no envían un valor de ETag al servicio tabla.  
...
- [Administración de simultaneidad usando Almacenamiento de Azure: aplicación de ejemplo](http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114)  
...
- Introducción al almacenamiento para [Blob](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-blobs/), [Tabla](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-tables/) y [Colas](http://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-queues/)
