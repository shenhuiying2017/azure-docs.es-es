...
**Cuentas de Blob Storage** son cuentas de almacenamiento especiales para almacenar los datos no estructurados como blobs (objetos) en Azure Storage. Con las cuentas de Blob Storage, ahora puede elegir entre los niveles de almacenamiento esporádico y frecuente en el nivel de cuenta, y los niveles de almacenamiento esporádico, frecuente y de archivo en el nivel de blob, según los patrones de acceso. Para optimizar costos, se recomienda encarecidamente almacenar los datos a los que no se accede casi nunca, con poca frecuencia y frecuentemente en los niveles de almacenamiento frecuente, esporádico y de archivo, respectivamente. Las cuentas de Blob Storage son similares a las de almacenamiento de uso general existentes y comparten las excelentes características de rendimiento, escalabilidad, disponibilidad y durabilidad que se usan en la actualidad, incluida la coherencia total de la API para blobs en bloques y blobs en anexos.

> [!NOTE]
> Las cuentas de Blob Storage solo admiten blobs en bloques y en anexos, pero no blobs en páginas.

Las cuentas de Blob Storage exponen el atributo **Access Tier** (Nivel de acceso) en el nivel de cuenta, que especifica el nivel de la cuenta de almacenamiento predeterminado como **Frecuente** o **Esporádico**. El nivel de cuenta de almacenamiento predeterminado se aplica a todos los blobs que no tienen un nivel explícito establecido en el nivel de blob. Si hay un cambio en el patrón de uso de los datos, también se puede cambiar de capa de almacenamiento en cualquier momento. El **nivel de archivo** (versión preliminar) solo puede aplicarse en el nivel de blob.
...
* Necesita usar tablas, colas o archivos, y desea que los blobs se almacenen en la misma cuenta de almacenamiento. La única ventaja técnica de almacenarlos en la misma cuenta es que tienen las mismas claves compartidas.
...
Cuando un blob se mueve a un nivel más esporádico (Permanente -> Esporádico, Permanente -> Archivo o Esporádico -> Archivo), la operación se factura como una operación de escritura en el nivel de destino, y se aplicarán los cargos de la operación de escritura (por 10 000) y de la escritura de datos (por GB) del nivel de destino. Si un blob se mueve a un nivel menos esporádico (Archivo -> Esporádico, Archivo -> Permanente -> Esporádico -> Permanente), la operación se factura como una operación de lectura desde el nivel de origen, y se aplicarán los cargos de la operación de lectura (por 10 000) y de la recuperación de datos (por GB) del nivel de origen.
...
* Solo admiten el almacenamiento de archivo las cuentas nuevas de Blob Storage creadas en el Este de EE. UU. 2, Este de EE. UU. u Oeste de EE. UU. después de inscribirse correctamente en la versión preliminar.
...
| **Duración mínima del almacenamiento** | N/D | N/D | 180 días
...
Las cuentas de Blob Storage usan un modelo de precios para el almacenamiento de blobs que se basa en el nivel de cada blob. Si se usa una cuenta de Almacenamiento de blobs, se aplicarán las siguientes consideraciones de facturación:
...
* **Cambio del nivel de almacenamiento**: el cambio de la capa de almacenamiento de la cuenta pasa de esporádico a frecuente conlleva un cargo igual a la lectura de todos los datos existentes en la cuenta de almacenamiento. Sin embargo, el cambio de la capa de almacenamiento de la cuenta cambia de frecuente a esporádico crea una carga que equivale a la escritura de todos los datos en el nivel de acceso esporádico.
...
Para supervisar las cuentas de almacenamiento existentes y recopilar estos datos, se puede usar Azure Storage Analytics, que realiza el registro y proporciona los datos de métricas de una cuenta de almacenamiento. Storage Analytics puede almacenar métricas que incluyen las estadísticas de transacciones agregadas y los datos de capacidad de las solicitudes al servicio Blob Storage tanto de cuentas de almacenamiento de uso general como cuentas de Blob Storage. Estos datos se almacenan en tablas conocidas de la misma cuenta de almacenamiento.
...
2. **¿Por qué y cuándo conviene empezar a usar cuentas de Almacenamiento de blobs?**
    ...
    Las cuentas de Blob Storage están especializadas en el almacenamiento de blobs e incorporan nuevas características centradas en los blobs. Es más, las cuentas de Almacenamiento de blobs son el método recomendado para el almacenamiento de blobs, ya que se introducirán funcionalidades futuras como el almacenamiento jerárquico y la organización en niveles basadas en este tipo de cuenta. Sin embargo, cada usuario debe decidir en qué momento desea realizar la migración en función de sus requisitos empresariales.
...
4. **¿Se pueden almacenar objetos en dos capas de almacenamiento en la misma cuenta?**
    ...
    Sí. El atributo *'Access Tier'* (Nivel de acceso) establecido en un nivel de cuenta es el nivel predeterminado que se aplica a todos los objetos de dicha cuenta sin un nivel de conjunto explícito. Sin embargo, el almacenamiento por niveles de blob (versión preliminar) permite establecer el nivel de acceso en el nivel de objeto, independientemente de cuál el valor del nivel de acceso. Los blobs de cualquiera de los tres niveles de almacenamiento (permanente, esporádico o archivo) pueden existir en la misma cuenta.

5. **¿Se puede cambiar la capa de almacenamiento de una cuenta de Almacenamiento de blobs?**

    Si, para cambiar el nivel de almacenamiento establezca el atributo *'Access Tier'* (Nivel de acceso) en la cuenta de almacenamiento. El cambio del nivel de almacenamiento se aplica a todos los objetos almacenados en la cuenta que no tengan un nivel explícito establecido. El cambio de la capa de almacenamiento de frecuente a esporádico incurre en gastos de operaciones de escritura (por 10 000) y de escritura de datos (por GB) (solo cuentas de Blob Storage), mientras que el cambio de esporádico a frecuente incurre en gastos de operaciones de lectura (por 10 000) y de recuperación de datos (por GB) para leer todos los datos de la cuenta.
...
7. **¿Los blobs en el nivel de almacenamiento esporádico se comportan de forma diferente a los del nivel de almacenamiento frecuente?**
    ...
    Los blobs de la capa de almacenamiento frecuente tienen la misma latencia que los de las cuentas de almacenamiento de uso general. Los blobs de la capa de almacenamiento esporádico tienen una latencia similar (en milisegundos) a los de las cuentas de almacenamiento de uso general. Los blobs del nivel de almacenamiento de archivo tienen varias horas de latencia.
