...
2.  Reemplace el contenido de SampleJob.cs por el código siguiente.
         ...
         {
             // Trabajo programado simple que puede invocarse manualmente enviando una solicitud
             // HTTP POST a la ruta de acceso "/jobs/sample".
             public class SampleJob : ScheduledJob
             {
                 ...
                 protected override void Initialize(ScheduledJobDescriptor scheduledJobDescriptor, CancellationToken cancellationToken)
                 {
                     ...
                     // Crear un nuevo contexto con el nombre de esquema proporcionado.
                     context = new todolistContext(Services.Settings.Name);
                 }
            
                 public async override Task ExecuteAsync()
                 {            
                     // Intentar obtener el token de acceso a Twitter almacenado de la configuración de la aplicación.  
                     ...
                     // Crear un nuevo autorizador para obtener acceso a las API de Twitter v1.1
                     // con las credenciales de usuario único de OAUth 2.0.
                     ...
                     // Establecer las credenciales para el autorizador.
                     auth.CredentialStore = store;
            
                     // Crear un nuevo contexto de LINQ to Twitter.
                     TwitterContext twitter = new TwitterContext(auth);
            
                     // Obtener el identificador del tweet almacenado más reciente.
                     long lastTweetId = 0;
                     ...
                     // Ejecutar una búsqueda que devuelva un resultado filtrado.
                     ...
                     // Quitar retweets y respuestas y registrar el número de tweets.
                     ...
                     // Almacenar nuevos tweets en la tabla Updates.
    ...
    En el código anterior, debe reemplazar las cadenas *todolistService* y *todolistContext* por el espacio de nombres y el valor DbContext del proyecto descargado, que son <em>nombre\_servicio\_móvil</em>Service y <em>nombre\_servicio\_móvil</em>Context, respectivamente.
...
2.  Copie la dirección URL del servicio móvil desde la ventana del explorador abierta, anexe la ruta de acceso `/tables/samplejob` a la dirección URL y, a continuación, ejecute una nueva solicitud POST para esta dirección URL mediante una utilidad HTTP, como Fiddler.

    Se inicia el método **ExecuteAsync** en el equipo local.
...
Publicación del servicio y registro del nuevo trabajo
-----------------------------------------------------
...
2.  En el [Portal de administración de Azure](https://manage.windowsazure.com/), haga clic en Servicios móviles y, a continuación, en la aplicación.
...
1.  (Opcional) En el [Portal de administración de Azure](https://manage.windowsazure.com/), haga clic en Manage para la base de datos asociada al servicio móvil.

    ![](./media/mobile-services-dotnet-backend-schedule-recurring-tasks/manage-sql-azure-database.png)

2.  Asimismo, en dicho portal, ejecute una consulta para ver los cambios realizados por la aplicación. La consulta puede ser similar a la siguiente, pero use el nombre del servicio móvil como nombre de esquema en lugar de `todolist`.
