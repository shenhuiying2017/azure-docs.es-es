<properties 
	pageTitle="Administración de clústeres de Hadoop en HDInsight usando el portal de Azure | Azure" 
...
- **Suscripción de Azure**. Azure es una plataforma basada en suscripción. Para obtener información acerca de cómo obtener una suscripción, consulte [Opciones de compra][azure-purchase-options], [Ofertas para miembros][azure-member-offers] o [Prueba gratuita][azure-free-trial].
...
* [Apertura de la consola de comandos de Hadoop](#hadoopcmd)
...
2. Haga clic en **NUEVO** en la parte inferior de la página, después en **SERVICIOS DE DATOS**, a continuación en **HDINSIGHT** y, finalmente, en **CREACIÓN RÁPIDA**.

3. Proporcione los valores de **Nombre de clúster**, **Tamaño de clúster**, **Contraseña de administración del clúster** y una **cuenta de almacenamiento** de Azure y después haga clic en **Crear clúster de HDInsight**. Después de que el clúster se crea y está en funcionamiento, el estado muestra  *Running*.
	...
	> [AZURE.NOTE] Una vez que se elige una cuenta de almacenamiento de Azure para su clúster de HDInsight, la única forma de cambiar la cuenta de almacenamiento es eliminar el clúster y crear un clúster nuevo con la cuenta de almacenamiento que desee.
...
- Algunos de los componentes nativos de Java, como Mahout, Cascading, se pueden ejecutar en el clúster como archivos JAR. Estos archivos JAR se pueden distribuir al almacenamiento de blobs de Azure (WASB) y someterse a los clústeres de HDInsight usando los mecanismos de envío de trabajo de Hadoop. Para obtener más información, consulte [Envío de trabajos de Hadoop mediante programación][hdinsight-submit-jobs]. 
	...
	>[AZURE.NOTE] Si tiene problemas con la implementación de los archivos jar en los clústeres de HDInsight o con la llamada de los archivos jar en los clústeres de HDInsight, póngase en contacto con [Soporte técnico de Microsoft][hdinsight-support].
...
2. Busque la ubicación del certificado, haga clic con el botón secundario en él, después haga clic en **Instalar certificado** y, por último, instale el certificado en el almacén personal del equipo. Edite las propiedades del certificado para asignarle un nombre descriptivo.
...
* [Administración de HDInsight mediante la interfaz de línea de comandos entre plataformas][hdinsight-admin-cross-platform]
...
[hdinsight-support]: http://azure.microsoft.com/es-es/support/options/
[makecert-info]: http://msdn.microsoft.com/es-es/library/bfsktky3(v=vs.110).aspx
