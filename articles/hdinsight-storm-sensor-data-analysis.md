<properties title="An&aacute;lisis de los datos de sensor con Storm y HDInsight" pageTitle="An&aacute;lisis de los datos de sensor con Apache Storm y Microsoft Azure HDInsight (Hadoop)" description="Obtenga informaci&oacute;n sobre c&oacute;mo utilizar Apache Storm para procesar los datos de sensor en tiempo real con HDInsight (Hadoop)" metaKeywords="Azure hdinsight storm, Azure hdinsight realtime, azure hadoop storm, azure hadoop realtime, azure hadoop real-time, azure hdinsight real-time" services="hdinsight" solutions="" documentationCenter="big-data" authors="larryfr" videoId="" scriptId="" />
...
> [AZURE.NOTE] En <https://github.com/Blackmist/hdinsight-eventhub-example> está disponible una versión completa de este proyecto.

## Requisitos previos

-   Una suscripción de Azure

-   Visual Studio con el [SDK de Microsoft Azure para .NET][SDK de Microsoft Azure para .NET]

-   [Java y JDK][Java y JDK]

-   [Maven][Maven]

-   [Git][Git]

> [AZURE.NOTE] Java, JDK, Maven y Git también están disponibles en el administrador de paquetes [Chocolatey NuGet][Chocolatey NuGet].

## Creación del panel

El panel se utiliza para mostrar información del sensor casi en tiempo real. En este caso, el panel es una aplicación de ASP.NET hospedada en un sitio web de Azure. El objetivo principal de la aplicación es servir como un centro de [SignalR][SignalR] que recibe información de la topología Storm cuando procesa mensajes.
...
>
> Para conocer un ejemplo del uso de la topología Storm para comunicarse con un sitio web Python utilizando WebSockets sin procesar, consulte el proyecto [Storm Tweet Sentiment D3 Visualization][Storm Tweet Sentiment D3 Visualization].

1.  En Visual Studio, cree una nueva aplicación C# utilizando la plantilla de proyecto **Aplicación web ASP.NET**. Dele el nombre de **Panel** a la nueva aplicación.
...
3.  En el cuadro de diálogo **Configurar sitio de Windows Azure**, introduzca un **nombre de sitio** y una **región** para el sitio web y, a continuación, haga clic en **Aceptar**. Se creará un Sitio web de Azure que albergará el panel.

4.  En **Explorador de soluciones**, haga clic con el botón secundario en el proyecto y seleccione **Agregar | Clase de concentrador SignalR (v2)**. Dé el nombre de **DashHub.cs** a la clase y agréguela al proyecto. Contendrá el concentrador SignalR que se utiliza para comunicar datos entre HDInsight y la página web del panel.

    > [AZURE.NOTE] Si está utilizando Visual Studio 2012, la plantilla **Clase de concentrador SignalR (v2)** no estará disponible. También puede agregar una **clase** sin formato denominada DashHub. También tendrá que instalar manualmente el paquete SignalR abriendo **Herramientas | Administrador de paquetes de la biblioteca | Consola del administrador de paquetes** y ejecutando el comando siguiente:
    >
...
6.  En **Explorador de soluciones**, haga clic con el botón secundario en el proyecto y seleccione **Agregar | Clase de inicio OWIN**. Llame a la nueva clase **Startup.cs**.

    > [AZURE.NOTE] Si está utilizando Visual Studio 2012, la plantilla **Clase de inicio OWIN** no estará disponible. También puede agregar una **clase** denominada Startup.
...
8.  En el **Explorador de soluciones**, haga clic con el botón secundario en el proyecto y, a continuación, haga clic en **Agregar | Página HTML**. Dé el nombre de **index.html** a la nueva página. Esta página contiene el panel en tiempo real de este proyecto. Recibirá información de DashHub y mostrará un gráfico utilizando D3.js.

9.  En **Explorador de soluciones**, haga clic con el botón secundario en **index.html** y seleccione **Establecer como página de inicio**.

10. Reemplace el código en el archivo **index.html** por el código siguiente.
...
11. En el **Explorador de soluciones**, haga clic con el botón secundario en el proyecto y, a continuación, haga clic en **Agregar | Página HTML**. Dé el nombre de **test.html** a la nueva página. Esta página puede usarse para probar DashHub y el panel mediante el envío y recepción de mensajes.

12. Reemplace el código en el archivo **test.html** por el código siguiente.
...
1.  Para comprobar que SignalR está funcionando y que el panel mostrará líneas de gráficos para los datos enviados a SignalR, abra una nueva ventana del navegador para la página **test.html** en este sitio web. Por ejemplo, **http://mydashboard.azurewebsites.net/test.html**.
...
1.  En el [Portal de Azure][Portal de Azure], seleccione **NUEVO | Bus de servicio | Centro de eventos | Creación personalizada**.

2.  En el cuadro de diálogo **Agregar un nuevo centro de eventos**, introduzca un **nombre del centro de eventos**, seleccione la **región** en donde se va a crear el centro y cree un nuevo espacio de nombres o seleccione uno existente. Finalmente, haga clic en la **flecha**.

3.  En el cuadro de diálogo **Configuración del centro de eventos**, introduzca los valores de **Recuento de particiones** y **Retención de mensajes**. En este ejemplo, utilice un recuento de particiones de 10 y una retención de mensajes de 1.
...
5.  Seleccione **Configurar** y después cree dos directivas de acceso mediante la información siguiente.
...
6.  Después de guardar las directivas, utilice el **Generador de claves de acceso compartido** en la parte inferior de la página para recuperar la clave de las directivas **devices** y **storm**. Guárdelas ya que se utilizarán después.
...
1.  En Visual Studio, cree un nuevo proyecto de **Escritorio de Windows** y seleccione la plantilla de proyecto **Aplicación de consola**. Asigne el nombre **SendEvents** al proyecto y haga clic en **Aceptar**.
...
5.  En el archivo **Program.cs**, establezca el valor de las variables siguientes al comienzo del archivo en los valores correspondientes recuperados del Centro de eventos del Portal de administración de Azure.
    ...
    | Establezca esto...     | Para esto...                                                              |
    |------------------------|---------------------------------------------------------------------------|
    | eventHubName           | El nombre el centro de eventos. Por ejemplo, **temperature**.             |
    | eventHubNamespace      | El espacio de nombres del centro de eventos. Por ejemplo, **sensors-ns**. |
    | sharedAccessPolicyName | La directiva creada con acceso de envío. Por ejemplo, **devices**.        |
    | sharedAccessPolicyKey  | La clave de la directiva con acceso de envío.                             |

6.  En el **Explorador de soluciones**, haga clic con el botón secundario en **SendEvents** y en **Agregar | Clase**. Llame a la nueva clase **Event.cs**. Describirá el mensaje enviado al Centro de eventos.

7.  Reemplace el contenido de **Event.cs** por el código siguiente.
...
8.  **Guarde todo** y ejecute la aplicación para rellenar el Centro de eventos con mensajes.
...
2.  En la parte inferior de la página, haga clic sucesivamente en **+NUEVO**, **Servicios de red**, **Red virtual** y, a continuación, en **Creación rápida**.
...
7.  En **Vista rápida**, anote el **ID. DE RED VIRTUAL**. Lo necesitará al aprovisionar los clústeres de Storm y de HBase.
...
4.  En la página **Detalles del clúster**, introduzca el nombre del nuevo clúster y seleccione **HBase** en **Tipo de clúster**. Seleccione la flecha para continuar.
...
1.  Utilice el Escritorio remoto para conectarse a su clúster de Storm y después copiar el archivo **%STORM\_HOME%\\examples\\eventhubspout\\eventhubs-storm-spout-0.9-jar-with-dependencies.jar** a su entorno de desarrollo local. Contiene el **events-storm-spout**.
...
Para enviar mensajes al panel de ASP.NET, utilice el [SDK del cliente SignalR para Java][SDK del cliente SignalR para Java].
...
4.  Cambie los directorios en el directorio **java-client\\signalr-client-sdk** y compile el proyecto en un archivo JAR utilizando los comandos siguientes.

        cd java-client\signalr-client-sdk
        mvn package

    > [WACOM.NOTE] Si recibe un error que la dependencia **gson** no se puede descargar, quite las líneas siguientes del archivo **java-client\\signalr-client-sdk\\pom.xml**.
    >
    > ``` <repositories>
    > <repository>
    > <id>central</id>
...
A continuación, modifique **pom.xml** para hacer referencia a las dependencias de este proyecto, así como a los complementos de Maven que se van a utilizar al compilar y empaquetar.
...
1.  Con un editor de texto, abra el archivo **pom.xml** y agregue lo siguiente a la sección **\<dependencies\>**. Puede agregarlos al final de la sección, después de la dependencia para junit.
    ...
    > [WACOM.NOTE] Tenga en cuenta que algunas dependencias están marcadas con un ámbito **proporcionado** para indicar que estas dependencias deben descargarse del repositorio de Maven y utilizarse para compilar y probar localmente la aplicación, pero que también estarán disponibles en su entorno de ejecución y no necesitan compilarse e incluirse en el JAR creado por este proyecto.
...
2.  Al final del archivo **pom.xml**, justo delante de la entrada **\</project\>**, agregue lo siguiente.
    ...
    -   Utilice **maven-compiler-plugin** para compilar la aplicación.
    -   Utilice **maven-shade-plugin** para generar un uberjar o fat jar, que contiene este proyecto y cualquier dependencia requerida.
    -   Utilice **exec-maven-plugin**, que le permite ejecutar la aplicación localmente sin un clúster de Hadoop.
...
3.  Use lo siguiente como contenido para el archivo **Config.properties**.
    ...
    Reemplace la **contraseña** con la clave para la directiva **storm** creada anteriormente en el Centro de eventos. Reemplace **namespace** con el espacio de nombres del Centro de eventos.
...
4.  Use lo siguiente como contenido del archivo **DashboardBolt.java**.
    ...
    Reemplace `http://yourwebsiteaddress` con la dirección del Sitio web de Azure en la que ha publicado el panel. Por ejemplo, http://mydashboard.azurewebsites.net.
...
3.  Conecte el clúster de Storm de HDInsight mediante el Escritorio remoto y copie el archivo **TemperatureMonitor-1.0-SNAPSHOT.jar** al directorio **c:\\apps\\dist\\storm\<número de versión\>**.

4.  Utilice el icono de la **línea de comandos de HDInsight** en el escritorio del clúster para abrir un nuevo símbolo del sistema y utilice los comandos siguientes para ejecutar la topología.
...
-   Para obtener más información sobre Apache Storm, consulte <https://storm.incubator.apache.org/>.

-   Para obtener más información sobre HBase con HDInsight, consulte [Información general de HBase con HDInsight][Información general de HBase con HDInsight]

-   Para obtener más información sobre SignalR, consulte [ASP.NET SignalR][ASP.NET SignalR].

-   Para obtener más información sobre D3.js, consulte [D3.js - Documentos orientados a datos][D3.js - Documentos orientados a datos]

-   Para obtener información sobre la creación de topologías en .NET, consulte [Desarrollo de aplicaciones de procesamiento de datos de transmisión por secuencias con SCP.NET y C# en Storm en HDInsight][Desarrollo de aplicaciones de procesamiento de datos de transmisión por secuencias con SCP.NET y C# en Storm en HDInsight].

  [SDK de Microsoft Azure para .NET]: http://azure.microsoft.com/es-es/downloads/archive-net-downloads/
  ...
  [SDK del cliente SignalR para Java]: https://github.com/SignalR/java-client
  [Información general de HBase con HDInsight]: http://azure.microsoft.com/es-es/documentation/articles/hdinsight-hbase-overview/
  [ASP.NET SignalR]: http://signalr.net/
  [D3.js - Documentos orientados a datos]: http://d3js.org/
