<properties
	pageTitle="Autenticación entre servicios de Azure AD mediante OAuth2.0 | Microsoft Azure"
...
# Llamadas entre servicios mediante las credenciales del cliente
...
|-----------|------|------------|
| response\_type | requerido | Especifica el tipo de respuesta solicitado. En un flujo de concesión de credenciales de cliente, el valor debe ser **client\_credentials**.|
| client\_id | requerido | Especifica el identificador de cliente de Azure AD del servicio web que realiza la llamada. Para buscar el identificador del cliente de la aplicación de llamada, en el Portal de administración de Azure, haga clic sucesivamente en **Active Directory**, en el directorio, en la aplicación y, finalmente, en **Configurar**.|
| client\_secret | requerido | Escriba una clave registrada para el servicio web de llamada de Azure AD. Para crear una clave, en el Portal de administración de Azure, haga clic sucesivamente en **Active Directory**, en el directorio, en la aplicación y, finalmente, en **Configurar**. |
...
| Parámetro | Descripción |
|-------------|-------------|
|access\_token |El token de acceso solicitado. El servicio web de llamada puede usar este token para autenticarse en el servicio web de recepción. |
|access\_type | Indica el valor de tipo de token. El único tipo que admite Azure AD es el **portador**. Para más información sobre los tokens de portador, consulte [OAuth2.0 Authorization Framework: Bearer Token Usage (RFC 6750)](http://www.rfc-editor.org/rfc/rfc6750.txt) [Marco de autorización de OAuth2.0: uso del token de portador (RFC 6750)].
