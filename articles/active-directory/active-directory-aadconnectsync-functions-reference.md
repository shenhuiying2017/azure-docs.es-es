...
**Comentarios**: esta función convierte ambos parámetros en la representación binaria y establece un bit en los siguientes valores:
...
**Descripción**. la función ConvertToBase64 convierte una cadena en una en base64 de Unicode. Convierte el valor de una matriz de enteros en su representación de cadena equivalente que se codifica con dígitos de base 64.
...
**Ejemplo**: si dn es "cn=Joe,ou=Atlanta,ou=GA,ou=US, dc=contoso,dc=com", `DNComponentRev([dn],3)` `DNComponentRev([dn],1,"DC")` Ambos devuelven US.
