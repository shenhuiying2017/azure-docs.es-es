...
En este momento, el punto de conexión v2.0 no admite `end_session_endpoint` de OpenID Connect. Esto significa que la aplicación no puede enviar una solicitud al punto de conexión v2.0 para finalizar la sesión de un usuario y borrar las cookies que estableció el punto de conexión v2.0.
Para cerrar la sesión de un usuario, la aplicación simplemente finaliza su propia sesión con el usuario y deja intacta la sesión del usuario con el punto de conexión v2.0. La próxima vez que el usuario intente iniciar sesión, verá una página de "elegir cuenta", en la que se enumeran las cuentas con inicio de sesión activo. En esa página, el usuario puede elegir cerrar la sesión de cualquier cuenta y finalizar la sesión con el punto de conexión v2.0.
