...
| code_challenge_method | opcional    | Método utilizado para codificar `code_verifier` para el parámetro `code_challenge`. Puede ser `plain` o `S256`.  Si se excluye, se asume que `code_challenge` es texto no cifrado si se incluye `code_challenge`.  Azure AAD v2.0 admite `plain` y `S256`. Para obtener más información, consulte [PKCE RFC](https://tools.ietf.org/html/rfc7636). |
...
| code_verifier | opcional              | El mismo code_verifier que se usó para obtener el authorization_code.  Se requiere si PKCE se utilizó en la solicitud de concesión de código de autorización.  Para obtener más información, consulte [PKCE RFC](https://tools.ietf.org/html/rfc7636).                                                                                                                                                                                                                                                                                             |
#### <a name="successful-response"></a>Respuesta correcta
