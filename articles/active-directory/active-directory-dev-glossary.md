...
Una clase de [aplicación cliente](#client-application) registrada en Azure AD, diseñada para permitir el inicio de sesión y el [consentimiento](#consent) a partir de las cuentas de usuario que se aprovisionan en cualquier [inquilino](#tenant) de Azure AD, incluidos los inquilinos que no sean el que se registró originalmente el cliente. Por el contrario, una aplicación registrada como único inquilino solo permite inicios de sesión desde cuentas de usuario aprovisionadas en el mismo inquilino que donde se registra la aplicación. De forma predeterminada, las aplicaciones [cliente nativas](#native-client) son aplicaciones multiinquilino, mientras que las aplicaciones [cliente web](#web-client) tienen la capacidad de seleccionar entre uno y varios inquilinos.
...
- Permisos "delegados", que solicitan acceso [basado en el ámbito](#scopes) con autorización delegada del [propietario del recurso](#resource-owner) que ha iniciado sesión, y se manifiestan en tiempo de ejecución como [notificaciones "scp"](#claim) en el [token de acceso](#access-token) del cliente.
- Permisos de "aplicación", que solicitarán acceso [basado en roles](#roles) bajo las credenciales o identidad de la aplicación cliente y se manifiestan en tiempo de ejecución como [notificaciones de "roles"](#claim) en el token de acceso del cliente.
...
Un servidor de recursos expone las API e impone el acceso a sus recursos protegidos mediante [ámbitos](#scopes) y [roles](#roles), con la plataforma de autorización de OAuth 2.0. Algunos ejemplos son la API de Azure AD Graph que proporciona acceso a los datos del inquilino de Azure AD y las API de Office 365 que proporcionan acceso a datos como el correo, calendario y documentos, que también son accesibles desde la [API de Microsoft Graph][Microsoft-Graph].
...
Los roles son cadenas definidas por recursos (por ejemplo "Aprobador de gastos" o "Solo lectura"), administradas en el [Portal de Azure clásico][AZURE-classic-portal] a través del [manifiesto de aplicación](#application-manifest) del recurso, y almacenadas en la [propiedad appRoles][AAD-Graph-Sp-Entity] del recurso. Una aplicación cliente solicita [permiso](#permissions) para acceder a un rol de "aplicación", que se presenta al recurso en tiempo de ejecución en la [notificación "roles"](#claim) del [token de acceso](#access-token) del cliente. Los administradores de Azure AD pueden asignar usuarios a los roles de "usuario" a través del [Portal de Azure clásico][AZURE-classic-portal].

Para obtener una explicación detallada de los roles de aplicación expuestos por la API de Azure AD Graph, consulte los [ámbitos de permisos de API Graph][AAD-Graph-Perm-Scopes]. Para ver un ejemplo de implementación paso a paso, consulte la entrada de blob [Role based access control in cloud applications using Azure AD][Duyshant-Role-Blog] \(Control de acceso basado en roles en aplicaciones en la nube con Azure AD).
...
Los ámbitos son cadenas definidas por recursos (por ejemplo "Mail.Read" o "Directory.ReadWrite.All"), administradas en el [Portal de Azure clásico][AZURE-classic-portal] a través del [manifiesto de aplicación](#application-manifest) del recurso, y almacenadas en la [propiedad oauth2Permissions][AAD-Graph-Sp-Entity] del recurso. Una aplicación cliente solicita [permiso](#permissions) para acceder, que se presenta al recurso en tiempo de ejecución en la [notificación "scp"](#claim) del [token de acceso](#access-token) del cliente.

Como procedimiento recomendado para la convención de nomenclatura, utilice un formato "resource.operation.constraint". Para obtener una explicación detallada de los ámbitos expuestos por la API de Azure AD Graph, consulte los [ámbitos de permisos de API Graph][AAD-Graph-Perm-Scopes]. Para los ámbitos expuestos por los servicios de Office 365, consulte la [referencia a los permisos de la API de Office 365][O365-Perm-Ref].

## token de seguridad
Un documento firmado con notificaciones, como un token OAuth2 o una aserción SAML 2.0. En el caso de una [concesión de autorización](#authorization-grant) de OAuth 2.0, un [token de acceso](#access-token) (OAuth2) y un [token de identificador] \(OpenID Connect) son un tipo de token de seguridad, que se implementan como un [JSON Web Token (JWT)][JWT].

## objeto de entidad de servicio
Cuando se registra o se actualiza una aplicación en el [Portal de Azure clásico][AZURE-classic-portal], el portal crea o actualiza un objeto de aplicación y un [objeto de entidad de servicio](#application-object) correspondiente para dicho inquilino. El objeto de aplicación *define* globalmente la configuración de identidad de la aplicación (por todos los inquilinos a los que se le ha concedido acceso a la aplicación asociada) y es la plantilla desde la que *proceden* los objetos de entidad de servicio correspondientes para su uso local en tiempo de ejecución (en un inquilino específico).

Para más información, consulte [Objetos Application y objetos ServicePrincipal][AAD-App-SP-Objects].

## inicio de sesión
El proceso de una [aplicación cliente](#client-application) que inicia la autenticación del usuario final y captura el estado relacionado tras la autenticación, con el fin de adquirir un [token de seguridad](#security-token) y establecer el ámbito de la sesión de la aplicación en ese estado. El estado puede incluir artefactos, como información de perfil de usuario, e información derivada de las notificaciones de tokens.

La función de inicio de sesión de una aplicación se utiliza normalmente para implementar-el inicio de sesión único (SSO) y puede ir precedida de una función de "suscripción", que es el punto de entrada para que un usuario final obtenga acceso a una aplicación (después del primer inicio de sesión). La función de suscripción se usa para recopilar y conservar el estado adicional específico del usuario y puede requerir el [consentimiento del usuario](#consent).
...
## tenant
Una instancia de un directorio de Azure AD se conoce como inquilino de Azure AD. Proporciona una variedad de características, como un servicio de registro de aplicaciones integradas, la autenticación de cuentas de usuario y aplicaciones registradas, y los puntos de conexión REST necesarios para admitir varios protocolos, como OAuth2 y SAML. Los puntos de conexión incluyen el [punto de conexión de autorización](#authorization-endpoint), el [punto de conexión de token](#token-endpoint) y el punto de conexión "común" utilizado por las [aplicaciones multiinquilino](#multi-tenant-application).

Consulte [Obtención de un inquilino de Azure Active Directory][AAD-How-To-Tenant] para más información sobre las diversas maneras de acceder a un inquilino.

## punto de conexión de token
Uno de los puntos de conexión implementados por el [servidor de autorización](#authorization-server) para admitir las [concesiones de autorización](#authorization-grant) de OAuth2. Dependiendo de la concesión, se puede utilizar para proporcionar un [token de acceso](#access-token) (y posiblemente un token de "actualización") a un [cliente](#client-application), así como un [token de identificador](#ID-token) cuando la concesión se utiliza junto con el protocolo [OpenID Connect][OpenIDConnect].
...
Similar a cómo se usa un objeto de entidad de servicio para representar una instancia de aplicación, un objeto de entidad de seguridad de usuario es otro tipo de entidad de seguridad, que representa a un usuario. La [entidad de usuario][AAD-Graph-User-Entity] de Azure AD Graph define el esquema de un objeto de entidad de seguridad de usuario. La entidad de usuario consta de propiedades relacionadas con el usuario, como el nombre y apellido, el nombre principal de usuario, su pertenencia a roles de directorio, etc., proporcionando la configuración de la identidad de usuario necesaria para que Azure AD establezca una entidad de seguridad de usuario en tiempo de ejecución. La entidad de seguridad de usuario se utiliza para representar a un usuario autenticado al registrar la delegación del [consentimiento](#consent) o tomar decisiones de control de acceso, entre otras.

## cliente web
Un tipo de [aplicación cliente](#client-application) que ejecuta todo el código en un servidor web y, por tanto, se considera un cliente "confidencial" debido a su capacidad para almacenar las credenciales de forma privada o confidencial en el servidor. Consulte los [tipos de cliente y perfiles de OAuth2][OAuth2-Client-Types] para más información.
