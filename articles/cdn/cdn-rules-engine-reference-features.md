...
La clave de cifrado que se usará para cifrar y descifrar los valores de token viene determinada por la clave principal y las opciones de clave de copia de seguridad en la página de autenticación de token. Tenga en cuenta que las claves de cifrado son específicas de la plataforma.
...
Valor|Resultado
---|----
Enabled|Hace que el servidor perimetral no tenga en cuenta las mayúsculas y minúsculas al comparar las direcciones URL de los parámetros de autenticación basada en token.
...
**Propósito**: limita el ancho de banda para la respuesta de nuestros servidores perimetrales.
...
Valor|Resultado
--|--
Sobrescribir|Garantiza que se llevarán a cabo las siguientes acciones:<br/> - Sobrescribe el encabezado Cache-Control generado por el servidor de origen. <br/>- Agrega a la respuesta el encabezado Cache-Control generado por la característica Max-Age externa.
Pass Through|Garantiza que el encabezado Cache-Control generado por la característica Max-Age externa nunca se agregará a la respuesta. <br/> Si el servidor de origen genera un encabezado Cache-Control, se pasará directamente al usuario final. <br/> Si el servidor de origen no genera un encabezado Cache-Control, esta opción puede provocar el encabezado de respuesta no contenga un encabezado Cache-Control.
Add if Missing|Si no se recibió un encabezado Cache-Control desde el servidor de origen, esta opción agrega el encabezado Cache-Control generado por la característica Max-Age externa. Esta opción es útil para garantizar que todos los recursos se asignarán a un encabezado Cache-Control.
...
Una clave de caché es la ruta de acceso relativa que identifica un recurso con fines de almacenamiento en caché. En otras palabras, nuestros servidores buscarán una versión almacenada en caché de un recurso de acuerdo con su ruta de acceso definida por su clave de caché.
...
- Esta acción no afecta a las revalidaciones de caché del explorador al servidor perimetral. Estos tipos de revalidaciones dependen de los encabezados Cache-Control o Expires enviados al explorador, que se pueden personalizar con la característica Max-Age externa.
...
- Establezca la unidad de tiempo en "Off" para asignar un intervalo de max-age interna predeterminada de 7 días para las solicitudes que no tienen asignada una indicación de max-age en su encabezado Cache-Control o Expires.
...
**Propósito**: controla la generación de encabezados Expires mediante el servidor perimetral cuando está activa la característica Max-Age externa.
...
Valor|Resultado
--|--
Sobrescribir|Garantiza que se llevarán a cabo las siguientes acciones:<br/>- Sobrescribe el encabezado Expires generado por el servidor de origen.<br/>- Agrega a la respuesta el encabezado Expires generado por la característica Max-Age externa.
Pass Through|Garantiza que el encabezado Expires generado por la característica Max-Age externa nunca se agregará a la respuesta. <br/> Si el servidor de origen genera un encabezado Expires, se pasará directamente al usuario final. <br/>Si el servidor de origen no genera un encabezado Expires, esta opción puede provocar el encabezado de respuesta no contenga un encabezado Expires.
Add if Missing| Si no se recibió un encabezado Expires desde el servidor de origen, esta opción agrega el encabezado Expires generado por la característica Max-Age externa. Esta opción es útil para garantizar que todos los recursos se asignarán a un encabezado Expires.
...
Si habilita esta característica, se generarán encabezados Cache-Control:max-age y Expires en nuestros servidores perimetrales y se enviarán al cliente HTTP. De forma predeterminada, estos encabezados sobrescribirán los creados por el servidor de origen. Sin embargo, pueden utilizarse las características Tratamiento de encabezados Cache-Control y Tratamiento de encabezados Expires para modificar este comportamiento.

Información importante:

- Esta acción no afecta a las revalidaciones de caché del servidor perimetral al servidor de origen. Estos tipos de revalidaciones vienen determinados por los encabezados Cache-Control/Expires recibidos desde el servidor de origen, y pueden personalizarse con las características Max-Age interna predeterminada y Forzar Max-Age interna.
...
- Si se establece esta característica en un valor negativo, nuestros servidores perimetrales enviarán un encabezado Cache-Control:no-cache y una hora de expiración establecida en el pasado con cada respuesta al explorador. Aunque un cliente HTTP no almacenará la respuesta en caché, esta configuración no afectará a la capacidad de nuestros servidores perimetrales para almacenar en caché la respuesta del servidor de origen.
- Al establecer la unidad de tiempo en "Off" se deshabilitará esta característica. Los encabezados Cache-Control/Expires almacenados en caché con la respuesta del servidor de origen se pasarán directamente al explorador.
...
- Esta característica no afecta a las revalidaciones de caché del explorador al servidor perimetral. Estos tipos de revalidaciones dependen de los encabezados Cache-Control o Expires enviados al explorador.
- Esta característica no tiene un efecto visible en la respuesta entregada por un servidor perimetral al solicitante. Sin embargo, puede afectar a la cantidad de tráfico de revalidación enviado desde nuestros servidores perimetrales al servidor de origen.
- Para configurar esta característica:
...
Una solicitud de no almacenar en caché se produce cuando el cliente HTTP envía un encabezado Cache-Control:no-cache o Pragma:no-cache en la solicitud HTTP.
...
Valor|Resultado
-|-
Enabled|Impide que nuestros servidores perimetrales respondan a una solicitud de intervalo de bytes no válido con un código de estado 416 No se puede satisfacer el intervalo solicitado. En su lugar, nuestros servidores proporcionarán el recurso solicitado y devolverán 200 OK al cliente.
...
Tenga en cuenta que este intervalo de tiempo se inicia cuando expira la vigencia máxima del recurso, no cuando se produce la revalidación errónea. Por lo tanto, el período máximo durante el cual puede proporcionarse un recurso sin una revalidación correcta es la cantidad de tiempo especificadao por la combinación de max-age más max-stale. Por ejemplo, si un recurso se almacenó en caché a las 9:00 con un valor de max-age de 30 minutos y un valor de max-stale de 15 minutos, un intento de revalidación erróneo a las 9:44 hará que un usuario final reciba el recurso almacenado en caché obsoleto, mientras que un intento de revalidación erróneo a las 9:46 hará que el usuario final reciba un mensaje 504 Tiempo de espera agotado para la puerta de enlace.

Los valores configurados para esta característica se invalidan con los encabezados Cache-Control:must-revalidate o Cache-Control:proxy-revalidate recibidos desde el servidor de origen. Si alguno de los encabezados se recibe desde el servidor de origen cuando un recurso se almacena inicialmente en caché, el servidor perimetral no proporcionará un recurso almacenado en caché obsoleto. En este caso, si el servidor perimetral no puede revalidar con el origen una vez transcurrido el intervalo de max-age del recurso, el servidor perimetral devolverá un mensaje 504 Tiempo de espera agotado para la puerta de enlace.
...
Determina si se entregará el contenido almacenado en caché cuando se produzca un error durante la revalidación de caché o al recuperar el contenido solicitado desde el servidor de origen del cliente.

Valor|Resultado
-|-
Enabled|Cuando se produce un error durante una conexión a un servidor de origen, se servirá contenido obsoleto al solicitante.
Disabled|El error del servidor de origen se reenviará al solicitante.

**Comportamiento predeterminado**: deshabilitado.

###<a name="stale-while-revalidate"></a>Obsoleto durante revalidación
**Propósito**: mejora el rendimiento al permitir que nuestros servidores perimetrales sirvan contenido obsoleto al solicitante mientras se lleva a cabo la revalidación.
...
Valor|Resultado
--|--
Enabled | El encabezado de respuesta Age se incluirá en la respuesta enviada al solicitante.
Disabled | El encabezado de respuesta Age se excluirá de la respuesta enviada al solicitante.
...
La opción Header name (Nombre de encabezado) define el nombre del encabezado de solicitud personalizado donde se almacenará la dirección IP del cliente.
...
El propósito principal de este campo personalizado es que permite determinar qué valores de encabezado de solicitud y respuesta se almacenarán en los archivos de registro.
...
**Propósito**: determina el conjunto de métodos HTTP adicionales que pueden almacenarse en caché en nuestra red.
...
- De forma predeterminada, solo se almacenarán en caché las solicitudes cuyo cuerpo sea inferior a 14 Kb. Use la característica Tamaño del cuerpo de solicitud almacenable en caché para establecer el tamaño máximo del cuerpo de la solicitud.
