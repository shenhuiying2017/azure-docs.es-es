...
Almacenamiento de blobs de Azure de bajo coste es una solución de almacenamiento sólida, de uso general y compatible con Hadoop que se integra sin problemas con HDInsight. A través de una interfaz de sistema de archivos distribuidos (HDFS) de Hadoop, el conjunto completo de componentes de HDInsight puede operar directamente en datos en almacenamiento de blobs de Azure. En este tutorial, aprenderá a configurar un contenedor para almacenamiento de blobs y, a continuación, hará frente a los datos dentro.

Almacenar los datos en un almacenamiento de blobs de Azure hace que elimine de forma segura los clústeres de HDInsight que se usan para los cálculos sin perder los datos del usuario.
...
![Los clústeres de Hadoop en HDInsight acceden y almacenan grandes cantidades de datos en el almacenamiento de blobs de Azure en la nube compatible con Hadoop rentable y escalable.](./media/hdinsight-use-blob-storage/HDI.ASVArch.png "Arquitectura de almacenamiento de HDInsight")
...
- **Contenedores de las cuentas de almacenamiento que se conectan a un clúster:** dado que el nombre y la clave de la cuenta se almacenan en *core-site.xml*, tiene acceso total a los blobs de dichos contenedores.
...
Varios trabajos de WebHCat, incluidos Hive, MapReduce, streaming de Hadoop y Pig, pueden llevar una descripción de cuentas de almacenamiento y metadatos con ellos. (Actualmente esto funciona para Pig con cuentas de almacenamiento pero no para metadatos). En la sección [Acceso a un blob usando PowerShell](#powershell) de este artículo, existe una muestra de esta característica. Para obtener más información, consulte [Uso de un clúster de HDInsight con cuentas de almacenamiento y tiendas de metadatos alternativas](http://social.technet.microsoft.com/wiki/contents/articles/23256.using-an-hdinsight-cluster-with-alternate-storage-accounts-and-metastores.aspx).
...
##<a id="preparingblobstorage"></a>Preparación de un contenedor para el almacenamiento de blobs de Azure
...
###Creación de un contenedor de blobs para HDInsight usando el portal de Azure
...
[Azure PowerShell][powershell-install] se puede usar para crear contenedores. El siguiente script de PowerShell muestra cómo crear un contenedor de blobs en una cuenta de almacenamiento de Azure existente:
...
##<a id="addressing"></a>Archivos de dirección en almacenamiento de blobs de Azure

El esquema URI para acceder a los archivos del almacenamiento de blobs de Azure es:
...
Consulte [Instalación y configuración de Azure PowerShell][powershell-install] para obtener información sobre la instalación y la configuración de Azure PowerShell en su estación de trabajo. Puede usar la consola de Windows PowerShell o Windows PowerShell ISE para ejecutar los cmdlets de Azure PowerShell.
...
**Ejemplo de uso de Azure PowerShell para acceder a una cuenta de almacenamiento sin definir**
...
En este artículo, ha aprendido a usar el almacenamiento de blobs de Azure con HDInsight y que el almacenamiento de blobs de Azure es un componente fundamental de HDInsight. Esto le permite crear soluciones de adquisición de datos de archivado escalable y a largo plazo con el almacenamiento de blobs de Azure; además de usar HDInsight para desbloquear la información que hay dentro de los datos almacenados.
