<properties 
	pageTitle="Consultar Big Data desde almacenamiento de blobs compatible con Hadoop | Azure" 
	description="HDInsight usa el almacenamiento de blobs compatible con Hadoop como el almacén de big data para HDFS. Obtenga información acerca de cómo realizar consultas desde el almacenamiento de blobs y almacene los resultados de su análisis." 
...
> La mayoría de los comandos HDFS como <b>ls</b>, <b>copyFromLocal</b> y <b>mkdir</b>, entre otros, seguirán funcionando según lo previsto. Únicamente los comandos específicos de la implementación nativa de HDFS (a la que nos referiremos como DFS), como <b>fschk</b> y <b>dfsadmin</b>, exhibirán comportamientos diferentes en el almacenamiento de blobs de Azure.
...
* **Compartir y reutilizar los datos:** los datos de HDFS se encuentran dentro del clúster de cálculo. Solo las aplicaciones que tienen acceso al clúster de cálculo pueden utilizar los datos usando la API HDFS. Se tiene acceso a los datos en el almacenamiento de blobs mediante las API HDFS o con la [API de REST de almacenamiento Blobs][blob-storage-restAPI]. Por lo tanto, puede utilizarse un conjunto mayor de aplicaciones (incluidos otros clústeres de HDInsight) y herramientas para generar y consumir los datos.
* **Archivo de datos:** almacenar los datos en un almacenamiento de blobs hace que los clústeres de HDInsight que se usan para los cálculos se eliminen de forma segura sin perder los datos del usuario. 
* **Coste de almacenamiento de datos:** almacenar datos en DFS de largo plazo es más costoso que el almacenamiento de datos en el almacenamiento de blobs, ya que el coste de un clúster de cálculo es mayor que el coste de un contenedor de almacenamiento de blobs. Además, dado que los datos no tienen que volver a cargarse para la generación de cada clúster de cálculo, está ahorrando así los costes de carga de datos.
* **Escalado horizontal elástico:** si bien HDFS proporciona un sistema de archivos de escalado horizontal, el escalado se determina por el número de nodos que aprovisione para el clúster. Cambiar el escalado puede ser un proceso más complicado que basarse en las capacidades de escalado elástico del almacenamiento de blobs que obtiene automáticamente.
* **Replicación geográfica:** sus contenedores de almacenamiento de blobs pueden replicarse geográficamente a través del Portal de Azure. Aunque esta opción proporciona recuperación geográfica y redundancia de datos, una conmutación por error en la ubicación de replicación geográfica afectará gravemente al rendimiento y puede incurrir en costes adicionales. Nuestra recomendación es elegir la replicación geográfica con cuidado y solo si el valor de los datos compensa el coste adicional.
...
> [AZURE.NOTE] La sintaxis para enviar los archivos al emulador de almacenamiento (que se ejecuta en el emulador de HDInsight) es <i>wasb://&lt;NombreDelContenedor&gt;@storageemulator</i>.



El esquema de URI proporciona tanto acceso no cifrado con el prefijo  *wasb:* como acceso cifrado SSL con  *wasbs*. Se recomienda usar  *wasbs* siempre que sea posible, incluso al obtener acceso a los datos que residen en el mismo centro de datos de Azure.
...
> [AZURE.NOTE] El nombre de archivo es <i>hadoop-examples.jar</i> en clústeres de las versiones 1.6 y de 2.1 HDInsight.
...
**Ejemplo de PowerShell para tener acceso a una cuenta de almacenamiento sin definir**
