...
Además, HDInsight ofrece la capacidad de obtener acceso a los datos almacenados en el almacenamiento de blobs. La sintaxis para obtener acceso al almacenamiento de blobs es la siguiente:
...
* **Reutilización y uso compartido de los datos:** Los datos de HDFS se ubican dentro del clúster de cálculo. Solamente las aplicaciones que tengan acceso al clúster de cálculo podrán usar los datos usando la API HDFS. Se puede obtener acceso a los datos del almacenamiento de blobs a través de las API HDFS o las [API REST de almacenamiento de blobs][blob-storage-restAPI]. Por lo tanto, se puede usar un conjunto mayor de aplicaciones (incluyendo otros clústeres de HDInsight) y herramientas para producir y consumir los datos.
...
[Azure PowerShell][powershell-install] se puede usar para crear contenedores de blobs. A continuación se muestra un ejemplo de script de PowerShell:
	...
	$subscriptionName = "<SubscriptionName>"	# Nombre de suscripción de Azure
	$storageAccountName = "<AzureStorageAccountName>" # La cuenta de almacenamiento que creará
	$containerName="<BlobContainerToBeCreated>" # El nombre del contenedor de blobs que creará

	# Conectarse a la cuenta de Azure y seleccionar la suscripción actual
	Add-AzureAccount # La conexión expirará dentro de unas horas.
...
> [WACOM.NOTE] La sintaxis para enviar los archivos al emulador de almacenamiento (que se ejecuta en el emulador de HDInsight) es <i>wasb://&lt;ContainerName&gt;@storageemulator</i>.
...
> [WACOM.NOTE] El nombre del archivo es  <i>hadoop-examples.jar</i> en las versiones 1.6 y 2.1 de los clústeres de HDInsight.
