...
# Preguntas más frecuentes sobre el Bus de servicio
...
- [Preguntas generales acerca de la mensajería del Bus de servicio de Azure](#general-questions-about-azure-service-bus-messaging)
- [Procedimientos recomendados del Bus de servicio](#service-bus-best-practices)
- [Precios del Bus de servicio](#service-bus-pricing)
- [Cuotas del Bus de servicio](#service-bus-quotas)
...
## Preguntas generales acerca de la mensajería del Bus de servicio de Azure

### ¿Qué es la mensajería del Bus de servicio de Azure?

[Mensajería del Bus de servicio](service-bus-messaging-overview.md) es una plataforma de nube de mensajería asíncrona que le permite enviar datos entre sistemas desacoplados. Microsoft ofrece esto como un servicio, lo que significa que el usuario no tendrá que hospedar su propio hardware para poder utilizarlo.

### ¿Qué es un espacio de nombres del Bus de servicio?

Un espacio de nombres proporciona un contenedor con un ámbito para el desvío de recursos del bus de servicio en la aplicación. Es necesario crear uno para usar el Bus de servicio y es uno de los primeros pasos a realizar para empezar.

### ¿Qué es una cola del Bus de servicio de Azure?

Una [cola del Bus de servicio](service-bus-queues-topics-subscriptions.md) es una entidad en la que se almacenan los mensajes. Las colas son especialmente útiles cuando tiene varias aplicaciones o varias partes de una aplicación distribuida que necesitan comunicarse entre sí. La cola es similar a un centro de distribución en el sentido de que se recibe múltiples productos (mensajes) que luego se envían desde esa ubicación.

### ¿Qué son los temas y las suscripciones del Bus de servicio?

Un tema se puede considerar como una cola y cuando utiliza varias suscripciones, se convierte en un modelo de mensajería más enriquecido. Básicamente es una herramienta de comunicación de uno a varios. Este modelo de publicación/suscripción (o *pub/sub*) permite que cuando una aplicación envía un mensaje a un tema con varias suscripciones, ese mensaje sea recibido por varias aplicaciones.

### ¿Qué es el servicio de retransmisión del Bus de servicio de Azure?

El [servicio de retransmisión](service-bus-relay-overview.md) proporciona la capacidad de hospedar de forma transparente y obtener acceso a los servicios WCF desde cualquier lugar. En otras palabras, este servicio permite crear aplicaciones híbridas que se pueden ejecutar en los centros de datos de Azure y en un entorno empresarial local.

### ¿Qué es una entidad con particiones?

Un único agente de mensajes controla una cola o tema convencional, que se almacena en un almacén de mensajería. Las [colas o temas con particiones](service-bus-partitioning.md) están administrados por varios agentes de mensajes y se almacenan en varios almacenes de mensajería. Esto significa que el rendimiento general de una cola o tema particionado ya no está limitado por el rendimiento de un solo agente o almacén de mensajería. Además, una interrupción temporal de un almacén de mensajería no hace que una cola o tema con particiones deje de estar disponible.
...
## Procedimientos recomendados del Bus de servicio

### ¿Cuáles son algunos de los procedimientos recomendados del Bus de servicio de Azure?

-   [Procedimientos recomendados para mejorar el rendimiento mediante la mensajería asincrónica del Bus de servicio][]\: en este artículo se describe cómo optimizar el rendimiento al intercambiar mensajes asíncronos.

-   [Procedimientos recomendados para aislar aplicaciones ante desastres e interrupciones del Bus de servicio][]\: en este artículo se analiza la mejor protección para los puntos de conexión de retransmisión, colas y temas, y la replicación activa y pasiva frente a interrupciones del centro de datos.

###¿Qué debo saber antes de crear entidades de mensajería?   

Las siguientes propiedades de una cola y tema son inmutables. Tenga esto en cuenta al aprovisionar las entidades ya que esto no se puede modificar, sin crear una nueva entidad de reemplazo.
...
Para más información sobre los precios del Bus de servicio, consulte [Precios del Bus de servicio][Pricing overview]. Además de los precios indicados, se le cobrará por las transferencias de datos asociadas para salidas del centro de datos en el que se aprovisiona la aplicación.
...
Para obtener una lista de cuotas y límites del Bus de servicio, consulte [Cuotas del Bus de servicio][].
...
#### Tamaño de la cola o el tema

Especifique el tamaño máximo de la cola o el tema al crear la cola o el tema. Esta cuota puede tener un valor de 1, 2, 3, 4 o 5 GB. Si se alcanza el tamaño máximo de la cola establecido, se rechazarán los mensajes entrantes adicionales y el código de llamada recibirá una excepción.
...
#### Número de agentes de escucha simultáneos en una retransmisión
...
#### Número de agentes de escucha simultáneos por espacio de nombres
...
### ¿Cómo se puede migrar un espacio de nombres a otra suscripción de Azure?

Puede utilizar los comandos de PowerShell (los encontrará [aquí][]) para mover un espacio de nombres de una suscripción de Azure a otra. Para ejecutar la operación, el espacio de nombres tiene que estar ya activo. También el usuario que ejecuta los comandos tiene que ser un administrador en las suscripciones de origen y de destino.

## Solución de problemas del Bus de servicio de Azure

[Información general sobre las excepciones][]

### ¿Cuáles son algunas de las excepciones generadas por las API de mensajería del Bus de servicio de Azure y sus acciones sugeridas?
...
El artículo [Excepciones de mensajería del Bus de servicio][Exceptions overview] describe algunas excepciones con acciones sugeridas.

### ¿Qué es una firma de acceso compartido y qué lenguajes admiten la generación de una firma?

Las firmas de acceso compartido son un mecanismo de autenticación basado en URI y valores hash seguros SHA-256. Para más información sobre cómo generar sus propias firmas en el nodo, PHP, Java y C#, vea el artículo [Las firmas de acceso compartido][] .
...
[Procedimientos recomendados para aislar aplicaciones ante desastres e interrupciones del Bus de servicio]: service-bus-outages-disasters.md
...
[Información general sobre las excepciones]: service-bus-messaging-exceptions.md
[Las firmas de acceso compartido]: service-bus-sas-overview.md
