<properties 
   pageTitle="Tutorial de REST de Bus de servicio"
   description="Cree una sencilla aplicación host de Bus de servicio que expone una interfaz basada en REST."
...
En este tema se describe cómo compilar una sencilla aplicación host de Bus de servicio que expone una interfaz basada en REST. REST permite a un cliente web, como un explorador web, tener acceso a la API del Bus de servicio a través de solicitudes HTTP.
...
El primer paso es crear un espacio de nombres de servicio y obtener una clave de firma de acceso compartido (SAS). Un espacio de nombres de servicio proporciona un límite de aplicación para cada aplicación que se expone a través del Bus de servicio. El sistema genera una clave SAS automáticamente cuando se crea un espacio de nombres de servicio. La combinación del espacio de nombres de servicio y la clave SAS proporciona las credenciales de Bus de servicio para autenticar el acceso a una aplicación.

### Para crear un espacio de nombres de servicio y obtener una clave SAS

1. Para crear un espacio de nombres en el portal de administración de Azure, siga los pasos descritos en [Creación o modificación de un espacio de nombres de servicio de Bus de servicio](https://msdn.microsoft.com/library/hh690931.aspx).

2. En la ventana principal del portal de administración de Azure, haga clic en el nombre del espacio de nombres de servicio que creó en el paso anterior.
...
4. Agregue una referencia a **System.ServiceModel.dll** al proyecto:
	1. En el **Explorador de soluciones**, haga clic en la carpeta **Referencias** bajo la carpeta del proyecto y, a continuación, haga clic en **Agregar referencia**. 
	2. Haga clic en la pestaña **.NET** del cuadro de diálogo **Agregar referencia** y desplácese hacia abajo hasta que vea **System.ServiceModel**. Selecciónelo y haga clic en **Aceptar**.
...
9. Dentro de la interfaz `IImageContract`, declare un método para la operación sencilla que el contrato `IImageContract` expone en la interfaz y aplique el atributo `OperationContractAttribute` al método que desea exponer como parte del contrato público de Bus de servicio.
...
6. Después del elemento `<services>`, cree un elemento `<behaviors>` con el contenido siguiente, reemplazando "SAS\_KEY" por la clave de *firma de acceso compartido* (SAS) que ha obtenido en el portal de administración de Azure en el paso 1.
...
1. En la declaración de función `Main()`, cree una variable para almacenar el espacio de nombres del proyecto del Bus de servicio.
...
1. Cree el host del servicio web, utilizando la dirección URI creada anteriormente en esta sección.
...
Ahora que ha creado una aplicación que utiliza el servicio de Retransmisión de bus de servicio, vea los temas siguientes para obtener más información acerca de la mensajería retransmitida:

- [Información general sobre la arquitectura de Azure Service Bus](fundamentals-service-bus-hybrid-solutions.md#relays)
