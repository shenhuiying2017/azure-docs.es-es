...
3.	Configure el conector de SFTP de la siguiente forma:
	...
	- **Configuración del paquete** 
		...
		- **Tecla del host del servidor SSH**: especifique la huella digital de la tecla del host pública para el servidor SSH.
		- **Carpeta raíz**: especifique una ruta de acceso a la carpeta raíz.
		- **Cifrar cifrado**: especifique el cifrado.
...
5. Acceda a la aplicación de API que acaba de crear mediante Examinar -> Aplicaciones de API -> <Nombre de la aplicación de API que acaba de crear>; puede ver que el componente "Seguridad" no está configurado. 
...
6.	Ahora puede usar el conector de SFTP en el flujo. Puede utilizar el archivo recuperado desde el desencadenador de SFTP ("TriggerOnFileAvailable") en otras acciones del flujo. 
	...
	**Nota:** el desencadenador de SFTP  "TriggerOnFileAvailable" eliminará el archivo recuperado después de procesar el archivo.
...
8.	Configure las propiedades de entrada para el desencadenador de SFTP de la forma siguiente:
	...
	- **Máscara de archivo**: especifique la máscara de archivo que se aplicará para recuperar archivos. '*' recupera todos los archivos en la carpeta especificada.
...
7.	De un modo similar, puede usar las acciones de SFTP en el flujo. Puede usar la acción "Cargar archivo" para cargar un archivo en el servidor de SFTP. Configure las propiedades de entrada para la acción "Cargar archivo" de la siguiente manera:
	...
	- **Carpeta temporal**: si se proporciona, el adaptador cargará el archivo a la'Ruta a la carpeta temporal' y una vez que se realiza la carga, el archivo se moverá a la 'Ruta de la carpeta'. El'Ruta a la carpeta temporal' debe estar en el mismo disco físico que la 'Ruta de la carpeta' para asegurarse de que la operación de mover es atómica. La carpeta temporal solo puede usarse cuando la propiedad 'Anexar si existe' está deshabilitada.
