<properties pageTitle="Uso de un servicio móvil back-end de JavaScript" metaKeywords="server scripts, mobile devices, Azure, scheduler" description="Proporciona ejemplos acerca de cómo definir, registrar y usar scripts de servidor en Servicios móviles de Azure." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Work with server scripts in Mobile Services" authors="ricksal" solutions="" manager="dwrede" editor="" />
...
+ [Operaciones de tabla]
	...
	+ [Direccionamiento del Trabajo con usuarios de tabla][Trabajo con usuarios]
+ [API personalizadas][Delimitador de API personalizadas]
...
+ [Programador de trabajos]
	+ [Direccionamiento del de scripts de trabajos programados]
...
- El segundo argumento es siempre un [objeto user][Objeto user] que representa al usuario que envió la solicitud. 

- El tercer argumento es siempre un [objeto request][Objeto request] mediante el que puede controlar la ejecución de la operación solicitada y la respuesta enviada al cliente.
...
>[WACOM.NOTE]Debe usarse el nombre del para la función registrada en la operación de eliminación porque _del_ es una palabra clave reservada en JavaScript. 
...
+ En el [Portal de administración de Azure][Portal de administración]. Puede obtenerse acceso a los scripts para las operaciones de tabla en la pestaña **Scripts** para una tabla dada. A continuación se muestra el código predeterminado registrado en el script de inserción para la tabla `TodoItem`. Puede reemplazar este código por su propia lógica de negocios personalizada.
...
+ Mediante el control de código fuente. Cuando tenga el control de código fuente habilitado, simplemente cree un archivo con el nombre <em>`<table>`</em>.<em>`<operation>`</em>.js en la subcarpeta .\service\table en el repositorio git, donde <em>`<table>`</em> es el nombre de la tabla y <em>`<operation>`</em> es la operación de tabla que se registra. Para obtener más información, consulte [control de código fuente y código compartido][Control de código fuente, código compartido y funciones auxiliares].
...
<p>Es posible que la operación no responda cuando un script cuente con una ruta de código en la que no se invoque <b>execute</b> ni <b>respond</b>.</p></div>
...
Cuando se llama a la función **execute**, se utiliza el valor `item`, [query][objeto query] o `id` que se pasó como el primer argumento en la función de script para realizar la operación. Para una operación de inserción, actualización o consulta, puede modificar el objeto o la consulta antes de llamar a **execute**: 
...
Cuando el runtime de Servicios Móviles llama a funciones de API personalizada, se proporcionan dos objetos [request][objeto request] y [response][objeto response]. Estos objetos ofrecen la funcionalidad de [express.js library], que los scripts pueden aprovechar. La siguiente API personalizada llamada **hello** es un ejemplo muy sencillo que devuelve _Hello, world!_ en respuesta a una solicitud POST:

		exports.post = function(request, response) {
		    response.send(200, "{ message: 'Hello, world!' }");
		} 

La función **send** en el [objeto request] devuelve la respuesta deseada al cliente. Este código se invoca mediante el envío de una solicitud POST a la siguiente dirección URL:
...
+ En el [Portal de administración de Azure][Portal de administración]. Los scripts de la API personalizada se crean y modifican en la pestaña **API**. El código del script del servidor se encuentra en la pestaña **Scripts** de una determinada API personalizada. A continuación se muestra el script que invoca la solicitud POST en el extremo de la API personalizada `CompleteAll`. 
	...
	Los métodos de permisos de acceso a la API personalizada se encuentran asignados en la pestaña Permisos. Para ver cómo se creó esta API personalizada, consulte [Llamada a una API personalizada desde el cliente].  

+ Mediante el control de código fuente. Cuando disponga de un control de código fuente habilitado, simplemente cree un archivo con el nombre <em>`<api_personalizada>`</em>.js en la subcarpeta .\service\api en el repositorio git, donde <em>`<api_personalizada>`</em> es el nombre de la API personalizada que se registra. El script contienen una función _exported_ para cada método HTTP que ofrece la API personalizada. Los permisos se definen en un archivo .json complementario. Para obtener más información, consulte [control de código fuente y código compartido][Control de código fuente, código compartido y funciones auxiliares].
...
Una solicitud HTTP POST invoca esta función de la API personalizada para el siguiente extremo:
		...
		https://<servicio>.azure-mobile.net/api/orderpizza
...
###<a name="api-routes"></a>Direccionamiento del de varias rutas en una API personalizada
...
+ `https://<servicio>.azure-mobile.net/api/calculator/add`
+ `https://<servicio>.azure-mobile.net/api/calculator/sub`
...
El objeto **api** que se pasa a la función **register** expone una función para cada método HTTP (**get**, **post**, **put**, **patch**, **delete**). Estas funciones registran una ruta a una función definida para un método HTTP específico. Cada función usa dos parámetros, el primero es el nombre de ruta y el segundo es la función registrada en la ruta. 
...
###<a name="scheduler-scripts"></a>Direccionamiento del de scripts de trabajos programados
...
>[WACOM.NOTE]Cuando cuente con un control de código fuente habilitado, podrá editar archivos de script de trabajos programados directamente en la subcarpeta .\service\scheduler del repositorio git. Para obtener más información, consulte [Inserción de compartido de código mediante el control de código fuente].
...
+ **crypto**: Proporciona la funcionalidad crypto de OpenSSL. Para obtener más información, consulte la [documentación de Node.js][API crypto].
+ **path**: Contiene utilidades para trabajar con rutas de archivos. Para obtener más información, consulte la [documentación de Node.js][API path].
+ **querystring**: Contiene utilidades para trabajar con cadenas de consulta. Para obtener más información, consulte la [documentación de Node.js][API querystring].
...
+ **url**: Contiene utilidades para redistribuir y resolver direcciones URL. Para obtener más información, consulte la [documentación de Node.js][API url].
+ **util**: Contiene varias utilidades, como una comprobación del tipo de objeto y un formato de cadena. Para obtener más información, consulte la [documentación de Node.js][API util]. 
+ **zlib**: Ofrece la funcionalidad de compresión, como gzip y deflate. Para obtener más información, consulte la [documentación de Node.js][API zlib]. 
...
###<a name="shared-code-source-control"></a>Direccionamiento del compartido de código mediante el control de código fuente
...
###<a name="helper-functions"></a>Direccionamiento del de funciones auxiliares
...
Los archivos de script se cargan en el directorio compartido mediante [control de código fuente][Uso compartido de código con control de código] o mediante la [herramienta de la línea de comandos][Uso de la herramienta de la línea de comandos].

###<a name="app-settings"></a>Direccionamiento del de configuración de aplicaciones

Servicios móviles permite almacenar de forma segura valores como la configuración de la aplicación, que puede tener acceso a los scripts de servidor en tiempo de ejecución.  Al agregar datos a la configuración de la aplicación del servicio móvil, los pares nombre/valor se almacenan cifrados y se puede acceder a ellos en los scripts de servidor sin codificarlos de forma rápida en el archivo de script. Para obtener más información, consulte [Configuración de aplicación].
...
###<a name="access-tables"></a>Direccionamiento del a tablas desde scripts
...
Los scripts de la API personalizada pueden obtener acceso al [objeto tables] desde la propiedad <strong>service</strong> del [objeto reques] proporcionado. Esta línea de código obtiene el objeto [tables] de la solicitud:

		var todoItemsTable = request.service.tables.getTable('TodoItem');

<div class="dev-callout"><strong>Nota:</strong>
<p>Las funciones compartidas no pueden obtener acceso al objeto <strong>tables</strong> directamente. En una función compartida, debe pasar el objeto tables a la función.</p></div>
...
Puede encontrar un ejemplo final en la muestra de código aquí: [Acceso a parámetros personalizados][Incorporación de parámetros personalizados].

###<a name="bulk-inserts"></a>Direccionamiento del de inserciones en masa

Si usa un bucle **for** o **while** para insertar directamente una gran cantidad de elementos en una  tabla (1000, por ejemplo), es posible que se encuentre con una limitación de la conexión SQL que provoque que se produzcan errores en las inserciones. Nunca se puede completar su solicitud, o puede devolver un Error de servidor interno HTTP 500.  Para evitar este problema, puede insertar los elementos en lotes de 10, por ejemplo. Una vez que se haya insertado el primer lote, envíe el siguiente, y así sucesivamente.
...
####<a name="joins"></a>Direccionamiento del de tablas relacionales
...
Una ejecución correcta de **open** provoca que la conexión de base de datos se pase a la función **success** como un parámetro. Puede invocar cualquiera de las siguientes funciones en el objeto **connection**: *close*, *queryRaw*, *query*, *beginTransaction*, *commit*, and *rollback*.
...
<div class="dev-callout"><strong>Nota:</strong>
<p>Para ver los registros para el servicio móvil, inicie sesión en el <a href="https://manage.windowsazure.com/">Portal de administración</a>, seleccione el servicio móvil y, a continuación, seleccione la pestaña <strong>Logs</strong>.</p></div>
...
[Definición de scripts de tablas]: #execute-operation
[Invalidación de la respuesta predeterminada]: #override-response
[Modificación de una operación]: #modify-operation
[Invalidación correcta y con error]: #override-success-error
...
[Trabajo con usuarios]: #work-with-users
[Direccionamiento del de scripts de trabajos programados]: #scheduler-scripts
[Limitación del acceso a tablas]: #authorize-tables
...
[Acceso a los encabezados de solicitud de API personalizadas]: #get-api-headers
...
<!-- URLs. -->
[Referencia del script del servidor de Servicios móviles]: http://msdn.microsoft.com/es-es/library/windowsazure/jj554226.aspx
[Programación de trabajos de back-end en Servicios móviles]: /es-es/develop/mobile/tutorials/schedule-backend-tasks/
[objeto request]: http://msdn.microsoft.com/es-es/library/windowsazure/jj554218.aspx
[objeto response]: http://msdn.microsoft.com/es-es/library/windowsazure/dn303373.aspx
[objeto User]: http://msdn.microsoft.com/es-es/library/windowsazure/jj554220.aspx
...
[función eliminar]: http://msdn.microsoft.com/es-es/library/windowsazure/jj554215.aspx
...
[objeto query]: http://msdn.microsoft.com/es-es/library/windowsazure/jj613353.aspx
...
[objeto tables]: http://msdn.microsoft.com/es-es/library/windowsazure/jj614364.aspx
[objeto mssql]: http://msdn.microsoft.com/es-es/library/windowsazure/jj554212.aspx
[objeto console]: http://msdn.microsoft.com/es-es/library/windowsazure/jj554209.aspx
...
[objeto service]: http://msdn.microsoft.com/es-es/library/windowsazure/dn303371.aspx
