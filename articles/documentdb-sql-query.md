<properties title="Consulta con SQL de Base de datos de documentos" pageTitle="Consulta con SQL de Base de datos de documentos | Azure" description="Base de datos de documentos permite consultar documentos usando una gram&aacute;tica similar a la de SQL en documentos JSON jer&aacute;rquicos sin necesidad de una esquema expl&iacute;cito ni la creaci&oacute;n de &iacute;ndices secundarios." metaKeywords="" services="documentdb"  documentationCenter="" solutions="data-management" authors="bradsev" manager="jhubbard" editor="cgronlun" videoId="" scriptId="" />

<tags ms.service="documentdb" ms.workload="data-services" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="08/20/2014" ms.author="spelluru" />

# Base de datos de documentos de consulta

Base de documentos de Azure admite la consulta de documentos con un lenguaje SQL (lenguaje de consulta estructurado) familiar en documentos JSON jerárquicos. Base de documentos realmente no tiene esquemas. En virtud de su compromiso con el modelo de datos JSON que se encuentra directamente en el motor de base de datos, proporciona índices automáticos de documentos JSON sin necesidad de un esquema explícito o de la creación de índices secundarios.
Durante el diseño del lenguaje de consulta de Base de datos de documentos, teníamos dos objetivos en mente:

-   **Adoptar SQL**: en lugar de inventar un nuevo lenguaje de consulta, queríamos adoptar el lenguaje SQL. Después de todo, SQL es uno de los lenguajes de consulta más familiares y populares. El lenguaje SQL de Base de datos de documentos de Azure proporciona un modelo de programación formal para consultas enriquecidas en documentos JSON.
-   **Ampliar SQL**: igual que una base de datos de documentos JSON capaz de ejecutar JavaScript directamente en el motor de base de datos, queríamos usar el modelo de programación de JavaScript como base para nuestro lenguaje de consulta SQL. El lenguaje de consulta de Base de datos de documentos de Azure tiene su origen en el sistema de tipo de JavaScript, una evaluación de la expresión y una invocación de función. A su vez, este proporciona un modelo de programación natural para proyecciones relacionales, navegación jerárquica por documentos JSON, autocombinaciones, invocación de funciones definidas por el usuario (UDF) escritas íntegramente en JavaScript entre otras características.
...
En este tutorial, presentamos las capacidades y la gramática del lenguaje de consulta de Base de datos de documentos a través de una serie de ejemplos. Asimismo, abordamos el modo en que se puede consultar Base de datos de documentos usando la API de REST y los SDK (incluido LINQ).

# Tareas iniciales

Para consultar SQL de Base de datos de documentos en el trabajo, empezaremos con unos documentos JSON sencillos y realizaremos algunas consultas fáciles con él. Tenga en cuenta estos dos documentos JSON sobre dos familias. No olvide que, con Base de datos de documentos, no es preciso que creemos ningún esquema ni índice secundario de forma explícita. Solo tenemos que insertar los documentos JSON en una recopilación de Base de datos de documentos y, posteriormente, realizar consultas.
Aquí vemos un documento JSON sencillo correspondiente a la familia Andersen, los padres, hijos (y sus mascotas), dirección e información de registro. El documento tiene cadenas, números, booleanos, matrices y propiedades anidadas.
...
La consulta siguiente devuelve todos los nombres proporcionados de los niños de la familia cuyo identificador coincida con `WakefieldFamily`.
...
-   Como SQL de Base de datos de documentos trabaja en valores JSON, trata entidades en forma de árbol en lugar de filas y columnas. Por consiguiente, el lenguaje permite que se haga referencia a los nodos del árbol a cualquier profundidad arbitraria, como `Node1.Node2.Node3…..Nodem`, similar al lenguaje SQL relacional que hace alusión a la referencia de dos partes de `<table>.<column>`.
...
-   Base de datos de documentos solo admite documentos JSON estrictos. Esto significa que el sistema de tipo y las expresiones se restringen para tratar únicamente tipos JSON. Consulte [JSON specification] (http://www.json.org/) para obtener más detalles.
-   Una recopilación de Base de datos de documentos es un contenedor sin esquemas de documentos JSON. Las relaciones de las entidades de datos en todos los documentos de una recopilación se capturan implícitamente por contención y no por relaciones entre PK y FK. Se trata de un aspecto importante que merece la pena señalar teniendo en cuenta las combinaciones internas descritas posteriormente en este documento.

# Índices de Base de datos de documentos

Antes de adentrarnos en el lenguaje SQL de Base de datos de documentos, merece la pena explorar el diseño de los índices de Base de datos de documentos.

El objetivo de los índices de base de datos es atender consultas en sus diversas formas con un consumo de los recursos mínimo (como CPU, E/S) mientras se proporcionan un buen rendimiento y bajas latencias. A menudo, la elección de los índices adecuados para consultar una base de datos requiere mucha planificación y experimentación. Este enfoque plantea un desafío para las bases de datos sin esquemas en las que los datos no cumplen un esquema estricto y evolucionan rápidamente.

Por consiguiente, cuando diseñamos el subsistema de índices de Base de datos de documentos, establecimos los objetivos siguientes:

-   Indexación de documentos sin necesidad de esquema: el subsistema de índices no requiere información del esquema alguna ni la realización de ninguna suposición a priori sobre el esquema de los documentos.

-   Compatibilidad con consultas eficaces, jerárquicas enriquecidas y relacionales: el índice admite el lenguaje de consulta de Base de datos de documentos de forma eficaz, incluida la compatibilidad con las proyecciones jerárquicas y relacionales.

-   Compatibilidad con consultas coherentes frente a un volumen de escrituras sostenido: en el caso de las cargas de trabajo de alto rendimiento de escritura con consultas coherentes, el índice se actualiza paulatinamente, de forma eficaz y en línea frente a un volumen de escrituras sostenido. La actualización coherente del índice es fundamental para atender las consultas con el nivel de coherencia con el que el usuario ha configurado el servicio documental.

-   Compatibilidad con la arquitectura multiempresa: una vez proporcionado el modelo basado en la reserva para la gobernanza de recursos de los inquilinos, se realizan actualizaciones de los índices sin sobrepasar el presupuesto de los recursos del sistema (CPU, memoria, IOPS) aplicado por réplica.

-   Almacenamiento eficaz: para obtener rentabilidad, se enlaza la sobrecarga de almacenamiento en el disco del índice y es predecible. Esto es fundamental porque Base de datos de documentos permite que el desarrollador haga concesiones basadas en el coste entre la sobrecarga de índices y el rendimiento de las consultas.

Consulte [DocumentDB samples] (http://code.msdn.microsoft.com/Azure-DocumentDB-NET-Code-6b3da8af\#content) en MSDN para obtener información sobre cómo configurar la directiva de índices de una recopilación. Adentrémonos ahora en los detalles del lenguaje SQL de Base de datos de documentos.
...
La cláusula `FROM <from_specification>` es opcional a menos que el origen se filtre o proyecte posteriormente en la consulta. El objetivo de esta cláusula es especificar el origen de datos sobre el que debe operar la consulta. Normalmente, toda la recopilación es el origen pero, en su lugar, puede especificarse un subconjunto de la recopilación.

Una consulta como `SELECT * FROM Families` indica que toda la recopilación Familias es el origen sobre el que se va a enumerar. Se puede usar un identificador **ROOT** especial para representar la recopilación en lugar de usar el nombre de la misma.
Las reglas de enlace que se imponen por consulta son las siguientes:

-   Puede establecerse para la recopilación un alias como en `SELECT f.id FROM Families AS f` o simplemente `SELECT f.id FROM Families f`. Aquí`f` es el equivalente de `Families`. `AS` es una palabra clave opcional para establecer para el identificador un alias.

-   Tenga en cuenta que, una vez establecido un alias, no podrá enlazarse el origen original. Por ejemplo, `SELECT Familes.id FROM Families f` no es válido sintácticamente porque el identificador "Familias" ya no puede resolverse.
...
El origen también se puede reducir a un subconjunto más pequeño. Por ejemplo, si a alguien le interesa solo enumerar un subárbol en cada documento, la subraíz podría convertirse en el origen, como en el ejemplo siguiente.
...
Aunque en el ejemplo anterior se usó una matriz como origen, también podría usarse un objeto como origen como se muestra en el ejemplo siguiente. Cualquier valor JSON válido (que no sea Undefined) que se pueda encontrar en el origen se tendrá en cuenta para su inclusión en el resultado de la consulta. Si algunas familias no tienen un valor `address.state`, se excluirán en el resultado de la consulta.
...
En la consulta siguiente se solicitan documentos que contienen una propiedad de nombre y que el valor de la propiedad es `AndersenFamily`. Cualquier otro documento que no tenga una propiedad de nombre o en el que no coincida el valor `AndersenFamily`, se excluirá.
...
Asimismo, se admiten los operadores unarios **+,-, ~ y NO** y pueden usarse en consultas como se muestra a continuación:
...
Además de los operadores binarios y unarios, también se permiten las referencias de propiedad. Por ejemplo, `SELECT * FROM Families f WHERE f.isRegistered` devolvería los documentos JSON que incluyen la propiedad `isRegistered` y el valor de la propiedad es igual al valor `true`. Cualquier otro valor (false, null, Undefined, <number>, <string>, <object>, <array> etc.) dará lugar a la exclusión del documento de origen del resultado.
...
Para otros operadores de comparación como \>, \>=, !=, \< y \<=
...
Estos operan en valores booleanos. Las tablas de verdad lógica para estos operadores son como se muestran a continuación.
...
<tbody>
<tr class="odd">
<td align="left"><p><strong>O bien</strong></p></td>
...
En el siguiente ejemplo se muestra una consulta SELECT típica:
...
En el ejemplo siguiente, proyectamos dos propiedades anidadas `f.address.state` y `f.address.city`:
...
Analicemos el rol de `$1` aquí. La cláusula `SELECT` debe crear un objeto JSON y, como no se proporciona ninguna clave, usamos nombres de las variables de argumentos implícitos que empiezan por `$1`. Por ejemplo, esta consulta devuelve dos variables de argumentos implícitos, etiquetadas como `$1` y `$2`.
...
Además de las referencias de propiedad, la cláusula **SELECT** también admite expresiones escalares como constantes, expresiones aritméticas, expresiones lógicas, etc. Por ejemplo, aquí hay una sencilla consulta "Hola mundo".
...
Otra característica clave del lenguaje SQL de Base de datos de documentos es la creación de matrices u objetos. En el ejemplo anterior, observe que creamos un nuevo objeto JSON. Del mismo modo, también se pueden construir matrices como se muestra a continuación.
...
En el ejemplo siguiente se amplía esto para mostrar cómo devolver valores primitivos JSON, es decir, en el nivel de hoja del árbol JSON.
...
Admitimos el operador especial (\*) para proyectar el documento tal cual. Al usarse, debe ser el único campo proyectado. Mientras que una consulta como `SELECT * FROM Families f` es válida, `SELECT VALUE * FROM Families f` y `SELECT *, f.id FROM Families f` no lo son.
...
Agregamos una nueva construcción mediante la palabra clave **IN** en el lenguaje SQL de Base de datos de documentos para que sea compatible con la iteración sobre matrices JSON. El origen FROM proporciona compatibilidad con la iteración. Empecemos con el ejemplo siguiente:
...
Analicemos ahora otra consulta que realice una iteración sobre elementos secundarios de la recopilación. Observe la diferencia en la matriz de salida. En este ejemplo se divide `children` y se quita el formato de los resultados, convirtiéndose en una sola matriz.
...
Esto puede usarse más veces para filtrar por cada entrada individual de la matriz como en el ejemplo siguiente.
...
En una base de datos relacional, la necesidad de combinar en tablas es muy importante. Es la consecuencia lógica de diseñar esquemas normalizados. Al contrario que esto, Base de datos de documentos aborda un modelo de datos desnormalizados de documentos sin esquemas. Este es el equivalente lógico de una "autocombinación".

La sintaxis que se admite en el lenguaje es <from_source1> JOIN <from_source2> JOIN ... JOIN <from_sourcen>. Generalmente, esto devolvería un conjunto de tuplas **N** (tupla con valores **N**). Cada tupla tendrá valores producidos iterando todos los alias de la recopilación sobre sus conjuntos correspondientes. En otras palabras, se trata de un producto cruzado completo de los conjuntos que participan en la combinación.

En los ejemplos siguientes se muestra cómo funcionan JOIN. En el ejemplo que vemos a continuación, el resultado está vacío porque el producto cruzado de cada documento del origen y un conjunto vacío está vacío.
...
El ejemplo siguiente es una combinación más convencional:
...
Lo primero que se debe tener en cuenta es que `from_source` de **JOIN** es un iterador aquí. De modo que el flujo en este caso es el siguiente

-   Amplía cada elemento secundario **c** en la matriz.
-   Aplica un producto cruzado con la raíz del documento **f** con cada elemento secundario **c** cuyo formato se quitó en el primer paso.
-   Por último, proyecta solo la propiedad de nombre **f** del objeto raíz.

El primer documento (`AndersenFamily`) incluye únicamente un elemento secundario, de modo que el conjunto de resultados contiene solo un objeto correspondiente a este documento. En el segundo documento (`WakefieldFamily`) se incluyen dos elementos secundarios. De este modo, el producto cruzado produce un objeto independiente para cada elemento secundario, lo que da lugar a dos objetos, uno para cada elemento secundario correspondiente a este documento. Tenga en cuenta que los campos raíces de estos dos documentos serán los mismos, justo como esperaría en un producto cruzado.
...
En el ejemplo siguiente, hay un filtro adicional por `pet`. Este excluye todas las tuplas donde el nombre de mascota no sea "Shadow". Tenga en cuenta que podemos crear tuplas a partir de matrices, filtrar por cualquiera de los elementos de la tupla y proyectar cualquier combinación de los elementos.
...
-   La posibilidad de realizar operaciones CRUD transaccionales de alto rendimiento y consultas en los documentos de una recopilación en virtud de una mayor integración del tiempo de ejecución de JavaScript en el motor de base de datos.
...
Junto con los tipos especificados anteriormente, el lenguaje SQL de Base de datos de documentos ofrece soporte para las funciones definidas por el usuario (UDF). En particular, se admiten las UDF escalares allí donde los desarrolladores puedan proporcionar cero o muchos argumentos y devolver un solo resultado de argumento. Cada uno de estos argumentos se comprueba para ver si se trata de valores JSON legales.

La gramática del lenguaje SQL de Base de datos de documentos se amplía para admitir una lógica de aplicación personalizada usando estas funciones definidas por el usuario. Las funciones definidas por el usuario pueden registrarse con Base de datos de documentos de Azure y, a continuación, se puede hacer referencia a ellas como parte de una consulta de SQL. De hecho, las UDF están exquisitamente diseñadas para su invocación por parte de las consultas. Como consecuencia a esta opción, las UDF no tienen acceso al objeto de contexto que los otros tipos de JavaScript (procedimientos y desencadenadores almacenados) tienen. Puesto que las consultas se ejecutan como de solo lectura, pueden ejecutarse en réplicas principales o secundarias. Por consiguiente, las UDF están diseñadas para ejecutarse en réplicas secundarias, a diferencia de otros tipos de JavaScript.
...
El ejemplo anterior crea una UDF cuyo nombre es `SQRT`. Acepta un solo valor `number` JSON y calcula la raíz cuadrada del número usando la biblioteca Math.

Ahora podemos usar esta UDF en una consulta de una proyección.
...
La UDF también puede usarse en un filtro como se muestra en el ejemplo siguiente:
...
El lenguaje SQL de Base de datos de documentos no realiza conversiones implícitas a diferencia de JavaScript. Por ejemplo, una consulta como `SELECT * FROM Person p WHERE p.Age = 21` coincide con documentos en los que se incluye la propiedad Age cuyo valor es 21. Cualquier otro documento cuya propiedad Age coincida con la cadena "21" u
otras variaciones posiblemente infinitas como "021", "21,0", "0021", "00021", etc. no coincidirá.
Esto contrasta con JavaScript allí donde los valores de cadena se convierten implícitamente en número (basándose en operador, ej.: ==). Esta opción es fundamental para conseguir una coincidencia de índices eficaz en el lenguaje SQL de Base de datos de documentos.
...
En primer lugar, para el sistema de tipo, admitimos todos los tipos primitivos JSON (tipos numéricos, booleanos, de cadena y null). Solo se admiten estos tipos JSON. Se admiten las siguientes expresiones escalares.
...
-   Expresiones aritméticas: entre estas se incluyen expresiones aritméticas comunes o valores numéricos y booleanos. Para ver la lista completa, consulte la especificación de SQL anterior.
...
La sintaxis es `input.SelectMany(x => f(x))`, donde `f` es una expresión escalar que devuelve un tipo de recopilación.
...
Los operadores anteriores pueden ser compuestos para formar consultas más eficaces. Como Base de datos de documentos admite recopilaciones anidadas, dicha composición puede concatenarse o anidarse.

### Concatenación

La sintaxis es `input(.|.SelectMany())(.Select()|.Where())*`. Una consulta concatenada puede empezar por una consulta `SelectMany` seguida de varios operadores `Select` o `Where`.
...
En los ejemplos siguientes se muestra POST para una consulta de Base de datos de documentos realizada a una recopilación que incluye dos documentos de ejemplo que hemos revisado hasta el momento. La consulta tiene un filtro sencillo por la propiedad de nombre JSON. Tenga en cuenta el uso de `x-ms-documentdb-isquery` y el tipo de contenido: encabezados `application/sql` que van a denotar que la operación es una consulta.
...
El cliente de .NET se itera automáticamente a través de todas las páginas de resultados de la consulta de los bloques foreach, como se muestra anteriormente. Las opciones de consulta incluidas en la sección de la API de REST también están disponibles en el SDK de .NET usando las clases `FeedOptions` y `FeedResponse` en el método CreateDocumentQuery. El número de páginas puede controlarse usando el valor `MaxItemCount`.

Los desarrolladores también pueden controlar de forma explícita la paginación creando `IDocumentQueryable` con el objeto `IQueryable` y, a continuación, leyendo los valores `ResponseContinuationToken` y pasándolos de nuevo como `RequestContinuationToken` en `FeedOptions`. `EnableScanInQuery` puede establecerse para permitir análisis si la directiva de índices configurada no puede admitir la consulta.

Consulte [DocumentDB .NET samples] (http://code.msdn.microsoft.com/Azure-DocumentDB-NET-Code-6b3da8af\#content) para obtener más ejemplos sobre consultas.
...
1.  [Introducción a Base de datos de documentos de Azure][Introducción a Base de datos de documentos de Azure]
...
4.  [Niveles de coherencia de Base de datos de documentos][Niveles de coherencia de Base de datos de documentos]
5.  SQL ANSI 2011: <http://www.iso.org/iso/iso_catalogue/catalogue_tc/catalogue_detail.htm?csnumber=53681>
6.  JSON <http://json.org/>
7.  Especificación de JavaScript <http://www.ecma-international.org/publications/standards/Ecma-262.htm>
8.  LINQ [http://msdn.microsoft.com/es-es/library/bb308959.aspx][http://msdn.microsoft.com/es-es/library/bb308959.aspx]
9.  Técnicas de evaluación de consultas para bases de datos de gran tamaño <http://dl.acm.org/citation.cfm?id=152611>
...
13. G. Graefe. The Cascades framework for query optimization. IEEE Data Eng. Bull., 18(3): 1995.
  ...
  [Niveles de coherencia de Base de datos de documentos]: ../documentdb-consistency-levels
  [Introducción a Base de datos de documentos de Azure]: ../documentdb-introduction
  [http://msdn.microsoft.com/es-es/library/bb308959.aspx]: http://msdn.microsoft.com/es-es/library/bb308959.aspx
