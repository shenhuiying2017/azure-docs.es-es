...
-   [Creación de suscripciones](#create-subscription)
...
-   [Eliminación de temas y suscripciones](#delete)
...
2.  Escriba **npm install azure** en la ventana de comandos. Esto debería devolver el resultado siguiente:
...
se obtiene el tema especificado (si existe) o se crea un nuevo tema con el nombre especificado. El código siguiente usa **createTopicIfNotExists** para crear un tema denominado "MyTopic":
...
**createServiceBusService** también admite opciones adicionales, lo que
permite invalidar la configuración predeterminada de los temas, como el período de vida de los mensajes
o el tamaño máximo de los temas. En el siguiente ejemplo se muestra cómo
establecer el tamaño máximo de los temas en 5 GB y el período de vida en 1 minuto:
...
<strong>Nota:</strong>
<p>Las suscripciones son permanentes y seguirán existiendo hasta que se eliminen o se elimine el tema al que están asociadas. Si su aplicación contiene lógica para crear una suscripción, primero debe comprobar si la suscripción ya existe mediante el método <strong>getSubscription.</strong> método.</p>
</div>
...
> Ya que el filtro predeterminado se aplica automáticamente a todas las suscripciones nuevas, primero debe eliminar el filtro predeterminado o <strong>MatchAll</strong> invalidará todos los filtros que especifique. Puede eliminar la regla predeterminada mediante el método <strong>deleteRule</strong> del objeto <strong>ServiceBusService</strong> .

En el ejemplo que aparece a continuación se crea una suscripción denominada "HighMessages" con un filtro **SqlFilter** que solo selecciona los mensajes con una propiedad **messagenumber** personalizada superior a 3:
...
Del mismo modo, en el ejemplo que aparece a continuación se crea una suscripción denominada
"LowMessages" con un filtro **SqlFilter** que solo selecciona los mensajes con
una propiedad **messagenumber** igual a 3 o inferior:
...
Los temas y suscripciones son permanentes, por lo que deben eliminarse explícitamente
a través del Portal de administración de Azure o mediante programación.
