...
2. Vaya a "Web y móvil > Aplicaciones de API" y busque "Conector de HDInsight".
3. Proporcione detalles genéricos, por ejemplo, el nombre, el plan de servicio de aplicación, etc., en la primera hoja.
4. Como parte de la configuración del paquete, proporcione el nombre de usuario y la contraseña del clúster de HDInsight.
...
Acceda a la aplicación de API que acaba de crear mediante Examinar -> Aplicaciones de API -> <Nombre de la aplicación de API que acaba de crear>. Verá el siguiente comportamiento. El componente 'seguridad' muestra 0, lo que significa que no hay ningún certificado de administración cargado.

![][2] 

Para cargar el certificado de administración en la aplicación de API, debe hacer lo siguiente:
1. Haga clic en el componente 'Seguridad'.
2. Haga clic en  Cargar certificado' en la hoja 'Seguridad' que se abre.
3. Busque y seleccione el archivo de certificado en la hoja siguiente:
4. Una vez seleccionado el certificado, haga clic en Aceptar.
...
- Al crear o editar una aplicación lógica, elija la aplicación de API del conector de HDInsight creado como acción y muestra todas las acciones disponibles.
...
Puede hacer clic en 'Ejecutar' para iniciar manualmente la aplicación lógica para probar el escenario.
