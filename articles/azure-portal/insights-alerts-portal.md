<properties
	pageTitle="Uso de Azure Portal para crear alertas de los servicios de Azure | Microsoft Azure"
...
# Uso de Azure Portal para crear alertas de los servicios de Azure
...
## Creación de una regla de alerta de una métrica con Azure Portal

1. En el [portal](https://portal.azure.com/), busque el recurso que desea supervisar y selecciónelo.

2. Seleccione **Alertas** o **Reglas de alerta** en la sección SUPERVISIÓN. El texto y el icono pueden variar ligeramente en los distintos recursos.
...
4. Asígnele un **nombre** a la regla de alerta y elija una **descripción**, que también muestra los correos electrónicos de notificación.
5. Seleccione la **métrica** que desea supervisar y, luego, elija un valor de **Condición** y **Umbral** para la métrica. También debe elegir el **período** de tiempo de la regla de métrica que se debe cumplir antes de que se desencadene la alerta. Por ejemplo, si usa el período "PT5M" y la alerta busca una CPU por encima del 80 %, la alerta se desencadena cuando la CPU ha estado por sobre el 80 % durante 5 minutos. Una vez que se desencadena por primera vez, se vuelve a desencadenar cuando la CPU se mantiene por debajo del 80% durante 5 minutos. La CPU se mide cada 1 minuto.
...
7. Si desea enviar una notificación a otras direcciones de correo electrónico cuando se active la alerta, agréguelas en el campo **Correos electrónicos de administradores adicionales**. Separe las direcciones de correo electrónico con punto y coma, de la siguiente manera: *email@contoso.com;correoelectrónico2@contoso.com*
...
- **Deshabilitar** la alerta, si desea dejar de recibir notificaciones de esa alerta de manera temporal, o **habilitarla** si desea reanudar las notificaciones para esa alerta.
