...
AKS admite la creación de grupos de nodo habilitados para GPU. Azure proporciona actualmente única o varias máquinas virtuales habilitadas para GPU. Las máquinas virtuales habilitadas para GPU están diseñadas para cargas de trabajo de proceso intensivo, uso intensivo de gráficos y visualización. [Aquí](https://docs.microsoft.com/azure/virtual-machines/windows/sizes-gpu) se puede encontrar una lista de máquinas virtuales habilitadas para GPU.
...
Cree el clúster de AKS con nodos de tamaño `Standard_NC6`.
...
Describa uno de los nodos para confirmar que las GPU son programables. Esto se puede encontrar en la sección `Capacity`. Por ejemplo, `alpha.kubernetes.io/nvidia-gpu:  1`.
...
Para demostrar que las GPU funcionan correctamente, programe una carga de trabajo compatible con la GPU con la solicitud de recursos adecuada. En este ejemplo se va a ejecutar un trabajo [Tensorflow](https://www.tensorflow.org/versions/r1.1/get_started/mnist/beginners) en el [conjunto de datos MNIST](http://yann.lecun.com/exdb/mnist/).

El siguiente manifiesto de trabajo incluye un límite de recursos de `alpha.kubernetes.io/nvidia-gpu: 1`. Las bibliotecas y herramientas de depuración adecuadas de CUDA estarán disponibles en el nodo en `/usr/local/nvidia` y deben montarse en el pod con la especificación de volumen adecuada, tal y como se muestra a continuación.

Copie el manifiesto y guárdelo como **samples-tf-mnist-demo.yaml**.
```
apiVersion: batch/v1
kind: Job
metadata:
...
Supervise el progreso del trabajo hasta la finalización correcta mediante el comando [kubectl get jobs][kubectl-get] con el argumento `--watch`.
```
...
Determine el nombre del pod para ver los registros mediante la visualización de los pod completados.
```
...
Con el nombre del módulo de la salida del comando anterior, consulte los registros del pod para confirmar que se ha detectado el dispositivo de GPU adecuado, en este caso, `Tesla K80`.
```
...
Quite los objetos Kubernetes asociados creados en este paso.
```
...
¿Está interesado en ejecutar cargas de trabajo de Machine Learning en Kubernetes? Consulte la documentación de Kubeflow para más información.

> [!div class="nextstepaction"]
> [Guía de usuario de Kubeflow][kubeflow-docs]
