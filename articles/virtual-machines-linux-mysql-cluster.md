<properties title="Using load-balanced sets to clusterize MySQL on Linux" pageTitle="Uso de conjuntos de carga equilibrada para crear clústeres en MySQL en Linux" description="Un artículo que ilustra los modelos para configurar un clúster de Linux de alta disponibilidad y carga equilibrada en Azure utilizando MySQL como ejemplo" metaKeywords="mysql, linux, cluster, azure, ha, high availability, corosync, pacemaker, drbd, heartbeat" services="virtual-machines" solutions="" documentationCenter="" authors="jparrel" videoId="" scriptId="" manager="timlt" />
...
**No instale MySQL en este momento**. Los scripts de instalación de Debian y Ubuntu inicializarán un directorio de datos de MySQL "/var/lib/mysql", pero dado que el directorio se sustituirá por un sistema de archivos DRBD, necesitamos hacer esto más tarde.
...
Después de hacer esto, inicialice el recurso usando "drbdadm" en ambas máquinas virtuales:
...
Si examina el contenido de /proc/drbd ("sudo cat /proc/drbd") en ambas máquinas virtuales, debe ver "Primary/Secondary" en "hadb01" y "Secondary/Primary" en "hadb02", en consonancia con la solución en este punto. El disco de 5 GB se sincronizará a través de la red 10.10.10.0/24 sin coste alguno para los clientes.
...
En "hadb01" ahora estamos preparados para montar los recursos DRBD. Debian y derivados usan `/var/lib/mysql` como directorio de datos de MySQL. Dado que no hemos instalado MySQL, crearemos el directorio y montaremos el recurso DRBD. En "hadb01":
...
También necesita habilitar las redes para MySQL si desea realizar consultas desde fuera de las máquinas virtuales, que es la finalidad de esta guía. En ambas máquinas virtuales, abara "/etc/mysql/my.cnf" y vaya a "bind-address", y cambie 127.0.0.1 por 0.0.0.0. Después de guardar el archivo, emita "sudo service mysql restart" en su primaria actual.
...
Una vez creado el extremo, iremos a "hadb02", Extremos y crearemos un nuevo extremo, pero elegiremos "lb-mysql" y después seleccionaremos MySQL en el menú desplegable. También puede usar la CLI de Azure para este paso.
...
Ejecute "sudo crm configure show" para realizar la comprobación. Ahora, cree un archivo (por ejemplo "/tmp/cluster.conf") con los siguiente recursos:
