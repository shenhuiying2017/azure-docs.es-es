...
Si cualquiera de los supuestos anteriores no se cumple, es recomendable ajustar la configuración de escalamiento en la pestaña *Escala*. Si esta acción no resuelve el problema, puede investigar si Base de datos SQL de Azure puede ser la causa del problema. Las próximas secciones cubren algunos enfoques diferentes para diagnosticar qué puede estar funcionando mal.
...
Actualmente esta es la edición predeterminada que usan los Servicios móviles. A continuación se indican algunas recomendaciones a la hora de seleccionar el nivel de la base de datos:

-   **Base de datos de 20 MB gratuita**: se usa solo para fines de desarrollo.
-   **Web y Business**: se usa para servicios de desarrollo y producción. Los dos niveles proporcionan la misma capacidad de rendimiento, aunque el nivel Web solamente admite bases de datos con un tamaño de hasta 5 GB. Para base de datos más grandes, use el nivel Business.
...
6.  Configure el servicio móvil para usar la base de datos que acaba de crear.

    -   Si está creando un nuevo servicio móvil, seleccione **+NUEVO**, **Proceso**, **Servicio móvil** y **Crear**. En la pantalla siguiente, rellene los valores, seleccione **Usar una base de datos SQL existente** en el campo **Base de datos** y después seleccione **Siguiente**. En la pantalla siguiente, asegúrese de seleccionar la base de datos creada en el paso 5 y después seleccione **Aceptar**.
    -   Si ya tiene un servicio móvil, navegue a la ficha **Configurar** correspondiente a ese servicio y seleccione **Cambiar base de datos** en la barra de herramientas. En la pantalla siguiente, seleccione **Usar una base de datos SQL existente** en el campo **Base de datos SQL** y después seleccione **Siguiente**. En la pantalla siguiente, asegúrese de seleccionar la base de datos creada en el paso 5 y después seleccione **Aceptar**.

A continuación se indican algunas recomendaciones a la hora de seleccionar el nivel correcto de la base de datos:

-   **Basic**: usar en el tiempo de desarrollo o para pequeños servicios de producción donde espera hacer las consultas de una en una a la base de datos.
...
5.  Navegue a la pestaña **Monitor**.
6.  Asegúrese de que se muestran las métricas correspondientes mediante el botón **Agregar métricas**. Entre las métricas se incluyen las siguientes:
    ...
    -   *Porcentaje de CPU* (solamente disponible en los niveles Basic, Standard y Premium)
    -   *Porcentaje de lecturas de datos físicos* (solamente disponible en los niveles Basic, Standard y Premium)
    -   *Porcentaje de escrituras en registro* (solamente disponible en los niveles Basic, Standard y Premium)
    -   *Almacenamiento*
...
La indización de tablas pequeñas puede no ser óptima porque el optimizador de consultas puede tardar más tiempo en atravesar el índice buscando datos que en realizar un simple examen de tabla. Por tanto, pudiera ser que los índices en tablas pequeñas nunca fueran usados y mucho menos mantenidos como datos en los cambios de tabla.
...
> [WACOM.NOTE]
> Ejecute esta consulta en la base de datos **master** del servidor; la vista **sys.resource\_stats** solamente está presente en esa base de datos.
...
> [WACOM.NOTE]
> Ejecute esta consulta en la base de datos **master** del servidor; la vista **sys.event\_log** solamente está presente en esa base de datos.
...
-   **No clúster**. Los índices no clúster se almacenan separadamente desde filas de datos y se usan para realizar una búsqueda basada en el valor de índice. Todos los índices no clúster de una tabla usan los valores de clave del índice en clúster como clave de búsqueda.
...
<!-- BLOG LINKS -->
  ...
  [Diagnóstico de problemas]: #Diagnosing
  [Indización]: #Indexing
  [Diseño de esquemas]: #Schema
  [Diseño de consultas]: #Query
  [Arquitectura del servicio]: #Architecture
  [Solución avanzada de problemas]: #Advanced
  [Portal de administración de Azure]: http://manage.windowsazure.com
  [Preguntas más frecuentes sobre la retirada de las ediciones Web y Business]: http://msdn.microsoft.com/es-es/library/azure/dn741330.aspx
  [Características de vista previa]: https://account.windowsazure.com/previewfeatures
  [Razones para usar los nuevos niveles de servicio]: http://msdn.microsoft.com/es-es/library/azure/dn369873.aspx#Reasons
  [Azure Management Portal - SQL Database Metrics]: ./media/mobile-services-sql-scale-guidance/3.png
  [Descripción del uso de recursos]: http://msdn.microsoft.com/es-es/library/azure/dn369873.aspx#Resource
