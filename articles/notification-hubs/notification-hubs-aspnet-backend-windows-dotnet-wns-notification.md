...
8. En el Explorador de soluciones, en el proyecto **(Windows 8.1)**, haga doble clic en **MainPage.xaml** para abrirlo en el editor de Visual Studio.
...
1. En el Explorador de soluciones, en el proyecto **compartido**, abra el archivo **App.xaml.cs**. Encuentre la llamada a `InitNotificationsAsync()` in the `OnLaunched()` . Marque como comentario o elimine la llamada a `InitNotificationsAsync()`. El controlador del botón agregado anteriormente inicializará los registros de notificación.
...
1. En el Explorador de soluciones, haga clic con el botón derecho en el proyecto **compartido**, a continuación en **Agregar** y, por último, en **Clase**. Asigne un nombre a la clase **RegisterClient.cs** y después haga clic en **Aceptar** para generar la clase.
