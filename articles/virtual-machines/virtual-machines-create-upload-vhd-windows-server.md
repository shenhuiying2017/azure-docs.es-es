...
1. Inicia sesión en el [Portal](http://manage.windowsazure.com).
...
8. Escriba un **Nombre** para el contenedor y seleccione la **Directiva de acceso**.
	...
	> [AZURE.NOTE]De manera predeterminada, el contenedor es privado y solo puede acceder a él el propietario de la cuenta. Para permitir el acceso de lectura público a los blobs del contenedor, pero no a las propiedades y los metadatos del contenedor, usa la opción de **Blob público**. Para permitir el acceso de lectura público completo al contenedor y a los blobs, usa la opción de **Contenedor público**.
...
> [AZURE.TIP]Para empezar a usar Azure PowerShell, consulte [Instalación y configuración de Azure PowerShell](../install-configure-powershell.md). Para obtener información general, consulta [Introducción a los cmdlets de Microsoft Azure](https://msdn.microsoft.com/library/azure/jj554332.aspx).
...
1. Desde la ventana de Azure PowerShell que usaste en el paso anterior, escribe un comando parecido a este:
	...
	Donde:- **BlobStorageURL** es la dirección URL de la cuenta de almacenamiento - **YourImagesFolder** es el contenedor del almacenamiento de blobs donde quieres almacenar las imágenes - **VHDName** es el nombre que quieres que muestre el portal para identificar el disco duro virtual - **PathToVHDFile** es la ruta de acceso completa y el nombre del archivo .vhd
...
> [AZURE.TIP]Para usar el cmdlet de Azure PowerShell en lugar del portal para agregar la imagen, usa el cmdlet **Add-AzureVMImage**. Por ejemplo:

>	`Add-AzureVMImage -ImageName <ImageName> -MediaLocation <VHDLocation> -OS <OSType>`

1. En el portal, en **Todos los elementos**, haz clic en **Máquinas virtuales**.
...
6. Tras completar los pasos anteriores, la nueva imagen aparecerá en la lista cuando elija la pestaña **Imágenes**.
	...
	> [AZURE.TIP]Si se produce un error al intentar crear una máquina virtual, con este mensaje de error: "El VHD https://XXXXX.. tiene un tamaño virtual no compatible de YYYY bytes. El tamaño debe ser un número entero (en MB)", significa que el disco duro virtual no es un número entero de MB y debe ser un disco duro virtual de tamaño fijo. Prueba a usar el cmdlet de PowerShell **Add-AzureVMImage** en lugar del portal para agregar la imagen (mira el paso 5 anterior). Los cmdlets de Azure se aseguran de que el disco duro virtual cumple los requisitos de Azure.

## Pasos siguientes ##

Después de crear una máquina virtual, intente crear una máquina virtual de SQL Server. Para obtener instrucciones, consulta [Aprovisionamiento de una máquina virtual de SQL Server en Microsoft Azure](virtual-machines-provision-sql-server.md).
