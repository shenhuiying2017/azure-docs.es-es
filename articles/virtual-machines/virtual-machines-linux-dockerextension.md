...
- Para crear rápidamente un prototipo de una aplicación o si ya conoce y usa Docker Machine, puede usar [el controlador de Azure de Docker Machine](./virtual-machines-linux-docker-machine.md) para poner en marcha los hosts del docker en Azure.
- Para obtener más control sobre la implementación de un entorno de Docker de forma reproducible y definida por la plantilla de Azure que incluya controles de acceso basados en roles, diagnósticos e implementaciones registradas y controladas, la extensión de VM de Docker de Azure con Azure Resource Manager es el mejor enfoque.
...
La extensión de VM de Docker para Azure se instala en la VM de Linux y se configura automáticamente. Mediante el uso de plantillas de Resource Manager, el entorno se puede volver a implementar de forma coherente. Se recomienda el uso de la extensión de VM de Docker para Azure para un desarrollador más resistente o entornos de producción, ya que tiene algunos controles adicionales en comparación con el uso simplemente de Docker Machine o la creación del host del docker usted mismo.
...
Una vez que haya terminado la implementación, aplique SSH a su nuevo host de Docker mediante el nombre DNS que proporcionó durante la implementación. Las herramientas de Docker ya estarán instaladas, por lo que vamos a intentar poner en marcha un contenedor nginx:
