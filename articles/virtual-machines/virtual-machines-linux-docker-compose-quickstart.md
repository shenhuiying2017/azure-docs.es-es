<properties
   ...
   description="Introducción rápida al trabajo con Compose y Docker en máquinas virtuales de Linux"
...
Empiece a usar Docker y [Compose](http://github.com/docker/compose) para definir y ejecutar una aplicación compleja en una máquina virtual de Linux en Azure. Con Compose (el sucesor de*Fig*), use un archivo de texto simple para definir una aplicación compuesta de varios contenedores de Docker. A continuación, gire la aplicación en un único comando que hace todo para ejecutarlo en la máquina virtual.

Como ejemplo, en este artículo se muestra cómo configurar rápidamente un blog de WordPress con una base de datos SQL MariaDB de back-end, pero también puede utilizar Compose para configurar aplicaciones más complejas.
...
Verá un resultado similar a `docker-compose 1.5.1`.
...
Ahora puede conectarse a WordPress directamente en la máquina virtual; para ello, vaya a `http://localhost:8080`. Si desea conectarse a la máquina virtual a través de Internet, primero configure un extremo HTTP en la máquina virtual que asigne el puerto público 80 al puerto privado 8080. Por ejemplo, si ha creado la máquina virtual mediante el modelo de implementación clásica, ejecute el siguiente comando de CLI de Azure:
...
Si trata de conectarse a `http://<cloudservicename>.cloudapp.net`, ahora debería ver la pantalla de inicio de WordPress, donde se puede completar la instalación e iniciar la aplicación.
...
* Consulte [Docker VM extension user guide](https://github.com/Azure/azure-docker-extension/blob/master/README.md) (Guía de usuario de extensión de VM Docker) para obtener más opciones de configuración de Docker y Compose en su máquina virtual Docker.
...
* Pruebe a integrar Docker Compose con un clúster de [Docker Swarm](virtual-machines-linux-docker-swarm.md). Consulte [Using Swarm with Compose](https://docs.docker.com/compose/swarm/) (Uso de Swarm con Compose) para los escenarios.
