...
En primer lugar, cree el grupo de recursos con [az group create](/cli/azure/group#az_group_create). En el ejemplo siguiente se crea un grupo de recursos denominado `myResourceGroup` en la ubicación `westus`:
...
Cree el grupo de seguridad de red con [az network nsg create](/cli/azure/network/nsg#az_network_nsg_create). En el ejemplo siguiente, se crea un grupo de seguridad de red denominado `myNetworkSecurityGroup`:
...
Para asociar la subred al grupo de seguridad de red, use [az network vnet subnet update](/cli/azure/network/vnet/subnet#az_network_vnet_subnet_update). En el ejemplo siguiente se asocia el nombre de la subred `mySubnet` al grupo de seguridad de red llamado `myNetworkSecurityGroup`:
...
Cree la vNic con [az network nic create](/cli/azure/network/nic#az_network_nic_create). En el ejemplo siguiente se crea una vNic llamada `myNic`, se conecta a la red virtual `myVnet``myVnet` y se crea un registro de nombre de DNS interno llamado `jenkins`:
