...
Es recomendable utilizar uno o varios discos independientes para almacenar los datos de una máquina virtual. Al crear una máquina virtual de Azure, ésta cuenta con un disco para el sistema operativo asignado a la unidad C y con un disco temporal asignado a la unidad D. **No use la unidad D para almacenar datos.** Como señala su nombre, la unidad D ofrece únicamente almacenamiento temporal. No ofrece redundancia ni copias de seguridad porque no reside en Almacenamiento de Azure.
...
4. En la sección **Discos** se muestra el disco 0, el disco 1 y el disco 2. El disco 0 es el disco del sistema operativo, el disco 1 es el disco temporal (que no debe usarse para almacenamiento de datos) y el disco 2 es el disco de datos que conectó a la máquina virtual. El disco de datos tiene una capacidad de 5 GB, en función de lo que se especificó al conectar el disco. Haga clic con el botón derecho en el disco 2 y seleccione **Inicializar**.

5.	Se le notificará que se borrarán todos los datos cuando se inicializa el disco. Haga clic en **Sí** para confirmar la advertencia e inicializar el disco. A continuación, haga clic de nuevo con el botón derecho en el disco 2 y seleccione **Nuevo volumen**.
