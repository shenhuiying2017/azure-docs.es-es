...
Los escenarios de difusión se habilitan mediante la inclusión de una o más *etiquetas* cuando se crea un registro en el centro de notificaciones. Cuando las notificaciones se envían a una etiqueta, todos los dispositivos registrados para la etiqueta recibirán la notificación. Puesto que las etiquetas son cadenas simples, no tendrán que aprovisionarse antes. Para obtener información sobre las etiquetas, consulte [Introducción a centros de notificaciones][].
...
4.  En el código anterior, reemplace los marcadores de posición `<hub name>` y `<connection string with listen access>` por su nombre del centro de notificaciones y la cadena de conexión para *DefaultListenSharedAccessSignature* que obtuvo anteriormente.
...
5.  En la clase **MainActivity** quite los campos privados para **NotificationHub** y **GoogleCloudMessaging**, y agregue un campo para **Notifications**:

        // private GoogleCloudMessaging gcm;
        // private NotificationHub hub;
        private Notifications notifications;

6.  Luego, en el método **onCreate**, quite la inicialización del campo **hub** y el método **registerWithNotificationHubs**. Agregue después las siguientes líneas que inicializan una instancia de la clase **Notifications**. El método debe contener las siguientes líneas:
...
7.  Luego, agregue el siguiente método:
...
<div class="dev-callout"><strong>Nota:</strong> 
    <p>Dado que el identificador de registro asignado por el servicio de mensajer&iacute;a en la nube de Google (GCM) puede cambiar en cualquier momento, debe registrarse para notificaciones frecuentemente para evitar errores de notificaci&oacute;n. En este ejemplo se registra la notificaci&oacute;n cada vez que se inicia la aplicaci&oacute;n. En las aplicaciones que se ejecutan con frecuencia, m&aacute;s de una vez al d&iacute;a, es posible que pueda omitir el registro para conservar el ancho de banda si pasa menos de un d&iacute;a del registro previo.</p>
</div>
...
3.  Envíe una nueva notificación desde el back-end de alguna de las siguientes formas:
    ...
    -   **Java/PHP:** ejecute la aplicación o script.
...
<!-- Anchors. --> <!-- Images. --> <!-- URLs.-->
  ...
  [Introducción a centros de notificaciones]: http://msdn.microsoft.com/en-us/library/jj927170.aspx
