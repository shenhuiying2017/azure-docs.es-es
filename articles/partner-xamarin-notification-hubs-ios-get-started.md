...
En este tutorial se demuestra el escenario de difusión sencillo con centros de notificaciones. Este tutorial exige los siguientes requisitos previos:
...
-   [Componente de Servicios móviles de Azure](http://components.xamarin.com/view/azure-mobile-services/)
    ...
    **Nota:**
    ...
    Debido a los requisitos de la configuración de las notificaciones de inserción, debe implementar y realizar una prueba de las notificaciones de inserción en un dispositivo compatible con iOS (iPhone o iPad) en lugar de hacerlo en un emulador.
...
1.  Seleccione una dirección de correo electrónico de usuario en **User Email Address**, escriba un nombre común en **Common Name** y una dirección de correo electrónico en **CA Email Address**, asegúrese de que **Saved to disk** se encuentra seleccionado, y, a continuación, haga clic en **Continue**.
...
2.  Escriba un nombre para la aplicación en **Description** y un valor para **Bundle Identifier**, active la opción "Push Notifications" de la sección "Servicios de aplicaciones" y, a continuación, haga clic en **Continue**.
  ...
  De esta forma, se genera el identificador de la aplicación y se solicita que envíe la información. Haga clic en **Submit**.
  ...
  Si elige proporcionar un valor **Bundle Identifier** distinto a **MobileServices.Quickstart**, también debe actualizar el identificador de agrupación de trabajos en el proyecto de Xcode.
...
4.  Haga clic en **Edit**, luego desplácese al final de la pantalla y haga clic en **Create Certificate...**, debajo de la sección **Development Push SSL Certificate**.
  ...
  ![][115] 
    ...
    **Nota:**
    ...
    Este tutorial usa un certificado de desarrollo. Se usa el mismo proceso cuando se registra un certificado de producción. Asegúrese de que establece el mismo tipo de certificado cuando carga el certificado en Servicios móviles.
...
1.  Una vez que el portal haya creado el certificado, haga clic en **Download** y, a continuación, en **Done**.
  ...
  ![](./media/partner-xamarin-notification-hubs-ios-get-started/mobile-services-ios-push-step9.png)
    ...
    <p>De forma predeterminada, el certificado de desarrollo descargado se llama <strong>aps_development.cer</strong>.</p>
    </div>
...
1.  Vuelva al [portal de aprovisionamiento de iOS](http://go.microsoft.com/fwlink/p/?LinkId=272456) (en inglés), seleccione **Provisioning Profiles**, seleccione **All** y, a continuación, haga clic en el botón **+** para crear un nuevo perfil. De esta forma, aparecerá el asistente **Add iOS Provisioning Profile**.
...
1.  En Xcode, abra el organizador, seleccione la vista de dispositivos, seleccione **Provisioning Profiles** en la sección **Library** del panel izquierdo e importe el perfil de aprovisionamiento que acaba de crear.

2.  En la izquierda, seleccione el dispositivo e importe de nuevo el perfil de aprovisionamiento.

3.  En Acceso a llaves, haga clic con el botón secundario en el nuevo certificado, haga clic en **Export**, escriba un nombre para el certificado, seleccione el formato **.p12** y, a continuación, haga clic en **Save**.
...
2.  Ahora tenemos que agregar una referencia al componente de Servicios móviles de Azure. En la vista Solution, haga clic con el botón secundario en la carpeta **Components** de su proyecto y elija **Get More Components**. Busque el componente **Servicios móviles de Azure** y agréguelo a su proyecto.
...
5.  Cree una clase **Constants.cs** con las variables siguientes:

         // Cadena de conexión y ruta del centro específicos de las aplicaciones de Azure
         public const string ConnectionString = "<Azure connection string>";
         public const string NotificationHubPath = "<Azure hub path>";

6.  En **AppDelegate.cs**, actualice **FinishedLaunching()** para que coincida con lo siguiente:

         public override bool FinishedLaunching(UIApplication application, NSDictionary launchOptions)
         {
             // NOTA: no llame a la implementación de base en una clase Model
             // Consulte http://docs.xamarin.com/guides/ios/application_fundamentals/delegates,_protocols,_and_events 
...
7.  Reemplace el método **RegisteredForRemoteNotifications()** de **AppDelegate.cs**:
         ...
         public override void RegisteredForRemoteNotifications(UIApplication application, NSData deviceToken)
         {
             ...
             Hub.UnregisterAllAsync (deviceToken, (error) => {
                 ...
                 NSSet tags = null; // Cree etiquetas si lo desea
...
8.  Reemplace el método **ReceivedRemoteNotification()** de **AppDelegate.cs**:
...
9.  Cree el método **ProcessNotification()** siguiente en **AppDelegate.cs**:
         ...
         {
             // Comprobar que el diccionario tenga la clave de aps.  Esta es la carga de la notificación que habría enviado.
             if (null != options && options.ContainsKey(new NSString("aps")))
             {
                 //Obtener el diccionario de aps.
                 NSDictionary aps = options.ObjectForKey(new NSString("aps")) as NSDictionary;

                 string alert = string.Empty;

                 //Extraer el texto de alerta.
                 //NOTA: si usa la alerta simple especificando simplemente "  aps:{alert:"alert msg here"}  "
                 //funcionará correctamente.  En cambio, si usa una alerta compleja con claves de localización y demás, 
                 //su objeto "alert" del diccionario aps será otro NSDictionary... Básicamente 
                 //json se vuelca justo en NSDictionary; por lo que es preciso tenerlo en cuenta.
                 if (aps.ContainsKey(new NSString("alert")))
                     alert = (aps [new NSString("alert")] as NSString).ToString();

                 //Si esto procedía de ReceivedRemoteNotification mientras la aplicación se estaba ejecutando,
                 // hay que procesar manualmente aspectos como el sonido, los distintivos y las alertas.
                 if (!fromFinishedLaunching)
                 {
                     //Mostrar manualmente una alerta
    ...
    **Nota:**
    ...
    Puede elegir la invalidación de **FailedToRegisterForRemoteNotifications()** para controlar situaciones, por ejemplo, cuando no haya conexión de red, entre otras.
...
2.  Agregue una referencia al SDK de Bus de servicio de Azure con el [paquete WindowsAzure.ServiceBus NuGet](http://nuget.org/packages/WindowsAzure.ServiceBus/). En el menú principal de Visual Studio, haga clic en **Tools**, luego en **Library Package Manager** y finalmente en **Package Manager Console**. Posteriormente, en la ventana de la consola, escriba:
         ...
         Install-Package WindowsAzure.ServiceBus y presione Entrar.
